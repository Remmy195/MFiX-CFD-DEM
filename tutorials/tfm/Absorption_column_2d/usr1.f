...
! Cumulative time spent on chem solver
      DOUBLE PRECISION, SAVE:: TIME_CHEM=0.0
! Max time spent on chem solver (across all procs)
      DOUBLE PRECISION:: MAX_TIME_CHEM
...
! Update the cpu consumed by chem solver:
      TOCK = get_elapsed_time()
      TIME_CHEM = TIME_CHEM + ( TOCK - TICK)
      call global_max_0d(TIME_CHEM,MAX_TIME_CHEM,PE_IO)
      IF(MYPE==PE_IO .AND. TOCK > 0) THEN
         write(*,1101)'Equilibrium chem. time [sec, % of MFIX] = ['&
            ,MAX_TIME_CHEM,'s, '&
            ,MAX_TIME_CHEM/TOCK*100.0,'%]'
