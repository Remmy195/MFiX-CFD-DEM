...
!                                                                      !
!  Comments: Write reaction rates in units of kmoles/sec.m^3 (SI)      !
!                                                                      !
...
! stored in the global variable for the liquid phase diffusivity
...
! liquid phase mass transfer coefficient for CO2 (no rxn) (cm/s or m/s)
      ...
      CASE (PSFIRST_LMTC)
! effectively no gas side mass transfer resistance; usr_c(2) was explicitly
! set to large_number in user check overriding any user value
         kg_abs = USR_C(2)
         IF (eps_pack > v_limiter .AND. eps_liq > v_limiter .AND. &
             wa_pack > 1.D-6) &
         kl_abs = USR_C(3)* (D_CO2inMEA*c_l(lRNH2)*kl_rxn)**0.5

      CASE (CONSTANT_MTC)
         IF (ep_gas > v_limiter) kg_abs = USR_C(2)
         IF (eps_pack > v_limiter .AND. eps_liq > v_limiter .AND. &
             wa_pack > 1.D-6) kl_abs = USR_C(3)
...
! Gas superficial mass velocity (kg/m^2.s)
...
! Calculate superficial gas mass velocity (kg/m^2.s)
...
! The gas constant R is in units m^3.Pa/kmol.K in SI.
! So the gas-phase mass transfer coefficient is in units kmol/(Pa.m^2.s)
      kg_abs = USR_C(2)*(sa_pack*diffg/(GAS_CONST*Tg))*group1
...
! Calculate superficial liquid mass velocity (kg/m^2.s)
      lliq = vliq*rol*epl
...
! Calculate the liquid-phase mass transfer coefficient (cm/s or m/s)
      kl_abs = usr_c(3)*(mul*GRAVITY/rol)**(1./3.)*group1
...
! interface. The concentration of CO2 is returned in units kmol/m^3.   !
!                                                                      !
! References:                                                          !
