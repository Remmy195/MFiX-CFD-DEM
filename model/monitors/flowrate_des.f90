...
contains
  ...
  subroutine monitor_flowrate_des(id, data)
    ...
    ! should be very close to each other. When both flow rates are non-zero in magnitude,
    ! we take the average of the two. Otherwise we take the flow rate which magnitude
    ! is the largest. Taking the average when both are non-zero mitigates the bias that
    ! tends to overestimate the flow rate when particles are accelerating while crossing
    ! the plane (typically for gravity-driven flows).
    ...
    ! rates.
    do i = 1,dsize
       if(abs(data_r(1,i))>zero.and.abs(data_r(2,i))>zero) then
          data(i) = 0.5*(data_r(1,i) + data_r(2,i))
       elseif(abs(data_r(1,i))>abs(data_r(2,i))) then
          data(i) = data_r(1,i)
       else
          data(i) = data_r(2,i)
       endif
  ...
  !                                                                    !
  !....................................................................!
  subroutine monitor_calc_flowrate_des (id, lc, psize, mask, weight, &
       vsize, values, var)
