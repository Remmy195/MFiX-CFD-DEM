...
! Particles become "normal" once they are at least one radius away from the plane.
          x0c = x0 + s*rp
...
! The weight includes the time scale so a mass flow rate is in kg/s
  ...
  subroutine monitor_collect_data_des(lid, get_value, psize, mask, &
    ...
    use discretelement, only: des_vel_new, omega_new, des_pos_new
    ...
    if(monitor_radius(lid)) &
         call get_value(lid, lc, psize, mask, weight, vsize, values, des_radius)

    if(monitor_pmass (lid)) &
         call get_value(lid, lc, psize, mask, weight, vsize, values, pmass)

    if(monitor_pcount (lid)) &
         call get_value(lid, lc, psize, mask, weight, vsize, values, pmass/pmass) ! pmass/pmass passes an array of ones as the variable

    if(monitor_pvol  (lid)) &
         call get_value(lid, lc, psize, mask, weight, vsize, values, pvol)

    if(monitor_ro_p  (lid)) &
         call get_value(lid, lc, psize, mask, weight, vsize, values, ro_sol)

    if(monitor_vel_x (lid)) &
         call get_value(lid, lc, psize, mask, weight, vsize, values, des_vel_new(:,1))

    if(monitor_vel_y (lid)) &
         call get_value(lid, lc, psize, mask, weight, vsize, values, des_vel_new(:,2))

    if(monitor_vel_z (lid)) &
         call get_value(lid, lc, psize, mask, weight, vsize, values, des_vel_new(:,3))

    if(monitor_pos_x (lid)) &
         call get_value(lid, lc, psize, mask, weight, vsize, values, des_pos_new(:,1))

    if(monitor_pos_y (lid)) &
         call get_value(lid, lc, psize, mask, weight, vsize, values, des_pos_new(:,2))

    if(monitor_pos_z (lid)) &
         call get_value(lid, lc, psize, mask, weight, vsize, values, des_pos_new(:,3))

    if(monitor_rot_x (lid)) &
         call get_value(lid, lc, psize, mask, weight, vsize, values, omega_new(:,1))

    if(monitor_rot_y (lid)) &
         call get_value(lid, lc, psize, mask, weight, vsize, values, omega_new(:,2))

    if(monitor_rot_z (lid)) &
         call get_value(lid, lc, psize, mask, weight, vsize, values, omega_new(:,3))

    if(monitor_t_p   (lid)) &
         call get_value(lid, lc, psize, mask, weight, vsize, values, des_t_s)

    do lc1 = 1, dimension_n_s
       if(monitor_x_p(lid,lc1)) &
            call get_value(lid, lc, psize, mask, weight, vsize, values, des_x_s(:,lc1))
    end do

    do lc1 = 1, des_usr_var_size
       if(monitor_des_usr_var(lid,lc1)) &
            call get_value(lid, lc, psize, mask, weight, vsize, values, des_usr_var(lc1,:))
    end do

    do lc1 = 1, NO_OF_DES_RXNS
       if(monitor_part_rrate(lid,lc1)) &
            call get_value(lid, lc, psize, mask, weight, vsize, values, Part_rrates_out(:,lc1))
    end do

    if(monitor_part_residence_time   (lid)) &
         call get_value(lid, lc, psize, mask, weight, vsize, values, residence_time)
