...
!---------------------------------------------------------------------//
      ...
      character(len=:), allocatable :: mfix_input_filename
...
! Units for data input and output: CGS/SI
...
#ifdef MFIX_VERSION
      CHARACTER(LEN=30) :: ID_VERSION = MFIX_VERSION
#else
#error "Error in build scripts; MFIX_VERSION undefined"
#endif


! Simulation time of the run.
      DOUBLE PRECISION :: TIME
! Start time of the run, usually 0
      DOUBLE PRECISION :: TSTART = 0.0d0
...
! Flag for nice DT LIST
      LOGICAL :: NICE_DT

! Nice DT array list
      DOUBLE PRECISION, ALLOCATABLE :: NICE_DT_LIST(:)
...
! If .TRUE. one of the species equations is being solved (gas and/or
! solids phases)
      LOGICAL :: ANY_SPECIES_EQ

! If .TRUE. one of the solids phase species equations is being solved
      LOGICAL :: ANY_SOLIDS_SPECIES_EQ
...
!    if elapsed_time > (batch_wallclock - term_buffer) then
...
! Run preprocessing only (mesh generation) and exit
      LOGICAL :: PPO
...
! Flag, when set to .FALSE., will update velocity
! of ONLY EXITING particles near Non-MASS_OUTFLOW boundaries
      LOGICAL :: FREEZE_EXIT_DES_VEL = .FALSE.
      ...
      CONTAINS
         ...
         SUBROUTINE GET_TUNIT(TLEFT, TUNIT)
            ...
            IF (TLEFT < 60.0d0) THEN
                TUNIT = 's'
            ELSE IF (TLEFT < 3600.0d0) THEN
                TLEFT = TLEFT/60.0d0
                TUNIT = 'm'
            ELSE
               TLEFT = TLEFT/3600.0d0
               TUNIT = 'h'
               IF (TLEFT >= 24.0d0) THEN
                   TLEFT = TLEFT/24.0d0
                   TUNIT = 'days'
               ENDIF
            ENDIF
