...
! Interval in number of time steps at which LOG file is written
      INTEGER :: NLOG
! Flag to display messages and residuals on the screen XXX issues/1301
      LOGICAL :: FULL_LOG
      LOGICAL :: TSS ! Time step summary
      INTEGER :: TSS_HDR  ! Write header every TSS_HDR iterations
...
! Time step summary info
      INTEGER, PARAMETER :: TSS_SIZE = 100
      CHARACTER(LEN=12) :: TSSH(TSS_SIZE)='' ! Header
      CHARACTER(LEN=12) :: TSSI(TSS_SIZE)='' ! Info
      LOGICAL :: TSSF(TSS_SIZE)
      INTEGER :: TSSL(TSS_SIZE)
      INTEGER :: TSSC=0
