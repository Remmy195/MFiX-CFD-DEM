...
! Old keyword for solids density :: Replaced by RO_s0
      ...
      LOGICAL :: IC_DES_FIT_TO_REGION

      DOUBLE PRECISION :: C (DIMENSION_USR_C)
      CHARACTER(LEN=20) :: C_NAME (DIMENSION_USR_C)
      
      DOUBLE PRECISION :: stiff_chem_min_species
...
! 23.3 Deprecated list:
!-----------------------------------------------------------------------
      NAMELIST / DEP_23_3 / IC_DES_FIT_TO_REGION

! 24.4 Deprecated list:
!-----------------------------------------------------------------------
      NAMELIST / DEP_24_4 / C, C_NAME, STIFF_CHEM_MIN_SPECIES
...
! 23.3 Release deprecated keywords
      STRING=''; STRING = '&DEP_23_3 '//trim(adjustl(INPUT))//'/'
      READ(STRING,NML=DEP_23_3,IOSTAT=IOS)
      IF(IOS == 0) CALL DEPRECATED(LINE_NO, INPUT, '23.3')

! 24.4 Release deprecated keywords
      STRING=''; STRING = '&DEP_24_4 '//trim(adjustl(INPUT))//'/'
      READ(STRING,NML=DEP_24_4,IOSTAT=IOS)
      IF(IOS == 0) CALL DEPRECATED(LINE_NO, INPUT, '24.4')
