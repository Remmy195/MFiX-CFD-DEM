...
MODULE output_manager
   ...
   use funits, only: dmp_log, unit_log, unit_res, unit_log1
   ...
   use time_mod, only:  get_elapsed_time, get_remaining_time, start_io, end_io
...
!                                                                      !
!----------------------------------------------------------------------!
   SUBROUTINE WRITE_OUTPUTS(EXIT_FLAG, FINISHED)
      ...
      IF(WRITE_VTK_FILES) THEN
         DO LC = 1, DIMENSION_VTK
            IF(CHECK_TIME(VTK_TIME(LC))) THEN
               ...
               IF(DISCRETE_ELEMENT) CALL WRITE_VTP_FILE(LC,0,TIME)
            ENDIF
         ENDDO
      ENDIF
...
!----------------------------------------------------------------------!
      ...
      DOUBLE PRECISION :: ELAPSED_TIME, REMAINING_TIME
      CHARACTER(LEN=9) :: CHAR_ELAPSED, CHAR_REMAINING
      CHARACTER(LEN=4) :: UNIT_ELAPSED, UNIT_REMAINING
...
! Write the elapsed time and estimated remaining time
 ...
 1100 FORMAT(/'Simulation time: ',g12.5,3x,'DT: ',g12.5,3x,'Remaining DEM NITs: ',A)

         ELAPSED_TIME = GET_ELAPSED_TIME()
         CALL GET_TUNIT(ELAPSED_TIME, UNIT_ELAPSED)
         CHAR_ELAPSED=''; WRITE(CHAR_ELAPSED,"(F9.2)") ELAPSED_TIME
         CHAR_ELAPSED = trim(adjustl(CHAR_ELAPSED))
! Estimated remaining time
         REMAINING_TIME = GET_REMAINING_TIME()
         CALL GET_TUNIT(REMAINING_TIME, UNIT_REMAINING)

         IF (.NOT.STEADY_STATE) THEN
            CHAR_REMAINING=''; WRITE(CHAR_REMAINING,"(F9.2)") REMAINING_TIME
            CHAR_REMAINING = trim(adjustl(CHAR_REMAINING))
         ELSE
            CHAR_REMAINING = '0.0'
            UNIT_REMAINING = 's'
         ENDIF
         ...
         SHOW_TIME = .FALSE.
#endif
         IF (FINISHED) SHOW_TIME = .TRUE.
         IF(SHOW_TIME) THEN
! Notify the user of usage/remaining wall times.
             WRITE(ERR_MSG,2000)                                           &
                 'Elapsed time:', trim(CHAR_ELAPSED), trim(UNIT_ELAPSED),  &
                 'Time remaining (est):',trim(CHAR_REMAINING), trim(UNIT_REMAINING)
             CALL LOG_MESSAGE(__FILE__, __LINE__, LOGLEVEL_STATUS, HEADER=.FALSE., FOOTER=.FALSE.)
         ENDIF
      ENDIF

 2000 FORMAT(2(A,1X,A,A,4X))
      RETURN
...
!-----------------------------------------------
   ...
   SUBROUTINE UPDATE_TIME_STEP_INFO_FLOAT(I,DATA)
   use output, only: TSSI, TSS_SIZE
...
! time
      CASE(1)
         WRITE(TSSI(I),'(F12.4)') DATA

! Fluid time step (DT)
      CASE(2)
         WRITE(TSSI(I),'(E10.2)') DATA

! Max residual value
      CASE(6)
         WRITE(TSSI(I),'(E8.2)') DATA

! Wall time  Fluid(10), DEM(11), PIC(12), CGDEM(13)
! Wall time elapsed (last -1), left(last)
      CASE(10,11,12,13,14, TSS_SIZE-1, TSS_SIZE)
         TVAL = DATA
         CALL GET_TUNIT(TVAL, TUNIT)
         WRITE(TSSI(I),'(F8.2,A4)') TVAL,TUNIT
         IF(TVAL<0.0) TSSI(I) = '     NA     '


   END SELECT

   RETURN
   END SUBROUTINE UPDATE_TIME_STEP_INFO_FLOAT

   SUBROUTINE UPDATE_TIME_STEP_INFO_INT(I,DATA)
   use output, only: TSSI
   IMPLICIT NONE
...
! NIT
      CASE(4)
         WRITE(TSSI(I),'(I4,1x)') DATA

! DEM/PIC iterations
      CASE(15)
         WRITE(TSSI(I),'(I8)') DATA

! PIP
      CASE(16)
         WRITE(TSSI(I),'(I12)') DATA
   END SELECT

   RETURN
   END SUBROUTINE UPDATE_TIME_STEP_INFO_INT

   SUBROUTINE UPDATE_TIME_STEP_INFO_CHAR(I,DATA)
   use output, only: TSSI
   IMPLICIT NONE
...
! Status
      CASE(3)
         WRITE(TSSI(I),'(A3)') DATA

! Max residual string
      CASE(5)
         WRITE(TSSI(I),'(A2,1x)') DATA

   END SELECT

   RETURN
   END SUBROUTINE UPDATE_TIME_STEP_INFO_CHAR

   SUBROUTINE SET_TIME_STEP_INFO_HEADER()
   use output, only: TSSI, TSSH, TSSF, TSSL, TSS_SIZE
   use discretelement, only: DISCRETE_ELEMENT
   use run, only: TFM_SOLIDS, DEM_SOLIDS, PIC_SOLIDS, CGP_SOLIDS
   IMPLICIT NONE
   ...
   IF(RO_g0/=ZERO) THEN
      ...
      TSSF(10) = .TRUE.
   ENDIF
   IF(DEM_SOLIDS.AND.(.NOT.CGP_SOLIDS).AND.(.NOT.SuperDEM)) THEN
      IF(RO_g0/=ZERO) THEN
         TSSF(11) = .TRUE.
         TSSF(15) = .TRUE.
      ENDIF
...
! 5 = Max residual string
   TSSH( 5) = 'Max'
   TSSL( 5) = 3

! 6 = Max residual value
   TSSH( 6) = 'Residual'
   TSSL( 6) = 8

! 7 = Combined Max residual string + value
   TSSH( 7) = 'Max Residual'
   TSSL( 7) = 12


! 10 = Fluid loop Wall time
   TSSH(10) = '    Fluid   '
   TSSL(10) = 12

! 11 = DEM loop Wall time
   TSSH(11) = '    DEM     '
   TSSL(11) = 12

! 12 = MPPIC loop Wall time
   TSSH(12) = '   MPPIC    '
   TSSL(12) = 12

! 13 = CGDEM loop Wall time
   TSSH(13) = '   CGDEM    '
   TSSL(13) = 12

! 14 = SuperDEM loop Wall time
   TSSH(14) = '   SQDEM    '
   TSSL(14) = 12

! 15 = DEM/PIC/CGDEM/SQP iterations
   TSSH(15) = '  NITs  '
   TSSL(15) = 8

! 16 = Number of particles/parcels
   TSSH(16) = '    PIP     '
   TSSL(16) = 12


! Last - 1 = Wall time elapsed
   TSSH( TSS_SIZE - 1) = '   Elapsed  '
   TSSL( TSS_SIZE - 1) = 12

! last = Wall time remaining
   TSSH( TSS_SIZE ) = '    Left    '
   TSSL( TSS_SIZE ) = 12


   RETURN
   END SUBROUTINE SET_TIME_STEP_INFO_HEADER

   SUBROUTINE PRINT_TIME_STEP_INFO_HEADER()
   use output, only: TSSH, TSSF, TSSC, TSSL, TSS_SIZE
   IMPLICIT NONE

   INTEGER :: I
   CHARACTER(LEN=64) :: FSTRING

   IF(MyPE/=Pe_IO) RETURN

   DO I = 1, TSS_SIZE
      WRITE(FSTRING,'(A,I2,A)') '(',TSSL(I),'("="))'
      IF(TSSF(I)) WRITE(unit_log1, FMT=FSTRING, ADVANCE="NO")
   ENDDO
   WRITE(unit_log1,'(A1)')
   DO I = 1, TSS_SIZE
      WRITE(FSTRING,'(A,I2,A)') '(A',TSSL(I),')'
      IF(TSSF(I)) WRITE(unit_log1, FMT=FSTRING, ADVANCE="NO") TSSH(I)
   ENDDO
   WRITE(unit_log1,'(A1)')
   DO I = 1, TSS_SIZE
      WRITE(FSTRING,'(A,I2,A)') '(',TSSL(I),'("="))'
   ...
   RETURN
   END SUBROUTINE PRINT_TIME_STEP_INFO_HEADER

   SUBROUTINE PRINT_TIME_STEP_INFO_LINE()
   use output, only:  TSSI, TSSF, TSSL, TSS_SIZE
   IMPLICIT NONE
   ...
   TSSI(7) = trim(TSSI(5))//'='//trim(TSSI(6))

   DO I = 1, TSS_SIZE
      WRITE(FSTRING,'(A,I2,A)') '(A',TSSL(I),')'
      IF(TSSF(I)) WRITE(unit_log1, FMT=FSTRING, ADVANCE="NO") TSSI(I)
   ENDDO
   WRITE(unit_log1,'(A1)')

   !TSSI(:) = '  -  '  ! commented out so the number of elements is the same in
   !"MfiX Status tab"

   RETURN
   END SUBROUTINE PRINT_TIME_STEP_INFO_LINE

   SUBROUTINE PRINT_TIME_STEP_INFO()
   use output, only: TSSI, TSSF, TSSL, TSSC, TSS_SIZE
   use residual
   use run
   ...
   CALL SET_TIME_STEP_INFO_HEADER()
   CALL UPDATE_TIME_STEP_INFO_FLOAT(1,time+dt)
   CALL UPDATE_TIME_STEP_INFO_FLOAT(2,dt)
   CALL UPDATE_TIME_STEP_INFO_CHAR(5,resid_string(8))

   CALL UPDATE_TIME_STEP_INFO_FLOAT(TSS_SIZE-1,GET_ELAPSED_TIME())
! Estimated remaining time
   REMAINING_TIME = GET_REMAINING_TIME()
   CALL UPDATE_TIME_STEP_INFO_FLOAT(TSS_SIZE,REMAINING_TIME)
   IF(TSSC==0) THEN
      CALL PRINT_TIME_STEP_INFO_HEADER()
   ELSE
      IF(TSS_HDR>0.AND.MOD(TSSC,TSS_HDR)==0) CALL PRINT_TIME_STEP_INFO_HEADER()
   ENDIF
   CALL PRINT_TIME_STEP_INFO_LINE()
   TSSC = TSSC + 1

   END SUBROUTINE PRINT_TIME_STEP_INFO

   SUBROUTINE PRINT_GRANULAR_STEP_INFO(II)
   use discretelement, only: PIP, IGHOST_CNT
   use discretelement, only: dtsolid
   use physprop, only: ro_g0

   IMPLICIT NONE
   DOUBLE PRECISION :: WALL_NOW, WALL_ELAP, WALL_LEFT
   INTEGER, INTENT(IN) :: II
   INTEGER :: NP

   IF(RO_g0/=ZERO) RETURN

   IF(MOD(II,NLOG)==0) THEN
      NP = PIP - IGHOST_CNT
      CALL GLOBAL_ALL_SUM(NP)
      CALL UPDATE_TIME_STEP_INFO_INT(16,NP)
      CALL PRINT_TIME_STEP_INFO
   ENDIF

   RETURN
   END SUBROUTINE PRINT_GRANULAR_STEP_INFO
