MODULE WRITE_RES0_MOD
   ...
   USE constant, only: usr_c, usr_c_name, c_e, c_f, ep_star, gravity, phi, phi_w, v_ex
   ...
   USE output
   USE param, only: dimension_usr_c
   USE physprop
   USE run
   USE rxns, only: nrr
...
!------------------------------------------------------------------------
      ...
      WRITE (UNIT_RES, REC=4) IMIN1, JMIN1, KMIN1, IMAX, JMAX, KMAX, IMAX1, &
         ...
         , DIMENSION_BC, DIMENSION_USR_C, DIMENSION_IS, DT, XMIN, XLENGTH, YLENGTH&
         , ZLENGTH, C_E, C_F, PHI, PHI_W
      CALL OUT_BIN_512 (UNIT_RES, USR_C, DIMENSION_USR_C, NEXT_RECA)
      NEXT_RECA = 1 + NEXT_RECA                  ! work around for -O3 compiler bug
      NEXT_RECA = NEXT_RECA - 1                    ! # is this still required in 2022?
      DO LC = 1, DIMENSION_USR_C
         WRITE (UNIT_RES, REC=NEXT_RECA) USR_C_NAME(LC)
...
! The paraview MFiX reader expects 60 characters for DESCRIPTION
      WRITE (UNIT_RES, REC=NEXT_RECA) RUN_NAME, DESCRIPTION(1:60), UNITS, RUN_TYPE, &
         COORDINATES
      NEXT_RECA = NEXT_RECA + 1
