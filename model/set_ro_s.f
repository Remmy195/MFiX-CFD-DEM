...
!                  RO_s(IJK,M) = RO_s0(M)
         ...
         ELSEIF (USR_ROs(M)) THEN
            DO IJK=IJKSTART3,IJKEND3
! for encompassing all fluid and flow boundary cells
               IF (WALL_AT(IJK)) CYCLE
...
! Close the file, cleanup, and exit.
 ...
 1100 FORMAT(//1X,/' From: CHECK_SET_ROs',/,' Error 1100:',     &
         ...
         ' fractions which do not sum to 1.'/,'   > myPE = ',I6)
...
!  interior.                                                           !
!                                                                      !
!  In the case of variable density, it is more logical for the user    !
!  to specify ep_s (ic_ep_s/bc_ep_s) as opposed to rop_s, since rop_s  !
!  is itself derived from ep_s and ro_s and ro_s itself is generally   !
!  not yet calculated. Unfortunately, MFIX does not store ep_s.        !
!  Instead it is function of rop_s at any time, which causes some      !
!  challenges during setup.                                            !
!                                                                      !
!  In particular, imposing the user specified ic/bc_ep_s on the field  !
...
! =>rop_s in the flow boundary; here ep_s may become inconsistent with
! ep_g in the flow cell if for some reason the boundary ro_s does not
! also match the interior fluid ro_s. this can happen in MO cell where
! we allow the user to specify certain BC values that are involved in
! calculating density.

! call this for MO since rop_s will not be set. or will it get caught in flow_to_vel?
...
!                                                                      !
!``````````````````````````````````````````````````````````````````````!
      SUBROUTINE IC_USR_ROS_CORRECTION(M)
...
! ic_ros, which together which ic_ep_s was used to define an ic_rop_s
      ...
      END SUBROUTINE IC_USR_ROS_CORRECTION
...
!  Author: J.CARNEY                                                    !
!                                                                      !
!  Purpose: This should probably only apply/impact MO cells            !
!                                                                      !
!``````````````````````````````````````````````````````````````````````!
