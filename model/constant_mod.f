...
!---------------------------------------------------------------------//
   use error_manager
   Use param, only: dim_m, dimension_usr_c
!---------------------------------------------------------------------//
...
!     Fr = Constant with units of pressure. Its value is set in
!          set_constants and units are adjusted if necessary.
...
!   Pc and dPc/deps are calculated.
   ...
   PARAMETER(N_Pc=2d0, D_Pc=5d0, N_Pf=1.03d0, delta=1d-2)

! parameters used for solids pressure model when close-packed
! parameters for P_s = a_ps*(EP_star - EP_g)**b_ps
!    A_PS = constant with units of pressure. Its value is set in
!           set_constants and units are adjusted if necessary
   DOUBLE PRECISION :: A_PS
   INTEGER, PARAMETER :: B_PS = 10

! parameter used for solids viscosity model schaeffer when
! close-packed. Constant with units of pressure*time. Its
! value is set in set_constants and units are adjusted if
! necessary
...
! User defined constants
   DOUBLE PRECISION :: USR_C (DIMENSION_USR_C)

! Names of user defined constants (for output file only)
   CHARACTER(LEN=20) :: USR_C_NAME (DIMENSION_USR_C)
...
! Enter the value of all constants in various units (CGS or SI)
      IF (UNITS == 'SI') THEN
         ...
         Fr = 0.05d0   ! Pa
         A_PS = 1D24   ! Pa
         MAX_MU_s = 100.d0   ! Pa-s (kg/(m*s))
      ELSEIF (UNITS == 'CGS') THEN
         IF (GRAVITY == UNDEFINED) GRAVITY = 980.665D0  !cm/s^2
         GAS_CONST = 8314.56D4                !g.cm^2/(s^2.mol.K)
         to_SI = ONE                          !does not do anything in CGS. e.g.: calc_mu_g.f
         IF (LAM_HYS == UNDEFINED) LAM_HYS = 0.0001d0    ! cm
         Fr = 0.5d0   ! dyne/cm^2 (barye)
         A_PS = 1D25  ! dyne/cm^2 (barye)
         MAX_MU_S = 1000.d0   ! poise (g/(cm*s))
      ELSE
