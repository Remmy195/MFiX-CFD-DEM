...
MODULE STEP
   ...
   USE set_bc1_mod, only: set_bc1, set_bc1_dt
...
! Set wall boundary conditions and transient flow b.c.'s
      CALL SET_BC1
! Wall boundary calculations and setting conditions for quantities that
! should only be done every (successful) time step
      CALL SET_BC1_DT
! include here so they are set before calculations rely on value
! (e.g., calc_mu_g, calc_mu_s)
      CALL SET_EP_FACTORS

      CALL WRITE_OUTPUTS(EXIT_FLAG, .FALSE.)
...
! Second call to check for maximum velocity at inlet to avoid
! convergence problems; consider that jet/ramp will alter bc
! velocity values so this should be updated
...
! call get_stats(IER)
      ...
      CALL WRITE_OUTPUTS(EXIT_FLAG, .TRUE.)
