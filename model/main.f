...
! DISTIO variable for specifying the mfix version
      CHARACTER(LEN=512) :: version = 'RES = 01.8'
...
! used for activating check_data_30
   ...
   CONTAINS
      ...
      subroutine run_mfix0(mfix_dat_filename, filename_len, cmdline_loglevel)

         USE ERROR_MANAGER, ONLY: LOGLEVEL
         USE EXIT, ONLY: MFIX_EXIT, CHECK_EXIT_FLAG
         USE MPI_UTILITY
         INTEGER, INTENT(IN) :: FILENAME_LEN
         CHARACTER(LEN=FILENAME_LEN), INTENT(IN) :: MFIX_DAT_FILENAME

         INTEGER, INTENT(IN) :: CMDLINE_LOGLEVEL
         loglevel = cmdline_loglevel

! do we want to do this?  cgw 2023-07-05
!#ifdef __GFORTRAN__
!         call gfortran_init
!#endif
...
!---------------------------------------------------------------------
      ...
      USE check_output_control_mod, only: check_output_monitor
      ...
      USE discretelement, only: GluedSphereDEM
      USE discretelement
      USE drag, only: f_gs
      ...
      USE monitor_functions, only: monitor_tavg_restart
      USE output, only: dbgprn_layout
      USE output_manager, only: init_output_vars, write_outputs
      ...
      USE randomno, only: init_random_seed
...
!---------------------------------------------------------------------
      ...
      USE get_stl_data_mod, only: get_stl_data_is
      USE mpi_utility, only: bcast
      USE open_files_mod, only: open_files, open_mesh_file
      USE param1, only: n_spx
      USE run, only: run_type, run_name, ppo, generate_mesh, mfix_input_filename
      USE physprop, only : mmax
...
! It is not allowed to remesh during a restart.
! If PPO=.TRUE. force RUN_TYPE to be 'NEW'
      IF(PPO) RUN_TYPE='NEW'
...
! Second call to check bcs in case some stls are flush with the MFiX box
! boundaries
...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
      ...
      USE run, only: ppo
      USE time_mod, only: get_elapsed_time, get_io_time, get_paused_time
      use read_part_input_mod

      IMPLICIT NONE
      DOUBLE PRECISION:: elapsed_time, io_time, paused_time
      logical, save :: finalized = .false.
      if (finalized) return
      finalized = .true.

! Skip finalization when doing preprocessing only
      IF(PPO) RETURN

! Call user-defined subroutine after time-loop.
      IF (CALL_USR) CALL USR3

      IF(DISCRETE_ELEMENT) CALL WRITE_PART_OUTPUT

! Write time used in the .OUT file. (FIXME this writes to screen not OUT file)
      elapsed_time = get_elapsed_time()
      io_time = get_io_time()
      paused_time = get_paused_time()
      CALL WRITE_OUT3 (elapsed_time, io_time, paused_time)
