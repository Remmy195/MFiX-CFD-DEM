...
! If no solids, set the tmp EPs to zero and clear solid velocity
            ...
            ELSE
               IF (MAX_VEL .eq. ZERO) THEN
                  EPs_TMP(M) = BC_EP_s(BCV, M)
               ELSE
                  EPs_TMP(M) = BC_EP_s(BCV,M) * (VEL_TMP(M) / MAX_VEL)
               ENDIF
! Overwrite the current BC value.
               SELECT CASE(BC_PLANE(BCV))
...
! If the net change in solids volume fraction is greater than 0.01,
...
! with an updated value.
         ...
         WRITE(ERR_MSG,1000) BCV, BC_AREA(BCV), NPpDT, int(NPpDT/DTSOLID), &
            PI_FACTOR(BCV_I), PI_COUNT(BCV_I), DEM_MI_TIME(BCV_I), MAX_VEL
         CALL LOG_MESSAGE(__FILE__, __LINE__, LOGLEVEL_INFO, HEADER=.FALSE., FOOTER=.FALSE.)

         DO MM = 1, PHASE_CNT
            M = PHASE_LIST(MM)
            WRITE(ERR_MSG,1010) M, NPMpSEC(M)*DTSOLID, int(NPMpSEC(M)), VEL_TMP(M)
            CALL LOG_MESSAGE(__FILE__, __LINE__, LOGLEVEL_INFO, HEADER=.FALSE., FOOTER=.FALSE.)
         ENDDO


 1000 FORMAT(2/,2X,'For mass inlet BC: ', I3,/,&
         4x,'Boundary Area = ', ES11.4,/,&
         4X,'Total no. particles injected per solids time step = ', ES15.8,/,&
         4X,'Total no. particles injected per second = ', I10,/,&
         4X,'PI_FACTOR = ', ES15.8, ' PI_COUNT = ', I5,/,&
         4X,'Start DES_MI_TIME = ', ES15.8,/'  ',/,&
         4X,'Uniform inlet velocity = ', ES11.4,/'  ',/,&
         4X,'For each solid phase,',/'  ')

 1010 FORMAT(6x,'For solid phase ', I3, ':',/,&
         8X,'No. particles injected per solids time step = ', ES15.8,/,&
         8X,'No. particles injected per second = ', I10,/,&
         8X,'Calculated inlet velocity = ', ES11.4,/'  ')
...
!---------------------------------------------------------------------//
      ...
      use desgrid, only: IofPOS, JofPOS, KofPOS, IofbcPOS, JofbcPOS, KofbcPOS
...
! Set the search area a little bigger than the inlet area.
         ...
         SELECT CASE (BC_PLANE(BCV))
         CASE('N','S')
            BND1 = min(IofbcPOS(BC_X_e(BCV))+1,dg_IMAX1) - &
               max(IofbcPOS(BC_X_w(BCV))-1,dg_IMIN1)
            BND2 = min(KofbcPOS(BC_Z_t(BCV))+1,dg_KMAX1) - &
               max(KofbcPOS(BC_Z_b(BCV))-1,dg_KMIN1)

         CASE('E','W')
            BND1 = min(JofbcPOS(BC_Y_n(BCV))+1,dg_JMAX1) - &
               max(JofbcPOS(BC_Y_s(BCV))-1,dg_JMIN1)
            BND2 = min(KofbcPOS(BC_Z_t(BCV))+1,dg_KMAX1) - &
               max(KofbcPOS(BC_Z_b(BCV))-1,dg_KMIN1)
         ...
         CASE('T','B')
            BND1 = min(IofbcPOS(BC_X_e(BCV))+1,dg_IMAX1) - &
               max(IofbcPOS(BC_X_w(BCV))-1,dg_IMIN1)
            BND2 = min(JofbcPOS(BC_Y_n(BCV))+1,dg_JMAX1) - &
               max(JofbcPOS(BC_Y_s(BCV))-1,dg_JMIN1)
         END SELECT
...
! own as well as the start/end array positions for each BC.
      ...
      DO BCV_I = 1, DEM_BCMI
         ...
         I_w = max(IofbcPOS(BC_X_w(BCV))-1,dg_IMIN1)
         I_e = min(IofbcPOS(BC_X_e(BCV))+1,dg_IMAX1)
         J_s = max(JofbcPOS(BC_Y_s(BCV))-1,dg_JMIN1)
         J_n = min(JofbcPOS(BC_Y_n(BCV))+1,dg_JMAX1)
         K_b = max(KofbcPOS(BC_Z_b(BCV))-1,dg_KMIN1)
         K_t = min(KofbcPOS(BC_Z_t(BCV))+1,dg_KMAX1)
