...
module check_cell_movement_mod
   ...
   use discretelement, only: XE, YN, ZT, iglobal_id, GluedSphereDEM, gsp_explicit, gsp_implicit
...
! skipping particles that do not exist
         IF(IS_NORMAL(L)) THEN
            ...
            IF((I > IEND1 .OR. I < ISTART1) .OR. &
               ...
               IF(.NOT.SMALL_CELL_AT(IJK)) THEN  !JFD: Keep particles in small cut cells

! This particle cannot stay in the current cell.
                  PINC(IJK) = PINC(IJK) - 1
