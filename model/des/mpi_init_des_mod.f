...
!-----------------------------------------------
      ...
      ELSE
         ...
         ! pmass is added into iGhostPack
         ...
         IF(DES_EXPLICITLY_COUPLED) THEN
            ...
            IF(ANY_SOLIDS_SPECIES_EQ)iParticlePacketSize = iParticlePacketSize + 1
         ENDIF

         IF(GSP_EXPLICIT) THEN
            ! sc2gpc_vec(:,3), gp_sa, gid_list
            iParticlePacketSize = iParticlePacketSize + 5
            ! gid_list packed for ghost particles
            iGhostPacketSize = iGhostPacketSize + 1
            IF(ENERGY_EQ) THEN
               ! gp_kps, gp_neighsid and gp_neighsa
               iParticlePacketSize = iParticlePacketSize + 13
               ! gp_kps is packed for ghost particles
               iGhostPacketSize = iGhostPacketSize + 1
               ! for intra heat transfer, sc2gpc_vec need to be packed for ghost particles
               iGhostPacketSize = iGhostPacketSize + 3
            ENDIF
         ELSEIF(GSP_IMPLICIT) THEN
! Only need to pass glueSurface (@renjieke maybe will have glueDiameter in future)
            iParticlePacketSize = iParticlePacketSize + 1
            iGhostPacketSize = iGhostPacketSize + 1
! Pass glueDiameter and glueBounding
            iParticlePacketSize = iParticlePacketSize + 2
            iGhostPacketSize = iGhostPacketSize + 2
! OMOI, only need for par cross mpi BC, no need for ghost cell
            iParticlePacketSize = iParticlePacketSize + 3
! glueQuat, for par cross mpi bc and for ghost particles
            iParticlePacketSize = iParticlePacketSize + 4
            iGhostPacketSize = iGhostPacketSize + 4
         ENDIF

         ! Calculate the size of neighbor data
         iPairPacketSize = 11
!SuperDEM,omoi,super_r,super_mn,super_q
         IF (SuperDEM) then
            iParticlePacketSize = iParticlePacketSize +12
            iGhostPacketSize    = iGhostPacketSize   + 9
         endif
         IF (SuperDEM) then
!contact_point_a and contact_pint_b
             iPairPacketSize = iPairPacketSize + 8
         endif
...
! @renjieke maximum rec buf size sometimes is not sufficient for gsp explicit,
! Double the size might help
      if(gsp_explicit) imaxbuf = imaxbuf * 2
