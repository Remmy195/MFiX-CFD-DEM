MODULE WRITE_RES1_DES
   ...
   use discretelement, only: NGluedParticles, GluedSphereDEM
   use discretelement, only: gsp_explicit, gsp_implicit
   use functions, only: is_nonexistent, is_any_ghost
...
! Write glued particle array data
      INTERFACE WRITE_RES_pARRAY_GSP
         MODULE PROCEDURE WRITE_RES_pARRAY_GSP_1B
         MODULE PROCEDURE WRITE_RES_pARRAY_GSP_1I
         MODULE PROCEDURE WRITE_RES_pARRAY_GSP_1D
         MODULE PROCEDURE WRITE_RES_pARRAY_GSP_1L
      END INTERFACE
...
! Setup data for particle array data collection:
         ...
         if(rk_debug) then
            ...
            ncnt = 0
            DO LL = 1, MAX_PIP
               if(is_any_ghost(LL)) then
                  gcnt = gcnt + 1
               elseif(is_nonexistent(LL)) then
                  ncnt = ncnt + 1
               else
                  cnt = cnt + 1
               endif
            ENDDO
            call GLOBAL_ALL_SUM(cnt)
         endif
...
! Write out the initial data.
      ...
      IF (gsp_explicit) CALL WRITE_RES_DES(lNEXT_REC, NGluedParticles)    ! Number of Glued Particles

      IF(rk_debug .and. mype == pe_io) THEN
         write(*,*) "pROOTCNT =",pROOTCNT
         write(*,*) "manually count based on particle state", cnt
         if(cnt /= pROOTCNT) write(*,*) "PIP/ghost_cnt is not updated properly"
      ENDIF
...
!                                                                      !
! Purpose: Write scalar doubles to RES file.                           !
!``````````````````````````````````````````````````````````````````````!
...
!``````````````````````````````````````````````````````````````````````!
! Subroutine: WRITE_RES_PARRAY_1L                                      !
!                                                                      !
! Purpose: Write scalar logicals to RES file.                          !
!``````````````````````````````````````````````````````````````````````!
...
!``````````````````````````````````````````````````````````````````````!
! Subroutine: WRITE_RES_PARRAY_GSP_1B                                  !
! Author: Hang Zhou                                    Date: Feb 2024  !
! Purpose: Write scalar bytes of glued particles to RES file.          !
!``````````````````````````````````````````````````````````````````````!
      SUBROUTINE WRITE_RES_PARRAY_GSP_1B(lNEXT_REC, INPUT_B)
      ...
      input_i(:) = input_b
      IF(myPE == PE_IO) CALL OUT_BIN_512i(RDES_UNIT, input_i, NGluedParticles, lNEXT_REC)

      deallocate(input_i)

      RETURN
      END SUBROUTINE WRITE_RES_PARRAY_GSP_1B

!``````````````````````````````````````````````````````````````````````!
! Subroutine: WRITE_RES_PARRAY_GSP_1I                                  !
! Author: Hang Zhou                                    Date: Feb 2024  !
! Purpose: Write scalar integers of glued particles to RES file.       !
!``````````````````````````````````````````````````````````````````````!
      SUBROUTINE WRITE_RES_PARRAY_GSP_1I(lNEXT_REC, INPUT_I)
      ...
      RETURN
      END SUBROUTINE WRITE_RES_PARRAY_GSP_1I

!``````````````````````````````````````````````````````````````````````!
! Subroutine: WRITE_RES_PARRAY_GSP_1D                                  !
! Author: Hang Zhou                                    Date: Feb 2024  !
! Purpose: Write scalar doubles of glued particles to RES file.        !
!``````````````````````````````````````````````````````````````````````!
      SUBROUTINE WRITE_RES_PARRAY_GSP_1D(lNEXT_REC, INPUT_D)
      ...
      IF(myPE == PE_IO) CALL OUT_BIN_512(RDES_UNIT,INPUT_D, NGluedParticles, lNEXT_REC)

      RETURN
      END SUBROUTINE WRITE_RES_PARRAY_GSP_1D

!``````````````````````````````````````````````````````````````````````!
! Subroutine: WRITE_RES_PARRAY_GSP_1L                                  !
! Author: Hang Zhou                                    Date: Feb 2024  !
! Purpose: Write scalar logicals of glued particles to RES file.       !
!``````````````````````````````````````````````````````````````````````!
      SUBROUTINE WRITE_RES_PARRAY_GSP_1L(lNEXT_REC, INPUT_L)
...
! Loop counters
      ...
      allocate(INPUT_I(size(INPUT_L)))

      IF(myPE == PE_IO) THEN
         DO LC = 1, size(INPUT_L)
            INPUT_I(LC) = merge(1,0,INPUT_L(LC))
         ENDDO
         CALL OUT_BIN_512i(RDES_UNIT,INPUT_I, NGluedParticles, lNEXT_REC)
      ENDIF

      deallocate(INPUT_I)

      RETURN
      END SUBROUTINE WRITE_RES_PARRAY_GSP_1L
