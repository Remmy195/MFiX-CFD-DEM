...
MODULE READ_RES1_DES
   ...
   use discretelement, only: GluedSphereDEM, NGluedParticles,gsp_explicit,gsp_implicit
   use error_manager
   ...
   INTERFACE READ_RES_pARRAY_GSP
      MODULE PROCEDURE READ_RES_pARRAY_GSP_1B
      MODULE PROCEDURE READ_RES_pARRAY_GSP_1I
      MODULE PROCEDURE READ_RES_pARRAY_GSP_1D
      MODULE PROCEDURE READ_RES_pARRAY_GSP_1L
   END INTERFACE READ_RES_pARRAY_GSP
...
! Integer Error Flag
      ...
      IF(bDIST_IO) THEN
         ...
         IF (gsp_explicit) READ(RDES_UNIT, REC=5) NGluedParticles
...
!``````````````````````````````````````````````````````````````````````!
! Subroutine: READ_RES_pARRAY_1B                                       !
!                                                                      !
! Purpose: Read scalar bytes from RES file.                            !
!``````````````````````````````````````````````````````````````````````!
...
!``````````````````````````````````````````````````````````````````````!
! Subroutine: READ_RES_pARRAY_1I                                       !
!                                                                      !
! Purpose: Read scalar integers from RES file.                         !
!``````````````````````````````````````````````````````````````````````!
...
!                                                                      !
! Purpose: Read scalar doubles from RES file.                          !
!``````````````````````````````````````````````````````````````````````!
...
!                                                                      !
! Purpose: Read scalar logicals from RES file.                         !
!``````````````````````````````````````````````````````````````````````!
...
!``````````````````````````````````````````````````````````````````````!
! Subroutine: READ_RES_pARRAY_GSP_1B                                   !
! Author: Hang Zhou                                    Date: Feb 2024  !
! Purpose: Read scalar bytes of glued particles from RES file.         !
!``````````````````````````````````````````````````````````````````````!
      SUBROUTINE READ_RES_pARRAY_GSP_1B(lNEXT_REC, OUTPUT_B)
      ...
      IF(myPE == PE_IO) THEN
         CALL IN_BIN_512i(RDES_UNIT, OUTPUT_I, NGluedParticles, lNEXT_REC)
      ENDIF
      call BCAST(OUTPUT_I, PE_IO)
      OUTPUT_B(:) = OUTPUT_I(:)
      deallocate(output_i)
      ...
      RETURN
      END SUBROUTINE READ_RES_pARRAY_GSP_1B

!``````````````````````````````````````````````````````````````````````!
! Subroutine: READ_RES_pARRAY_GSP_1I                                   !
! Author: Hang Zhou                                    Date: Feb 2024  !
! Purpose: Read scalar integers of glued particles from RES file.      !
!``````````````````````````````````````````````````````````````````````!
      SUBROUTINE READ_RES_pARRAY_GSP_1I(lNEXT_REC, OUTPUT_I)
      ...
      call BCAST(OUTPUT_I, PE_IO)
      RETURN
      END SUBROUTINE READ_RES_pARRAY_GSP_1I



!``````````````````````````````````````````````````````````````````````!
! Subroutine: READ_RES_pARRAY_GSP_1D                                   !
! Author: Hang Zhou                                    Date: Feb 2024  !
! Purpose: Read scalar doubles of glued particles from RES file.       !
!``````````````````````````````````````````````````````````````````````!
      SUBROUTINE READ_RES_pARRAY_GSP_1D(lNEXT_REC, OUTPUT_D)
      ...
      IF(myPE == PE_IO) CALL IN_BIN_512(RDES_UNIT, OUTPUT_D, NGluedParticles, lNEXT_REC)
      CALL BCAST(OUTPUT_D, PE_IO)
      
      RETURN
      END SUBROUTINE READ_RES_pARRAY_GSP_1D


!``````````````````````````````````````````````````````````````````````!
! Subroutine: READ_RES_pARRAY_GSP_1L                                   !
! Author: Hang Zhou                                    Date: Feb 2024  !
! Purpose: Read scalar logicals of glued particles from RES file.      !
!``````````````````````````````````````````````````````````````````````!
      SUBROUTINE READ_RES_pARRAY_GSP_1L(lNEXT_REC, OUTPUT_L)
...
! Loop counters
      ...
      allocate(lBUF_I(NGluedParticles))
      IF(myPE == PE_IO) THEN
         CALL IN_BIN_512i(RDES_UNIT, lBUF_I, NGluedParticles, lNEXT_REC)
         ...
         ENDDO
      ENDIF
      CALL BCAST(OUTPUT_L, PE_IO)
      deallocate(lBUF_I)


      RETURN
      END SUBROUTINE READ_RES_pARRAY_GSP_1L
