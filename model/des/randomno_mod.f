! vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvC
!                                                                      C
...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
    ...
    PRIVATE
    PUBLIC :: uni_rno, nor_rno, log_nor_rno, init_random_seed, repeatable
    CONTAINS
...
! Through Computer Experiments, Springer, 1994.
      ...
      do i = 1, ceiling(real(nsize/2.0))
         ...
         if(2*i.le.nsize) y(2*i) = x(2) * w * sigma + mean
      end do
