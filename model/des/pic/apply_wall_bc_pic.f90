  ...
  do np = 1, max_pip
     ...
     if(is_normal(np)) then
        ...
        facet_lp: do lc1=1, facets_at_dg(dg_ijk)%count
           ...
           ! JFD: Only consider parcels moving towards the wall. This avoids bouncing
           ! off the wrong facet when the geometry is concave.
           if(dot_product(Nw, des_vel_new(np,:)) >= 0.0d0) cycle facet_lp
...
! JFD: Ignore small cut cells
  if(small_cell_at(lijk)) pic_wall_at = .false.
