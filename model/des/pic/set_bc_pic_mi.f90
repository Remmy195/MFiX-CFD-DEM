...
!-----------------------------------------------
  ...
  use mfix_pic, only: dim_pic_mi_bin
...
!-----------------------------------------------
  ...
  real :: pflow, frac
  !for polydispersity
  real :: POLYPFLOW(dim_pic_mi_bin), MEDIAN_DIA(dim_pic_mi_bin)
  integer :: NUMBIN
  integer :: I
...
1202 FORMAT('WARNING 1202: The system is initiated with no particles',&
  ...
  if(pic_bcmi > 0) then
     ...
     do bcv_i = 1, pic_bcmi
        ...
        pic_mi(bcv_i)%polyremdr(:,:) = 0.0 !poly parcel remainder
        pic_mi(bcv_i)%polypflow(:,:) = 0.0 !poly parcel count

        if(bc_area(bcv) > epsilon(0.0d0)) then

           procArea = getBCProcArea(bcv)
           ! this brings back the amount of bc area assigned to processors
           do phase=1,dim_m       
              CALL getBCPFlow(bcv,phase,pflow,polypflow,median_dia,NUMBIN)
              !make specific to mono-disperse
              if((pflow > 0.0d0).and.(BC_PSD_TYPE(bcv,phase).EQ.'MONO'))then
                 frac = procArea/bc_area(bcv)   !fraction is ratio <1
                 !number of particles per processor (mono)
                 pic_mi(bcv_i)%pflow(phase) = pflow*frac 
              endif
              !incorporate polypflow possibility
              if(BC_PSD_TYPE(bcv,phase).NE.'MONO')then
                 frac = procArea/bc_area(bcv)   !fraction is ratio <1
                 !loop over the bins and assign a particle flow from each
                 pic_mi(bcv_i)%numbin(phase) = NUMBIN
                 do I = 1, NUMBIN, 1
                    pic_mi(bcv_i)%polypflow(I,phase) = polypflow(I)*frac
                    pic_mi(bcv_i)%median_dia(I,phase) = median_dia(I)
                 end do
                 !to prevent overflow on polyremdr during assignment
                 pic_mi(bcv_i)%median_dia(NUMBIN+1,phase) = median_dia(NUMBIN)
              endif
           enddo
           !same for mono and poly
           if(procArea > 0.0d0) then
              ! do everything like normal
              !this assigns spacing by radius
              call set_bc_pic_offset(bcv_i, bcv)
              !this blocks cells that involve stl
...
!                                                                      !
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^!
 subroutine getBCPFlow(lbcv,lphase,pflow,polypflow,median_dia,NUMBIN)
...
! polydisperse information
  ...
  use mfix_pic, only: dim_pic_mi_bin, pic_numbin

  implicit none

  integer, intent(in   ) :: lbcv, lphase
  real, intent(out) :: pflow, polypflow(dim_pic_mi_bin),median_dia(dim_pic_mi_bin)
  integer, intent(out) :: NUMBIN

  double precision :: mflow, vel, ro_sb

! local management of polydisperse
! counter
  INTEGER :: I 
! left and right hand endpoints of the bins
...
! Mass flow of particle at BC  (this does not change for poly)
  ...
  if((mflow>zero).and.(BC_PSD_TYPE(lbcv,lphase).EQ.'MONO')) then
...
! MEDIAN_DIAM(I) the diameter at the center of bin I.
  ...
  !collect information on PSD
  if ((mflow>zero).and.(BC_PSD_TYPE(lbcv,lphase).EQ.'NORMAL'.OR.&
                        BC_PSD_TYPE(lbcv,lphase).EQ.'LOG_NORMAL')) then
     PSD_MEAN_DP = BC_PSD_MEAN_DP(lbcv,lphase)
     PSD_STDEV = BC_PSD_STDEV(lbcv,lphase)
     PSD_MIN_DP = BC_PSD_MIN_DP(lbcv,lphase)
     PSD_MAX_DP = BC_PSD_MAX_DP(lbcv,lphase)

    !create increment for the bins
     Z_INCREMENT = (PSD_MAX_DP-PSD_MIN_DP)/REAL(NUMBIN)

     if(BC_PSD_TYPE(lbcv,lphase).EQ.'NORMAL') then 
     ...
     if(BC_PSD_TYPE(lbcv,lphase).EQ.'LOG_NORMAL')then
  ...
  if((mflow>zero).and.(BC_PSD_TYPE(lbcv,lphase).EQ.'CUSTOM')) then
     !pick up the keyframe count for distribution
     KF = kf_count(2) !2 means BC
     !isolate which keyframe carries the BC/M combo
     if (KF > 0) then
     !pick which KF is associated with the current BC
        DO NKF = 1, kf_count(2), 1  !2 means BC
           if(rlist_CV(NKF,2).EQ.lbcv)then 
             KF = NKF
           end if
        END DO
     ...
     !check if custom distribution for this BCV and phase match
     if (localM.EQ.lphase) then
     ...
     ! Note that here ZCDF(1) = 0 and ZCDF(NUMBIN+1) = 1.0
        NUMBIN = dist_data(KF,2)%nrows - 1
        ZCDF(1) = dist_data(KF,2)%y(1) ! This should always be zero
        DO I = 1, NUMBIN, 1
           !data is stored under dist_data
           !pick up the median diameters and associate CDF
           MEDIAN_DIA(I) = HALF * (dist_data(KF,2)%x(I) + dist_data(KF,2)%x(I+1))
           ZCDF(I+1) = dist_data(KF,2)%y(I+1)
        END DO
     else
        !there is no associate custom distribution and error out
        WRITE(*,*) "Something went wrong with CUSTOM BCV=",lbcv,&
                   "and PHASE=",lphase
     end if !phase check
  endif !custom

  !Now I need to transform each bin and CDF/median_dia into an
  !associate pflow which for the polycase will be polypflow(bin#)

  if ((mflow>zero).and.(BC_PSD_TYPE(lbcv,lphase).NE.'MONO')) then
     !switching to counting approach
     !pick up mean particle volume of system
     VOL_MEAN = mean_volume(2,lbcv,lphase)
     !mass of a mean particle
     MASS_MEAN = VOL_MEAN*ro_sb
     !estimate total number of particles and scale by CDF (dtsolid in
     !pic_flow_bcs)
     PART_ESTIMATE = (mflow)/MASS_MEAN/(ZCDF(NUMBIN+1) - ZCDF(1))

    ! calculate the pflow for each bin in the distribution
    ! The CDF() for each bin will give us the weighting function
    ! regardless of the poly implementation
    polypflow(:)=0.0d0
    DO I = 1, NUMBIN, 1
    ! This represents the number of PARCELS in the bin, and includes
    ! fractional parcels      
       polypflow(I) = (ZCDF(I+1)-ZCDF(I))*PART_ESTIMATE/ &
                      bc_pic_mi_const_statwt(lbcv,lphase)
    END DO
  else
     polypflow(:)=zero
...
!  logical :: ldebug = .true.
  ...
  do phase=1,des_mmax
     ...
     if((pflow > 0.0d0).and.(BC_PSD_TYPE(lbc_map,phase).EQ.'MONO'))then
        lrad  = d_p0(phase)*half
        lstat_wt = bc_pic_mi_const_statwt(lbc_map,phase)
        eff_rad = max(lrad*(lstat_wt)**(1.0/3.0), eff_rad)
     endif
     if(BC_PSD_TYPE(lbc_map,phase).NE.'MONO')then
        numbin = pic_mi(lbcv)%numbin(phase)
        ! manage polydisperse option (based on largest particle)
        do ni = 1, numbin, 1
          polypflow = pic_mi(lbcv)%polypflow(ni,phase)
          if (polypflow > 0.0d0)then
             lrad = pic_mi(lbcv)%median_dia(ni,phase)*half
             lstat_wt = bc_pic_mi_const_statwt(lbc_map,phase)
             eff_rad = max(lrad*(lstat_wt)**(1.0/3.0), eff_rad)
          endif
        end do
...
!---------------------------------------------------------------------//
  ...
  do phase=1,des_mmax
     ...
     if(BC_PSD_TYPE(lbc_map,phase).NE.'MONO') then
        numbin = pic_mi(lbcv)%numbin(phase)
        ! manage polydisperse option
        do ni = 1, numbin, 1
          polypflow = pic_mi(lbcv)%polypflow(ni,phase)
          if (polypflow > 0.0d0) then
             lrad = pic_mi(lbcv)%median_dia(ni,phase)*half
             lstat_wt = bc_pic_mi_const_statwt(lbc_map,phase)
             !effective radius will be based on largest particle
