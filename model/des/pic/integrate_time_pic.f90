...
SUBROUTINE INTEGRATE_TIME_PIC(PIC_ITERS)
  ...
  ! USE constant
! packing limit (now using des_epg_clip instead of ep_star)
...
! time frequency for collision damping model
  DOUBLE PRECISION :: ONE_OVER_TAU_D(DIMENSION_3)
...
! last velocity
  DOUBLE PRECISION, allocatable :: DES_VEL_NEW_LAST(:,:)
...
!  find a new DTSOLID_PIC_CFL through MANAGE_PIC_CFL
  ...
  EPg_MFP = 0.98d0 * (ONE - DES_EPG_CLIP)
  ...
  ! Calculate a collision time frequency before any looping begins (moved)
  !IF(PIC_COLLISION_DAMPING) CALL MAKE_TAU_D(AVG_MASS_VEL, ONE_OVER_TAU_D)
  ! NEW   Keep all old velocities for collision damping model
  if(PIC_COLLISION_DAMPING)then
    allocate(DES_VEL_NEW_LAST(MAX_PIP,LC_BND))
    DO NP = 1, MAX_PIP
       DES_VEL_NEW_LAST(NP,:) = DES_VEL_NEW(NP,:)
       if (ldebug) then
          write(*,*) "DES_VEL_NEW_LAST = ", DES_VEL_NEW_LAST(NP,1), &
                "     DES_VEL_NEW = ", DES_VEL_NEW(NP,1)
       endif
    END DO
  endif
...
! AVINASH:  Please check this implementation
  if (PIC_COLLISION_DAMPING) then
     !calculate average mass velocity as if a normal pic step has occurred
     CALL MAKE_TAU_D(AVG_MASS_VEL, ONE_OVER_TAU_D)
     !Loop over the entire velocity field again and recalculate velocity
     !based on equation (13) from 2010 (ORourke and Snider)
     !(last vel+(dt/2taud)*mass avg vel at current time step)/(1+(dt/2taud))
    DO NP = 1, MAX_PIP
     !  pick up location of particle
        IJK = PIJK(NP,4)
        DO LC = 1, LC_BND
            if(ONE_OVER_TAU_D(IJK)/=ZERO) then
               TIME_RATIO = DTSOLID * 0.5d0 * ONE_OVER_TAU_D(IJK)
               ADJ_VEL(LC) = (DES_VEL_NEW_LAST(NP,LC) + &
                           (TIME_RATIO * AVG_MASS_VEL(IJK,LC))) / &
                           (ONE + TIME_RATIO)
        ...
        END DO
     END DO
     deallocate(DES_VEL_NEW_LAST)
  end if

!  if (PIC_COLLISION_DAMPING) deallocate(DES_VEL_NEW_LAST)
...
!     specifying ALL as only choice to user (no addition to input file)
...
! JFD:
! When entering this subroutine, DTSOLID is set to the fluid DT
...
!  collision damping time frequency
   DOUBLE PRECISION, intent(out) :: ONE_OVER_TAU_D(DIMENSION_3)
...
!  std dev all direction
   DOUBLE PRECISION:: SIGMA_ALL
!  radial function
...
!  create tau_d arithmetic factor eta(1-eta) eqn 33/48
   ARITHFAC2 = ((ONE+PIC_CD_E)*0.5)*(ONE-(ONE+PIC_CD_E)*0.5)
...
!         ONE is a reset to a fluid-full cell to reduce effect
          G_0 = max((ONE - DES_EPG_CLIP) / ((ONE - DES_EPG_CLIP) - (ONE - EP_g(IJK))),ONE)

!  create standard deviation function for this parcel in 3 directions
          SIGMA(:) = sqrt(SUM_STD_DEV(IJK,:))
          SIGMA_ALL = sqrt(SIGMA(1)**2 + SIGMA(2)**2 + SIGMA(3)**2)
!  create sauter radius for this parcel location
          SAUTER_RAD = SUM_LOCAL_VOL(IJK)/SUM_LOCAL_SA(IJK)

!  create collision damping time factor for this parcel location
          ONE_OVER_TAU_D(IJK) = ARITHFAC *((ONE - EP_G(IJK))*SIGMA_ALL/SAUTER_RAD) * &
                                 G_0 * ARITHFAC2
