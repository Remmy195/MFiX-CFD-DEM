...
!---------------------------------------------------------------------//
  ...
  use mfix_pic, only: pic_mi_max_skip
...
! for poly implementation
  double precision :: polypflow, polyremdr
  integer :: NUMBIN, II
...
!......................................................................!
  ...
  do bcv = 1, pic_bcmi
     ...
     do phase=1,des_mmax
      ...
      if (BC_PSD_TYPE(bc_map,phase).EQ.'MONO') then      
        ...
        if(ldebug) then
           write(*,*) ' '
           write(*,*) 'BC_PSD Type:', BC_PSD_TYPE(bc_map,phase)
           write(*,*) '      phase:', phase
        ...
        pic_lp: do lc1=1, iseed
           ...
           if(check_stl) then
              do while(pic_mi(bcv)%blocked(i,j,k) .or. &
                ...
                if(skipped >= pic_mi_max_skip ) then
                   pic_mi_max_skip = pic_mi_max_skip*2
                   WRITE (ERR_MSG, 1000)bc_map,pic_mi_max_skip
                   CALL LOG_WARNING()
           ...
           if(any_solids_species_eq .or. energy_eq ) &
                des_t_s(max_pip) = bc_t_s(bc_map,phase)

           ! Set species mass fractions
           if((energy_eq .and. c_ps0(phase)==undefined) .or. any_solids_species_eq)&
...
! Residence time
      ...
      if(BC_PSD_TYPE(bc_map,phase).NE.'MONO') then

        NUMBIN = pic_mi(bcv)%numbin(phase)
        if (NUMBIN>0) then

          do II = 1, NUMBIN, 1
          ! calculate number of parcels (parcels/s)*s = parcels
            polypflow = pic_mi(bcv)%polypflow(II,phase)*dtsolid
          ! adding any parts of parcels from last pass
            polypflow = polypflow + pic_mi(bcv)%polyremdr(II,phase)
          ! make it a whole number
            iseed = floor(polypflow)
          ! represents a partial parcel left from last dt sweep
          ! rather than adding mass to next bin, this will keep particle
          ! distribution correct over time
             pic_mi(bcv)%polyremdr(II,phase) = polypflow - real(iseed)

          if(ldebug) then
             write(*,*) ' '
             write(*,*) '     BC_PSD_TYPE:', BC_PSD_TYPE(bc_map,phase)
             write(*,*) '           phase:', phase
             write(*,*) 'pic_mi%polypflow:', pic_mi(bcv)%polypflow(II,phase)
             write(*,*) '       polypflow:', polypflow
             write(*,*) '            seed:', iseed
             write(*,*) '       polyremdr:', pic_mi(bcv)%polyremdr(II,phase)
          endif

          lro   = bc_ro_s(bc_map, phase)
          lrad  = pic_mi(bcv)%median_dia(II,phase)*half
          lvol  = (4.0/3.0)*pi*lrad**3
          lmass = lvol*lro
          lnmax = nmax(phase)
          ...
          if(ldebug) then
             ...
             write(*,*) '    lvol:', lvol
             write(*,*) '   lmass:', lmass
             write(*,*) '  statwt:', lstat_wt
          endif

          poly_pic_lp: do lc1=1, iseed
...
1000 FORMAT('Warning: Unable to seed parcel along mass inlet BC#: ',I4, &
           /'Doubling pic_mi_max_skip, new value = ',I8)
