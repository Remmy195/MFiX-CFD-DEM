...
! CGDEM
   use discretelement, only: CGDEM, cgp_stat_wt, cgp_scaling_method, SuperDEM, sqp_vol
! MPPIC
...
! Max number of "data" rows allowed for custom distribution
     integer, parameter :: max_rows = 1001
...
! Verify the number or rows does not exceed the max allowed
             if(dist_data(i,lIndx)%nrows>max_rows) then
               write(err_msg, 1110) trim(filename),dist_data(i,lIndx)%nrows,max_rows
1110           format('Error reading custom PSD file file : ',a/ ,&
                      'Attempting to read ',I6,' rows.'/         , &
                      'Number of rows exceeds the maximum value of ',I6)
               call log_error()
             end if
...
!        d_cubed = d_p0(m)**3.0D0 * loop_length
      ...
      if(SuperDEM) then ! correct volume for superquadic shape
         mean_volume = mean_volume / (pi*d_p0(lM)**3/6.0d0) * sqp_vol(lM)
      endif
