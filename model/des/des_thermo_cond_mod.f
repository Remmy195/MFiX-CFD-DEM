...
!  Comments:                                                           !
!                                                                      !
!  REF: Batchelor and OBrien, "Thermal or electrical conduction       !
...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^!
      ...
      FUNCTION DES_CONDUCTION(I, J, CENTER_DIST, iM, iIJK, Ri, Rj)
...
! radius for particle I and J
      DOUBLE PRECISION, INTENT(IN), OPTIONAL :: Ri, Rj
...
! Determine the radius of the larger and smaller particle
         IF(present(Ri) .and. present(Rj)) THEN
            MIN_RAD = MIN(Ri, Rj)
            MAX_RAD = MAX(Ri, Rj)
         ELSE
...
! Calculate
      COSB = (R1**2 - R2**2 + CENTER_DIST_CORR**2)/(2.0d0 * R1 * CENTER_DIST_CORR)
! Check to ensure that cosb is less than or equal to one. If cosb is
...
! But what does it mean when cosb < -1?   cgw 2023-09-11
! Thus, there is no particle-fluid-particle heat transfer.
      IF( COSB .GT. 1.0d0 .OR. COSB .LT. -1.0d0) THEN ! avoid domain error in acos
         RADIUS = -1.0d0
      ELSE
...
! Increment level counter
 ...
 1000 FORMAT(/1X,70('*'),/' From: DES_COND_EQ',/, ' Message: ',        &
     ...
     DOUBLE PRECISION FUNCTION EVAL_H_PFP(RLENS_dim,S,OLAP_dim,RP)
      ...
      DOUBLE PRECISION :: Rout,Rkn, log_num, log_den, log_arg, omo, omomk
      DOUBLE PRECISION :: s1mRkn2, s1mRout2
...
! Unphysical inputs, set EVAL_H_PFP to zero and return
      IF(RLENS<=ONE.OR.OLAP>ONE.OR.KN==ZERO) THEN
         EVAL_H_PFP = ZERO
         RETURN
      ENDIF
      ...
      ELSEIF(OLAP.le.TWO)THEN
         Rout = sqrt(RLENS**2 - (ONE - OLAP)**2) ! Here RLENS > ONE - OLAP so we can take the sqrt.
      ELSE
         WRITE(ERR_MSG,1000) OLAP_dim,OLAP, RP , &
                             RLENS_dim, RLENS  , &
                             S,KN
         CALL LOG_ERROR()

1000  FORMAT(2x,'Error 2000: From EVAL_H_PFP: Excessive overlap (Olap > Diam) detected',/ &
             2x,'OLAP_dim, OLAP, RP = ',E14.8,2x,E14.8,2x,E14.8 / &
             2x,'RLENS_dim, RLENS   = ', E14.8,2x,E14.8 /&
             2x,'S, Kn              = ',E14.8,2x,E14.8)

      ENDIF
      Rout     = MIN(Rout,ONE)
      s1mRout2 = sqrt(ONE - Rout**2) ! Rout is <=1 so we can take the sqrt.

! Cannot take log of a negative number, set EVAL_H_PFP to zero and return
      log_num = ONE - OLAP - s1mRout2
      log_arg = log_num/Kn
      IF(log_num<=ZERO.OR.log_arg<=ZERO) THEN
         EVAL_H_PFP = ZERO
         RETURN
      ENDIF
...
!     Particle in contact (below code verified)
         TERM1      =     PI*(omo**2    - omomk**2)/KN
         TERM2      = TWO*PI*( s1mRout2 - omomk)
         TERM3      = TWO*PI*omo*log(log_arg)
         EVAL_H_PFP = TERM1 + TERM2 + TERM3
      ELSE
         IF(omomk**2 .GT. ONE) THEN
            Rkn = ZERO
         ELSE
            Rkn=sqrt(ONE - omomk**2)
         ENDIF
         s1mRkn2 = sqrt(ONE - Rkn**2)

         TERM1 = (Rkn**2/(TWO*KN)) + s1mRout2 - s1mRkn2

         log_den = ONE - OLAP - s1mRkn2
         log_arg = log_num/log_den
         if(log_arg>ZERO) then
            TERM2 = (ONE-OLAP)*log(log_arg)
...
! Index of particle being looped over
      ...
      ! Correct for overlap using "area" correction terms
         ...
         ENDIF
      ! Compute inner Rad
         R_in = sqrt(2.0D0*OLAP_ACTUAL*Rpart-OLAP_ACTUAL*OLAP_ACTUAL)
      ...
      ! Compute heat transfer (particle-wall)
         Q_pw = 2.0D0*R_in*Keff*(TWall-TPart)
      ENDIF
