...
MODULE STL_PREPROC_DES
   ...
   use geometry, only: DO_K, IMAX, JMAX, KMAX
...
! Pre-procssing for the des in order to assign facets to grid cells.
      WRITE(ERR_MSG,"('Preprocessing geometry for DES.')")
...
!      IF(.NOT.CARTESIAN_GRID) CALL CONVERT_DEFAULT_WALLS_TO_STL
      IF(BC_DEFAULT_WALLS) CALL CONVERT_DEFAULT_WALLS_TO_STL
...
! Pre-procssing for the des in order to assign facets to grid cells.
      WRITE(ERR_MSG,"('DES geometry preprocessing complete.')")
      CALL LOG_MESSAGE(__FILE__, __LINE__, LOGLEVEL_INFO, HEADER=.FALSE., FOOTER=.FALSE.)
...
! Allocate the data storage array.
      ...
      IGNORE_STL_EDGE_INTERSECTION(:,:)   = .FALSE.

! Do not set IGNORE_STL_EDGE_INTERSECTION for PIC simulations
      IF(MPPIC) RETURN
...
! The two facets share the same edge. Now test the angle between facets
                                 ...
                                 IF(FACET_DP > COS_STL_NB_ANGLE) THEN
                                    ...
                                    IGNORE_STL_EDGE_INTERSECTION(FACET_ID, EDGE) = .TRUE.
                                    IGNORE_STL_EDGE_INTERSECTION(NB_FACET_ID, NB_EDGE) = .TRUE.
...
! Add to list of STL groups
         ...
         STL_GROUP(N_STL_GROUP)%BC_ID    = BCV
         STL_GROUP(N_STL_GROUP)%IS_ID    = -1
...
! Add to list of STL groups
            ...
            ENDIF
            STL_GROUP(N_STL_GROUP)%END    = N_FACETS_DES
! Set BC_ID for each facet of the group
            BC_ID_STL_FACE(STL_GROUP(N_STL_GROUP)%START:STL_GROUP(N_STL_GROUP)%END) = BCV
         ENDIF
      ENDDO
      STL_END(BCWALLS_STL)=N_FACETS_DES
...
! Extents of the BC region with respect to the fluid grid.
      ...
      DO ISV=1, DIMENSION_IS
         ...
         IF(trim(IS_TYPE(ISV)) == 'IMPERMEABLE') THEN
            ...
            if(IS_I_w(ISV)==IS_I_e(ISV)) then
               lXw = XE(IS_I_w(ISV)); lXe = XE(IS_I_e(ISV))
            else
               lXw = XE(IS_I_w(ISV)-1); lXe = XE(IS_I_e(ISV))
            endif

            if(IS_J_s(ISV)==IS_J_n(ISV)) then
               lYs = YN(IS_J_s(ISV)); lYn = YN(IS_J_n(ISV))
            else
               lYs = YN(IS_J_s(ISV)-1); lYn = YN(IS_J_n(ISV))
            endif

            IF(DO_K) THEN
               if(IS_K_b(ISV)==IS_K_t(ISV).AND.KMAX/=1) then
                  lZb = ZT(IS_K_b(ISV)); lZt = ZT(IS_K_t(ISV))
               else
...
! Add to list of STL groups
            ...
            STL_GROUP(N_STL_GROUP)%BC_ID    = -1
            STL_GROUP(N_STL_GROUP)%IS_ID    = ISV
            IF(N_STL_GROUP== 1) THEN
               STL_GROUP(N_STL_GROUP)%START = 1
            ELSE
               STL_GROUP(N_STL_GROUP)%START    = STL_GROUP(N_STL_GROUP - 1)%END + 1
...
! Add to list of STL groups
         ...
         STL_GROUP(N_STL_GROUP)%END    = N_FACETS_DES
! Set BC_ID for each facet of the group
         BC_ID_STL_FACE(STL_GROUP(N_STL_GROUP)%START:STL_GROUP(N_STL_GROUP)%END) = -1
...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^!
      ...
      SAME_X = COMPARE(pXw, pXe)
      SAME_Y = COMPARE(pYs, pYn)
      SAME_Z = COMPARE(pZb, pZt)

! West Face
      IF(.NOT.SAME_Y.AND..NOT.SAME_Z) THEN
         N_FACETS_DES = N_FACETS_DES+1
...
! South Face
      IF(.NOT.SAME_X.AND..NOT.SAME_Z) THEN
         N_FACETS_DES = N_FACETS_DES+1
         VERTEX(1,:,N_FACETS_DES) = (/pXw, pYs, pZb/)
         VERTEX(2,:,N_FACETS_DES) = (/pXe, pYs, pZb/)
...
! Bottom Face
      IF(.NOT.SAME_X.AND..NOT.SAME_Y) THEN
         N_FACETS_DES = N_FACETS_DES+1
         VERTEX(1,:,N_FACETS_DES) = (/pXw, pYs, pZb/)
         VERTEX(2,:,N_FACETS_DES) = (/pXe, pYs, pZb/)
