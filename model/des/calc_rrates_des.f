...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^!
   SUBROUTINE CALC_RRATES_DES(NP, pRgp, pRgc, pRPhase, pHoRg, pSUMRg)
      ...
      USE rxns
      USE sendrecv
      USE toleranc, only: ZERO_X_gs, COMPARE, CHEM_MIN_SPECIES_FLUID, CHEM_MIN_SPECIES_SOLID, DES_MIN_PMASS_FRAC
      USE usr
...
! Calculate user defined reaction rates.
      ...
      IF(ARRHENIUS_RRATES_DES) THEN
         CALL CALC_ARRHENIUS_RRATES_DES(NP, pM, IJK, DES_RATES)
      ELSE
...
! Check if there are enough reactants for this reaction
        do lN = 1, DES_Reaction(H)%nSpecies
         ...
         if(DES_Reaction(H)%Species(lN)%MWxStoich .lt. zero) then
           if(M == 0) then
             if (X_g(IJK,NN) .le. chem_min_species_fluid) then
! There is an insignificant amount of reactant. Skip this reaction.
               DES_RATES(H) = ZERO
               CYCLE RXN_LP
             endif
           else
             if (DES_X_s(NP,nn) .le. chem_min_species_solid) then
               DES_RATES(H) = ZERO
               CYCLE RXN_LP
             endif
             ! check if the particle mass is big enough to consider reactions.
             if(pmass(NP) .le. des_min_pmass(M)*des_min_pmass_frac) then
               DES_RATES(H) = ZERO
               CYCLE RXN_LP
             endif
           endif
...
! Enthalpy transfer associated with mass transfer. (gas/solid)
                  IF(M /= mXfr) RxH = RxH + lRate*CALC_H(T_g(IJK),0,NN)
               ELSE
...
! Discrete Solids Phase:
            ELSE
! Consumption/Formation of solids phase species.
