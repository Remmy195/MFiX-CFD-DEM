...
!---------------------------------------------------------------------//
      ...
      use discretelement, only: CGDEM, cgp_stat_wt, GluedSphereDEM, GENER_PART_CONFIG
...
! Correction for CGDEM particle size
...
! diameter and/or density of the particle may have changed.
      IF(RUN_TYPE /= 'NEW' .AND. ANY_SOLIDS_SPECIES_EQ) RETURN
...
! if GluedSphereDEM and not auto seeding force to set all spheres into same phase index
               IF (GSP_EXPLICIT .AND. .NOT. GENER_PART_CONFIG) EXIT M_LP_ABS
                  !write(*,*) "enter forced phase define"
                  !if use manually seeding, then set pijk in read_gp_info instead here!
                  !PIJK(L,5) = M
                  !ENDIF
               IF( dDp < SMALL_NUMBER .AND. dRho < SMALL_NUMBER) THEN
                  !write(*,*) "enter diameter and rhos dependent phase"
                  PIJK(L,5) = M
                  EXIT M_LP_ABS
               ENDIF             
            ENDDO M_LP_ABS
