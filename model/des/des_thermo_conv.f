...
! Size of particle array on this process.
      use discretelement, only: MAX_PIP, des_usr_var, gp_sa
! Flag to use glued particle dem
      use discretelement, only: GluedSphereDEM, glueSurface
...
! Calculate heat transfer for a real particle in the parcel, then scale W times
         ...
         elseif (gsp_explicit) then
            GAMMAxSA(NP) = GAMMA * gp_sa(NP)
         elseif (gsp_implicit) then
            GAMMAxSA(NP) = GAMMA * glueSurface(NP)
         else
            GAMMAxSA(NP) = GAMMA* l4Pi*DES_RADIUS(NP)*DES_RADIUS(NP)
         endif
