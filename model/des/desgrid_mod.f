...
MODULE DESGRID
   ...
   use discretelement, only: des_vel_new, des_usr_var, GluedSphereDEM, gid_list, gsp_explicit, gsp_implicit
   use discretelement, only: dimn
   use discretelement, only: xe, yn, zt
   use discretelement, only: iglobal_id
   use discretelement, only: remove_rogue_particles
   use error_manager, only: err_msg, loglevel_error, loglevel_info, loglevel_warning, log_message, ival
   ...
   use mass_outflow_dem_mod, only: delete_particle, delete_particle_gsp
...
! constants required for functions computing local and global ijkvalues
      ...
      logical:: rogue_warning_printed = .False. ! Only print one message

      contains

!''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''!
!  Function: set dg_pic_max_init = 128                                 !
!......................................................................!
      subroutine set_dg_init()
! dg_pic_max_init for glued particle DEM
! not sure what this is but leave as if condition for now
      if (GluedSphereDEM) then
         dg_pic_max_init = 128
      endif
      end subroutine set_dg_init
...
!                                                                      !
!  Purpose: Calculate the cell I index containing the given position.  !
!           This version uses NINT instead of FLOOR.                   !
!           This is meant to be used to identify BC plane locations.   !
!......................................................................!
      integer function iofbcpos(fpos)
      implicit none
      double precision fpos
      iofbcpos = nint((fpos-dg_xstart)*dg_dxinv) + dg_istart1 - 1
      end function iofbcpos
...
!           This is meant to be used to identify BC plane locations.   !
!......................................................................!
      integer function jofbcpos(fpos)
      implicit none
      double precision fpos
      jofbcpos = nint((fpos-dg_ystart)*dg_dyinv) + dg_jstart1 - 1
      end function jofbcpos
...
!           This is meant to be used to identify BC plane locations.   !
!......................................................................!
      integer function kofbcpos(fpos)
      implicit none
      double precision fpos
      kofbcpos = nint((fpos-dg_zstart)*dg_dzinv) + dg_kstart1 - 1
      end function kofbcpos
...
!------------------------------------------------------------------------
      subroutine desgrid_neigh_build ()
      use discretelement, only: n_gsp_deleted
      implicit none
!-----------------------------------------------
! Local variables
!-----------------------------------------------
...
! Integer error flag
      ...
      integer :: owner
      double precision :: pos_and_vel(6)
      logical :: do_exit ! exit (or cycle) loop, not mfix!  rename me please.
...
!$omp    is_coarse_dg_x, is_coarse_dg_y, is_coarse_dg_z, max_radius, err_msg, pos_and_vel, ier, mype, owner) &
!$omp    shared(max_pip, neighbor_index, dg_pijk, NO_K, des_pos_new, des_vel_new,dg_pic, factor_RLM, &
!$omp           dd, max_isize, des_radius, gid_list,iglobal_id, &
!$omp           dg_xstart, dg_ystart, dg_zstart, dg_dxinv, dg_dyinv, dg_dzinv, &
!$omp           dg_istart1,dg_iend1, dg_jstart1, dg_jend1, dg_kstart1,dg_kend1, des_usr_var, GluedSphereDEM, &
!$omp           gsp_explicit, gsp_implicit, N_GSP_DELETED,&
!$omp           do_exit, pe_io, remove_rogue_particles)
...
!$omp do reduction (+: N_GSP_DELETED)
      do lcurpar =1,max_pip
!$  if (.false.) then
...
!$  endif
         ...
         if (.NOT. IS_NORMAL(lcurpar)) cycle
         lijk = dg_pijk(lcurpar)
         lic = dg_iof_lo(lijk)
         ljc = dg_jof_lo(lijk)
         lkc = dg_kof_lo(lijk)

         ! Check if this NORMAL-particle is in a GHOST CELL
         if ((lic.lt.dg_istart1).or.(lic.gt.dg_iend1).or. &
             (ljc.lt.dg_jstart1).or.(ljc.gt.dg_jend1).or. &
             (lkc.lt.dg_kstart1).or.(lkc.gt.dg_kend1)) then

         ! Store position and velocity for the error message.
         ! Need to keep track of the rank where out of bounds occurs
         ! (OWNER).
            do_exit = .true.
            IER(myPE) = 1
            OWNER = merge(myPE, 0, IER(myPE) == 1)

            if (NO_K) then
               pos_and_vel(1:2) = des_pos_new(lcurpar,1:2)
               pos_and_vel(4:5) = des_vel_new(lcurpar,1:2)
            else
               pos_and_vel(1:3) = des_pos_new(lcurpar,1:3)
               pos_and_vel(4:6) = des_vel_new(lcurpar,1:3)
            endif

            IF(REMOVE_ROGUE_PARTICLES) THEN
               IF(gsp_explicit) THEN
                  N_GSP_DELETED = N_GSP_DELETED + 1
                  CALL DELETE_PARTICLE_GSP(lcurpar)
               ELSE
                  CALL DELETE_PARTICLE(lcurpar)
               ENDIF
               do_exit = .true.
            ENDIF
...
! skip spheres belong to same glue particles, rigid body transformation and rotation
! so no inner collision
               if (gsp_explicit) THEN
                  if (gid_list(lneigh) .eq. gid_list(lcurpar)) cycle
               endif
...
!$omp end parallel
      ...
      CALL GLOBAL_ALL_SUM(N_GSP_DELETED)

! Error message when a particle goes out of bounds (OOB).
! Collect data from rank where OOB occurs.
      CALL GLOBAL_ALL_SUM(IER)
      CALL GLOBAL_ALL_SUM(OWNER)
      IF (sum(IER) /= 0) THEN
         call bcast(pos_and_vel,owner)
         IF (myPE == PE_IO) THEN
            if (NO_K) then
               WRITE (ERR_MSG, 1106) pos_and_vel(1:2), pos_and_vel(4:5)
            else
               WRITE (ERR_MSG, 1107) pos_and_vel(1:3), pos_and_vel(4:6)
            endif
            if (remove_rogue_particles) then
               if (.not. rogue_warning_printed) then
                  ERR_MSG(1) =  &
                     "Rogue particles removed.  Further warnings will not be printed."
                  rogue_warning_printed = .True.
                  CALL LOG_WARNING()
               endif
            else
               CALL LOG_ERROR()
 ...
 1106 FORMAT('Error: NORMAL particle found in GHOST cell.',   &
            ...
            /' b) Reducing the value of the variable NEIGHBOR_SEARCH_N ',&
             '(Solids>DEM>Max steps between neighbor search)',           &
            /' c) Increasing the particle and/or wall stiffness ',       &
             "(Solids>DEM>Normal spring constant or Young's modulus)",   &
            /'Current particle location (x, y coordinates):',G11.4,', ',G11.4, &
            /'Current particle velocity (u, v components) :',G11.4,', ',G11.4)

 1107 FORMAT('Error: NORMAL particle found in GHOST cell.',   &
            /'Particle may have gone OUT-OF-BOUNDS.',                    &
            /'To resolve this issue, please consider:',                  &
            /' a) Verify all wall boundary conditions are defined',      &
            /' b) Reducing the value of the variable NEIGHBOR_SEARCH_N ',&
            ...
            /' c) Increasing the particle and/or wall stiffness ',       &
             "(Solids>DEM>Normal spring constant or Young's modulus)",   &
            /'Current particle location (x, y, z coordinates):',G11.4,', ',G11.4,', ',G11.4, &
            /'Current particle velocity (u, v, w components) :',G11.4,', ',G11.4,', ',G11.4)
