...
! Calculate the Prandtl Number
      ...
      if(CGDEM) then
         DP = 2.0d0*DES_CGP_RPR(NP)
      else if (SuperDEM) then
! calculate volume equivalent diameter for superquadric particle
         DP=2.0d0* (3.0D0*PVOL(np)/4.0d0/pi)**THIRD
      else if (GSP_EXPLICIT) then
         gid = gid_list(np)
         DP = glueDiameter(gid)
      else if (GSP_IMPLICIT) then
         DP = 2.0d0 * (3.0d0*pvol(np)/4.0d0/pi)**third
      else
...
! Tavassoli modeL, a model modified based on GUNN
         ...
         CASE DEFAULT
            ...
            call log_error()
            ERROR STOP 'INVALID DES CONVECTION MODEL'  ! log_error should have terminated the run
      END SELECT
