...
! JFD: Artificially set flag to -1 if there is an internal surface.
! Then the pressure gradient will not use the pressure on the other side
! of the internal surface
            if(ip_at_e(ijk)) flag_hi = -1
            if(ip_at_e(lo)) flag_lo = -1

            del_phi(1,ijk) = calc_del_phi(flag_lo, flag_hi, &
...
! Then the pressure gradient will not use the pressure on the other side
! of the internal surface
            if(ip_at_n(ijk)) flag_hi = -1
            if(ip_at_n(lo)) flag_lo = -1

            del_phi(2,ijk) = calc_del_phi(flag_lo, flag_hi, &
                 phi(ijk), dx_c, phi_lo, dx_lo, phi_hi, dx_hi)
...
! Then the pressure gradient will not use the pressure on the other side
! of the internal surface
               if(ip_at_t(ijk)) flag_hi = -1
               if(ip_at_t(lo)) flag_lo = -1

               del_phi(3,ijk) = calc_del_phi(flag_lo, flag_hi, &
                    phi(ijk), dx_c, phi_lo, dx_lo, phi_hi, dx_hi)
