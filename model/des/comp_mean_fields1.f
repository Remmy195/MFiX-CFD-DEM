...
! Accumulate solid phase volumes across cells and filter cells.
! For each particle:
! 1. Skip non-normal particles
! 2. If filter size is zero, add the particle volume directly to the cell
! 3. Otherwise, distribute the particle volume across filter cells
!    weighted by the particle's contribution to each cell
!$omp parallel default(none) &
!$omp private(NP, VOL_WT, M, LC, IJK, VOLXWEIGHT) &
!$omp shared(MAX_PIP, PVOL, PIJK, &
!$omp       FILTER_WEIGHT, SOLVOLINC, &
!$omp       FILTER_CELL, FILTER_SIZE)
!$omp do
      do NP=1,MAX_PIP
