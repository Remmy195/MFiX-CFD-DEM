...
MODULE write_des_data_mod
   ...
   use discretelement, only: superdem, super_r, super_mn, super_q
   use error_manager, only: loglevel_error, log_message, ival, ivar
   use fldvar, only: theta_m
   use functions, only: is_nonexistent, fluid_at, is_any_ghost
   ...
   use run, only: any_solids_species_eq, energy_eq, run_name, run_type, time
...
! SuperDEM
      IF(SuperDEM) &
          CALL VTP_WRITE_DATA('Q_ROTATION', super_q(:,1:4))
...
! Return the effective diameter of the particle.
      IF(MPPIC) THEN
