         ...
         IF (DO_K) THEN
            IF(K==KMIN3 .OR. K==KMIN2 .OR. K==KMAX2 .OR. K==KMAX3)THEN
               ...
               ELSEIF (CYCLIC_Z) THEN
                  ICBC_FLAG(IJK) = 'c--'
               ELSEIF(BC_DEFAULT_WALLS) THEN
                  ICBC_FLAG(IJK) = 'W--'
               ELSE
                  ICBC_FLAG(IJK) = 'B--' ! Edges of the MFiX Box when no default
                                         ! walls are applied.
                                         ! This is a temporary setting so inlet
                                         ! and outlets can be defined without
                                         ! having to set a wall BC first.
               ENDIF
            ENDIF
         ENDIF
...
! If no errors, sync up the ghost cell layers.
 ...
 1100 FORMAT('Error 1100 (PE ',A,') : No initial or boundary ',        &
         'conditions specified in','the following cells:',/             &
         '    I       J       K')
...
! Remove the temporary flag that identifies the MFiX Box edges
! When no default walls are applied, this will leave
! ICBC_FLAG blank and the data check will catch that the BC
! was not defined.
! Only do this when running the simulation, not when generating the mesh
      IF(.NOT.PPO) THEN
         ...
         DO I = IMIN3, IMAX3
            ...
            ALONG_BOX = (ICBC_FLAG(IJK)(1:1) .EQ. 'B')

! The 12 edges of the MFiX box are not used but need to be set to 'W--'
            IF(I==IMIN3.AND.J==JMIN3.AND.ALONG_BOX) ICBC_FLAG(IJK) = 'W--'
            IF(I==IMIN3.AND.J==JMAX3.AND.ALONG_BOX) ICBC_FLAG(IJK) = 'W--'
            IF(I==IMAX3.AND.J==JMIN3.AND.ALONG_BOX) ICBC_FLAG(IJK) = 'W--'
            IF(I==IMAX3.AND.J==JMAX3.AND.ALONG_BOX) ICBC_FLAG(IJK) = 'W--'

            IF(J==JMIN3.AND.K==KMIN3.AND.ALONG_BOX) ICBC_FLAG(IJK) = 'W--'
            IF(J==JMIN3.AND.K==KMAX3.AND.ALONG_BOX) ICBC_FLAG(IJK) = 'W--'
            IF(J==JMAX3.AND.K==KMIN3.AND.ALONG_BOX) ICBC_FLAG(IJK) = 'W--'
            IF(J==JMAX3.AND.K==KMAX3.AND.ALONG_BOX) ICBC_FLAG(IJK) = 'W--'

            IF(I==IMIN3.AND.K==KMIN3.AND.ALONG_BOX) ICBC_FLAG(IJK) = 'W--'
            IF(I==IMIN3.AND.K==KMAX3.AND.ALONG_BOX) ICBC_FLAG(IJK) = 'W--'
            IF(I==IMAX3.AND.K==KMIN3.AND.ALONG_BOX) ICBC_FLAG(IJK) = 'W--'
            IF(I==IMAX3.AND.K==KMAX3.AND.ALONG_BOX) ICBC_FLAG(IJK) = 'W--'
         ENDDO
         ENDDO
         ENDDO

         WHERE(ICBC_FLAG(:)(1:1) .EQ. 'B') ICBC_FLAG(:) = '   '
      ENDIF
