...
MODULE vtk_out_mod
   ...
   USE parse
   USE physprop, only: mmax, nmax, mw_g, MW_MIX_g, mu_g, c_pg, c_ps, k_g
...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
      ...
      DO LC = 1, DIMENSION_VTK
         ...
         IF(DISCRETE_ELEMENT) CALL WRITE_VTP_FILE(LC,1,TIME)
      ENDDO
...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
  SUBROUTINE WRITE_VTU_FILE(LCV,MODE)
      ...
      DOUBLE PRECISION ::  SFC1, SFC2, SFC3
...
!     Location of W-momentum cells for original (uncut grid)
      ...
      IF(VTK_DATA(VTK_REGION)=='C') THEN
         ...
         DO PASS=WRITE_HEADER,WRITE_DATA
            ...
            IF(VTK_RO_g(VTK_REGION).AND.ALLOCATED(RO_G)) &
               CALL WRITE_SCALAR_IN_VTU_BIN('RO_G',RO_G,PASS)

            IF(VTK_MU_g(VTK_REGION).AND.ALLOCATED(MU_G)) &
               CALL WRITE_SCALAR_IN_VTU_BIN('MU_G',MU_G,PASS)

            IF(VTK_K_g(VTK_REGION).AND.ALLOCATED(K_G)) &
               CALL WRITE_SCALAR_IN_VTU_BIN('K_G',K_G,PASS)
...
! Specific heat
            IF(VTK_C_Pg(VTK_REGION).AND.ALLOCATED(C_PG)) &
               CALL WRITE_SCALAR_IN_VTU_BIN('Gas_specific_heat',C_PG,PASS)

            DO M = 1,MMAX
               IF(VTK_C_Ps(VTK_REGION,M).AND.ALLOCATED(C_PS)) THEN
                  WRITE(SUBM,*)M
                  CALL WRITE_SCALAR_IN_VTU_BIN('Solids_specific_heat_'//ADJUSTL(SUBM),C_PS(:,M),PASS)
               ENDIF
            END DO
...
! Gas molar fractions
          ...
          IF(VTK_SCALAR_CC(VTK_REGION).AND.ALLOCATED(CENTER_S)) &
             CALL WRITE_VECTOR_IN_VTU_BIN('Scalar center',CENTER_S(:,1),CENTER_S(:,2),CENTER_S(:,3),PASS)
...
! Bypass re-indexng when writing the mesh file (reset re-indexing value)
      ...
      ELSEIF(VTK_DATA(VTK_REGION)=='G') THEN
         ...
         DO PASS=WRITE_HEADER,WRITE_DATA
            ...
            CALL WRITE_STL_GEOMETRY_IN_VTU_BIN(PASS)

            CALL GLOBAL_ALL_SUM(STL_COLLISION_COUNT)
!            CALL GLOBAL_ALL_SUM(STL_WEAR_N)
            CALL GLOBAL_ALL_SUM(STL_FNORM)
            CALL GLOBAL_ALL_SUM(STL_FTAN)

            CALL WRITE_STL_SCALAR_IN_VTU_BIN('Collision_count', STL_COLLISION_COUNT, PASS)
!            CALL WRITE_STL_SCALAR_IN_VTU_BIN('Wear_normal', STL_WEAR_N, PASS)
            CALL WRITE_STL_VECTOR_IN_VTU_BIN('STL_FNORM', STL_FNORM(:,1), STL_FNORM(:,2), STL_FNORM(:,3), PASS)
            CALL WRITE_STL_VECTOR_IN_VTU_BIN('STL_FTAN', STL_FTAN(:,1), STL_FTAN(:,2), STL_FTAN(:,3), PASS)
            CALL WRITE_STL_VECTOR_IN_VTU_BIN('STL_NORMAL_VEL', STL_NORMAL_VEL(1,:), STL_NORMAL_VEL(2,:), STL_NORMAL_VEL(3,:), PASS)
            CALL WRITE_STL_VECTOR_IN_VTU_BIN('STL_TANGENTIAL_VEL', STL_TANGENTIAL_VEL(1,:), STL_TANGENTIAL_VEL(2,:), STL_TANGENTIAL_VEL(3,:), PASS)
      ...
      IF(MODE==0) CALL UPDATE_AND_CLOSE_PVD_FILE(TIME)
...
!vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvC
!                                                                      C
!  Module name: WRITE_STL_GEOMETRY_IN_VTU_BIN                          C
!  Purpose: Write Geometry and connectivity in a vtu file              C
!           Binary format                                              C
!                                                                      C
!  Author: Jeff Dietiker                              Date: 07-Aug-24  C
!  Reviewer:                                          Date:            C
!                                                                      C
!  Revision Number #                                  Date: ##-###-##  C
!  Author: #                                                           C
...
!                                                                      C
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
  SUBROUTINE WRITE_STL_GEOMETRY_IN_VTU_BIN(PASS)
...
! Get number of cells and points
         ...
         DO NG=1,N_STL_GROUP
            ...
            DO GEO = 1, VTK_GEO_MAX
               ...
               IF(TRIM(VTK_GEO(VTK_REGION,GEO))==TRIM(STL_GROUP(NG)%FILENAME).OR.  &
                  TRIM(VTK_GEO(VTK_REGION,GEO))=='GROUP_'//NG_CHAR  ) THEN
                  VTK_GEO_GROUP = NG
...
110   FORMAT(A)
      ...
      END SUBROUTINE WRITE_STL_GEOMETRY_IN_VTU_BIN



!vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvC
!                                                                      C
!  Module name: WRITE_STL_SCALAR_IN_VTU_BIN                            C
...
!                                                                      C
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
  SUBROUTINE WRITE_STL_SCALAR_IN_VTU_BIN(VAR_NAME,VAR,PASS)

      IMPLICIT NONE
      INTEGER :: I,IJK

      CHARACTER (*) :: VAR_NAME
      DOUBLE PRECISION, DIMENSION(DIM_STL) ::  VAR
...
!           For each scalar, write a tag, with corresponding offset
         ...
         ELSEIF(PASS==WRITE_DATA) THEN
            ...
            DO NG=1,N_STL_GROUP
               IF(STL_GROUP(NG)%CONVERT_TO_VTU) THEN
                  DO IJK = STL_GROUP(NG)%START, STL_GROUP(NG)%END
                     WRITE(UNIT_VTU) REAL(VAR(IJK))
                  ENDDO
               ENDIF
            ENDDO
...
90    FORMAT(A,A,A,I12,A)
      ...
      END SUBROUTINE WRITE_STL_SCALAR_IN_VTU_BIN

!vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvC
!                                                                      C
!  Module name: WRITE_STL_VECTOR_IN_VTU                                C
...
!                                                                      C
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
  SUBROUTINE WRITE_STL_VECTOR_IN_VTU_BIN(VAR_NAME,VARX,VARY,VARZ,PASS)

      IMPLICIT NONE
      INTEGER :: IJK

      CHARACTER (*) :: VAR_NAME
      DOUBLE PRECISION, DIMENSION(DIM_STL) ::  VARX,VARY,VARZ
...
90    FORMAT(A,A,A,I12,A)
      ...
      END SUBROUTINE WRITE_STL_VECTOR_IN_VTU_BIN
...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
  SUBROUTINE WRITE_CUT_SURFACE_VTK
      ...
      INTEGER, PARAMETER :: max_cut_face_node = 15

      IF(myPE/=0) RETURN

      ALLOCATE(FACE_CONNECTIVITY(DIMENSION_MAX_CUT_CELL,max_cut_face_node))
...
!                  CALL EVAL_STL_FCT_AT('SCALAR',IJK,NODE_OF_CORNER,F_COPY,CLIP_FLAG,BCID2)
            ...
            IF(N_CUT_FACE_NODES > max_cut_face_node) THEN
               WRITE(ERR_MSG,2000)'FATAL ERROR: There are too many nodes in a cut cell.', &
...
!======================================================================
      ...
      !                        ENDIF
                  ...
                  IF(N_FACE_NODES > max_cut_face_node) THEN
                     WRITE(ERR_MSG,2000)'FATAL ERROR: There are too many nodes in a cut cell.', &
                                      'Please verify geometry and mesh settings.'
                     CALL LOG_ERROR()
                  ENDIF
...
! Fill histogram
      ...
      IF(GLOBAL_NUMBER_OF_CELLS_WITH_THIS_CRITERION == 0) THEN
         WRITE (ERR_MSG, 10) ' WARNING: INVALID RANGE FOR ', TRIM(VARNAME), GLOBAL_MIN_Q, GLOBAL_MAX_Q
         CALL LOG_WARNING()
      ENDIF

10    FORMAT(A,A, /1X,'MIN = ',E14.8, /1x,'MAX = ',E14.8,/1x, &
            'This is likely an indication of a bad mesh, please verify the geometry and mesh settings.')
