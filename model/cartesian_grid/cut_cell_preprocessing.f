...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
  SUBROUTINE CUT_CELL_PREPROCESSING
      ...
      DOUBLE PRECISION :: PP_START_TIME

#ifndef QUIET
      IF(.NOT.CG_HEADER_WAS_PRINTED) CALL PRINT_CG_HEADER
#endif
      PP_START_TIME = GET_ELAPSED_TIME()
...
!======================================================================
! Gather data and write surface(s) defined by all cut cells
!======================================================================
...
!      CALL SET_GHOST_CELL_FLAGS_OLD
      ...
      WRITE(ERR_MSG,20)'Cartesian grid preprocessing completed in ', get_elapsed_time() - pp_start_time, 's'
      CALL LOG_STATUS()
...
1000  FORMAT('Info: THIS IS A COLD DEM GRANULAR FLOW SIMULATION.'/   &
             '      SKIPPING CARTESIAN GRID PREPROCESSING.'/)
...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
  SUBROUTINE PRINT_CG_HEADER
      ...
      WRITE(ERR_MSG,'(A)')' MFiX with Cartesian grid.'
      CALL LOG_STATUS()
...
! Consolidate flags into the master flag
