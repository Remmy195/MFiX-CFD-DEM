...
MODULE GET_STL_DATA_MOD
   ...
   USE bc, only: bc_wall_vel, bc_wall_omega, bc_wall_rot_center
...
!           preprocessing switches to STL procedure                    C
!                                                                      C
...
!            WRITE(*,*)' ALL FACES READ SUCCESSFULLY.'
      ...
      WRITE(ERR_MSG,*) '' // nl // &
         ...
         ' and is provided for convenience (it is not used by MFiX).'
      CALL LOG_INFO()
...
!vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvC
!                                                                      C
!  Module name: ALLOCATE_STL_ARRAYS                                    C
!  Purpose: Allocate STL arrays                                        C
!                                                                      C
!  Author: Jeff Dietiker                              Date: 07-AUG-24  C
!  Reviewer:                                          Date: **-***-**  C
!                                                                      C
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
      SUBROUTINE ALLOCATE_STL_ARRAYS
      IMPLICIT NONE
      INTEGER :: NN, NG, VV

!      print *,'allocating STL arrays ',DIM_STL

      IF(.NOT.ALLOCATED(VERTEX)) ALLOCATE(VERTEX(3,3,DIM_STL))
      IF(.NOT.ALLOCATED(NORM_FACE)) ALLOCATE(NORM_FACE(3,DIM_STL))
      IF(.NOT.ALLOCATED(STL_NORMAL_VEL)) ALLOCATE(STL_NORMAL_VEL(3,DIM_STL))
      STL_NORMAL_VEL(:,:) = ZERO
      IF(.NOT.ALLOCATED(STL_TANGENTIAL_VEL)) ALLOCATE(STL_TANGENTIAL_VEL(3,DIM_STL))
      STL_TANGENTIAL_VEL(:,:) = ZERO

      IF(.NOT.ALLOCATED(STL_FACET_ALONG_BOX)) ALLOCATE(STL_FACET_ALONG_BOX (DIM_STL))
      IF(.NOT.ALLOCATED(BC_ID_STL_FACE)) ALLOCATE(BC_ID_STL_FACE (DIM_STL))
      BC_ID_STL_FACE (:) = -1
      IF(.NOT.ALLOCATED(IGNORE_STL_EDGE_INTERSECTION)) ALLOCATE(IGNORE_STL_EDGE_INTERSECTION(DIM_STL,0:3))

      IF(.NOT.ALLOCATED(STL_COLLISION_COUNT)) ALLOCATE(STL_COLLISION_COUNT(DIM_STL))
      IF(.NOT.ALLOCATED(STL_WEAR_N)) ALLOCATE(STL_WEAR_N(DIM_STL))
      IF(.NOT.ALLOCATED(STL_FNORM)) ALLOCATE(STL_FNORM(DIM_STL,3))
      IF(.NOT.ALLOCATED(STL_FTAN)) ALLOCATE(STL_FTAN(DIM_STL,3))

      RETURN
      END SUBROUTINE ALLOCATE_STL_ARRAYS

!vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvC
!                                                                      C
!  Module name: GET_STL_FACET_COUNT                                    C
!  Purpose: Get the number of facets in all STL files                  C
!           so STL arrays can be allocated.                            C
!                                                                      C
!  Author: Jeff Dietiker                              Date: 08-AUG-24  C
!  Reviewer:                                          Date: **-***-**  C
!                                                                      C
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
      SUBROUTINE GET_STL_FACET_COUNT

      IMPLICIT NONE
      INTEGER :: NN, NG, N_FACETS_BUFFER
      ...
      INTEGER :: BCV,ISV, NUMBER_OF_GEOMETRY_FILES,NUMBER_OF_BC_PATCHES
!      INTEGER :: BC_PATCH(0:DIMENSION_BC),L2,NF1,NF2
!      LOGICAL :: BC_PATCH_FOUND_IN_STL(DIMENSION_BC)
      CHARACTER(LEN=120) :: FNAME
!      CHARACTER(LEN=120) :: SCALE_MSG
!      CHARACTER(LEN=120) :: TRANSLATE_MSG
!      integer :: stl_unit, nf
!      LOGICAL :: MULTISTL
!---------------------------------------------------------------------
      ...
      N_FACETS_BUFFER = 6 !  Default BC

! DETERMINE WHICH BOUNDARY CONDITIONS NEED STL FILE
      WRITE(ERR_MSG,100)'From GET_STL_FACET_COUNT: Counting STL facets...'
      CALL LOG_STATUS()

      DO BCV = 1, DIMENSION_BC
         IF (BC_DEFINED(BCV)) THEN
            IF(.NOT.IS_CG(BC_TYPE_ENUM(BCV))) N_FACETS_BUFFER = N_FACETS_BUFFER + 12  !  Regular BC
            IF(USE_STL.AND.BC_TYPE(BCV)(1:3)=='CG_') THEN
      ...
      DO ISV = 1, DIMENSION_IS
         IF(trim(IS_TYPE(ISV)) == 'IMPERMEABLE') N_FACETS_BUFFER = N_FACETS_BUFFER + 12  !  Regular IS
...
! Read each geometry_####.stl or is_####.stl file
      DO NN = 1, NUMBER_OF_GEOMETRY_FILES
         WRITE(ERR_MSG,2000) 'Counting facets from '//TRIM(geometryfile(NN))//' ...'
         CALL LOG_INFO()
...
! OPEN geometry.stl ASCII FILE
         ...
         DO WHILE(KEEP_READING)
            ...
            IF(TRIM(TEST_CHAR) == 'facet') N_FACETS = N_FACETS + 1
...
930      CLOSE(333)
      ...
      WRITE(ERR_MSG, *) 'Total number of STL facets found =',N_FACETS, nl,&
                        ' Maximum additional facets needed for BC/IS = ',N_FACETS_BUFFER
      CALL LOG_INFO()

! STL arrays will be allocated with DIM_STL, which accounts for STL
! facets (N_FACETS) and regular ICs and ISs (N_FACETS_BUFFER)
! N_FACETS_BUFFER is larger than needed (12 facets per BC or IS) because
! we don't know yet if the BC/IS is a plane (2 facets) or a volume (12
! facets)

      DIM_STL = N_FACETS + N_FACETS_BUFFER

! If using a geometry.msh (legacy, not really used anymore), add
! 1 Million to DIM_STL so we can allocate arrays
      if(USE_MSH) DIM_STL = DIM_STL + 1000000
...
!     call log_error()
 ...
 4000 FORMAT(1X,A,F10.4,' to ',F10.4)
 5000 FORMAT(1X,A,F10.4)
      END SUBROUTINE GET_STL_FACET_COUNT
...
!---------------------------------------------------------------------
      ...
      CALL GET_STL_FACET_COUNT
      CALL ALLOCATE_STL_ARRAYS
      ...
      ENDIF
      IF(N_QUADRIC > 0) THEN
         WRITE(ERR_MSG,*) 'ERROR: Both quadric(s) and STL input are specified.', nl,&
             'MFiX handles only one type of surface input.'
         CALL LOG_ERROR()
      ENDIF
      IF(USE_MSH) THEN
          WRITE(ERR_MSG,*) "Error: Both MSH and STL input are specified",        nl,&
              "MFiX handles only one type of surface input."
         CALL LOG_ERROR()
      ENDIF
...
! VERIFY THAT THERE IS AT LEAST ONE STL FILE TO READ
      IF(NUMBER_OF_BC_PATCHES==0) THEN
         IF(MyPE == PE_IO) THEN
            WRITE(ERR_MSG,'(A)') 'Warning: No Cartesian grid boundary condition specified.', nl,&
                    'If using cut cells, at least one BC_TYPE must start with CG (for example CG_NSW)', nl,&
...
! Read each geometry_####.stl file
      DO NN = 1, NUMBER_OF_GEOMETRY_FILES
         ...
         STL_GROUP(N_STL_GROUP)%BC_ID    = BC_PATCH(NN)
...
930      WRITE(ERR_MSG,100) 'Done reading file.'
      ...
      IF(SCALE_STL/=ONE) THEN
          WRITE(SCALE_MSG,*) '  Scaling factor:',SCALE_STL, nl
      ELSE
          SCALE_MSG = ''
      ENDIF
      ABSTRANS = dabs(TX_STL)+dabs(TY_STL)+dabs(TZ_STL)
      IF(ABSTRANS>TOL_STL) THEN
          WRITE(TRANSLATE_MSG,*)'  Translation vector (X,Y,Z)=',TX_STL,TY_STL,TZ_STL, nl
      ELSE
          TRANSLATE_MSG = ''
      END IF
      WRITE(ERR_MSG, *) 'BC STL file(s) successfully read.',            nl,&
          'Total number of BC facets read =',N_FACETS + IGNORED_FACETS, nl,&
          'Number of valid facets         =',N_FACETS,                  nl,&
          'Number of ignored facets       =',IGNORED_FACETS,            nl,&
          'BC Geometry range from STL file:',                           nl,&
          TRIM(SCALE_MSG),                                                 &
          TRIM(TRANSLATE_MSG),                                             &
          '  X-range = ', XMIN_STL,XMAX_STL,                            nl,&
          '  Y-range = ', YMIN_STL,YMAX_STL,                            nl,&
          '  Z-range = ', ZMIN_STL,ZMAX_STL
      CALL LOG_INFO()
...
! Close FACETS_IGNORED.stl
      IF(mype.eq.pe_io) then
         ...
         WRITE(ERR_MSG,*) 'The file FACETS_IGNORED.stl was successfully written and is provided for convenience (it is not used by MFiX).'
         CALL LOG_STATUS()
      ENDIF
      ...
      IF(mype.eq.pe_io.and..true.) then
         ...
         write(stl_unit,*)'endsolid vcg'
         close(stl_unit, status = 'keep')
         WRITE(ERR_MSG,*) 'The file FACETS_READ.stl was successfully written and is provided for convenience (it is not used by MFiX).'
         CALL LOG_STATUS()
      ENDIF
      ...
      IF(.FALSE.) THEN
         DO NG=1,N_STL_GROUP
            ...
            WRITE(*,*) 'BC_ID    : ', STL_GROUP(NG)%BC_ID
            WRITE(*,*) 'IS_ID    : ', STL_GROUP(NG)%IS_ID
...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
      ...
      IMPLICIT NONE
      INTEGER :: NN, NG, IGNORED_FACETS, N_IS_FACETS
      LOGICAL :: PRESENT,KEEP_READING,IGNORE_CURRENT_FACET
      DOUBLE PRECISION ::v1x,v1y,v1z
      DOUBLE PRECISION ::v2x,v2y,v2z
      DOUBLE PRECISION ::v3x,v3y,v3z
...
! Add data to STL_GROUP
         ...
         STL_GROUP(N_STL_GROUP)%IS_ID    = IS_PATCH(NN)
         IF(N_STL_GROUP==1) THEN
            STL_GROUP(N_STL_GROUP)%START    = 1
         ELSE
            STL_GROUP(N_STL_GROUP)%START    = STL_GROUP(N_STL_GROUP - 1)%END + 1
...
930      WRITE(ERR_MSG,100) 'Done reading file.'
      ...
      N_FACETS = N_FACETS_DES
      N_IS_FACETS = N_FACETS_DES - N_FACETS_DES0 + IGNORED_FACETS

      XMIN_STL = MINVAL(VERTEX(:,1,N_FACETS_DES0 + 1:N_FACETS))
      XMAX_STL = MAXVAL(VERTEX(:,1,N_FACETS_DES0 + 1:N_FACETS))
      YMIN_STL = MINVAL(VERTEX(:,2,N_FACETS_DES0 + 1:N_FACETS))
      YMAX_STL = MAXVAL(VERTEX(:,2,N_FACETS_DES0 + 1:N_FACETS))
      ZMIN_STL = MINVAL(VERTEX(:,3,N_FACETS_DES0 + 1:N_FACETS))
      ZMAX_STL = MAXVAL(VERTEX(:,3,N_FACETS_DES0 + 1:N_FACETS))
      ...
      IF(N_IS_FACETS>0) THEN
         WRITE(ERR_MSG, *) 'IS STL file(s) successfully read.',               nl,&
             'Total number of IS facets read =',N_IS_FACETS,                  nl,&
             'Number of valid facets         =',N_FACETS_DES - N_FACETS_DES0,                 nl,&
             'Number of ignored facets       =',IGNORED_FACETS,               nl,&
             'IS Geometry range from STL file:',                              nl,&
             TRIM(SCALE_MSG),                                                    &
             TRIM(TRANSLATE_MSG),                                                &
             '  X-range = ', XMIN_STL,XMAX_STL,                               nl,&
             '  Y-range = ', YMIN_STL,YMAX_STL,                               nl,&
...
!      IF(ZMAX_STL>ZLENGTH) ZMAX_STL=ZLENGTH
      ...
      CALL ASSIGN_BC_ID_TO_IS_STL_GROUP
      CALL COPY_INIT_STL_DATA
      CALL BIN_FACETS_TO_DG

      IF(DISCRETE_ELEMENT .OR. MPPIC) CALL FLAG_FACETS_WHERE_EDGE_COLLISION_IS_IGNORED


      IF(.FALSE.) THEN
         IF(MYPE==PE_IO) THEN
            DO NG=1,N_STL_GROUP
               WRITE(*,*) 'STL GROUP   : ', ival(NG)
               WRITE(*,*) '   TYPE     : ', STL_GROUP(NG)%TYPE
               WRITE(*,*) '   FILENAME : ', STL_GROUP(NG)%FILENAME
               WRITE(*,*) '   ID       : ', ival(STL_GROUP(NG)%ID)
               WRITE(*,*) '   BC_ID    : ', ival(STL_GROUP(NG)%BC_ID)
               WRITE(*,*) '   IS_ID    : ', ival(STL_GROUP(NG)%IS_ID)
               WRITE(*,*) '   START    : ', ival(STL_GROUP(NG)%START)
               WRITE(*,*) '   END      : ', ival(STL_GROUP(NG)%END)
            ENDDO
         ENDIF
      ENDIF
...
!vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvC
!                                                                      C
!  Module name: MOVE_IS_STL                                            C
!  Purpose: Moves internal surface (STL facets)                        C
!                                                                      C
!  Author: Jeff Dietiker                              Date: 09-JUN-20  C
!  Reviewer:                                          Date: **-***-**  C
!                                                                      C
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
      SUBROUTINE MOVE_IS_STL
      use discretelement, only: dtsolid, cross
      use run, only: time
      use keyframe_mod, only: omega_conv_fact

      IMPLICIT NONE

      INTEGER :: GROUP_ID, BC_ID, IS_ID
      INTEGER :: N1, N2
      INTEGER :: V, N
      DOUBLE PRECISION :: Theta_deg(3), omega(3), OP(3)
      DOUBLE PRECISION, DIMENSION(3,3) :: Rx, Ry, Rz, R
      DOUBLE PRECISION :: VERTEX_OLD(3,3), VEL(3,3) ! vertex, xyz coordinate
      DOUBLE PRECISION :: CENTER(3) ! Center of STL triangle
      DOUBLE PRECISION :: AVG_VEL(3) ! Average triangle velocity
      LOGICAL :: ANY_IS
      ...
      ANY_IS = .FALSE.
      DO GROUP_ID =1, N_STL_GROUP
         IF(trim(STL_GROUP(GROUP_ID)%TYPE)=='IS_STL') THEN
            ANY_IS = .TRUE.
            IS_ID = STL_GROUP(GROUP_ID)%IS_ID
            N1    = STL_GROUP(GROUP_ID)%START
            N2    = STL_GROUP(GROUP_ID)%END

            IS_WALL_TRANSLATION(IS_ID,:) = IS_WALL_TRANSLATION(IS_ID,:) + IS_WALL_VEL(IS_ID,:)*DTSOLID
...
! The routines building the rotation matrices needs degrees.
            ...
            DO N=N1,N2
               ...
               VERTEX_OLD(:,:) = VERTEX(:,:,N)
               DO V=1,3
                  VERTEX(V,:,N) = MATMUL(R, VERTEX_INIT(V,:,N) - IS_WALL_ROT_CENTER(IS_ID,:)) + IS_WALL_ROT_CENTER(IS_ID,:)
               ENDDO
               ! Translation
               ...
               DO V=1,3
                  VERTEX(V,:,N) = VERTEX(V,:,N) + IS_WALL_TRANSLATION(IS_ID,:)
                  VEL(V,:) = ( VERTEX(V,:,N) - VERTEX_OLD(V,:) ) / DTSOLID
                  AVG_VEL(:) = AVG_VEL(:) + VEL(V,:)
               ENDDO

               AVG_VEL(:) = AVG_VEL(:) / 3.0D0

               STL_NORMAL_VEL(:,N) = DOT_PRODUCT(AVG_VEL(:),NORM_FACE(:,N)) * NORM_FACE(:,N)
               STL_TANGENTIAL_VEL(:,N) = AVG_VEL(:) - STL_NORMAL_VEL(:,N)

            ENDDO

         ELSEIF(trim(STL_GROUP(GROUP_ID)%TYPE)=='BC_STL') THEN
            BC_ID = STL_GROUP(GROUP_ID)%BC_ID
            N1    = STL_GROUP(GROUP_ID)%START
            N2    = STL_GROUP(GROUP_ID)%END
      ...
      IS_BIN_COUNT = IS_BIN_COUNT + 1
      IF(ANY_IS.AND.MOD(IS_BIN_COUNT,IS_REBIN_N)==0) THEN
         IS_BIN_COUNT=0
         CALL BIN_FACETS_TO_DG
      ENDIF

      RETURN
      END SUBROUTINE MOVE_IS_STL
...
!                                                                      C
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
      SUBROUTINE ASSIGN_BC_ID_TO_IS_STL_GROUP
      IMPLICIT NONE
      INTEGER :: G, NN, GROUP_ID, BCV, BC_ID

! Get the first free BC_ID
      DO BCV=1, DIMENSION_BC
         IF(BC_DEFINED(BCV)) BC_ID = BCV + 1
      ENDDO

! Assign an unused BC_ID to the STL group
      DO GROUP_ID = 1, N_STL_GROUP
         IF(TRIM(STL_GROUP(GROUP_ID)%TYPE)=='IS_STL') THEN
            STL_GROUP(GROUP_ID)%BC_ID = BC_ID
      ...
      RETURN
      END SUBROUTINE ASSIGN_BC_ID_TO_IS_STL_GROUP
...
! DETERMINE WHICH BOUNDARY CONDITIONS NEED STL FILE
      WRITE(ERR_MSG,100)'From Detect_stl_along_mfix_box:'// nl // &
...
! Convert the 'CG_###' BC type into a regular '###' BC type
         if(bc_patch(bcv)%belongs_to_box) then
            WRITE(ERR_MSG,110)'CG BC: ',bc_patch(bcv)%bc_id, ' is flush with MFiX box.'// nl // &
                              'It will be converted to regular BC.'// nl // &
                              'Original BC type : ', bc_type(bc_patch(bcv)%bc_id),nl // &
