...
!                                                                      C
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
   SUBROUTINE UPDATE_AND_CLOSE_PVD_FILE(TIMESTAMP)
      ...
      IF (.NOT. BDIST_IO) THEN
         ...
         ELSEIF(VTK_DATA(VTK_REGION) == "P") THEN
            ...
            group = ADJUSTL(VTK_PART_GROUP_STRING(VTK_REGION))
         ELSEIF(VTK_DATA(VTK_REGION) == "F") THEN
            FILENAME = FIND_VTK_BASENAME(0) // ".vtp"
	 ELSEIF(VTK_DATA(VTK_REGION) == "N") THEN
            FILENAME = FIND_VTK_BASENAME(0) // ".vtp"
         ELSE
      ...
      ELSE
         ...
         ELSEIF(VTK_DATA(VTK_REGION) == "P") THEN
            ...
            group = TRIM(VTK_PART_GROUP_STRING(VTK_REGION))
         ELSEIF(VTK_DATA(VTK_REGION) == "F") THEN
            FILENAME = FIND_VTK_BASENAME(0) // ".pvtp"
	 ELSEIF(VTK_DATA(VTK_REGION) == "N") THEN
            FILENAME = FIND_VTK_BASENAME(0) // ".pvtp"
...
! For particle data, group is the list of all solids phases present in the vtp
! file. This metadata can be used by the GUI to pick up info about the phase
! (only useful if only one phase is present). This would avoid having to store
! the phase ID for all particles.
      ...
      WRITE(UNIT_PVD, "(6X,A,E14.7,7A)") &
         '<DataSet timestep="',TIMESTAMP,'" ', & ! simulation time
         'group="',trim(group),'" part="0" ', & ! necessary file data
         'file="', TRIM(FILENAME),'"/>'
