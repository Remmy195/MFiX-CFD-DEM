...
MODULE GET_CUT_CELL_VOLUME_AND_AREAS_MOD
   ...
   USE geometry, ONLY: do_k, no_k, ayz, axz, axy, center_s
   USE geometry, ONLY: ayz_u, axy_u, axz_u, ayz_v, axz_v, axy_v, ayz_w, axz_w, axy_w
...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
  SUBROUTINE GET_CUT_CELL_VOLUME_AND_AREAS(IJK,TYPE_OF_CELL,N_NODES,CONNECT,X_NP,Y_NP,Z_NP)
      ...
      DOUBLE PRECISION, DIMENSION(3) :: APEX, C_E, C_N, C_T, C_W, C_S, C_B, C_C
      DOUBLE PRECISION :: V_E, V_N, V_T, V_W, V_S, V_B, V_C
...
! Now need to compute west, south and bottom centroids to store the cell center coordinates.
      CALL GET_POLYGON_AREA_AND_CENTROID(N_WEST_FACE_NODES,COORD_WEST_FACE_NODES,AREA_WEST,CENTROID_WEST,IERROR)

      CALL GET_POLYGON_AREA_AND_CENTROID(N_SOUTH_FACE_NODES,COORD_SOUTH_FACE_NODES,AREA_SOUTH,CENTROID_SOUTH,IERROR)

      IF(DO_K) CALL GET_POLYGON_AREA_AND_CENTROID(N_BOTTOM_FACE_NODES,COORD_BOTTOM_FACE_NODES,AREA_BOTTOM,&
      CENTROID_BOTTOM,IERROR)
      SELECT CASE (TYPE_OF_CELL)
         CASE('SCALAR')
...
! Storing cell center coordinates
               V_E = AREA_EAST   * (X_NODE(8) - X_MEAN)
               V_N = AREA_NORTH  * (Y_NODE(8) - Y_MEAN)
               V_T = AREA_TOP    * (Z_NODE(8) - Z_MEAN)
               V_W = AREA_WEST   * (X_MEAN - X_NODE(1))
               V_S = AREA_SOUTH  * (Y_MEAN - Y_NODE(1))
               V_B = AREA_BOTTOM * (Z_MEAN - Z_NODE(1))
               V_C = CUT_AREA    * DEL_H
               ...
               C_E = CENTROID_EAST     + ONE/3.0D0 * (APEX - CENTROID_EAST)
               C_N = CENTROID_NORTH    + ONE/3.0D0 * (APEX - CENTROID_NORTH)
               C_T = CENTROID_TOP      + ONE/3.0D0 * (APEX - CENTROID_TOP)
               C_W = CENTROID_WEST     + ONE/3.0D0 * (APEX - CENTROID_WEST)
               C_S = CENTROID_SOUTH    + ONE/3.0D0 * (APEX - CENTROID_SOUTH)
               C_B = CENTROID_BOTTOM   + ONE/3.0D0 * (APEX - CENTROID_BOTTOM)
               C_C = CENTROID_CUT      + ONE/3.0D0 * (APEX - CENTROID_CUT)

               CENTER_S(IJK,:)  =  (V_E*C_E + V_N*C_N + V_T*C_T + &
                                    V_W*C_W + V_S*C_S + V_B*C_B + &
                                    V_C*C_C) &
                                 / (V_E + V_N + V_T + &
                                    V_W + V_S + V_B + V_C)
...
! neighbor cell where the snapping occurred.
                     IJK_BC_ID_LOOKUP(:)=-99 ! Initialize, will trigger segfault
                                             ! if we try to access
                     N_LOOKUP = 0
