...
! conductivity must be additive. The nonlinear terms (eps^2) are
...
! communications with Ahmadi reveals a different approach
...
!                    (qxP_s + SMALL_NUMBER)*(EP_S(IJK,M)/SUM_EPS_CP)
!                 LAMBDA_s_p(IJK) = MIN(LAMBDA_s(IJK, M), MAX_MU_s)
!              ENDIF
!-----------------------------------------------------------------------
...
! Linearize PC around small deviation from max. packing: eg=ep_star+delta
                  Pc = (shearRate_gamma**2)*((ONE-ep_star_array(ijk)-EPS_f_min)**2)*(D_p(IJK,M)**2)*ro_s(IJK,M) / &
                                  ( delta**2) + a_ps * ((ONE-EP_G(IJK))-(ONE-ep_star_array(ijk))+delta)**b_ps
...
!  - second invariant of the deviatoric stress tensor: i2_devD_s       C
!    (global)                                                          C
!                                                                      C
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
