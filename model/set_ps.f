...
! Calculate the volume of the Point source cells
!---------------------------------------------------------------------//
...
! volume.
 ...
 2001 format(3x,i3,3x,es15.8)
  ...
  500 continue
      write(err_msg,"('Point source setup error: ',A)") trim(eMsg)
      call log_error()

  501 continue
      write(err_msg,"('Point source setup error: ',I3,2x,A)") myPE, trim(eMsg)
      call log_error()
