...
MODULE SET_BC0_MOD
   ...
   use bc, only: bc_massflow_g, bc_volflow_g, bc_massflow_s, bc_volflow_s
      ...
      DO L = 1, DIMENSION_BC
         IF (BC_DEFINED(L)) THEN
            ...
            CASE (P_OUTFLOW)
               ...
               CALL SET_BC0_OUTFLOWB(L,.FALSE.)
               CALL SET_BC0_EP(L,.FALSE.)

            CASE (MASS_OUTFLOW)
               CALL SET_BC0_OUTFLOWA(L)
               CALL SET_BC0_OUTFLOWB(L,.TRUE.)
               CALL SET_BC0_EP(L,.TRUE.)
...
!  for a PO boundary.                                                  C
!                                                                      C
!  Otherwise one has to wait for:                                      C
...
!--------------------------------------------------------------------//
      ...
      DO I = BC_I_W(BCV), BC_I_E(BCV)
         ...
         P_STAR(IJK) = ZERO
         IF (BC_P_G(BCV) /= UNDEFINED) P_G(IJK) = SCALE_PRESSURE(BC_P_G(BCV))
...
! bc_rop_s will not be set by a user in solids MO, however, the check
! routine will set it for convenience if bc_ep_s was set (and needed)
         DO M = 1, SMAX
...
!  CELL except for pressure since this field variable may not be set   C
!  until after the call to set_fluidbed_p.                             C
!                                                                      C
...
!                                                                      C
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^!
      SUBROUTINE set_bc0_outflowB(BCV,checkMO)
...
! index of boundary
      INTEGER, INTENT(IN) :: BCV
! mass outflow BC allow for user specification; don't override
      LOGICAL, INTENT(IN) :: checkMO
! Local variables
!--------------------------------------------------------------------//
...
! local indices
      INTEGER :: M
! flags whether mo has non-zero massflow
      LOGICAL :: BC_FLAGG, BC_FLAGS(DIM_M)
!--------------------------------------------------------------------//
! Only in MO BC where BC values are usr provided and may be needed for
! IC should we not seek to override any usr BC setting in outflow
! boundaries

      BC_FLAGG = .FALSE.
      BC_FLAGS(1:DIM_M) = .FALSE.
      IF(CHECKMO) THEN
...
! This is only 'false' in the case of a MO BC and the massflow needs to
! be converted to velocity (via density).  Here the BC value is used
! ONLY as an IC for this.
         IF (.NOT.(BC_FLAGG .AND. BC_T_G(BCV) /= UNDEFINED)) &
            T_G(IJK) = T_G(FIJK)
...
! should this only loop over tfm phases (smax?)?
         DO M = 1, MMAX
            IF (.NOT.(BC_FLAGS(M) .AND. BC_T_S(BCV,M) /= UNDEFINED)) &
...
!                                                                      C
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
      SUBROUTINE SET_BC0_EP(BCV,checkMO)
...
!--------------------------------------------------------------------//
      ...
      IF (checkMO) THEN
         IF((BC_MASSFLOW_G(BCV) /= UNDEFINED .AND. &
            BC_MASSFLOW_G(BCV) /= ZERO) .OR.&
            (BC_VOLFLOW_G(BCV) /= UNDEFINED .AND. &
...
! we should not allow these to be set except in MO for IC
         IF (.NOT.(BC_FLAGG .AND. BC_EP_G(BCV) /= UNDEFINED)) &
            EP_G(IJK) = EP_G(FIJK)

         DO M = 1, SMAX
            IF (.NOT.(BC_FLAGS(M) .AND. BC_EP_S(BCV,M) /= UNDEFINED)) &
...
!--------------------------------------------------------------------//
      ...
      DO I = BC_I_W(BCV), BC_I_E(BCV)
         ...
         DO M = 1, SMAX
! value of bc_rop_s may not be valid yet
           ROP_S(IJK,M) = BC_ROP_S(BCV,M)
           T_S(IJK,M) = BC_T_S(BCV,M)
           THETA_M(IJK,M) = BC_THETA_M(BCV,M)
           IF (NMAX(M) > 0) &
