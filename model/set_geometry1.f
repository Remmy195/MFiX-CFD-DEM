...
!                      Indices
      ...
      DOUBLE PRECISION ::  XG_C(0:DIMENSION_I)
      DOUBLE PRECISION ::  YG_C(0:DIMENSION_J)
      DOUBLE PRECISION ::  ZG_C(0:DIMENSION_K)
!
!-----------------------------------------------
!
!                      Indices
...
! CALCULATE THE X_COORDINATE OF EACH CELL CENTER
      IF (DO_I) THEN
         XG_C(0:1) = X_MIN
         DO I = IMIN1, IMAX2
...
! CALCULATE THE Y_COORDINATE OF EACH CELL CENTER
      IF (DO_J) THEN
         YG_C(0:1) = Y_MIN
         DO J = JMIN1, JMAX2
            YG_C(J) = YG_C(J-1) + DY(J) ! North face location
         ENDDO
         DO J = JMIN1, JMAX2
...
! CALCULATE THE Z_COORDINATE OF EACH CELL CENTER
      IF (DO_K) THEN
         ZG_C(0:1) = Z_MIN
         DO K = KMIN1, KMAX2
            ZG_C(K) = ZG_C(K-1) + DZ(K) ! Top face location
         ENDDO
         DO K = KMIN1, KMAX2
...
! Scalar cell center coordinates
         CENTER_S(IJK,1) = XG_C(I)
         CENTER_S(IJK,2) = YG_C(J)
         CENTER_S(IJK,3) = ZG_C(K)
