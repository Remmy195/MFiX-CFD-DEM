...
!  <description>
!    Name used to create output files. Limited to 60 characters.
!    The name should generate legal file names after appending extensions.
...
!  <valid value="cgs" note="All input and output in CGS units (g, cm, s, cal). DEPRECATED."/>
      UNITS = "SI"
!</keyword>
...
!  <valid value="QKQY" note=" Z. Qi, S.B. Kuang, T.S. Qiu, A.B. Yu,
! Lattice Boltzmann investigation on fluid flows through packed beds: Interaction between fluid rheology and bed properties,
! Powder Technology 369 (2020) 248-260 and S. Benyahia, Simulation of heavy particles fluidized with non-Newtonian fluids,
! Powder Technology  433 (2024) 119261"/>
...
!  <valid value="SQP_DIFELICE_GANSER" note= "Xi Gao, Jia Yu, Liqiang Lu, Cheng Li and William A Rogers,
! Development and validation of SuperDEM-CFD coupled model for simulating non-spherical particles
! hydrodynamics in fluidized beds, Chemical Engineering Journal, 2021,420: 127654. and
! G.H. Ganser, A rational approach to drag
!  prediction of spherical and nonspherical particles, Powder Technol. 77 (1993) 143-152.
!  This model requires specification of a sphericity and a reference length
! (typically the bed diameter)"/>

!  <valid value="SQP_DIFELICE_HOLZER_SOMMERFELD" note= "Xi Gao, Jia Yu, Liqiang Lu, Cheng Li and William A Rogers,
! Development and validation of SuperDEM-CFD coupled model for simulating non-spherical particles
! hydrodynamics in fluidized beds, Chemical Engineering Journal, 2021,420: 127654. and
! A. Hölzer, M. Sommerfeld, New simple correlation formula for the drag coefficient of nonspherical particles,
...
!  </description>
!  <valid value=".FALSE." note="Use Model A. See Bouillard, J.X.,
...
!<keyword dtype="REAL" category="Physical parameters" required="false" locked="false">
!  <description>
!    X-axial component of gravitational acceleration vector.
!  </description>
      GRAVITY_X = UNDEFINED
!</keyword>
...
!<keyword dtype="REAL" category="Physical parameters" required="false" locked="false">
!  <description>
!    Y-axial component of gravitational acceleration vector.
!  </description>
      GRAVITY_Y = UNDEFINED
!</keyword>
...
!<keyword dtype="REAL" category="Physical parameters" required="false" locked="false">
!  <description>
!    Z-axial component of gravitational acceleration vector.
!  </description>
      GRAVITY_Z = UNDEFINED
!</keyword>
...
! Note, this keyword is not included in any namelist, so don't add it to the docs
!<disabled_keyword dtype="REAL" category="Numerical parameters" required="false" locked="false">
!  <description>
...
!  <valid value="0" note="First-order upwinding (default)."/>
...
!    determined from the boundary values.  By default it is 100 times
!    the maximum inlet velocity.  This can be scaled value by adjusting
!    this additional multiplier [1.0d0].
!  </description>
...
!<keyword dtype="LOGICAL" category="Numerical parameters" required="false" locked="true">
!  <description>
!    Initialize random number generator with a fixed seed, to ensure repeatable results.
!  </description>
      REPEATABLE = .TRUE.
!</keyword>
...
!    It is strongly encouraged to keep the default value of 50.0.</description>
!  <range min="0.0" max="+Inf" />
      TMIN = 50.0D0
!</keyword>
...
!<keyword dtype="LOGICAL" category="Numerical parameters" required="false" locked="false">
!  <description>Terminate simulation and print stack trace on zero division errors.</description>
      FPE_TRAP_ZERO = .True.

!</keyword>

!<keyword dtype="LOGICAL" category="Numerical parameters" required="false" locked="false">
!  <description>Terminate simulation and print stack trace on numerical overflow.</description>
      FPE_TRAP_OVERFLOW = .True.
!</keyword>

!<keyword dtype="LOGICAL" category="Numerical parameters" required="false" locked="false">
!  <description>Terminate simulation and print stack trace on math domain errors
!    (square root or log of negative value, etc).</description>
      FPE_TRAP_INVALID = .True.
!</keyword>
...
!<keyword dtype="CHARACTER" category="Geometry and discretization" required="false" locked="true">
...
!<keyword dtype="CHARACTER" category="Gas phase" required="false" locked="false">
!  <description>
!    Gas viscosity model (default is Sutherland's law).
!  </description>
      MU_G_MODEL= 'SUTHERLAND'
!  <valid value='CONSTANT' note='Constant viscosity'/>
!  <valid value='SUTHERLAND' note='Sutherland&apos;s law'/>
!  <valid value='HERSCHEL-BULKLEY' note='Herschel-Bulkley (non-Newtonian) model'/>
!  <valid value='LENNARD_JONES' note='Lennard-Jones model. Bird, R., Stewart, W. and Lightfoot, E. (2002)
!  Transport Phenomena.2nd Edition, John Wiley and Sons, New York. Eq.(1.4-14) on Page 26 for the calculation
!  of viscosity. Eq.(E.2-1) on Page 866 for the collision integral.'/>
!  <valid value='CANTERA_POLY' note='Polynomial fit from species transport data using cantera.
!  https://cantera.org/dev/cxx/d8/d58/classCantera_1_1GasTransport.html#a267d20cdea7486fe84e722ee82d84b20'/>
!  <valid value='USR' note='User-defined function'/>
!</keyword>
...
!  </description>
      KG_MODEL= 'AIR'
!  <valid value='CONSTANT' note='Constant thermal conductivity.  K_g0 must be specified.'/>
!  <valid value='AIR' note='Temperature-dependent model for air'/>
!  <valid value='LENNARD_JONES' note='Lennard-Jones model. Bird, R., Stewart, W. and Lightfoot, E. (2002)
!  Transport Phenomena.2nd Edition, John Wiley and Sons, New York. Eq.(9.3-13) on Page 275 for the calculation
!  of thermal conductivity. Eq.(E.2-1) on Page 866 for the collision integral.'/>
!  <valid value='CANTERA_POLY' note='Polynomial fit from species transport data using cantera.
!  https://cantera.org/dev/cxx/d8/d58/classCantera_1_1GasTransport.html#a267d20cdea7486fe84e722ee82d84b20'/>
!  <valid value='USR' note='User-defined function'/>
!</keyword>
...
!<keyword dtype="LOGICAL" category="Gas phase" required="false" locked="true">
!  <description>Solve the multicomponent diffusion model.</description>
!  <valid value=".TRUE." note="Include multicomponent diffusion model in species equation."/>
!  <valid value=".FALSE." note="Do not include multicomponent diffusion model."/>
      Multi_Component_Diffusion = .FALSE.
!</keyword>

! Note, the -- used below is handled specially in namelistparser.py to allow a list inside
!  valid values, since XML converts newlines to spaces in attributes
!
!<keyword dtype="LOGICAL" category="Gas phase" required="false" locked="true">
!  <description>Compute diffusion coefficients from kinetic theory.</description>
!  <valid value=".FALSE." note="Use constant diffusion coefficients.
!      Diffusion data can be obtained from:
!        -- Taylor, R & Krishna, R,  Multicomponent Mass Transfer, Wiley, 1993,  p. 134."/>
!  <valid value=".TRUE." note="Compute diffusion coefficients from kinetic theory.
!     Parameters in Lennard-Jones intermolecular potential model can be obtained from:
!        -- Table E.1 in Transport Phenomena, by Bird-Stewart-Lightfoot, Wiley, 2002, p. 864.
!        -- Appendix E: Data. In Liquid-State Physical Chemistry https://doi.org/10.1002/9783527676750.app5
!        -- Appendix C in The Properties of Gases and Liquids by Reid-Prausnitz-Sherwood, McGraw-Hill, 1977, p. 678."/>
...
!<keyword dtype="LOGICAL" category="Gas phase" required="false" locked="true">
!  <description>Include thermal diffusion in multicomponent diffusion model.</description>
!  <valid value=".TRUE." note="Compute thermal diffusion."/>
!  <valid value=".FALSE." note="Ignore effects of thermal diffusion."/>
      dif_thermal = .FALSE.
!</keyword>
...
!  </description>
!  <arg index="1" id="Species" min="1" max="DIM_N_G"/>
      LJeps(:DIM_N_G) = UNDEFINED
!</keyword>

!<keyword dtype="INTEGER" category="Gas phase" required="false" locked="false">
!  <description>
!    Geometrical configuration of molecule.
!  </description>
!  <valid value='0' note='Monatomic'/>
!  <valid value='1' note='Linear'/>
!  <valid value='2' note='Nonlinear'/>
!  <arg index="1" id="Species" min="1" max="DIM_N_G"/>
      config_transport(:DIM_N_G) = UNDEFINED_I
!</keyword>
...
!  <arg index="2" id="Species" min="1" max="DIM_N_G"/>
      Dabg(:DIM_N_G, :DIM_N_G) = UNDEFINED
!</keyword>
...
!  </description>
!  <arg index="1" id="Species" min="1" max="DIM_N_G"/>
!  <arg index="2" id="Index" min="1" max="5"/>
      poly_mu_g(:DIM_N_G, :5) = UNDEFINED
!</keyword>
...
!<keyword dtype="CHARACTER" category="Solids phase" required="false" locked="true">
!  <description>
...
!  <valid value='CGP' note='Coarse-grained particle DEM'/>
!  <valid value='SQP' note='Superquadric particle DEM'/>
!  <valid value='GSP' note='Glued sphere particle DEM'/>
...
!  <description>
!    Solids phase viscous stress model [Algebraic].
!  </description>
...
!<keyword dtype="REAL" category="Initial condition" required="false" locked="true">
!  <description>Initial x-axial component of gas velocity in the IC region.</description>
!  <arg index="1" id="IC" min="1" max="DIMENSION_IC"/>
         IC_U_G(LC) = UNDEFINED
!</keyword>

!<keyword dtype="REAL" category="Initial condition" required="false" locked="true">
!  <description>Initial x-axial component of solids-phase velocity in the IC region.</description>
!  <arg index="1" id="IC" min="1" max="DIMENSION_IC"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
...
!<keyword dtype="REAL" category="Initial condition" required="false" locked="true">
!  <description>Initial y-axial component of gas velocity in the IC region.</description>
!  <arg index="1" id="IC" min="1" max="DIMENSION_IC"/>
         IC_V_G(LC) = UNDEFINED
!</keyword>

!<keyword dtype="REAL" category="Initial condition" required="false" locked="true">
!  <description>Initial y-axial component of solids-phase velocity in the IC region.</description>
!  <arg index="1" id="IC" min="1" max="DIMENSION_IC"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
...
!<keyword dtype="REAL" category="Initial condition" required="false" locked="true">
!  <description>Initial z-axial component of gas velocity in the IC region.</description>
!  <arg index="1" id="IC" min="1" max="DIMENSION_IC"/>
         IC_W_G(LC) = UNDEFINED
!</keyword>

!<keyword dtype="REAL" category="Initial condition" required="false" locked="true">
!  <description>Initial z-axial component of solids-phase velocity in the IC region.</description>
!  <arg index="1" id="IC" min="1" max="DIMENSION_IC"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
...
!  <description>Flag to abort if unsuccessful seeding occurs. </description>
!  <arg index="1" id="IC" min="1" max="DIMENSION_IC"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
          IC_DES_ABORT_ON_FAILED_SEEDING(LC,: DIM_M) = .TRUE.
!</keyword>

! Note, this keyword is not included in any namelist, so don't add it to the docs
!<disabled_keyword dtype="LOGICAL" category="Initial condition" required="false" locked="true">
!  <description>Check for particle overlap with STL geometry
...
!<keyword dtype="LOGICAL" category="Initial condition" required="false" locked="true">
!  <description>Initialize SQP with random orientation (quaternion) in the IC region.</description>
!  <arg index="1" id="IC" min="1" max="DIMENSION_IC"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
         IC_SQP_RANDOM_Q(LC,:DIM_M) = .FALSE.
!</keyword>

!<keyword dtype="LOGICAL" category="Initial condition" required="false" locked="true">
!  <description>Initialize GSP with random orientation (quaternion) in the IC region.</description>
!  <arg index="1" id="IC" min="1" max="DIMENSION_IC"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
         IC_GSP_RANDOM_Q(LC,:DIM_M) = .FALSE.
!</keyword>

!<keyword dtype="LOGICAL" category="Initial condition" required="false" locked="true">
!  <description>Initial conditions take precedence over data in particle_inpupt.dat file for this region and solids phase.</description>
!  <arg index="1" id="IC" min="1" max="DIMENSION_IC"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
         IC_OVERRIDE_PART_INPUT(LC,:DIM_M) = .FALSE.
!</keyword>
      ENDDO
...
!  <description>Apply default wall Boundary conditions along the MFIX box
!   (no-slip, adiabatic walls).
!  </description>
         BC_DEFAULT_WALLS = .TRUE.
!</keyword>
...
!    d(T_g)/dn + Hw (T_g - T_ref) = C, where n is the fluid-to-wall normal.
!  </description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
...
!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>
!    Specified gas phase wall temperature, when using constant temperature wall,
!    or free stream gas reference temperature T_ref, in diffusion boundary condition:
!    d(T_g)/dn + Hw (T_g - T_ref) = C, where n is the fluid-to-wall normal.
!  </description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
...
!    d(T_s)/dn + Hw (T_s - T_ref) = C, where n is the fluid-to-wall normal.
!  </description>
...
!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>
!    Specified constant solids phase wall temperature, when using constant temperature wall,
!    or free stream solids reference temperature T_ref, in diffusion boundary condition:
!    d(T_s)/dn + Hw (T_s - T_ref) = C, where n is the fluid-to-wall normal.
!  </description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
...
!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>X-axial component of gas velocity at the BC plane.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
         BC_U_G(LC) = UNDEFINED
!</keyword>

!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>X-axial component of solids-phase velocity at the BC plane.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
...
!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Y-axial component of gas velocity at the BC plane.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
         BC_V_G(LC) = UNDEFINED
!</keyword>

!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Y-axial component of solids-phase velocity at the BC plane.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
...
!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Z-axial component of gas velocity at the BC plane.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
         BC_W_G(LC) = UNDEFINED
!</keyword>

!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Z-axial component of solids-phase velocity at the BC plane.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
...
!    a run, this value and BC_Jet_g0 should be specified such that the
...
!<keyword dtype="LOGICAL" category="Boundary condition" required="false" locked="true">
!  <description>Flag to randomize SQP orientation (quaternion) along mass inlet BC.
!  </description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_IC"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
         BC_SQP_RANDOM_Q(LC, :DIM_M) = .FALSE.
!</keyword>

!<keyword dtype="LOGICAL" category="Boundary condition" required="false" locked="true">
!  <description>Flag to randomize GSP orientation (quaternion) along mass inlet BC.
!  </description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_IC"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
         BC_GSP_RANDOM_Q(LC, :DIM_M) = .FALSE.
!</keyword>

! Gas pressure
!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Starting time for ramped gas pressure at the BC plane.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
         BC_P_g_ramp_t0(LC) = ZERO
!</keyword>

!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Ending time for ramped gas pressure at the BC plane.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
         BC_P_g_ramp_t1(LC) = UNDEFINED
!</keyword>

!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Starting value for ramped gas pressure at the BC plane.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
         BC_P_g_ramp_v0(LC) = UNDEFINED
!</keyword>

!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Ending value for ramped gas pressure at the BC plane.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
         BC_P_g_ramp_v1(LC) = UNDEFINED
!</keyword>
! Gas phase temperature

!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Starting time for ramped gas temperature at the BC plane.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
         BC_T_g_ramp_t0(LC) = ZERO
!</keyword>

!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Ending time for ramped gas temperature at the BC plane.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
         BC_T_g_ramp_t1(LC) = UNDEFINED
!</keyword>

!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Starting value for ramped gas temperature at the BC plane.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
         BC_T_g_ramp_v0(LC) = UNDEFINED
!</keyword>

!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Ending value for ramped gas temperature at the BC plane.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
         BC_T_g_ramp_v1(LC) = UNDEFINED
!</keyword>

!
! Solids phase temperature
!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Starting time for ramped solids phase temperature at the BC plane.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
      BC_T_s_ramp_t0(LC,:DIM_M) = ZERO
!</keyword>

!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Ending time for ramped solids phase temperature at the BC plane.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
      BC_T_s_ramp_t1(LC,:DIM_M) = UNDEFINED
!</keyword>

!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Starting value for ramped solids phase temperature at the BC plane.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
      BC_T_s_ramp_v0(LC,:DIM_M) = UNDEFINED
!</keyword>

!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Ending value for ramped solids phase temperature at the BC plane.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
      BC_T_s_ramp_v1(LC,:DIM_M) = UNDEFINED
!</keyword>
!
! Solids phase granular temperature
!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Starting time for ramped solids phase granular temperature at the BC plane.</description>
!  <range min="0" max=""/>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
      BC_Theta_m_ramp_t0(LC,:DIM_M) = ZERO
!</keyword>

!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Ending time for ramped solids phase granular temperature at the BC plane.</description>
!  <range min="0" max=""/>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
      BC_Theta_m_ramp_t1(LC,:DIM_M) = UNDEFINED
!</keyword>

!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Starting value for ramped solids phase granular temperature at the BC plane.</description>
!  <range min="0" max=""/>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
      BC_Theta_m_ramp_v0(LC,:DIM_M) = UNDEFINED
!</keyword>

!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Ending value for ramped solids phase granular temperature at the BC plane.</description>
!  <range min="0" max=""/>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
      BC_Theta_m_ramp_v1(LC,:DIM_M) = UNDEFINED
!</keyword>

!
! x-component of gas velocity
!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Starting time for ramped x-axial component of gas velocity at the BC plane.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
      BC_U_g_ramp_t0(LC) = ZERO
!</keyword>

!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Ending time for ramped x-axial component of gas velocity at the BC plane.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
      BC_U_g_ramp_t1(LC) = UNDEFINED
!</keyword>

!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Starting value for ramped x-axial component of gas velocity at the BC plane.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
      BC_U_g_ramp_v0(LC) = UNDEFINED
!</keyword>

!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Ending value for ramped x-axial component of gas velocity at the BC plane.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
      BC_U_g_ramp_v1(LC) = UNDEFINED
!</keyword>

!
! x-component of solids phase velocity
!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Starting time for ramped x-axial component of solids-phase velocity at the BC plane.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
      BC_U_s_ramp_t0(LC,:DIM_M) = ZERO
!</keyword>

!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Ending time for ramped x-axial component of solids-phase velocity at the BC plane.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
      BC_U_s_ramp_t1(LC,:DIM_M) = UNDEFINED
!</keyword>

!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Starting value for ramped x-axial component of solids-phase velocity at the BC plane.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
      BC_U_s_ramp_v0(LC,:DIM_M) = UNDEFINED
!</keyword>

!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Ending value for ramped x-axial component of solids-phase velocity at the BC plane.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
      BC_U_s_ramp_v1(LC,:DIM_M) = UNDEFINED
!</keyword>
...
!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Starting time for ramped y-axial component of gas velocity at the BC plane.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
      BC_V_g_ramp_t0(LC) = ZERO
!</keyword>

!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Ending time for ramped y-axial component of gas velocity at the BC plane.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
      BC_V_g_ramp_t1(LC) = UNDEFINED
!</keyword>

!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Starting value for ramped y-axial component of gas velocity at the BC plane.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
      BC_V_g_ramp_v0(LC) = UNDEFINED
!</keyword>

!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Ending value for ramped y-axial component of gas velocity at the BC plane.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
      BC_V_g_ramp_v1(LC) = UNDEFINED
!</keyword>

!
! y-component of solids phase velocity
!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Starting time for ramped y-axial component of solids-phase velocity at the BC plane.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
      BC_V_s_ramp_t0(LC,:DIM_M) = ZERO
!</keyword>

!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Ending time for ramped y-axial component of solids-phase velocity at the BC plane.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
      BC_V_s_ramp_t1(LC,:DIM_M) = UNDEFINED
!</keyword>

!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Starting value for ramped y-axial component of solids-phase velocity at the BC plane.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
      BC_V_s_ramp_v0(LC,:DIM_M) = UNDEFINED
!</keyword>

!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Ending value for ramped y-axial component of solids-phase velocity at the BC plane.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
      BC_V_s_ramp_v1(LC,:DIM_M) = UNDEFINED
!</keyword>
...
!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Starting time for ramped z-axial component of gas velocity at the BC plane.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
      BC_W_g_ramp_t0(LC) = ZERO
!</keyword>

!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Ending time for ramped z-axial component of gas velocity at the BC plane.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
      BC_W_g_ramp_t1(LC) = UNDEFINED
!</keyword>

!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Starting value for ramped z-axial component of gas velocity at the BC plane.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
      BC_W_g_ramp_v0(LC) = UNDEFINED
!</keyword>

!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Ending value for ramped z-axial component of gas velocity at the BC plane.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
      BC_W_g_ramp_v1(LC) = UNDEFINED
!</keyword>

!
! z-component of solids phase velocity
!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Starting time for ramped z-axial component of solids-phase velocity at the BC plane.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
      BC_W_s_ramp_t0(LC,:DIM_M) = ZERO
!</keyword>

!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Ending time for ramped z-axial component of solids-phase velocity at the BC plane.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
      BC_W_s_ramp_t1(LC,:DIM_M) = UNDEFINED
!</keyword>

!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Starting value for ramped z-axial component of solids-phase velocity at the BC plane.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
      BC_W_s_ramp_v0(LC,:DIM_M) = UNDEFINED
!</keyword>

!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Ending value for ramped z-axial component of solids-phase velocity at the BC plane.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
      BC_w_s_ramp_v1(LC,:DIM_M) = UNDEFINED
!</keyword>


! Gas volumetric flow rate
!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Starting time for ramped volumetric flow rate through the boundary.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
      BC_VOLFLOW_g_ramp_t0(LC) = ZERO
!</keyword>

!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Ending time for ramped volumetric flow rate through the boundary.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
      BC_VOLFLOW_g_ramp_t1(LC) = UNDEFINED
!</keyword>

!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Starting value for ramped volumetric flow rate through the boundary.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
      BC_VOLFLOW_g_ramp_v0(LC) = UNDEFINED
!</keyword>

!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Ending value for ramped volumetric flow rate through the boundary.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
      BC_VOLFLOW_g_ramp_v1(LC) = UNDEFINED
!</keyword>

!
! Solids volumetric flow rate
!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Starting time for ramped solids volumetric flow rate through the boundary.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
      BC_VOLFLOW_s_ramp_t0(LC,:DIM_M) = ZERO
!</keyword>

!  <description>Ending time for solids volumetric flow rate through the boundary.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
      BC_VOLFLOW_s_ramp_t1(LC,:DIM_M) = UNDEFINED
!</keyword>

!  <description>Starting value for solids volumetric flow rate through the boundary.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
      BC_VOLFLOW_s_ramp_v0(LC,:DIM_M) = UNDEFINED
!</keyword>

!  <description>Starting value for solids volumetric flow rate through the boundary.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
      BC_VOLFLOW_s_ramp_v1(LC,:DIM_M) = UNDEFINED
!</keyword>

!
! Gas mass flow rate
!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Starting time for ramped gas mass flow rate through the boundary.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
      BC_MASSFLOW_g_ramp_t0(LC) = ZERO
!</keyword>

!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Ending time for ramped gas mass flow rate through the boundary.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
      BC_MASSFLOW_g_ramp_t1(LC) = UNDEFINED
!</keyword>

!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Starting value for ramped gas mass flow rate through the boundary.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
      BC_MASSFLOW_g_ramp_v0(LC) = UNDEFINED
!</keyword>

!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Starting value for ramped gas mass flow rate through the boundary.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
      BC_MASSFLOW_g_ramp_v1(LC) = UNDEFINED
!</keyword>
!
! Solids mass flow rate
!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Starting time for ramped solids mass flow rate through the boundary.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
      BC_MASSFLOW_s_ramp_t0(LC,:DIM_M) = ZERO
!</keyword>

!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Ending time for ramped solids mass flow rate through the boundary.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
      BC_MASSFLOW_s_ramp_t1(LC,:DIM_M) = UNDEFINED
!</keyword>

!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Starting value for ramped solids mass flow rate through the boundary.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
      BC_MASSFLOW_s_ramp_v0(LC,:DIM_M) = UNDEFINED
!</keyword>

!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Ending value for ramped solids mass flow rate through the boundary.</description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
      BC_MASSFLOW_s_ramp_v1(LC,:DIM_M) = UNDEFINED
!</keyword>


!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Moving wall translational velocity. This is only meant to be used
!  for granular DEM, CGP, SQP or GSP models to move walls in the tangential direction.
!  Example: a conveyor belt.
!  The first index is the BC ID, the second index is the component (1=x, 2=y, 3=z)
!  </description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
...
!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Moving wall angular velocity. This is only meant to be used
!  for granular DEM, CGP, SQP or GSP models rotate walls.
!  The rotation occurs around the center point defined with BC_WALL_ROT_CENTER.
!  Example: a smooth rotating drum.
!  The first index is the BC ID, the second index is the component (1=x, 2=y, 3=z)
!  </description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
!  <arg index="2" id="Component" min="1" max="3"/>
      BC_WALL_OMEGA(LC,:) = ZERO
!</keyword>


!<keyword dtype="REAL" category="Boundary condition" required="false" locked="false">
!  <description>Moving wall center of rotation. This is only meant to be used
!  for granular DEM, CGP, SQP or GSP models to rotate walls.
!  This keywords is used in conjunction with BC_WALL_OMEGA.
!  Example: a smooth rotating drum.
!  The first index is the BC ID, the second index is the component (1=x, 2=y, 3=z)
!  </description>
!  <arg index="1" id="BC" min="1" max="DIMENSION_BC"/>
!  <arg index="2" id="Component" min="1" max="3"/>
      BC_WALL_ROT_CENTER(LC,:) = ZERO
!</keyword>
...
!      Solids velocity through the surface is set to zero or to a
!      user-specified fixed value (i.e., solids momentum equation
!      for this direction is not solved)." />
...
!      Solids velocity in the X direction is set to zero or to a
!      user-specified fixed value (i.e., solids momentum equation
!      for this direction is not solved)." />
...
!      Solids velocity in the Y direction is set to zero or to a
!      user-specified fixed value (i.e., solids momentum equation
!      for this direction is not solved)." />
...
!      Solids velocity in the Z direction is set to zero or to a
!      user-specified fixed value (i.e., solids momentum equation
!      for this direction is not solved)." />
!  <valid value="STL"
...
!<keyword dtype="REAL" category="Internal surface" required="false" locked="false">
!  <description>Internal surface translational velocity. This is only meant to be used
!  for granular DEM, CGP, SQP or GSP models to move internal surfaces defined by STL files.
!  Example: a valve moving up and down.
!  The first index is the IS ID, the second index is the component (1=x, 2=y, 3=z)
!  </description>
!  <arg index="1" id="IS" min="1" max="DIMENSION_IS"/>
!  <arg index="2" id="Component" min="1" max="3"/>
      IS_WALL_VEL(LC,:) = ZERO
...
!<keyword dtype="REAL" category="Internal surface" required="false" locked="false">
!  <description>Internal surface angular velocity. This is only meant to be used
!  for granular DEM, CGP, SQP or GSP models to move internal surfaces defined by STL files.
!  The rotation occurs around the center point defined with IS_WALL_ROT_CENTER.
!  Example: a smooth rotating drum.
!  The first index is the IS ID, the second index is the component (1=x, 2=y, 3=z)
!  </description>
!  <arg index="1" id="IS" min="1" max="DIMENSION_IS"/>
!  <arg index="2" id="Component" min="1" max="3"/>
      IS_WALL_OMEGA(LC,:) = ZERO
!</keyword>

!<keyword dtype="REAL" category="Internal surface" required="false" locked="false">
!  <description>Internal surface center of rotation. This is only meant to be used
!  for granular DEM, CGP, SQP or GSP models to move internal surfaces defined by STL files.
!  This keywords is used in conjunction with IS_WALL_OMEGA.
!  Example: a mixer with rotating paddles.
!  The first index is the IS ID, the second index is the component (1=x, 2=y, 3=z)
!  </description>
!  <arg index="1" id="IS" min="1" max="DIMENSION_IS"/>
!  <arg index="2" id="Component" min="1" max="3"/>
      IS_WALL_ROT_CENTER(LC,:) = ZERO
!</keyword>

      ENDDO

!<keyword dtype="INTEGER" category="Internal surface" required="false" locked="false">
!  <description>Frequency of STL facets binning to the DES grid. This is only meant to be used
!  for granular DEM, CGP, SQP or GSP models to move internal surfaces defined by STL files.
!  </description>
      IS_REBIN_N = 10
!</keyword>
...
!<keyword dtype="REAL" category="Point source" required="false" locked="false">
!  <description>X-axial component of incoming gas velocity.</description>
!  <arg index="1" id="PS" min="1" max="DIMENSION_PS"/>
         PS_U_G(LC) = UNDEFINED
!</keyword>

!<keyword dtype="REAL" category="Point source" required="false" locked="false">
!  <description>Y-axial component of incoming gas velocity.</description>
!  <arg index="1" id="PS" min="1" max="DIMENSION_PS"/>
         PS_V_G(LC) = UNDEFINED
!</keyword>

!<keyword dtype="REAL" category="Point source" required="false" locked="false">
!  <description>Z-axial component of incoming gas velocity.</description>
!  <arg index="1" id="PS" min="1" max="DIMENSION_PS"/>
         PS_W_G(LC) = UNDEFINED
!</keyword>
...
!<keyword dtype="REAL" category="Point source" required="false" locked="false">
!  <description>X-axial component of incoming solids velocity.</description>
!  <arg index="1" id="PS" min="1" max="DIMENSION_PS"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
...
!<keyword dtype="REAL" category="Point source" required="false" locked="false">
!  <description>Y-axial component of incoming solids velocity.</description>
!  <arg index="1" id="PS" min="1" max="DIMENSION_PS"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
...
!<keyword dtype="REAL" category="Point source" required="false" locked="false">
!  <description>Z-axial component of incoming solids velocity.</description>
!  <arg index="1" id="PS" min="1" max="DIMENSION_PS"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
...
!<keyword dtype="INTEGER" category="Output Control" required="false" locked="false">
!  <description> Write header every TSS_HDR iterations on screen.
...
!    array for post-processing by postmfix. User needs to set NRR to 1
...
!<keyword dtype="REAL" category="Output control" required="false" locked="false">
!  <description>
!     Interval to sample monitor data when computing time-averaged data.
!     Data is saved in [MONITOR_NAME]_tavg.csv
!     A value of UNDEFINED turns off time-averaged computation.
!  </description>
!  <range min="0"/>
!  <arg index="1" id="Monitor" max="DIMENSION_MONITOR" min="1"/>
      MONITOR_TAVG_DT(:) = UNDEFINED
!</keyword>

!<keyword dtype="REAL" category="Output control" required="false" locked="false">
!  <description>
!     Starting time when computing time-averaged monitor data
!     (typically used to discard the initial transient data).
!  </description>
!  <range min="0"/>
!  <arg index="1" id="Monitor" max="DIMENSION_MONITOR" min="1"/>
      MONITOR_TAVG_START(:) = ZERO
!</keyword>

!<keyword dtype="REAL" category="Output control" required="false" locked="false">
!  <description>
!     Interval to reset monitor time-averaged data, used to compute many averages over
!     short periods of time.
!     (a value of UNDEFINED will compute the average from monitor_tavg_start till
!     the end of simulation).
!  </description>
!  <range min="0"/>
!  <arg index="1" id="Monitor" max="DIMENSION_MONITOR" min="1"/>
      MONITOR_TAVG_RESET_DT(:) = UNDEFINED
!</keyword>

!<keyword dtype="REAL" category="Output control" required="false" locked="false">
!  <description>
!     Tolerance to declare steady state. If all rates of change of time-averaged quantities are below this tolerance
!     for a duration longer than the corresponding monitor_tavg_ss_span, then steady state is declared and the
!     simulation will stop (a value of ZERO will turn of the steady state check.)
!  </description>
!  <range min="0"/>
!  <arg index="1" id="Monitor" max="DIMENSION_MONITOR" min="1"/>
      MONITOR_TAVG_SS_TOL(:) = ZERO
!</keyword>

!<keyword dtype="REAL" category="Output control" required="false" locked="false">
!  <description>
!     Time span to declare steady state. If all rates of change of time-averaged quantities are below the tolerance
!     monitor_ss_tol for a duration longer than monitor_tavg_ss_span, then steady state is declared and the
!     simulation will stop (a value of UNDEFINED will turn of the steady state check.)
!  </description>
!  <range min="0"/>
!  <arg index="1" id="Monitor" max="DIMENSION_MONITOR" min="1"/>
      MONITOR_TAVG_SS_SPAN(:) = UNDEFINED
!</keyword>
...
!<keyword dtype="LOGICAL" category="Output control" required="false" locked="false">
!  <description>Write the solids phase volume fraction in region.</description>
!  <arg index="1" id="Monitor" min="1" max="DIMENSION_MONITOR"/>
!  <arg index="2" id="Phase" min="1" max="DIM_M"/>
      MONITOR_EP_s(:,:) = .FALSE.
!</keyword>
...
!<keyword dtype="LOGICAL" category="Output control" required="false" locked="false">
!  <description>Write particle count in region.</description>
!  <arg index="1" id="Monitor" min="1" max="DIMENSION_MONITOR"/>
      MONITOR_pcount(:) = .FALSE.
!</keyword>
...
!<keyword dtype="LOGICAL" category="Output control" required="false" locked="false">
!  <description>Write particle x-axis position in region.</description>
!  <arg index="1" id="Monitor" min="1" max="DIMENSION_MONITOR"/>
      MONITOR_pos_x(:) = .FALSE.
!</keyword>
...
!<keyword dtype="LOGICAL" category="Output control" required="false" locked="false">
!  <description>Write particle y-axis position in region.</description>
!  <arg index="1" id="Monitor" min="1" max="DIMENSION_MONITOR"/>
      MONITOR_pos_y(:) = .FALSE.
!</keyword>
...
!<keyword dtype="LOGICAL" category="Output control" required="false" locked="false">
!  <description>Write particle z-axis position in region.</description>
!  <arg index="1" id="Monitor" min="1" max="DIMENSION_MONITOR"/>
      MONITOR_pos_z(:) = .FALSE.
!</keyword>
...
!    in model/usr_properties.f for calculating the gas phase
!    density, RO_g.
!  </description>
...
!    in model/usr_properties.f for calculating the solids phase
!    density, RO_s.
!  </description>
!  <valid value=".TRUE." note="Call user-defined function."/>
!  <valid value=".FALSE." note="Use MFIX default calculation."/>
...
!    in model/usr_properties.f for calculating the gas-solids phase
...
!    model/usr_properties.f for calculating the gas-solids phase drag
...
!    model/usr_properties.f for calculating the solids-solids phase
...
!  <arg index="1" id="User-defined ID" min="1" max="DIMENSION_USR_C"/>
      USR_C(1:DIMENSION_USR_C) = UNDEFINED
!</keyword>
...
!<keyword dtype="LOGICAL" category="Run Control" required="false" locked="false">
!  <description>Read a keyframe data file.
!   Example: KEYFRAME(3) = .TRUE.
!   will read the keyframe file keyframe_0003.csv.
!   The first column of keyframe_0003.csv stores the independent variable.
!   The next columns of keyframe_0003.csv store the dependent variables.
!  </description>
!  <arg index="1" id="Keyframe file ID" min="1" max="100"/>
...
!<keyword dtype="CHARACTER" category="Run Control" required="false" locked="false">
!  <description>Name of keyword controlled by a Keyframe datafile.
!  Example: KF_KEYWORD(3,1)='bc_v_g'
!  First index (3) means the keyframe data file is keyframe_0003.csv.
!  Second index (1) means the first dependent variable (second column) controls keyword bc_v_g.
!  See documentation for list of keywords that can be controlled by a keyframe.
!  </description>
!  <arg index="1" id="Keyframe file ID" min="1" max="100"/>
!  <arg index="2" id="Column number in CSV keyframe file" min="1" max="20"/>
      KF_KEYWORD(:,:) = "NONE"
!</keyword>

! Note, this keyword is not included in any namelist, so don't add it to the docs
!<disabled_keyword dtype="INTEGER" category="Run Control" required="false" locked="true">
!  <description>Independent variable identifier for a Keyframe datafile.
!  Example: KF_IV(3)=0
!  Means the first column (independent variable) of keyframe data file keyframe_0003.csv
!  represents time.
!  </description>
!  <arg index="1" id="Keyframe file ID" min="1" max="100"/>
!  <valid value="0" note="Keyframe file dependent variable is time."/>
!  <valid value="1" note="Keyframe file dependent variable is x-coordinate."/>
!  <valid value="2" note="Keyframe file dependent variable is y-coordinate."/>
!  <valid value="3" note="Keyframe file dependent variable is z-coordinate."/>
      KF_IV(:) = 0
!</disabled_keyword>

!<keyword dtype="CHARACTER" category="Run Control" required="false" locked="false">
!  <description>Keyframe interpolation method (applies to all variables in a Keyframe datafile.
!  Example: KF_INTERP(3)='LINEAR'
!  First index (3) means the keyframe data file is keyframe_0003.csv.
!  </description>
!  <arg index="1" id="Keyframe file ID" min="1" max="100"/>
!  <valid value="LINEAR" note="Linearly interpolate data between keyframes (default)."/>
!  <valid value="STEP" note="Keep data constant between keyframes."/>
      KF_INTERP(:) = "LINEAR"
!</keyword>

!<keyword dtype="CHARACTER" category="run control" required="false" locked="false">
!  <description>Angular velocity unit. This is only meant to be used
!  for granular DEM, CGP, SQP or GSP models to move STL files or group of
!  particles.
!  Example: OMEGA_UNIT = 'REV/S' means the angular velocity is expressed in revolutions per second.
!  </description>
!  <valid value="RAD/S" note="Radian per second"/>
!  <valid value="DEG/S" note="Degree per second"/>
!  <valid value="REV/S" note="Revolution per second"/>
!  <valid value="RAD/MIN" note="Radian per minute"/>
!  <valid value="DEG/MIN" note="Degree per minute"/>
!  <valid value="REV/MIN" note="Revolution per minute"/>
      OMEGA_UNIT = 'RAD/S'
!</keyword>
!#####################################################################!
!                        Chemical reactions                           !
...
!  <description>Flag to use Arrhenius reaction model for fluid phase reactions.</description>
!  <conflict keyword="USE_RRATES" value=".TRUE."/>
      ARRHENIUS_RRATES_FLUID = .FALSE.
!</keyword>

!<keyword dtype="LOGICAL" category="Chemical reactions" required="false" locked="false">
!  <description>Flag to use Arrhenius reaction model for discrete particle reactions.</description>
!  <conflict keyword="USE_RRATES" value=".TRUE."/>
      ARRHENIUS_RRATES_DES = .FALSE.
!</keyword>
...
!<keyword dtype="REAL" category="Chemical reactions" required="false" locked="false">
!  <description>
...
!<keyword dtype="REAL" category="Chemical reactions" required="false" locked="false">
!  <description>
!    Stiff chemistry minimum fluid reaction rate. If all fluid reaction rates are below this threshold, a
!    cell is said to be non-reactive and no ODE is set for this cell.
!  </description>
!  <dependent keyword="STIFF_CHEMISTRY" value=".TRUE."/>
!  <conflict keyword="USE_RRATES" value=".TRUE."/>
!  <range min="+0.0"/>
...
!<keyword dtype="REAL" category="Chemical reactions" required="false" locked="false">
!  <description>
!    Stiff chemistry minimum DPM (Discrete Phase Model) reaction rate. If all DPM reaction
!    rates are below this threshold, a particle/parcel is said to be non-reactive and no
!    ODE is set for this particle/parcel.
!  </description>
!  <dependent keyword="STIFF_CHEMISTRY" value=".TRUE."/>
!  <conflict keyword="USE_RRATES" value=".TRUE."/>
!  <range min="+0.0"/>
      STIFF_CHEM_MIN_RATE_DPM = 1.0D-20
!</keyword>

!<keyword dtype="REAL" category="Chemical reactions" required="false" locked="false">
!  <description>
!    Minimum species mass fraction in fluid. Reaction rates are not calculated
!    when reactants fall below this threshold.
!  </description>
!  <conflict keyword="USE_RRATES" value=".TRUE."/>
!  <range min="0.0" max="1.0"/>
      CHEM_MIN_SPECIES_FLUID = 1.0D-7
!</keyword>

!<keyword dtype="REAL" category="Chemical reactions" required="false" locked="false">
!  <description>
!    Minimum species mass fraction in solid. Reaction rates are not calculated
!    when reactants fall below this threshold.
!  </description>
!  <conflict keyword="USE_RRATES" value=".TRUE."/>
!  <range min="0.0" max="1.0"/>
      CHEM_MIN_SPECIES_SOLID = 1.0D-7
!</keyword>
...
!<keyword dtype="REAL" category="Chemical reactions" required="false" locked="false">
!  <description>
!    Minimum particle mass fraction for reactions.
!    When the ratio of particle mass to initial mass is below
!    this threshold, reactions which consume particle species
!    will be turned off for these particles.
!    If a particle size distribution is in effect, the initial
!    mass is the minimum mass for the distribution.
!     </description>
!  <conflict keyword="USE_RRATES" value=".TRUE."/>
!  <range min="0.0" max="1.0"/>
      DES_MIN_PMASS_FRAC = 0.0D0
!</keyword>
...
!<keyword dtype="INTEGER" category="Parallelization control" required="false" locked="false">
!  <description>
