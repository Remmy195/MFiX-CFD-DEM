...
! Check if there was an error opening the file.
      IF (ERROR_OPENING(IER)) THEN
         WRITE (ERR_MSG, 3010) trim(FILE_NAME), trim(IMSG(myPE))
         CALL LOG_ERROR()
      ENDIF
3010  FORMAT('Error opening file ', A/, 'Error message: ', A)

! Only PE_IO opens the RUN_NAME.TIM file.
      IF (myPE == PE_IO.AND.(.NOT.PPO)) THEN
         CALL OPEN_FILE(RUN_NAME, UNIT_LOG1, '.TXT', FILE_NAME, 'UNKNOWN', 'SEQUENTIAL', 'FORMATTED', 132, IER(myPE), IMSG(myPE))
      END IF
...
! Report errors.
 ...
 5030 FORMAT('Mesh file was successfully opened: ',A)
