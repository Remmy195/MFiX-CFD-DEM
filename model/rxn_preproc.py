...
print("Processing chemical reaction data from %s... " %
      os.path.basename(input_filename), end='')
...
lines = ["!This file was generated by %s\n" % sys.argv[0],
         "!from %s on %s\n" % (input_filename, time.ctime()),
         "!Edit at your own risk!!!\n"]
for (val, name) in output:
    lines.append("      INTEGER, PARAMETER :: %s = %s\n" % (name, val))

if os.path.exists(output_filename):
    lines1 = open(output_filename).readlines()
    if (len(lines) == len(lines1)
        and all(lines1[n].startswith('!') for n in range(3))
        and all(lines1[n] == lines[n] for n in range(3, len(lines)))):
        print("no change")
        sys.exit(0)
