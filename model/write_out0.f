...
!
      WRITE(UNIT_OUT,1000)trim(ID_VERSION),ID_HOUR,ID_MINUTE,ID_MONTH,ID_DAY,ID_YEAR
...
!
      ...
      DO L = 1, DIMENSION_USR_C
         IF (USR_C(L) /= UNDEFINED) WRITE (UNIT_OUT, 1190) USR_C_NAME(L), L, USR_C(L)
      END DO
...
!
      ...
      DO L = 1, DIMENSION_IC
         IF (IC_DEFINED(L)) THEN
            ...
            LOC(1) = LOCATION(IC_I_W(L),X_MIN + XMIN,DX) - HALF*DX(IC_I_W(L))
            LOC(2) = LOCATION(IC_I_E(L),X_MIN + XMIN,DX) + HALF*DX(IC_I_E(L))
            LOC(3) = LOCATION(IC_J_S(L),Y_MIN,DY) - HALF*DY(IC_J_S(L))
            LOC(4) = LOCATION(IC_J_N(L),Y_MIN,DY) + HALF*DY(IC_J_N(L))
            LOC(5) = LOCATION(IC_K_B(L),Z_MIN,DZ) - HALF*DZ(IC_K_B(L))
            LOC(6) = LOCATION(IC_K_T(L),Z_MIN,DZ) + HALF*DZ(IC_K_T(L))
      ...
      DO L = 1, DIMENSION_BC
         IF (BC_DEFINED(L)) THEN
            ...
            IF (.not.IS_CG(BC_TYPE_ENUM(L))) THEN
               LOC(1) = LOCATION(BC_I_W(L),X_MIN + XMIN,DX) - HALF*DX(BC_I_W(L))
               LOC(2) = LOCATION(BC_I_E(L),X_MIN + XMIN,DX) + HALF*DX(BC_I_E(L))
               LOC(3) = LOCATION(BC_J_S(L),Y_MIN,DY) - HALF*DY(BC_J_S(L))
               LOC(4) = LOCATION(BC_J_N(L),Y_MIN,DY) + HALF*DY(BC_J_N(L))
               LOC(5) = LOCATION(BC_K_B(L),Z_MIN,DZ) - HALF*DZ(BC_K_B(L))
               LOC(6) = LOCATION(BC_K_T(L),Z_MIN,DZ) + HALF*DZ(BC_K_T(L))
      ...
      DO L = 1, DIMENSION_IS
         IF (IS_DEFINED(L)) THEN
            ...
            ELSE IF (IS_TYPE(L)=='SEMIPERMEABLE' .OR. &
               ...
               WRITE (UNIT_OUT, 1713)
            ENDIF
            LOC(1) = LOCATION(IS_I_W(L),X_MIN + XMIN,DX) - HALF*DX(IS_I_W(L))
            LOC(2) = LOCATION(IS_I_E(L),X_MIN + XMIN,DX) + HALF*DX(IS_I_E(L))
            LOC(3) = LOCATION(IS_J_S(L),Y_MIN,DY) - HALF*DY(IS_J_S(L))
            LOC(4) = LOCATION(IS_J_N(L),Y_MIN,DY) + HALF*DY(IS_J_N(L))
            LOC(5) = LOCATION(IS_K_B(L),Z_MIN,DZ) - HALF*DZ(IS_K_B(L))
            LOC(6) = LOCATION(IS_K_T(L),Z_MIN,DZ) + HALF*DZ(IS_K_T(L))
...
! Point Sources
      ...
      DO L = 1, DIMENSION_PS
         IF (PS_DEFINED(L)) THEN
            ...
            LOC(1) = LOCATION(PS_I_W(L),X_MIN + XMIN,DX) - HALF*DX(PS_I_W(L))
            LOC(2) = LOCATION(PS_I_E(L),X_MIN + XMIN,DX) + HALF*DX(PS_I_E(L))
            LOC(3) = LOCATION(PS_J_S(L),Y_MIN,DY) - HALF*DY(PS_J_S(L))
            LOC(4) = LOCATION(PS_J_N(L),Y_MIN,DY) + HALF*DY(PS_J_N(L))
            LOC(5) = LOCATION(PS_K_B(L),Z_MIN,DZ) - HALF*DZ(PS_K_B(L))
            LOC(6) = LOCATION(PS_K_T(L),Z_MIN,DZ) + HALF*DZ(PS_K_T(L))
            WRITE (UNIT_OUT, 1755) &
 ...
 1750 FORMAT(//,3X,'9. POINT SOURCES')
...
!
      WRITE (UNIT_OUT, 2000)
      DO L = 1, DIMENSION_MONITOR
         IF (MONITOR_DEFINED(L)) THEN
            WRITE (LCHAR,*) L
            WRITE (UNIT_OUT, 2010) ADJUSTL(LCHAR)
            WRITE (MTCHAR,*) MONITOR_TYPE(L)
            WRITE (UNIT_OUT, 2011) ADJUSTL(MTCHAR), TRIM(MONITOR_TYPENAME(L))
            IF(MONITOR_TYPE(L)<100) THEN
               WRITE (UNIT_OUT, 2012) 'CELL DATA'
            ELSE
               WRITE (UNIT_OUT, 2012) 'PARTICLE DATA (DEM, CG-DEM, PIC)'
            ENDIF
            WRITE (UNIT_OUT, 2013) TRIM(ADJUSTL(MONITOR_NAME(L)))//'.csv'

            LOC(1) = LOCATION(MONITOR_I_W(L),X_MIN + XMIN,DX) - HALF*DX(MONITOR_I_W(L))
            LOC(2) = LOCATION(MONITOR_I_E(L),X_MIN + XMIN,DX) + HALF*DX(MONITOR_I_E(L))
            LOC(3) = LOCATION(MONITOR_J_S(L),Y_MIN,DY) - HALF*DY(MONITOR_J_S(L))
            LOC(4) = LOCATION(MONITOR_J_N(L),Y_MIN,DY) + HALF*DY(MONITOR_J_N(L))
            LOC(5) = LOCATION(MONITOR_K_B(L),Z_MIN,DZ) - HALF*DZ(MONITOR_K_B(L))
            LOC(6) = LOCATION(MONITOR_K_T(L),Z_MIN,DZ) + HALF*DZ(MONITOR_K_T(L))

! Location
            WRITE (UNIT_OUT, 2020) MONITOR_X_W(L), LOC(1), MONITOR_X_E(L), LOC(2), MONITOR_Y_S&
               (L), LOC(3), MONITOR_Y_N(L), LOC(4), MONITOR_Z_B(L), LOC(5), MONITOR_Z_T(L), &
               LOC(6)
            WRITE (UNIT_OUT, 2030) MONITOR_I_W(L), MONITOR_I_E(L), MONITOR_J_S(L), MONITOR_J_N(L), &
               MONITOR_K_B(L), MONITOR_K_T(L)

! List of variables
            WRITE (UNIT_OUT, 2040)
            DO VAR = 1, MONITOR_VAR_COUNT(L)
               WRITE (UNIT_OUT, 2041) TRIM(MONITOR_VARNAME_LIST(L,VAR))
            END DO

         ENDIF ! is monitor defined?
      END DO   ! loop over monitors
...
!
      RETURN
 1000 FORMAT('Multiphase Flow with Interphase eXchanges'/34X,'Version: ',A,/20X,&
         'Time: ',I2,':',I2,20X,'Date: ',I2,'-',I2,'-',I4)
 1010 FORMAT(/7X,'Host : ',A50,/,1X,79('_'))
...
!
 1800 FORMAT(//,3X,'10. OUTPUT DATA FILES:',/7X,'Extension',T18,&
         'Description',T59,'Interval for writing')
 1801 FORMAT(7X,A4,T18,A,T61,G12.5)
!
 1900 FORMAT(//,3X,'11. TOLERANCES',/7X,&
...
!
 2000 FORMAT(//,3X,'12. MONITORS')
 2010 FORMAT(/7X,'Monitor region no : ',A3)
 2011 FORMAT(9X,   'Monitor type    : ',A3, ' (',A,')')
 2012 FORMAT(9X,   'Data type       : ',A)
 2013 FORMAT(9X,   'File name       : ',A)
 2020 FORMAT(9X,39X,' Specified  ',5X,' Simulated  ',/9X,&
         'X coordinate of west face   (MONITOR_X_w) = ',G12.5,5X,G12.5/,9X,&
         'X coordinate of east face   (MONITOR_X_e) = ',G12.5,5X,G12.5/,9X,&
         'Y coordinate of south face  (MONITOR_Y_s) = ',G12.5,5X,G12.5/,9X,&
         'Y coordinate of north face  (MONITOR_Y_n) = ',G12.5,5X,G12.5/,9X,&
         'Z coordinate of bottom face (MONITOR_Z_b) = ',G12.5,5X,G12.5/,9X,&
         'Z coordinate of top face    (MONITOR_Z_t) = ',G12.5,5X,G12.5/)
 2030 FORMAT(9X,'I index of cell at west     (MONITOR_I_w) = ',I4,/,9X,&
         'I index of cell at east     (MONITOR_I_e) = ',I4,/,9X,&
         'J index of cell at south    (MONITOR_J_s) = ',I4,/,9X,&
         'J index of cell at north    (MONITOR_J_n) = ',I4,/,9X,&
         'K index of cell at bottom   (MONITOR_K_b) = ',I4,/,9X,&
         'K index of cell at top      (MONITOR_K_t) = ',I4)
 2040 FORMAT(/9X,'List of variables: ')
 2041 FORMAT(9X,A)
...
!
      ...
      deallocate (array3)
!
 2000 FORMAT(//,3X,'13. INITIAL AND BOUNDARY CONDITION FLAGS',/7X,&
