MODULE SOLVE_SPECIES_EQ_MOD
   ...
   use fldvar, only: ep_g, u_g, v_g, w_g, x_g, x_go, rop_g, rop_go, T_g
   use fldvar, only: ep_s, u_s, v_s, w_s, x_s, x_so, rop_s, rop_so
   use functions, only: fluid_at, flow_at, zmax, wall_at
   ...
   use physprop, only: nmax, dif_g, dif_s, divJi
...
! species index
      INTEGER :: LN, LM
! Number of species to loop over: NMAX when using chi scheme,
! (NMAX - 1)  otherwise
      INTEGER :: N_SPECIES
...
! ---------------------------------------------------------------->>>
      IF (SPECIES_EQ(0)) THEN
         IF(chi_scheme) then
            call set_chi(DISCRETIZE(7), X_g, NMAX(0), &
                         U_g, V_g, W_g)
            N_SPECIES = NMAX(0)
         ELSE
            N_SPECIES = NMAX(0) - 1
...
! calculate the source terms to be used in the a matrix and b vector
                  ...
                  S_P(IJK) = APO + ZMAX(-divJi(IJK,LN))+&
                     (ZMAX(SUM_R_G(IJK))+ROX_GC(IJK,LN))*VOL(IJK)
                  S_C(IJK) = APO*X_GO(IJK,LN) + ZMAX(divJi(IJK,LN)) + &
...
! usr sources
            IF(CALL_USR_SOURCE(7)) CALL CALC_USR_SOURCE(GAS_SPECIES, &
                                 A_M, B_M, lM=0, lN=lN)
...
! Check for linear solver divergence.
         ...
         ENDDO    ! end do loop (ln = 1, n_species)
!
         IF(chi_scheme) then
            call unset_chi()
         ELSE
!
! calculate the values of X(nmax0) i.e. last species
!
!!$omp    parallel do private(IJK)
            DO IJK = ijkstart3, ijkend3
               SumXg = zero
               DO LM = 1, N_SPECIES
                  SumXg = SumXg + X_G(IJK,LM)
               ENDDO
               X_G(IJK,NMAX(0)) = one - SumXg
               if(X_G(IJK,NMAX(0)) <  small_number) X_G(IJK,NMAX(0)) = zero
            ENDDO
         ENDIF
      ENDIF
         ...
         IF (SPECIES_EQ(M)) THEN
            IF(chi_scheme) THEN
               ...
               DO LN = 1, N_SPECIES
                  ...
                  ENDDO
               ENDDO
               call set_chi(DISCRETIZE(7), X_S_temp, N_SPECIES, &
...
! usr sources
            IF(CALL_USR_SOURCE(7)) CALL CALC_USR_SOURCE(SOLIDS_SPECIES,&
                                 A_M, B_M, lM=M, lN=lN)
...
! Check for linear solver divergence.
            ...
            ELSE
!
! calculate the values of X(nmax(m)) i.e. last species
!
!!$omp    parallel do private(IJK)
               DO IJK = ijkstart3, ijkend3
                  SumXg = zero
                  DO LM = 1, N_SPECIES
                     SumXg = SumXg + X_S(IJK,M,LM)
                  ENDDO
                  X_S(IJK,M,NMAX(M)) = one - SumXg
                  if(X_S(IJK,M,NMAX(M)) <  small_number) X_S(IJK,M,NMAX(M)) = zero
               ENDDO
!
            ENDIF
