...
!!    This floods console with messages
!!      if(mfix_isnan) then
!!         print*,'NaN detected'
!!      endif
      RETURN
...
LOOP_FLUID : DO IJK = IJKSTART3, IJKEND3
         ...
         IF (FLUID_AT(IJK)) THEN
            IF(ABS(U_G(IJK)) > MAX_INLET_VEL .OR. &
               ...
               WRITE(ERR_MSG,1000) MAX_INLET_VEL, I_OF(IJK), J_OF(IJK), K_OF(IJK), &
            ...
            IF (.NOT.DES_CONTINUUM_COUPLED .OR. DES_CONTINUUM_HYBRID) THEN
               DO M = 1, MMAX
                 IF(ABS(U_S(IJK,M)) > MAX_INLET_VEL .OR. &
                   ...
                   CHECK_VEL_BOUND = .TRUE.
                   WRITE(ERR_MSG,1010) MAX_INLET_VEL, I_OF(IJK), J_OF(IJK), K_OF(IJK), M, &
      ...
      RETURN
 1000 FORMAT('Velocity exceeds limit: ', G12.5, /&
            'in cell: ','I = ',I4,2X,' J = ',I4,2X,' K = ',I4, /&
            ' Epg = ', G12.5, ' Ug = ', G12.5, ' Vg = ', G12.5, ' Wg = ', G12.5, /&
            'To change the limit, adjust the scale factor MAX_INLET_VEL_FAC.')

 1010 FORMAT('Velocity exceeds limit: ',  G12.5,/&
            'in cell: ','I = ',I4,2X,' J = ',I4,2X,' K = ',I4,' M = ',I4, /&
...
!vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv!
!  Function: CAP_BC_VEL                                                !
!  Purpose: Cap updated boundary velocities to the max_inlet_vel       !
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^!

      DOUBLE PRECISION FUNCTION CAP_BC_VEL(BC_VEL, SGN)
      use toleranc, only: max_inlet_vel

      IMPLICIT NONE
!---------------------------------------------------------------------//
! index for boundary condition
      DOUBLE PRECISION, INTENT(IN) :: BC_VEL
      DOUBLE PRECISION, INTENT(IN) :: SGN

      CAP_BC_VEL=BC_VEL
      IF (ABS(BC_VEL) >= MAX_INLET_VEL) &
         CAP_BC_VEL=SGN*MAX_INLET_VEL

      END FUNCTION CAP_BC_VEL
...
! length of input data line
      INTEGER :: LINE_LEN
! maximum column that non-blank characters are
