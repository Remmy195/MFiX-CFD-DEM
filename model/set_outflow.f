...
!                                                                      C
!  This routine sets the value of quantities that are derived or with  C
...
! Regardless of flow direction PI boundary reflects neighboring fluid
! for volume fraction settings.
! However ro_s will get properly updated in the eventual call to
! physical_prop_ros. This simply maintains ep_s for that call
            ROP_S(IJK,M) = ROP_S(FIJK,M)
            RO_S(IJK,M) = RO_S(FIJK,M)
            RO_G(IJK) = RO_G(FIJK)
         ELSE
...
! set ep_g accordingly
      EP_G(IJK) = ONE - SUM_EPS

! now that ep_g in the boundary cell is known, define the bulk density
! of the gas phase in the boundary cell; note ro_g will get properly
! updated in the eventual call to physical_prop_rog. This simply
! maintains ep_g for that call
...
! this won't be fully correct since ro_s and therefore rop_s in the
! flow cell is not updated yet based on above specification
      IF(KT_TYPE_ENUM == GHD_2007) THEN
         nTOT = zero
         DO M = 1, SMAX
             nM = ROP_S(IJK,M)*6d0/ &
