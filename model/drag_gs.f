...
!-----------------------------------------------
      ...
      USE DERIVED_TYPES
!     USE run  only: SQP_DIFELICE_GANSER, SQP_DIFELICE_HOLZER_SOMMERFELD
...
! Force a ZERO drag coefficient for MMS cases.
            ...
            ELSE
               ...
               CASE (TGS)
                  CALL DRAG_TGS(DgA,EPg,Mu,ROPg,VREL,DPM,DPA,phis)
...
!SuperDEM drag only
               CASE (SQP_DIFELICE_GANSER)
                  CALL DRAG_SQP_DIFELICE_GANSER(M,DgA,EPg,Mu,ROg,VREL,DPM,&
                                                UGC, VGC, WGC)
!SuperDEM drag only
               CASE (SQP_DIFELICE_HOLZER_SOMMERFELD)
                  CALL DRAG_SQP_DIFELICE_HOLZER_SOMMERFELD(M,DgA,EPg,Mu,ROg,&
                                                       VREL,DPM,UGC, VGC, WGC)
! Qi, Kuang, Qiu and Yu drag law. To be used with
! Herschel-Bulkley non-Newtonian viscosity
               CASE (QKQY)
                  CALL DRAG_QKQY(DgA,EPg,Mu,ROPg,VREL,DPM)
...
!vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvC
!                                                                      C
!  Subroutine: DRAG_QKQY                                               C
!  Purpose: Calculate the Qi, Kuang, Qiu and Yu gas-solids drag        C
!           coefficient.  To be used with                              C
!           with Herschel-Bulkley non-Newtonian viscosity              C
!                                                                      C
!  Author: Sofiane Benyahia                          11 JAN. 2024      C
!                                                                      C
!  Literature/Document References:                                     C
!  Z. Qi, S.B. Kuang, T.S. Qiu, A.B. Yu,                               C
!  Lattice Boltzmann investigation on fluid flows through              C
!  packed beds: Interaction between fluid rheology and bed properties, C
!  Powder Technology,Volume 369,2020,Pages 248-260,                    C
!  https://doi.org/10.1016/j.powtec.2020.05.046.                       C
!                                                                      C
!                                                                      C
!  Benyahia S. (2024). Powder Technology  433 (2024) 119261            C
!  Simulation of heavy particles fluidized with non-Newtonian fluids.  C
!  https://doi.org/10.1016/j.powtec.2023.119261                        C
!                                                                      C
!vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvC

      SUBROUTINE DRAG_QKQY(lDgA,EPg,Mug,ROPg,VREL,DPM)
...
!-----------------------------------------------
      USE param
      USE param1
      use physprop, only: HB_tau0, HB_k0, HB_n
      IMPLICIT NONE
!-----------------------------------------------
! Dummy arguments
!-----------------------------------------------
...
! Non-Newtonian Re
      IF(Mug > ZERO) THEN
! Note the presence of gas volume fraction in ROPG
         RE = DPM**HB_n*(VREL*EPg)**(2d0-HB_n)*(ROPg/EPg)/(HB_K0+HB_Tau0*(DPM/(VREL*EPg))**HB_n)
      ELSE
         RE = LARGE_NUMBER
      ENDIF
      ...
      logRe = dlog10(Re)-1.5d0

      if((Re > small_number)) then
! Single sphere drag coefficient
         C_d = (2.654d0/(HB_n**(-1.9d0)+3.213d0) + (4.8d0/(HB_n**0.08d0*dsqrt(RE))))**2
! Crowding function
         beta0 = (2.65d0*HB_n*(epg+one)**HB_n) - ((5.3d0-3.5d0*epg**HB_n)*epg**2*dexp(-0.5d0*(logRe)**2)) &
              + ((2.65d0*(epg+one)**(2-HB_n) - 2.65d0*HB_n*(epg+one)**HB_n)/(one+dexp(-(2d0*logRe))))

! below is equivalent to equation (15) in https://doi.org/10.1016/j.powtec.2020.05.046
         lDgA = 0.75D0 * C_d * VREL * (ROPg*Epg) / (Epg**beta0 * DPM)
      else
         lDgA = zero
      endif

      RETURN
      END SUBROUTINE DRAG_QKQY
...
! Note the presence of gas volume fraction in ROPG
      ...
      F = F + phis**3d0*RE*(0.95D0+0.61D0*(phis**3)/(1.0-phis)**2)
      lDgA = F*F_STOKES
