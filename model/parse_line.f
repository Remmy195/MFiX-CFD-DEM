...
! Initialize logicals for parsing reaction data.
      ...
      IF(TFM_RXN) THEN
         CALL PARSE_RXN (LINE, NO_OF_RXNS, RXN_NAME, RXN_CHEM_EQ,      &
            usrDH, usrfDH, arrhenius_coeff, reverse_calc, rxn_order_array, &
	    third_body_model, third_body_coeff, press_rxn_model, press_rxn_coeff,&
	    arrhenius_coeff_press, LT_coeff, rate_fit_model, rate_fit_coeff)
         READ_FLAG = .FALSE.
         RETURN
      ELSEIF(DES_RXN) THEN
         CALL PARSE_RXN (LINE, NO_OF_DES_RXNS, DES_RXN_NAME,           &
            DES_RXN_CHEM_EQ, DES_usrDH, DES_usrfDH,                    &
	    DES_arrhenius_coeff, DES_reverse_calc, DES_rxn_order_array)
         READ_FLAG = .FALSE.
         RETURN
      ENDIF
!
...
! User defined heat of reaction partitions: Allocate/Initialize
      ...
      usrfDH(:,:) = UNDEFINED
! User defined arrhenius coefficient: Allocate/Initialize
      IF(Allocated( arrhenius_coeff )) GoTo 100
      Allocate( arrhenius_coeff( DIMENSION_RXN, 3 ))
      arrhenius_coeff(:,:) = UNDEFINED
! Array about how to calculate reverse reaction rates: Allocate/Initialize
      IF(Allocated( reverse_calc )) GoTo 100
      Allocate( reverse_calc( DIMENSION_RXN ))
      reverse_calc(:) = UNDEFINED_C
! User defined species reaction orders: Allocate/Initialize
      IF(Allocated( rxn_order_array )) GoTo 100
      Allocate( rxn_order_array( DIMENSION_RXN ))
      rxn_order_array(:) = UNDEFINED_C
! Species applied as third body: Allocate/Initialize
      IF(Allocated( third_body_model )) GoTo 100
      Allocate( third_body_model( DIMENSION_RXN ))
      third_body_model(:) = UNDEFINED_C
! Coefficients of third body: Allocate/Initialize
      IF(Allocated( third_body_coeff )) GoTo 100
      Allocate( third_body_coeff( DIMENSION_RXN, DIM_N_G ))
      third_body_coeff(:,:) = UNDEFINED
! Models used for reaction rates of pressure-dependent reactions: Allocate/Initialize
      IF(Allocated( press_rxn_model )) GoTo 100
      Allocate( press_rxn_model( DIMENSION_RXN ))
      press_rxn_model(:) = UNDEFINED_C      
! Coefficient used for reaction rates of pressure-dependent reactions:
! Allocate/Initialize
      IF(Allocated( press_rxn_coeff )) GoTo 100
      Allocate( press_rxn_coeff( DIMENSION_RXN, 5 ))
      press_rxn_coeff(:, :) = UNDEFINED   
! User defined arrhenius coefficient for pressure-dependent reactions: 
! Allocate/Initialize
      IF(Allocated( arrhenius_coeff_press )) GoTo 100
      Allocate( arrhenius_coeff_press( DIMENSION_RXN, 1, 3 ))
      arrhenius_coeff_press(:,:,:) = UNDEFINED
! Coefficients for Landau_Teller reactions: Allocate/Initialize
      IF(Allocated( LT_coeff )) GoTo 100
      Allocate( LT_coeff( DIMENSION_RXN, 2 ))
      LT_coeff(:,:) = UNDEFINED
! Model to specify the optional rate-constant fit expression: Allocate/Initialize
      IF(Allocated( rate_fit_model )) GoTo 100
      Allocate( rate_fit_model( DIMENSION_RXN ))
      rate_fit_model(:) = UNDEFINED_C
! Coefficients for rate fit expressions: Allocate/Initialize
      IF(Allocated( rate_fit_coeff )) GoTo 100
      Allocate( rate_fit_coeff( DIMENSION_RXN, 9 ))
      rate_fit_coeff(:,:) = UNDEFINED     
...
! multiple lines.
      MORE_ChemEq = .FALSE.
! Flag indicating that the rxn orders spans additional lines.
      MORE_RxnOrder = .FALSE.
! Flag indicating that the parameters for pressure-related reactions
! spans additional lines.
      MORE_PressParam = .FALSE.
! Flag indicating that the parameters for optional rate-constant fit 
! expression spans additional lines.
      MORE_fitParam = .FALSE.
! Flag indicating that the coefficients of third body spans additional lines.
...
! User defined arrhenius coefficient: Allocate/Initialize
      IF(Allocated( DES_arrhenius_coeff )) GoTo 100
      Allocate( DES_arrhenius_coeff( DIMENSION_RXN, 3 ))
      DES_arrhenius_coeff(:,:) = UNDEFINED
! Array about how to calculate reverse reaction rates: Allocate/Initialize
      IF(Allocated( DES_reverse_calc )) GoTo 100
      Allocate( DES_reverse_calc( DIMENSION_RXN ))
      DES_reverse_calc(:) = UNDEFINED_C
! User defined species reaction orders: Allocate/Initialize
      IF(Allocated( DES_rxn_order_array )) GoTo 100
      Allocate( DES_rxn_order_array( DIMENSION_RXN ))
      DES_rxn_order_array(:) = UNDEFINED_C
! Logical indicating that the code is in the middle of parsing a
! reaction construct.
