...
! When specifying the number of particles or solid mass in an IC region with any
! DEM-type solid model, the void fraction is left UNDEFINED.
! This will trigger a data check error if left UNDEFINED.
! Here we set the void fraction to ONE so we can pass the data check.
! The actual value of the void fraction will be computed after the DEM seeding takes place.
            DO M=MMAX+1,MMAX+DES_MMAX
               IF(SOLIDS_MODEL(M) == 'DEM'.OR.SOLIDS_MODEL(M) == 'CGP'.OR.SOLIDS_MODEL(M) == 'SQP'.OR.SOLIDS_MODEL(M) == 'GSP') THEN
                  IF(IC_DES_NP(L,M) > 0 .OR. IC_DES_SM(L,M) > ZERO) EPGX = ONE
               ENDIF
            END DO
