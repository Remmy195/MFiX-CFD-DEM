...
! Local variables
!-----------------------------------------------
! Max number of axis cells (DIM_I, DIM_J, DIM_K)
      INTEGER :: DIM_A
...
!-----------------------------------------------
      ...
      IF(AXIS=='X') DIM_A = DIM_I
      IF(AXIS=='Y') DIM_A = DIM_J
      IF(AXIS=='Z') DIM_A = DIM_K

      IF(NA>DIM_A) THEN
          WRITE(ERR_MSG, 1000) AXIS, AXIS_INDEX, NA, AXIS_INDEX, DIM_A,AXIS,AXIS_INDEX,DIM_A,AXIS_INDEX
          CALL LOG_ERROR()
      ENDIF

 1000 FORMAT('Number of cells in ',A1,'-direction is too large.',   &
             /'Value specified (',A1,'MAX): ',I6,                          &
             /'Maximum value  (DIM_',A1,'): ',I6,                          &
             /'Either reduce the number of cells in ',A1,'-direction,',    &
             /'i.e. set ',A1,'MAX<',I6,                                    &
             /'or increase DIM_',A1, ' in model/param_mod.f and rebuild the solver.')
...
! If A_MIN and A_MAX and ALENGHT are defined, make sure they are consistent
      ELSEIF( ALENGTH /= UNDEFINED .AND. A_MIN /= UNDEFINED .AND. A_MAX /= UNDEFINED) THEN
         IF(.NOT.COMPARE(A_MAX-A_MIN,ALENGTH)) THEN
            ...
            CALL LOG_ERROR()
 1107       FORMAT(&
            'Error 1107: Incorrect domain size information in ',A1,'-direction.', /&
            A1, 'LENGTH, ',A1,'_MIN, and ', A1, '_MAX are not consistent.')
         ENDIF
