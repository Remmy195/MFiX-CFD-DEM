...
! have an undefined volume fraction. A second check is performed on
! the solids.
...
! Error for regular IC regions when solving species equations.
 ...
 1110 FORMAT('Error 1110: IC_X_g(',I3,',:) do not sum to 1 and the ',&
...
! Error for slightly compressible flows with MW_AVG specified.
 ...
 1111 FORMAT('Error 1111: IC_X_g(',I3,',:) do not sum to 1 and the ',&
 ...
 1112 FORMAT('Error 1112: IC_X_g(',I3,',:) do not sum to 1 or 0 and they',/&
             'are not needed.')
 ...
 1002 FORMAT(&
     'Error 1002: Turbulent kinetic energy must be greater than or equal to 0.0'/&
      'Illegal value: ',A,' = ',A)
...
! friction model
      use run, only: PRINCETON, GUO_BOYCE
...
! If IC_EP_G is undefined, then ROP_s and EP_s should be too.
 ...
 1401 FORMAT('Error 1400: Inconsistent solids phase ',I2,' ',          &
         'information for IC',/'region ',I3,'. ',A,' should not be specified.')
...
! check that solids phase m velocity components are initialized
         IF(BASIC_IC) THEN
            ...
            IF(GRANULAR_ENERGY .AND. IC_THETA_M(ICV,M)==ZERO.AND.(.NOT.SKIP(M))) THEN
               IF(PRINCETON) THEN
                  WRITE(ERR_MSG, 1002)'SRIVASTAVA',trim(iVar('IC_Theta_M',ICV,M))
                  CALL LOG_ERROR()
               ENDIF
               IF(GUO_BOYCE) THEN
                  WRITE(ERR_MSG, 1002)'GUO_BOYCE',trim(iVar('IC_Theta_M',ICV,M))
                  CALL LOG_ERROR()
               ENDIF
            ENDIF
...
! Summing to zero is not an error for PATCH IC regions.
 ...
 1402 FORMAT('Error 1402: IC_X_s(',I3,',',I2,',:) do not sum to 1 ', &
 ...
 1403 FORMAT('Error 1403: IC_X_s(',I3,',',I2,':) do not sum to 1 ',  &
 ...
 1404 FORMAT('Error 1404: IC_X_s(',I3,',',I2,':) do not sum to 1 ',  &
         'or 0 and',/'they are not needed.')
...
! Verify that the volume fractions sum to one.
 ...
 1410 FORMAT('Error 1410: Illegal initial condition region : ',I3,/    &
      'Sum of volume fractions does not equal 1.')
 ...
 1002 FORMAT('Error 1002: Zero initial granular temperature is not allowed', /&
      'when using the ',A,' frictional model.',/&
      'Please set a positive value for ',A)
