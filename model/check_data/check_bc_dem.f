...
!---------------------------------------------------------------------//
! User specified BC
      use bc, only: MASS_INFLOW, MASS_OUTFLOW, P_INFLOW, P_OUTFLOW, CG_MI, CG_PO, OUTFLOW, BC_TYPE_ENUM, BC_TYPE
...
! Determine the number of mass inlets that contain DEM solids.
         CASE (MASS_INFLOW)
            M_LP: DO M=1,M_TOT
               IF((SOLIDS_MODEL(M)=='DEM'.OR.SOLIDS_MODEL(M)=='CGP' .OR.SOLIDS_MODEL(M)=='SQP' .OR.SOLIDS_MODEL(M)=='GSP') .AND.  &
...
! Particle Size Distribution data check
...
!vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv!
!                                                                      !
! Subroutine: CHECK_BC_PSD                                             !
! Author: J.Dietiker                                  Date: 01-Dec-21  !
!                                                                      !
! Purpose: Check BC_PSD keywords.                                      !
! Moved the data check in this routine so it can be used both          !
! for DEM and PIC.                                                     !
!                                                                      !
!vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv!
      SUBROUTINE CHECK_BC_PSD(BCV,M)
...
! Coarse Grain DEM
      use discretelement, only: CGDEM,CGP_STAT_WT,CGP_SCALING_METHOD
! SQP polydispersity
      use discretelement, only: SuperDEM, SQP_POLY
! MPPIC
      use mfix_pic, only: DES_STAT_WT, MPPIC
! Global Parameters:
!---------------------------------------------------------------------//
...
! particles generated ?!?
            ...
            IF(SuperDEM .and. TRIM(BC_PSD_TYPE(BCV,M))/='MONO') SQP_POLY = .TRUE.
