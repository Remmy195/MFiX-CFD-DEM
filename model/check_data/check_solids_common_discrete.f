...
! particles generated ?!?
            ...
            IF(SuperDEM .and. TRIM(IC_PSD_TYPE(ICV,M))/='MONO') SQP_POLY = .TRUE.
...
! Notify that interpolation is not support for thermo variables
 ...
 2000 FORMAT('The selected interpolation scheme (',A,    &
         ') is not',/'supported by the DES Species equation implemen', &
         'tation. All energy',/'equation variables will use the ',     &
         'centroid method for interphase',/'data exchange.')

 2200 FORMAT('The DMP implementation of the stiff ',          &
            'chemistry solver',/,'does not support interpolation.')
...
! Cannot use interpolation when no scheme is selected.
      ...
      CASE ('DPVM_SATELLITE')
         DES_INTERP_SCHEME_ENUM = DES_INTERP_SATELLITE
