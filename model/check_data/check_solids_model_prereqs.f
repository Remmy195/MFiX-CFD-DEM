#include "error.inc"

MODULE CHECK_SOLIDS_MODEL_PREREQS_MOD

   use error_manager

CONTAINS

!vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv!
!                                                                      !
!  SUBROUTINE: CHECK_SOLIDS_MODEL_PREREQS                              !
!  Purpose: Check the distributed parallel namelist variables.         !
!                                                                      !
!  Author: P. Nicoletti                               Date: 14-DEC-99  !
!  Reviewer: J.Musser                                 Date: 16-Jan-14  !
!                                                                      !
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^!
      SUBROUTINE CHECK_SOLIDS_MODEL_PREREQS

! Modules
!---------------------------------------------------------------------//
! EP_STAR should not be used for any lagrangian model
      use constant, only: ep_star
! Flag: Use DES E-L model
      use discretelement, only: DISCRETE_ELEMENT
! Flag: Use TFM and DEM solids models.
      use discretelement, only: DES_CONTINUUM_HYBRID
! Flag: gas/solids E-L simulation, otherwise granular flow.
      use discretelement, only: DES_CONTINUUM_COUPLED
! Flag: Fluid affects particles, but particles do not impact fluid.
      use discretelement, only: DES_ONEWAY_COUPLED
! Number of discrete solids phases.
      use discretelement, only: DES_MMAX
! Print E-L data.
      use discretelement, only: PRINT_DES_DATA
! Flag: Use CGDEM E-L model
      use discretelement, only: CGDEM

! Flag: Use SuperDEM E-L model
      use discretelement, only: SuperDEM, SQP_a, SQP_b, SQP_c

! Flag: Use GluedSphereDEM E-L model
      use discretelement, only: GluedSphereDEM, GSP_EXPLICIT, GSP_IMPLICIT, GSP_MODEL

! Clipping linit for void fraction
      use discretelement, only: DES_EPG_CLIP

! Maximum number of solids phases.
      use param, only: DIM_M

! constants
      use param1, only: UNDEFINED, ZERO, ONE

! Number of phases specified by the user.
      use physprop, only: MMAX, SMAX
! User specified constant gas density
      use physprop, only: RO_g0

! Flag: Use MPPIC E-L model
      use mfix_pic, only: MPPIC
! Flag: Method of manufactured solutions (MMS)
      use mms, only: USE_MMS

      use run, only: SOLIDS_MODEL
! Flag: Use DES E-E solids model
      use run, only: TFM_SOLIDS, TFM_COUNT
      use run, only: DEM_SOLIDS, DEM_COUNT
      use run, only: PIC_SOLIDS, PIC_COUNT
      use run, only: CGP_SOLIDS, CGP_COUNT
      use run, only: SQP_SOLIDS, SQP_COUNT
      use run, only: GSP_SOLIDS, GSP_COUNT

! Flag: Solve species eq.
      USE run, only: SPECIES_EQ, ANY_SPECIES_EQ, ANY_SOLIDS_SPECIES_EQ
! Kinetic theory model for TFM solids.
      use run, only: KT_TYPE

! Number of scalar equations to solve
      USE scalars, only: NSCALAR, phase4scalar
! Phase associated with scalar transport.
      USE scalars, only: phase4scalar
! Skip data check when doing preprocessing only
      USE run, only:ppo

! check statistic weight
      USE ic, only: ic_pic_const_statwt

      implicit none

! Local Variables
!---------------------------------------------------------------------//
! Loop counter
      INTEGER :: M ! Phase index
! Error indicator for mmax
      LOGICAL :: ERR_MMAX
! Dummy integer
      INTEGER :: N
!......................................................................!

      IF(PPO) RETURN

! Moved this here but will need to discuss if this is best route
! going forward
      ERR_MMAX = .FALSE.
      IF (MMAX < 0) ERR_MMAX = .TRUE.
      IF(TRIM(KT_TYPE) == 'GHD') THEN
         IF (MMAX+1 > DIM_M) ERR_MMAX = .TRUE.
      ELSE
         IF (MMAX > DIM_M) ERR_MMAX = .TRUE.
      ENDIF
      IF((USE_MMS).AND.(MMAX > 1)) ERR_MMAX = .TRUE.

! Check MMAX
      IF (ERR_MMAX) THEN
         WRITE(ERR_MSG, 1000) iVal(DIM_M)
         CALL LOG_ERROR()
      ENDIF
 1000 FORMAT('Error 1000: MMAX out of range. Min: 0, Max: ',A)

     TFM_COUNT = 0
     DEM_COUNT = 0
     PIC_COUNT = 0
     CGP_COUNT = 0
     SQP_COUNT = 0
     GSP_COUNT = 0
! Loop over the phases to see what was specified.
      DO M=1, MMAX
         SOLIDS_MODEL(M) = trim(adjustl(SOLIDS_MODEL(M)))
         SELECT CASE(SOLIDS_MODEL(M))
         CASE ('TFM'); TFM_COUNT = TFM_COUNT + 1
         CASE ('DEM'); DEM_COUNT = DEM_COUNT + 1
         CASE ('PIC'); PIC_COUNT = PIC_COUNT + 1
         CASE ('CGP'); CGP_COUNT = CGP_COUNT + 1
         CASE ('SQP'); SQP_COUNT = SQP_COUNT + 1
         CASE ('GSP'); GSP_COUNT = GSP_COUNT + 1
         CASE DEFAULT
            WRITE(ERR_MSG,1001) iVar('SOLIDS_MODEL',M), SOLIDS_MODEL(M)
            CALL LOG_ERROR()

 1001 FORMAT('Error 1001: Unknown solids model: ',A,' = ',A)

         END SELECT
      ENDDO

! Set the number of discrete phases.
      DES_MMAX = DEM_COUNT + PIC_COUNT + CGP_COUNT + SQP_COUNT + GSP_COUNT

! Set the number of TFM phases.  (should be equivalent to tfm_count)
      MMAX = MMAX - DES_MMAX
      SMAX = MMAX   ! USE smax in the TFM code for the number of TFM phases
! For GHD theory increase MMAX by one to serve as 'mixture' phase
      IF(TRIM(KT_TYPE) == 'GHD') THEN
         MMAX = MMAX + 1
         TFM_COUNT = TFM_COUNT + 1
         SOLIDS_MODEL(MMAX) = 'TFM'
      ENDIF


! Clear out the unused phases.
      SOLIDS_MODEL((MMAX+DES_MMAX)+1:DIM_M) = '---'

! Set the runtime flags:
      TFM_SOLIDS = (TFM_COUNT > 0)
      DEM_SOLIDS = (DEM_COUNT > 0)
      PIC_SOLIDS = (PIC_COUNT > 0)
      CGP_SOLIDS = (CGP_COUNT > 0)
      SQP_SOLIDS = (SQP_COUNT > 0)
      GSP_SOLIDS = (GSP_COUNT > 0)

      if(CGP_SOLIDS .OR. SQP_SOLIDS .OR. GSP_SOLIDS) DEM_SOLIDS = .true.

! MPPIC and continuum solids don't mix.
      IF(PIC_SOLIDS .AND. TFM_SOLIDS)THEN
         WRITE(ERR_MSG, 1002)
         CALL LOG_ERROR()
      ENDIF
 1002 FORMAT('Error 1002: MPPIC solids and continuum solids cannot be combined.')

! MPPIC and DEM solids don't mix.
      IF(PIC_SOLIDS .AND. DEM_SOLIDS)THEN
         WRITE(ERR_MSG, 1003)
         CALL LOG_ERROR()
      ENDIF
 1003 FORMAT('Error 1003: MPPIC solids and DES solids cannot be combined.')

! MPPIC and CGP solids don't mix.
      IF(PIC_SOLIDS .AND. CGP_SOLIDS)THEN
         WRITE(ERR_MSG, 1005)
         CALL LOG_ERROR()
      ENDIF
 1005 FORMAT('Error 1005: MPPIC solids and CGP solids cannot be combined.')

 ! MPPIC and SQP solids don't mix.
      IF(PIC_SOLIDS .AND. SQP_SOLIDS)THEN
         WRITE(ERR_MSG, 1006)
         CALL LOG_ERROR()
      ENDIF
 1006 FORMAT('Error 1006: MPPIC solids and SQP solids cannot be combined.')

 ! MPPIC and GSP solids don't mix.
      IF(PIC_SOLIDS .AND. GSP_SOLIDS)THEN
         WRITE(ERR_MSG, 1007)
         CALL LOG_ERROR()
      ENDIF
 1007 FORMAT('Error 1007: MPPIC solids and GSP solids cannot be combined.')

! temporary move for now since these rely on definition of mmax/smax
! Set variable ANY_SPECIES_EQ
      M = max(SMAX, DES_MMAX)
      ANY_SPECIES_EQ = any(SPECIES_EQ(:M))
      ANY_SOLIDS_SPECIES_EQ = any(SPECIES_EQ(1:M))

! Check phase specification for scalars
      DO N = 1, NScalar
         IF(Phase4Scalar(N) < 0 .OR. Phase4Scalar(N) > MMAX) THEN
            WRITE(ERR_MSG,1004) iVar('Phase4Scalar',N), &
               iVal(phase4scalar(N))
            CALL LOG_ERROR()
         ENDIF
      ENDDO
 1004 FORMAT('Error 1004: Invalid or unknown input: ',A,' = ',A)

! Set the DEM runtime flag.
      DISCRETE_ELEMENT = DEM_SOLIDS .OR. PIC_SOLIDS .OR. CGP_SOLIDS.OR. SQP_SOLIDS .OR. GSP_SOLIDS
! Set the MMPIC runtime flag.
      MPPIC = PIC_SOLIDS
! Set the CGDEM runtime flag.
      CGDEM = CGP_SOLIDS
! Set the SuperDEM runtime flag.
      SuperDEM = SQP_SOLIDS
! Set the GluedSphereDEM runtime flag.
      GluedSphereDEM = GSP_SOLIDS

      IF(GluedSPhereDEM) THEN
         gsp_explicit = (gsp_model == 'EXPLICIT')
         gsp_implicit = (gsp_model == 'IMPLICIT')
      ENDIF
! Set the Hybrid flag.
      DES_CONTINUUM_HYBRID = ((DEM_SOLIDS .OR. CGP_SOLIDS .OR. SQP_SOLIDS .OR. GSP_SOLIDS).AND. TFM_SOLIDS)
! Set flag for coupled simulations
      DES_CONTINUUM_COUPLED = .NOT.(RO_g0 == 0.0d0)

      IF(DES_CONTINUUM_HYBRID) CALL HYBRID_HACK

! Overwrite user settings if no Lagrangian solids
      IF(.NOT.DISCRETE_ELEMENT) THEN
         DES_CONTINUUM_COUPLED = .FALSE.   ! This keyword might get removed.
         PRINT_DES_DATA = .FALSE.
         DES_ONEWAY_COUPLED = .FALSE.
      ENDIF

      IF(DISCRETE_ELEMENT.AND.EP_STAR/=UNDEFINED) THEN
         IF(DES_EPG_CLIP==UNDEFINED) THEN
            DES_EPG_CLIP = EP_STAR
            EP_STAR = UNDEFINED
            WRITE(ERR_MSG, 2000) DES_EPG_CLIP
            CALL LOG_WARNING()
         ELSE  ! DES_EPG_CLIP is defined
            IF(DES_EPG_CLIP<ZERO.OR.DES_EPG_CLIP>ONE) THEN
               WRITE(ERR_MSG, 2010) DES_EPG_CLIP
               CALL LOG_ERROR()
            ENDIF
         ENDIF
      ENDIF


2000 FORMAT('Warning 2000: Specifying EP_STAR for DEM/PIC/CGP/SQP/GSP simulations is deprecated.', &
            /'Unsetting EP_STAR and setting DES_EPG_CLIP to ',E14.8)
2010 FORMAT('Error 2010: Invalid value for DES_EPG_CLIP, must be between zero and one.', &
            /'Current value of DES_EPG_CLIP: ',E14.8)

      RETURN

      END SUBROUTINE CHECK_SOLIDS_MODEL_PREREQS


!vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv!
!                                                                      !
!  SUBROUTINE: Hybrid_HACK                                             !
!  Purpose: Check the distributed parallel namelist variables.         !
!                                                                      !
!  Author: P. Nicoletti                               Date: 14-DEC-99  !
!  Reviewer: J.Musser                                 Date: 16-Jan-14  !
!                                                                      !
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^!
      SUBROUTINE HYBRID_HACK

! Modules
!---------------------------------------------------------------------//
! Maximum number of solids phases.
      use param, only: DIM_M
      use error_manager
      use run, only: SOLIDS_MODEL

! Local variables
!---------------------------------------------------------------------//
      integer :: TFM_MAX
      integer :: DEM_MIN
      integer :: M
!......................................................................!

! Initialize the loop variables.
      TFM_MAX = -DIM_M
      DEM_MIN =  DIM_M

! Loop over the phases to see what was specified.
      DO M=1, DIM_M
         SELECT CASE(SOLIDS_MODEL(M))
         CASE ('TFM'); TFM_MAX = max(M, TFM_MAX)
         CASE ('DEM', 'CGP','SQP', 'GSP'); DEM_MIN = min(M, DEM_MIN)
         END SELECT
      ENDDO

      write(*,*)'  MAX TFM index: ', TFM_MAX
      write(*,*)'  MIN DEM index: ', DEM_MIN

      IF(DEM_MIN < TFM_MAX) THEN
         WRITE(ERR_MSG, 2000)
         CALL LOG_ERROR()
      ENDIF

 2000 FORMAT('Error 2000: Illegal phase specification for hybrid ',    &
         'model. All TFM',/'solids must be defined before DEM solids.')
      RETURN

   END SUBROUTINE HYBRID_HACK

END MODULE CHECK_SOLIDS_MODEL_PREREQS_MOD
