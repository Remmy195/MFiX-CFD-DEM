...
! User defined reaction names from reaction blocks @(RXNS)
      ...
      use parse, only: reverse_calc, rxn_order_array
      use parse, only: DES_reverse_calc, DES_rxn_order_array
...
! Undefined indicates that no reaction block was found in the input file.
...
! species.inc file.
      ...
      IF(NO_OF_RXNS > 0) THEN
         CALL CHECK_CHEMICAL_RXNS_COMMON(MFIX_DAT, NO_OF_RXNS, RXN_NAME,         &
            RXN_CHEM_EQ, usrDH, usrfDH, REACTION, reverse_calc, rxn_order_array)
      ENDIF

      IF(NO_OF_DES_RXNS > 0) THEN
         CALL CHECK_CHEMICAL_RXNS_COMMON(MFIX_DAT, NO_OF_DES_RXNS, DES_RXN_NAME, &
            DES_RXN_CHEM_EQ, DES_usrDH, DES_usrfDH, DES_REACTION, DES_reverse_calc, &
	    DES_rxn_order_array)
      ENDIF
 ...
 1000 FORMAT('Reaction rates for fluid reactions will be automatically computed based on Arrhenius expression.',/, &
             'Any custom reaction rates coded in usr_rates.f will be ignored.')

 1100 FORMAT('Reaction rates for DES reactions will be automatically computed based on Arrhenius expression.',/, &
             'Any custom reaction rates coded in usr_rates_des.f will be ignored.')
...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^!
      SUBROUTINE CHECK_CHEMICAL_RXNS_COMMON(MFIX_DAT, COUNT, NAME, CHEM_EQ, DH,  &
         fDH, RXN_ARRAY, REVERSECALC, RXNORDER)
...
! Array about how to calculate the rate constant for reverse reactions.
      CHARACTER(len=*), INTENT(IN) :: REVERSECALC(DIMENSION_RXN)
! Array of user defined species reaction order.
      CHARACTER(len=*), INTENT(IN) :: RXNORDER(DIMENSION_RXN)
...
! Take the data read from the data file and populate the reaction block.
         CALL setReaction(This, NMAX(0), SPECIES_ALIAS_g(:),lMMAX,     &
            NMAX(1:lMMAX), SPECIES_ALIAS_s(:,:), DH(L), fDH(L,:), REVERSECALC(L), RXNORDER(L))
...
! heat of reaction is given, flag error and exit.
 ...
 1200 FORMAT('Error 1200: Inconsistent user input. Energy equations ', &
         ...
         ' was detected',' for chemical',/' reaction ',A,'.')
...
! check and should never be true.
 ...
 1201 FORMAT('Error 1201: Inconsistent data returned from setReaction ',  &
