...
! Check that Bdist_IO is not enabled for restart
      if (bDist_IO) then
         if (RES_BACKUPS /= UNDEFINED_I .or. RES_BACKUP_DT /= UNDEFINED) then
             WRITE(ERR_MSG, "(A)")&
             "BDIST_IO cannot be enabled when RES_BACKUPS is used."
             CALL LOG_ERROR()
         endif
      endif
...
! Verify that the number of requested processes (numPEs) matches
...
! Initialize BACKGROUND_IJKEND3 (for re-indexing and dynamic load balance)
 ...
 1001 FORMAT('Warning 1001: Setting NODES',A1,' to default: NODES',A1,'=1.')
