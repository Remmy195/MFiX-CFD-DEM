...
MODULE CHECK_DATA_30_MOD
   ...
   use error_manager, only: err_msg, iVAR, loglevel_error, loglevel_info, loglevel_warning, log_message
...
4000  FORMAT(&
'Time = ', G12.5, /'The sum of mass fractions is not equal to one.')
...
4100  FORMAT(//'Statistics of sum of gas species mass fraction', / &
      ...
      IF (REPORT_SPECIES(0)) THEN
         ...
         ! Fortran G12.5 right-padding is inconsistent
...
4200  FORMAT(//'Statistics of sum of solids phase ', I2, ' species ', &
      ...
      DO M = 1, SMAX
         IF (REPORT_SPECIES(M)) THEN
            lSUM = SUM(COUNT(M, :))
            WRITE (ERR_MSG, 4200) M, MIN_VAL(M), MAX_VAL(M), trim(iVAR('X_s', M))
            CALL LOG_MESSAGE(__FILE__, __LINE__, LOGLEVEL_INFO, HEADER=.FALSE., FOOTER=.FALSE.)
            ! fixme inconsistent right-padding
...
! tolerance for force and exit.
      ...
      RETURN
5000  FORMAT(5X, 'Simulation time = ', G12.5, / &
