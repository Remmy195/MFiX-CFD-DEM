...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^!
      ...
      SELECT CASE(L)
      CASE(2); CALL WRITE_LINF_NORM
      CASE(1); CALL WRITE_INT_VOLS
      END SELECT
...
! Loop index
      ...
      DO NP = 1, MAX_PIP
         !IF(PEA(NP,1) .AND. .NOT.PEA(NP,4)) THEN
            ...
            X_ERR = DES_USR_VAR(1,NP) - DES_POS_NEW(NP,1)
            Y_ERR = DES_USR_VAR(2,NP) - DES_POS_NEW(NP,2)
            Z_ERR = DES_USR_VAR(3,NP) - DES_POS_NEW(NP,3)
