...
! Alias particle temperature.
      Tp = DES_T_s(NP)
! Calculate real particle diameter for CGDEM.
      IF(CGDEM) THEN 
         Dp0 = 2.0d0 * DES_RADIUS(NP)/(DES_CGP_STW(NP)**(1.0d0/3.0d0))
      ELSE
...
! Reaction rates:
!`````````````````````````````````````````````````````````````````````//
! Change the gas constant in the denominator based on cgs/si units
! R = 8314.56d4	erg/mol-K
! R = 8314.56	J/kmol-K
! Concentration of water vapor at droplet surface (mol-Vapor/cm^3)
      Cm_H2O = Psat_H2O(Tp) / (8314.56 * T_G(IJK))
...
! Concentration of water vapor in bluk gas phase (mol-Vapor/cm^3)
      Cg_H2O = (Y_H2O * P_G(IJK)) / (8314.56 * T_G(IJK))
...
! Scale reaction rate for CGDEM
      IF(CGDEM)  DES_RATES(:) = DES_RATES(:)*DES_CGP_STW(NP)
...
! Do not multiply by 10.0d0 if using SI units - Avinash
...
! Change the denominator based on cgs/si units
      ref_Patm = ref_P/1.01325d5

! The terms in the multiplier are changed from 1.87d-6 and 27.5d-6 since SI units is being used - Avinash Vaidheeswaran
      IF( ref_T < 450.0d0) THEN
         Diff_H2O_Air = (1.87d-10/ref_Patm) * (ref_T**2.072d0)
      ELSE
         Diff_H2O_Air = (27.5d-10/ref_Patm) * (ref_T**1.632d0)
      ENDIF
...
! Reynods Number
      IF(MU_g(IJK) > ZERO) THEN
         cal_NRe = Dp0 * VREL * RO_g(IJK) / MU_g(IJK)
      ELSE
