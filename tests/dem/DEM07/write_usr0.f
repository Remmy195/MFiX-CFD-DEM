...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^!
      ...
      CALL WRITE_DAT_HEADER('POST_POS_VEL.dat','Pos')
...
!----------------------------------------------------------------------!
      ...
      use discretelement, only: KN, KN_W, MEW, MEW_W, MEW_RW
...
!         10x,'DES_EN_WALL_INPUT = ',T30,G12.4)
 ...
 1200 FORMAT(/7X,'Time',7x,A,13X,A)
