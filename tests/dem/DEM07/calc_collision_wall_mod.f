...
! local normal and tangential forces
      ...
      DOUBLE PRECISION, DIMENSION(DIMN) :: FTAN, FNORM, OVERLAP_T, FNORMC
...
! Update by Avinash Vaidheeswaran - April 2021
! Contribution of normal contact force
! (Torque due to rolling friction is only a function of normal contact force)
            FNORMC(:) = -(KN_DES_W * OVERLAP_N * NORMAL(:))

! Rolling friction
            if(MEW_RW>ZERO) then
               OMEGA_MAG = sqrt(dot_product(OMEGA_NEW(LL,:),OMEGA_NEW(LL,:)))
               if(OMEGA_MAG > ZERO) then
                 RFTOW = -MEW_RW*sqrt(dot_product(FNORMC,FNORMC))*OMEGA_NEW(LL,:)/OMEGA_MAG
                 TOW(LL,:) = TOW(LL,:) + RFTOW
               endif
            endif
