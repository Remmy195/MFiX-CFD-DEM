...
#TODO add time step summary to dashboard
# Add tooltips for NIT
# Make "project version" readonly
...
class Dashboard:
    ...
    def update_dashboard(self, status):
        ...
        digits = 2 # show 0.01 seconds

        elapsed_time = status.get('elapsed_time', 0)
        paused_time = status.get('paused_time', 0)
        io_time = status.get('io_time', 0)
        remaining_time = status.get('remaining_time', 0)
        paused = status.get('paused', False)

        ui.lineedit_simtime.setText(
        ...
        ui.lineedit_io_time.setText( num_to_time(io_time,digits=digits))
        ...
        # vs total wall time, which affects the calculation.  So when paused,
        ...
        if dt is not None:
            ...
            if rg > 0:
                ui.progressbar_dt.setValue(int((dt-self.dt_min)/rg*100))
        ...
        if nit is not None:
            ...
            if rg > 0:
                ui.progressbar_nit.setValue(int((nit-self.nit_min)/rg*100))
