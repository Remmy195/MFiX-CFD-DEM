...
class SpeciesHandler:
    ...
    def species_mol_weight(self, species):
        ...
        fs_lower = {k.lower():v for k,v in self.fluid_species.items()}
        if s_lower in fs_lower:
            return fs_lower[s_lower].get('mol_weight')
        for (p, subdict) in self.solids_species.items():
            ss_lower = {k.lower():v for k,v in subdict.items()}
            if s_lower in ss_lower:
                return ss_lower[s_lower].get('mol_weight')
        return None
