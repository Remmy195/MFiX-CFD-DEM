...
from mfixgui.tools import safe_float, sqp_volume
...
class ICS:
    ...
    def init_ics(self):
        ...
        ui.checkbox_keyword_ic_override_part_input_args_IC_P.post_update = self.handle_ic_override
        ...
        ui.tablewidget_fluid_mass_fraction.key = 'ic_x_g'
        ui.groupbox_solids_composition.key = 'ic_x_s'
        ui.tablewidget_solids_mass_fraction.key = 'ic_x_s'

        ui.combobox_ic_psd.activated.connect(self.handle_ic_psd)
        ui.combobox_ic_psd.setToolTip("Select particle size distribution.")
        ui.label_ic_psd.keys = ui.combobox_ic_psd.keys = ['ic_psd_type',
                                                          'ic_psd_mean_dp',
                                                          'ic_psd_stdev',
                                                          'ic_psd_min_dp',
                                                          'ic_psd_max_dp']
        ui.label_ic_psd.setToolTip("Select particle size distribution.")

        cb = ui.combobox_solids_content_type
        cb.activated.connect(self.handle_ics_solids_content_type)

        for (i, msg) in enumerate(('Specify volume fraction.',
                                   'Specify total particle mass.',
        ...
        ui.groupbox_dem_seeding.setToolTip("Particle seeding options.")
        ...
        ui.checkbox_keyword_ic_gsp_random_q_args_IC_P.setVisible(False) # avoid seeing double

    def handle_ic_override(self):
        ...
        IC0 = self.ics_current_indices[0]
        model = None if P is None else self.project.get_value('solids_model', args=[P], default='TFM')
        is_dem = model in ('DEM', 'CGP','SQP','GSP')
        gener_part_config = self.project.get_value('gener_part_config', default=False)
        override = self.project.get_value('ic_override_part_input',
                                          args=[IC0,P],
                                          default=False)
        if not is_dem:
            return
        if gener_part_config:
            ...
            enable = override
            enable = True # TODO look at CSV file header and to determine which inputs to disable XXX

        for w in (ui.frame_solids_parameters,
                  ui.groupbox_solids_composition,
                  ui.tablewidget_solids_mass_fraction):
    ...
    def handle_ic_dem_seeding(self, clicked):
        ...
        height = ui.checkbox_keyword_ic_sqp_random_q_args_IC_P.height() # height of checkbox
        if not clicked:
    ...
    def handle_ics_solids_content_type(self, idx):
        ...
        ui.label_solids_content_units.setText('kg' if idx==1 else '')
        ...
        self.ics_estimate_particles()
        set_combobox_tooltip(ui.combobox_solids_content_type)
...
#           self.update_ic_ep_g(indices=self.ics_current_indices)
    ...
    def handle_ics_solids_content(self, widget, val_dict, args):
        if not self.ics_current_indices:
            return
        if not self.ics_current_solid:
            return
        ...
        if self.project.solver in (DEM,CGP,SQP,GSP,GSP) and not self.project.get_value('gener_part_config', default=False):
            key = 'ic_ep_s'
            ...
            if total > 0 or inventory:
                ...
                if rsp == 'yes':
                    ...
                    if self.project.get_value('particles', default=0)>0:
        ...
        for k in ['ic_ep_s', 'ic_des_sm', 'ic_des_np']:
            if k == widget.key:
                prev_val = self.project.get_value(k, args=[IC0,P])
                continue
            for IC in self.ics_current_indices:
                self.retain_keyword(k, args=[IC,P])
                self.unset_keyword(k, args=[IC,P])

        self.project.submit_change(widget, val_dict, args)
        self.update_ic_ep_g(indices=self.ics_current_indices)
        self.ics_estimate_particles()
        if self.ics_check_overpack(IC0):
            region_name = self.ics.get(IC0,{}).get('region','')
            ...
            if prev_val != val_dict.get(widget.key):
                self.warning("Resetting %s to %s" %
                             (widget.key.upper(), prev_val),
                             popup=True)

            for IC in self.ics_current_indices:
                if prev_val is not None:
                    self.update_keyword(widget.key, prev_val, args=[IC,P])
                else:
                    self.unset_keyword(widget.key, args=[IC,P])
            widget.setText(str(prev_val) if prev_val is not None else '')


    def ics_check_overpack(self, IC):
        # return True if IC is overpacked
        # issues/1296 ensure IC not overpacked
        if not all(self.project.get_value('close_packed',
                                          default=True,
                                          args=[i])
                   for i in range(1,1+len(self.solids))):
            return False # Check is skipped
        ...
        if self.project.solver in (TFM, SINGLE):
            ...
            limit_key = 'des_epg_clip'
        #ep_star = self.project.get_value('ep_star')
        #des_epg_clip = self.project.get_value('des_epg_clip')
        limit_val = self.project.get_value(limit_key)
        if limit_val is None:
            return False
        if ic_ep_g < limit_val:
            return True
        return False
    ...
    def ics_estimate_particles(self):
        ...
        if ic_ep_s == 0 or ic_des_np == 0 or ic_des_sm == 0:
            ...
            # Compute single particle volume
            solids_model = self.project.get_value('solids_model', default='TFM', args=[P])
            is_dem = solids_model in ('DEM', 'CGP', 'SQP', 'GSP')
            if solids_model == 'SQP':
                a = self.project.get_value('sqp_a', default=1, args=[P])
                b = self.project.get_value('sqp_b', default=1, args=[P])
                c = self.project.get_value('sqp_c', default=1, args=[P])
                m = self.project.get_value('sqp_m', default=2, args=[P])
                n = self.project.get_value('sqp_n', default=2, args=[P])
                V = sqp_volume(a,b,c,m,n)
            elif solids_model == 'GSP':
                solid_name = list(self.solids.keys())[P-1]
                solid = self.solids[solid_name]
                V = solid.get('volume', None)
                if V is None:
                    V = 𝜋 * d_p0**3 / 6 # default is single-sphere
                    gsp_dir = "GSP-%s"%P
                    fname = gsp_dir+"/gsp_properties.dat"
                    if os.path.exists(fname):
                        self.print_internal("Reading %s"%fname, color='blue')
                        try:
                            with open(fname) as f:
                                for line in f:
                                    if '=' in line:
                                        tok = line.split('=')
                                        if tok[0].strip() == 'vol' and len(tok) == 2:
                                            V = float(tok[1])
                                            solid['volume'] = V
                                            break
                        except Exception as e:
                            self.warn("Error %s loading %s" % (e, fname))
                    else:
                        pass # Assume it's a sphere, keep value computed above

            else: # Sphere
                V = 𝜋 * d_p0**3 / 6

            # 1: User specified volume fraction, estimate mass and number
            ic_vol = sum(self.ic_compute_volume(IC) for IC in self.ics_current_indices)
            if ic_ep_s is not None and ic_vol != 0:
                if d_p0 is not None and V:
                    npart =  (ic_ep_s * ic_vol) / V / stat_wt
                if 𝜌:
                    mpart = 𝜌 * ic_ep_s * ic_vol

            # 2: User specified total mass (inventory), estimate volume fraction and number
            elif ic_des_sm is not None and V:
                if d_p0 is not None and 𝜌:
                    npart = ic_des_sm / (V*𝜌) / stat_wt

            # 3: User specified number of particles, estimate volume fraction and mass
            elif ic_des_np is not None and V!=0:
                if d_p0 is not None and 𝜌:
    ...
    def ic_compute_volume(self, IC):
        ...
        if self.project.get_value('no_k'):
            ...
            z_max = self.project.get_value('ic_z_t', args=[IC])
            z_min = self.project.get_value('ic_z_b', args=[IC])
        x_max = self.project.get_value('ic_x_e', args=[IC])
        x_min = self.project.get_value('ic_x_w', args=[IC])
        y_max = self.project.get_value('ic_y_n', args=[IC])
        y_min = self.project.get_value('ic_y_s', args=[IC])
    ...
    def ics_cancel_add(self):
        ...
        self.enable_widget(ui.tablewidget_regions)
        ...
        if row is not None:
            ...
            if nrows > 1:
                self.set_widget_enabled(ui.toolbutton_up, row != 0)
                self.set_widget_enabled(ui.toolbutton_down, row != nrows-1)
        else:
    ...
    def ics_add_regions_1(self, selections, indices=None, autoselect=False):
        ...
        if self.ics_region_dict is None:
            self.ics_region_dict = self.get_region_dict()
        ...
        for (i, region_name) in enumerate(selections):
            ...
            if region_data is None: # ?
                self.warn("no data for IC region %s" % region_name)
                continue
    ...
    def ics_delete_regions(self):
        ...
        nrows = tw.rowCount()
        row = get_selected_row(tw)
        self.set_widget_enabled(ui.toolbutton_up, row is not None and nrows > 1 and row != 0)
        self.set_widget_enabled(ui.toolbutton_down, row is not None and nrows > 1 and row != nrows-1)
    ...
    def handle_ics_region_selection(self):
        ...
        if not enabled:
            ...
            self.update_ics_solids_mass_fraction_table()
            self.fixup_ics_table(ui.tablewidget_fluid_mass_fraction)
            self.fixup_ics_table(ui.tablewidget_solids_mass_fraction)
    ...
    def update_ics_solids_mass_fraction_table(self):
        ...
        if not (P and self.solids_species.get(P) and self.ics_current_indices):
            ...
            self.disable_widget(ui.groupbox_solids_composition,
                                reason="No species defined." if P and self.ics_current_indices and not self.solids_species.get(P)
                                else "")
        ...
        is_dem = model in ('DEM', 'CGP','SQP','GSP')
        gener_part_config = self.project.get_value('gener_part_config', default=False)
        if is_dem and not gener_part_config:
            enable = self.project.get_value("ic_override_part_input",
                                            args=[IC0, P],
                                            default=False)
            enable = True # TODO look for species in csv header line XXX

        else:
            enable = True

        self.set_widget_enabled(ui.groupbox_solids_composition, enable)
        self.set_widget_enabled(table, enable)
    ...
    def setup_ics_fluid_tab(self):
        ...
        if not enabled:
            ...
            tw.setRowCount(0)
        self.fixup_ics_table(tw)
        self.set_widget_enabled(tw, enabled)
        ...
        def setup_key_widget(key, default=None, enabled=True, reason=None):
        ...
        self.set_widget_enabled(gb, enabled, reason="Requires turbulence model")
        for w in widget_iter(gb):
            if not hasattr(w, 'toolTip'):
                continue
            if enabled:
                ...
                self.disable_widget(w, reason=reason)
        ...
        self.set_widget_enabled(ui.groupbox_fluid_advanced, enabled)
        ...
        default = 0.0
        enabled = bool(energy_eq)
        setup_key_widget(key, default, enabled, reason='Requires energy equations.')
    ...
    def setup_ics_solids_tab(self, P):
        ...
        gener_part_config = self.project.get_value('gener_part_config', default=False)
        if not is_dem:
            reason = "Requires DEM/CGP/SQP/GSP solids."
            for i in (1,2): # Disable inventory/count
                item = get_combobox_item(cb,i)
        ...
        if not (is_dem and gener_part_config):
            #self.disable_widget(gb, reason=reason)
            gb.setVisible(False)
            #gb.setChecked(False)
            ...
            gb.setVisible(True)
            self.enable_widget(gb)
        if not is_dem:
            ui.checkbox_keyword_ic_override_part_input_args_IC_P.setVisible(False)
        else:
            ui.checkbox_keyword_ic_override_part_input_args_IC_P.setVisible(not gener_part_config)
            self.handle_ic_override()

        self.handle_ic_dem_seeding(gb.isChecked())
        ...
        cb = ui.checkbox_keyword_ic_override_part_input_args_IC_P
        if is_dem and not gener_part_config:
            ic_override = self.project.get_value('ic_override_part_input',
                                                 args=[IC0,P],
                                                 default=False)
            cb.setChecked(ic_override)
        ...
        def setup_key_widget(key, default=None, enabled=True, reason=None, update=True):
            for pat in ('label_%s', 'label_%s_units',
        ...
        friction_model = self.project.get_value('friction_model',
                                                default=DEFAULT_FRICTION_MODEL)
        exclude_min =  friction_model in ('SRIVASTAVA', 'GUO_BOYCE')
        for le in (ui.lineedit_keyword_ic_theta_m_args_IC_P,):
            le.exclude_min = exclude_min
            le.required = True

        # Particle size distribution
        #  Not available for TFM solids, #1356
        model = self.project.get_value('solids_model', args=[P], default='TFM')
        la = ui.label_ic_psd
        cb = ui.combobox_ic_psd
        if model == 'TFM':
            #reason = "Not available for TFM solids."
            #self.disable_widget(la, reason=reason)
            #self.disable_widget(cb, reason=reason)
            ...
            cb.setVisible(True)
            for w in (la, cb):
                self.set_widget_enabled(w, enabled=gener_part_config if is_dem else True,
                                        reason="Automatic particle generation (GENER_PART_CONFIG) is not enabled.")
            cb.clear()
            cb.addItem("Uniform")
            psds = list(self.psd.keys())
            cb.addItems(psds) # TODO: tooltips?
        ...
        cb = ui.combobox_solids_content_type
        for w in (le, cb):
            self.set_widget_enabled(w, gener_part_config if is_dem else True,
                                    reason="Automatic particle generation (GENER_PART_CONFIG) is not enabled.")
        ...
        key_sqp = 'ic_sqp_random_q'
        key_gsp = 'ic_gsp_random_q'
        cb_sqp = ui.checkbox_keyword_ic_sqp_random_q_args_IC_P
        cb_gsp = ui.checkbox_keyword_ic_gsp_random_q_args_IC_P
        if model in ('SQP', 'GSP'):
            cb_sqp.setVisible(model=='SQP')
            cb_gsp.setVisible(model=='GSP')
            self.enable_widget(cb_sqp)
            self.enable_widget(cb_gsp)
            cb_sqp.setChecked(bool(self.project.get_value(key_sqp, args=[IC0,P])))
            cb_gsp.setChecked(bool(self.project.get_value(key_gsp, args=[IC0,P])))
            self.add_tooltip(cb_sqp, key_sqp)
            self.add_tooltip(cb_gsp, key_gsp)
        else: #SQP checkbox is the placeholder
            cb_sqp.setVisible(False)
            cb_gsp.setVisible(False)
            #self.disable_widget(cb_sqp, reason='Only available for SQP/GSP solids.')
        ...
        key = 'ic_p_star'
        default = 0.0
        setup_key_widget(key, default, enabled, reason='Only available for TFM solids.')
        le = ui.lineedit_ic_p_star_args_IC
        le.setVisible(enabled)
        ui.label_ic_p_star.setVisible(enabled)
        ui.label_ic_p_star_units.setVisible(enabled)
        ...
        default = 0.0
        setup_key_widget(key, default, enabled,
                         reason="Only available with non-algebraic viscous stress model when using TFM solids.")
        ...
        setup_key_widget(key, default, enabled, reason="Only available for PIC solids.")
        ...
        enabled = solids_model in ('DEM','CGP','SQP','GSP') or bool(energy_eq)
        self.set_widget_enabled(ui.groupbox_solids_advanced, enabled,
                                reason="Requires energy equations when using %s model."%solids_model)
