...
from math import hypot, pi as 𝜋

from json import JSONDecoder

from qtpy import QtWidgets
from qtpy.QtWidgets import (QCheckBox, QHBoxLayout, QHeaderView, QLineEdit,
                            QTableWidgetItem, QWidget)


from qtpy.QtGui import QPixmap, QColor
from qtpy.QtCore import Qt, QTimer, QProcess
...
from mfixgui.tools import (append_row_column_triangular, drop_row_column_triangular,
                           format_key_with_args, keyword_args, safe_float, sqp_d0)
from mfixgui.tools.qt import (get_combobox_item, get_selected_row, get_pixmap,
...
TAB_MATERIALS, TAB_TFM, TAB_DEM, TAB_CGP, TAB_SQP, TAB_GSP, TAB_PIC = range(7)

gsp_preview_size = (100,100)
...
class SolidsHandler(SolidsTFM, SolidsDEM, SolidsPIC, SpeciesHandler):
    ...
    def init_solids_handler(self):
        self.solids = {} # keyed by name of solids phase
        self.solids_current_phase = None
        self.solids_species = {} #dict of dict, keyed by phase num
        self.psd = {}
        ...
        self.sq_popup = None # On-demand
        self.gs_popup = None # On-demand
        ui = self.ui.solids
        ui.dynamic_widgets = {}
        ...
        cb = ui.checkbox_des_rigid_motion
        cb.clicked.connect(self.set_des_rigid_motion)
        ...
        self.set_widget_enabled(tb, enable)
        ...
        self.add_tooltip(ui.groupbox_des_rigid_motion,
                         key = 'des_rigid_motion')

        # Note, with find-by-args this will have to select a table row, not the
        # groupbox
        # Should nmax_s be on the +/- buttons or on the groupbox?
        ui.groupbox_species.keys = ['species_alias_s', 'species_s', 'ro_xs0']# Locatability

        # force summary table to update on kw updates
        class TableWidgetProxy:
            def objectName(self):
                return "proxy"
            def updateValue(*args): # pylint: disable=no-self-argument
        ...
        cb = ui.combobox_solids_ks_model
        ui.label_solids_ks_model.keys = cb.keys = ['k_s0', 'ks_model']
        ...
        ui.pushbutton_sq_designer.clicked.connect(self.show_sq_designer)
        ui.pushbutton_gs_designer.clicked.connect(self.show_gs_designer)
        # Click on preview to open designer, as long as it's not blank
        ui.label_gsp_preview.mouseReleaseEvent = lambda args:(self.show_gs_designer() if not
                                                         self.gsp_preview_blank else None)
        ...
        for le in (ui.lineedit_keyword_sqp_m_args_P,
                   ui.lineedit_keyword_sqp_n_args_P):
            le.initpython = le.saved_value = default_values.sqp_m
            le.required = True
            le.post_update = self.update_sqp

        for le in (ui.lineedit_keyword_sqp_a_args_P,
                   ui.lineedit_keyword_sqp_b_args_P,
                   ui.lineedit_keyword_sqp_c_args_P):
            le.initpython = le.saved_value = default_values.sqp_a
            le.required = True
            le.post_update = self.update_sqp

        for i,le in enumerate((ui.lineedit_keyword_sqp_q1_args_P,
                               ui.lineedit_keyword_sqp_q2_args_P,
                               ui.lineedit_keyword_sqp_q3_args_P,
                               ui.lineedit_keyword_sqp_q4_args_P)):
            le.post_update = self.update_sqp
            le.initpython = le.saved_value = 1 if i==0 else 0
            le.required = True

        for i,le in enumerate((ui.lineedit_keyword_gsp_q1_args_P,
                               ui.lineedit_keyword_gsp_q2_args_P,
                               ui.lineedit_keyword_gsp_q3_args_P,
                               ui.lineedit_keyword_gsp_q4_args_P)):
            le.post_update = self.update_gsp
            le.initpython = le.saved_value = 1 if i==0 else 0
            le.required = True

        for le in (ui.lineedit_keyword_ep_star,
                   ui.lineedit_keyword_des_epg_clip,
                   ui.lineedit_keyword_des_epg_clip_2):
            le.post_update = lambda le=le: self.check_overpack(le)
                ...
                if key_s0 == 'ro_s0': # Extra handling: Density column changes
                    ...
                    if model == CONSTANT:
                        ...
                        for i in range(1, 1+nmax_s):
                            args = [phase,i]
                            for k in 'ro_xs0', 'x_s0':
                                if self.project.get_value(k, args=args) is None:
        ...
        for (name, key) in (
            ...
            if key_s0 == 'ro_s0':
                item.setToolTip(item.toolTip() + '<br>Requires densities for all species, and an inert species.')
        ...
        def set_solids_ks_model(idx):
            ...
            dem_phases = [p for p in range(1,1+len(self.solids))
                          if self.project.get_value('solids_model', default='TFM', args=[p]) in ('DEM','CGP','SQP','GSP')
                          and p != phase]
            if dem_phases:
                resp = self.message(text="Setting ks_model applies to all DEM/CGP/SQP/GSP phases\nAre you sure?",
                                    buttons=['yes','no'],
                                    default=['no'])
                if resp != 'yes':
                    return
        ...
        cb = ui.combobox_solids_ks_model
        cb.activated.connect(set_solids_ks_model)
        ...
        get_combobox_item(cb, 3).setToolTip(d['valids']['SQP']['note'])
        get_combobox_item(cb, 4).setToolTip(d['valids']['GSP']['note'])
        get_combobox_item(cb, 5).setToolTip(d['valids']['PIC']['note'])
    ...
    def set_des_rigid_motion(self, val):
        P = self.solids_current_phase
        ui = self.ui.solids
        gb = ui.groupbox_des_rigid_motion
        if not P:
            gb.setVisible(False)
            return
        gb.setVisible(val)
        self.update_keyword('des_rigid_motion', val, args=[P])

    def check_overpack(self, widget):
        limit_key = widget.key
        ...
        for IC in self.ics:
            if self.ics_check_overpack(IC):
                ...
                ic_ep_g = self.project.get_value('ic_ep_g', args=[IC])
                min_ep_g = min(min_ep_g, ic_ep_g)
                self.warning("IC region '%s' is overpacked (void fraction=%.2g)" %
                             (region_name, ic_ep_g),
                             popup=True)
        if warned:
            self.warning("Setting %s to %s" %
                         (limit_key, min_ep_g),
                         popup=True)
            self.update_keyword(limit_key, min_ep_g)


    def update_sqp(self):
        self.update_sqp_bounding_sphere()
        self.update_particle_preview()

    def update_sqp_bounding_sphere(self):
        P = self.solids_current_phase
        if P is None:
            return
        args = (self.project.get_value('sqp_'+v, args=[P])
                for v in 'abcmn')

        d = sqp_d0(*args)
        self.update_keyword('d_p0', d, args=[P])
        self.ui.solids.lineedit_keyword_d_p0_args_P.setText(str(d))
    ...
    def solids_update_tabs(self):
        ...
        if solver == SINGLE: # We shouldn't be here
            return
        enable_dict = {TFM: (True, True, False, False, False, False, False),
                       DEM: (True, False, True, False, False, False, False),
                       CGP: (True, False, False, True, False, False, False),
                       SQP: (True, False, False, False, True, False, False),
                       GSP: (True, False, False, False, False, True, False),
                       PIC: (True, False, False, False, False, False, True),
                       HYBRID: (True, True, True, False,  False, False, False)}

        enabled = enable_dict[solver] if self.solids else (True, False, False, False, False, False, False)
        ...
        if active_tab == TAB_DEM and solver==CGP:
            active_tab = TAB_CGP
        if active_tab == TAB_DEM and solver==SQP:
            active_tab = TAB_SQP
        if active_tab == TAB_DEM and solver==GSP:
            active_tab = TAB_GSP
    ...
    def solids_change_tab(self, tabnum):
        ui = self.ui.solids
        to_btn = [ui.pushbutton_solids_materials,
                  ui.pushbutton_solids_TFM,
                  ui.pushbutton_solids_DEM,
                  ui.pushbutton_solids_CGP,
                  ui.pushbutton_solids_SQP,
        ...
        target = tabnum
        if target in (TAB_CGP, TAB_SQP, TAB_GSP):
            target = TAB_DEM
        elif target == TAB_PIC:
            target -= 3
    ...
    def setup_solids(self, allow_disabled_tab=False):
        ...
        ui = self.ui.solids
        # Can we modify phases when paused?
        if self.lock_level:
            for w in (ui.toolbutton_solids_add_phase,
                      ui.toolbutton_solids_delete_phase,
    ...
    def setup_solids_tab(self, tabnum):
        if tabnum == TAB_MATERIALS:
            ...
            self.handle_solids_species_selection()

        elif tabnum == TAB_TFM:
            self.setup_solids_tfm_tab()
        elif tabnum in (TAB_DEM, TAB_CGP, TAB_SQP, TAB_GSP):
    ...
    def fixup_solids_table(self, tw, stretch_column=0):
        ...
        if tw == ui.tablewidget_solids or tw == ui.tablewidget_psd:
            if tw == ui.tablewidget_psd:
                height -= 8 # tweak to remove some extra space
            tw.setMaximumHeight(height)
            tw.setMinimumHeight(header_height+row_height*min(nrows,3))

            # I am not sure why this is necessary but it prevents extra blank space around
            # the toolbuttons
            if tw == ui.tablewidget_psd:
                ui.groupbox_psd.setMaximumHeight(height
                                                 + ui.label_ep_star.height()  # font height
                                                 + 8  # spacing (4 top and bottom)
                                                 + ui.frame_add_delete_edit_psd.height())
        else:
    ...
    def setup_combobox_solids_model(self):
        ...
        if self.solids_current_phase is not None:
            mod = self.project.get_value('solids_model', default='TFM', args=self.solids_current_phase)
            i = (0 if mod=='TFM'
                 else 1 if mod=='DEM'
                 else 2 if mod=='CGP'
                 else 3 if mod=='SQP'
                 else 4 if mod=='GSP'
                 else 5 if mod=='PIC'  else None)
            if i is None:
            ...
            i = (0 if solver in (TFM, HYBRID)
                 else 1 if solver == DEM
                 else 2 if solver == CGP
                 else 3 if solver == SQP
                 else 4 if solver == GSP
                 else 5)
            cb.setCurrentIndex(i)
            set_combobox_tooltip(cb)
    ...
    def handle_combobox_solids_model(self, index):
        ...
        phase = self.solids_current_phase
        model = ('TFM', 'DEM', 'CGP', 'SQP', 'GSP', 'PIC')[index]
    ...
    def set_solids_model(self, phase, model):
        ...
        # Issues/965
        if model in ('DEM','CGP','SQP','GSP') and energy_eq:
    ...
    def show_sq_designer(self):
        ui = self.ui.solids
        if self.sq_popup is None:
            # Import on demand
            from mfixgui.widgets.sq_designer import SQPopup
            self.sq_popup = SQPopup(self)
        P = self.P
        def gv(k, d):
            return self.project.get_value('sqp_'+k, args=[P], default=d)
        a,b,c = (gv(k, 0.01) for k in ('a','b','c'))
        d = gv('d_p0', None)
        m,n = (gv(k, 2) for k in ('m', 'n'))
        q =  (gv('q1', 1), gv('q2', 0), gv('q3', 0), gv('q4', 0))
        if d is None:
            d = sqp_d0(a,b,c,m,n)
        self.sq_popup.setup(self.save_sqp_params, P, a, b, c, d, m, n, q)
        ui.groupbox_sqp_shape.setDisabled(True)
        self.sq_popup.show()

    def save_sqp_params(self, P, a, b, c, d, m, n, q):
        def uk(k, v):
            self.update_keyword('sqp_'+k, v, args=[P])
        for k, v in ('m', m),('n', n),('a', a),('b', b),('c',c):
            uk(k,v)
        self.update_keyword('d_p0', d, args=[P])
        for i in range(4):
            uk('q%d'%(i+1), q[i])
    ...
    def show_gs_designer(self):
        if not self.P:
            return
        ui = self.ui.solids
        if self.gs_popup is None:
            # Import on demand
            from mfixgui.widgets.gs_designer import GSPopup
            self.gs_popup = GSPopup(self)
        P = self.P
        #def gv(k, d):
        #    return self.project.get_value('gsp_'+k, args=[P], default=d)
        #q =  (gv('q1', 1), gv('q2', 0), gv('q3', 0), gv('q4', 0))
        self.gs_popup.setup(self.save_gsp_params, P)
        ui.groupbox_gsp_shape.setDisabled(True)
        self.gs_popup.show()

    def save_gsp_params(self, P, bounding_diam=None): # callback from gs_popup
        ui = self.ui.solids
        if P != self.P:  # Selection changed while dialog open
            ui.tablewidget_solids.setCurrentCell(P-1,0)
        self.update_solids_options()
        ui.groupbox_gsp_shape.setDisabled(False)
        if bounding_diam is not None:
            self.update_keyword('d_p0', bounding_diam, args=[P])
        self.update_particle_preview()
    ...
    def check_rdf_type(self):
        ...
        if len(self.solids) > 1 and rdf_type not in RDF_TYPES[2:]:
            ...
            default = RDF_TYPES[0]
            self.warn("Radial distribution function '%s' invalid for monodisperse cases, setting to '%s'" %
                      (rdf_type, default),
                      popup=True)
            self.update_keyword(key, default)


    def check_fedors_landel(self):
        # issues/1427
        n = len(self.solids)
        key = 'fedors_landel'
        if n != 2 and self.project.get_value(key):
            self.message(text="Fedors/Landel packing model is valid for binary mixtures only.\nUsing constant packing correlation.")
    ...
    def solids_add_phase(self, name='Solid', diameter=default_values.d_p0,
        ...
        if self.project.solver == SINGLE: # Should not get here! this pane is disabled.
            ...
            model = SOLIDS_MODELS[self.project.solver]
        ...
        if model not in ('SQP',):# 'GSP'):
            self.update_keyword('d_p0', diameter, args=[n])
        elif model == 'SQP':
            self.update_keyword('sqp_a', default_values.sqp_a, args=[n])
            self.update_keyword('sqp_b', default_values.sqp_b, args=[n])
            self.update_keyword('sqp_c', default_values.sqp_c, args=[n])
            self.update_keyword('sqp_m', default_values.sqp_m, args=[n])
            self.update_keyword('sqp_n', default_values.sqp_n, args=[n])
            self.update_keyword('sqp_q1', default_values.sqp_q1, args=[n])
            self.update_keyword('sqp_q2', default_values.sqp_q2, args=[n])
            self.update_keyword('sqp_q3', default_values.sqp_q3, args=[n])
            self.update_keyword('sqp_q4', default_values.sqp_q4, args=[n])
        elif model == 'GSP':
            self.update_keyword('gsp_q1', default_values.gsp_q1, args=[n])
            self.update_keyword('gsp_q2', default_values.gsp_q2, args=[n])
            self.update_keyword('gsp_q3', default_values.gsp_q3, args=[n])
            self.update_keyword('gsp_q4', default_values.gsp_q4, args=[n])
            # Clear out possibly stale gsp data (?)
            gsp_dir = 'GSP-%s' % n
            if os.path.exists(gsp_dir):
                self.print_internal("Deleting %s" % gsp_dir, color='blue')
                try:
                    shutil.rmtree(gsp_dir)
                except Exception as e:
                    self.warn(str(e), popup=True)

        for key in 'des_interp_on', 'des_interp_mean_fields':
        ...
        # issues/1427 unset FEDORS_LANDEL for non-binary mixture
        self.check_fedors_landel()

        # issues/122 des_en_input, des_en_wall_input defaults
        # This is not needed for TFM but it's harmless to add (?)
        key = 'des_en_input'
        # filling in new column of table
        idx = n
        incr = n-1
        while incr >= 0:
            self.set_keyword_default(key, default_values.des_en_input, args=[idx])
            idx += incr
            incr -= 1
        key = 'des_en_wall_input'
        self.set_keyword_default(key, default_values.des_en_wall_input, args=[n])
    ...
    def handle_solids_table_selection(self):
        ...
        self.set_widget_enabled(ui.toolbutton_solids_delete_phase, enabled)
    ...
    def solids_handle_cgp_radiobutton(self, *args):
        ...
        if stat_wt:
            ...
            val = self.project.get_value('cgp_stat_wt', args=[P])
            if not val:
        ...
        if d_p0:
            ...
            val = self.project.get_value('cgp_d_p0', args=[P])
            if not val:
    ...
    def update_solids_detail_pane(self):
        ...
        if P is None:
            ...
            for item in (ui.label_pic_const_statwt,
                ...
                item.setVisible(show)
            blank = QPixmap(*gsp_preview_size)
            blank.fill(QColor('#ffffff'))
            ui.label_gsp_preview.setPixmap(blank)
            self.gsp_preview_blank = True
            return
        ...
        self.enable_widget(ui.detail_pane, ignore_lock=True)
        ...
        for key in ('d_p0', 'ro_s0', 'des_em', 'cgp_stat_wt',
                    'sqp_a', 'sqp_b', 'sqp_c', 'sqp_m', 'sqp_n',
                    'sqp_q1', 'sqp_q2', 'sqp_q3', 'sqp_q4',
                    'gsp_q1', 'gsp_q2', 'gsp_q3', 'gsp_q4'):
            default = None
            if key.endswith('q1'):
                default = 1
            elif key.endswith(('q2','q3','q4')):
                default = 0
            val = self.project.get_value(key, args=[P], default=default)
            w = get_widget(key)
            w.setText(as_str(val))
            w.saved_value = val # Reversion value for invalid input

        # d_p0 not required if model is SQP
        ui.lineedit_keyword_d_p0_args_P.required = (model!='SQP')
        if model == 'SQP':
            def gv(k,d): return self.project.get_value('sqp_'+k, args=[P], default=d)
            if self.project.get_value('d_p0', args=[P]) is None:
                # Compute bounding diameter
                val = sqp_d0(gv('a', 1),
                             gv('b', 1),
                             gv('c', 1),
                             gv('m', 2),
                             gv('n', 2))
                self.update_keyword('d_p0', val, args=[P])
        show = (model == 'CGP')
        ...
        for key in ('momentum_x_eq', 'momentum_y_eq', 'momentum_z_eq'):
            ...
            if model == 'TFM': # momentum eq only avail w/ TFM solid model
                ...
                self.enable_widget(cb, True)
                cb.setChecked(val)
            else:
                cb.setChecked(False)
                self.set_widget_enabled(cb, False, reason='Only available for TFM solids model.')

        # issues/1781
        enable = (model in ('DEM','CGP','SQP','GSP'))
        gb = ui.groupbox_des_rigid_motion
        cb = ui.checkbox_des_rigid_motion
        for w in (gb,cb):
            self.set_widget_enabled(w, enable,
                reason='Not available for %s model.' % model)
            if not enable:
                self.update_keyword('des_rigid_motion', False, args=[P])

        val = bool(self.project.get_value('des_rigid_motion', args=[P]))
        cb.setChecked(val)
        if not val:
            enable = False
        gb.setVisible(enable)
        for k in ('vel', 'omega', 'rot_center'):
            for i in (1,2,3):
                if enable:
                    val = self.project.get_value('des_rigid_motion_'+k,  args=[P,i])
                    txt = '' if val is None else str(val)
                else:
                    txt = ''
                getattr(ui, 'lineedit_keyword_des_rigid_motion_%s_args_P_%s'%(k,i)).setText(txt)
        ...
        cb = ui.checkbox_species_eq
        reason = 'Requires energy equations when using %s model.'%model
        self.set_widget_enabled(cb, (energy_eq) if model in ('DEM','CGP','SQP','GSP') else True,
                                reason=reason)
        ...
        for item in (ui.label_solids_viscosity_model,
            ...
            self.set_widget_enabled(item, enabled, reason="Only available for TFM solids.")

        if not enabled:
            for item in (ui.lineedit_keyword_mu_s0_args_P,
                         ui.label_mu_s0_units):
                self.disable_widget(item)
        else:
            self.enable_widget(ui.label_solids_viscosity_model)
        ...
        if enabled:
            cb = ui.combobox_solids_specific_heat_model
            item = get_combobox_item(cb, MIXTURE)
            enable_item = bool(self.solids_species.get(P, False))
            self.set_widget_enabled(item, enable_item)
            txt = "<b>Mixture</b>: Use MFiX default calculation."
            if not enable_item:
                txt += "<br>Requires defined species, and specific heat of each."
            item.setToolTip(txt)
            if enable_item is False and cb.currentIndex() == MIXTURE:
                val = self.get_retained_keyword('c_ps0', args=[P], default=default_values.c_ps0)
                self.update_keyword('c_ps0', val, args=[P])
                self.set_solids_specific_heat_model(CONSTANT)

        if not enabled:
            for item in (ui.lineedit_keyword_c_ps0_args_P,
                         ui.label_c_ps0_units):
                self.disable_widget(item, "Requires energy equations.")
        ...
        if enabled:
            ...
            for item in (ui.label_solids_ks_model,
                         ui.combobox_solids_ks_model):
                self.enable_widget(item)

            for item in (ui.lineedit_keyword_k_s0_args_P,
                         ui.label_k_s0_units):
                self.set_widget_enabled(item, idx > 0)
            ...
            for item in (ui.label_solids_ks_model,
                         ...
                         ui.label_k_s0_units):
                self.disable_widget(item,
                                    reason="Requires DEM/CGP/SQP/GSP or TFM solids and energy equations.")
                ui.combobox_solids_ks_model.setCurrentIndex(0)
                ui.lineedit_keyword_k_s0_args_P.setText('')
        set_combobox_tooltip(ui.combobox_solids_ks_model)
        ...
        enabled = (model in ('DEM','CGP','SQP','GSP') and energy_eq)
        for item in (ui.label_des_em,
                     ui.combobox_solids_emissivity_model,
                     ui.lineedit_keyword_des_em_args_P):
            self.set_widget_enabled(item, enabled, reason="Requires DEM/CGP/SQP/GSP solids and energy equations.")
        ...
        if enabled:
            ...
            set_combobox_tooltip(cb)
            if des_em == 0.0:
                self.disable_widget(ui.lineedit_keyword_des_em_args_P)
        ...
        # Advanced
        enabled = (model=='TFM')
        self.set_widget_enabled(ui.groupbox_advanced, enabled,
                                reason="Options for TFM model only")
        ...
        self.set_widget_enabled(ui.checkbox_enable_added_mass_force, (kt_type != 'GHD'),
                                reason="Added mass force not allowed with GDD model.")
    ...
    def update_solids_table(self):
        ...
        if kt_type == 'GHD' and len(self.solids) > 1:
            self.disable_widget(tb, reason='<b>kt_type=GHD</b>: only 2 solid phases permitted')
        elif kt_type in ('GD_99', 'GTSH') and self.solids:
            self.disable_widget(tb, reason= '<b>kt_type=%s</b>: only 1 solid phase permitted' % kt_type)
            ui.toolbutton_solids_add_phase.setEnabled(False)
        else:
            enabled = len(self.solids) < DIM_M
            reason='Maximum number of solid phases reached.'
            if self.particle_popup.db:
                self.set_widget_enabled(ui.toolbutton_solids_search, enabled,
                                      reason=reason)
            self.set_widget_enabled(ui.toolbutton_solids_add_phase, enabled,
                                  reason=reason)

    def show_sqp_options(self, enable):
        ui = self.ui.solids
        ui.groupbox_sqp_shape.setVisible(enable)
        ui.pushbutton_sq_designer.setVisible(enable)
        ui.label_d_p0.setText('Bounding diameter (optional)' if enable
                              else 'Diameter')

    def show_gsp_options(self, enable):
        ui = self.ui.solids
        ui.groupbox_gsp_shape.setVisible(enable)
        ui.pushbutton_gs_designer.setVisible(enable)
        ui.label_d_p0.setText('Bounding diameter' if enable
                              else 'Diameter')
        blank = QPixmap(*gsp_preview_size)
        blank.fill(QColor('#ffffff'))
        if enable and self.P:
            gsp_dir = "GSP-%s" % self.P
            fname = gsp_dir+'/thumbnail.png'
            if os.path.exists(fname):
                try:
                    pm = QPixmap(fname).scaled(*gsp_preview_size)
                    ui.label_gsp_preview.setPixmap(pm)
                    self.gsp_preview_blank = False
                    ui.label_gsp_preview.setScaledContents(False)
                    ui.label_gsp_preview.setVisible(True)
                except Exception as e:
                    print(e)
            else:
                self.gsp_preview_blank = False
                ui.label_gsp_preview.setPixmap(get_pixmap("sphere.png", *gsp_preview_size))
            fname = gsp_dir+'/gsp_properties.dat'
            nspheres = vol = area = equiv_diam = None
            nspheres = 1
            d_p0 = self.project.get_value('d_p0', args=[self.P],
                                          default=default_values.d_p0)
            vol = 𝜋 * d_p0**3 / 6
            equiv_diam = d_p0
            area =  𝜋 * d_p0**2
            if os.path.exists(fname):
                with open(fname) as f:
                    for line in f:
                        ...
                        if len(tok) != 2:
                            continue
                        key, val = tok[0].strip(), tok[1].strip()
                        if key == 'nspheres':
                            nspheres = int(val)
                        if key == 'equiv_diam':
                            equiv_diam = float(val)
                        if key == 'area':
                            area = float(val)
                        if key == 'vol':
                            vol = float(val)
                            solid_name = list(self.solids.keys())[self.P-1]
            ...
            ui.lineedit_gsp_num_spheres.setText(to_str(nspheres))
            ui.lineedit_gsp_equiv_diam.setText(to_str(equiv_diam))
            ui.lineedit_gsp_area.setText(to_str(area))
            ui.lineedit_gsp_vol.setText(to_str(vol))
    ...
    def update_solids_options(self):
        ...
        if P is None:
            for item in items:
                self.disable_widget(item)
                item.setChecked(False)
            self.show_sqp_options(self.project.solver==SQP)
            self.show_gsp_options(self.project.solver==GSP)

            return

        model =  self.project.get_value('solids_model', default='TFM', args=[P])
        species_eq = self.project.get_value('species_eq', default=True, args=[P])
        energy_eq = self.project.get_value('energy_eq', default=True)
        ...
        #issues/533, this is a also in update_solids_detail_pane
        enable = (bool(energy_eq) if model in ('DEM','CGP','SQP','GSP') else True)

        self.set_widget_enabled(ui.checkbox_species_eq, enable,
                                reason = 'Requires energy equations when using %s model.'%model)
        if not enable:
            species_eq = False
            self.update_keyword('species_eq', False, args=[P])
    ...
    def solids_delete_phase(self):
        ...
        try:
            ...
            if len(self.solids) < DIM_M:
                self.enable_widget(ui.toolbutton_solids_add_phase)
                self.set_widget_enabled(ui.toolbutton_solids_search, bool(self.particle_popup.db))
            ...
            self.monitors_delete_solids_phase(phase)
            ...
            # delete any GSP data and fix hole
            gsp_dir = 'GSP-%s'%phase
            if os.path.exists(gsp_dir):
                try:
                    self.print_internal('Deleting %s' % gsp_dir, color='blue')
                    ...
                    self.warn(str(e), popup=True)
                for n in range(phase+1, 11):
                    if os.path.exists('GSP-%s'%n):
                        try:
                            self.print_internal('GSP-%s -> GSP-%s' % (n,n-1), color='blue')
                            os.replace('GSP-%s'%n, 'GSP-%s'%(n-1))
                        except Exception as e:
                            self.warn(str(e), popup=True)
    ...
    def update_solids_species_groupbox(self):
        ...
        if phase is None:
            ...
            energy_eq = self.project.get_value('energy_eq', default=True)
            enabled = bool(self.solids_species.get(phase)
                           or
                           (species_eq)
                           or
                           (energy_eq and self.solids_specific_heat_model == MIXTURE))
    ...
    def update_solids_mass_fraction_total(self):
        ...
        if total != 1.0:
            item.setForeground(QColor("red" if table.isEnabled()
                                      else "#be0000"))
            ...
            item.setForeground(QColor("black" if table.isEnabled()
                                      else "#bebebe"))
    ...
    def solids_species_save(self):
        ...
        self.solids_species[phase] = dict((alias, deepcopy(data))
    ...
    def handle_solids_species_selection(self):
        ...
        row = get_selected_row(tw)
        enabled = (row is not None)
        self.set_widget_enabled(ui.toolbutton_solids_species_delete, enabled)
        self.set_widget_enabled(ui.toolbutton_solids_species_edit, enabled)
        if enabled:
    ...
    def handle_solids_psd_selection(self):
        ...
        row = get_selected_row(tw)
        enabled = (row is not None)
        self.set_widget_enabled(ui.toolbutton_delete_psd, enabled)
        self.set_widget_enabled(ui.toolbutton_edit_psd, enabled)
        if enabled:
            # Double-click signal comes too soon, while mouse button is still down
            # and disturbs row selection.
    ...
    def solids_normalize_keys(self):
        ...
        for (phase, phase_data) in self.solids_species.items():
            ...
            self.project.update_thermo_data(self.solids_species[phase], loading=True)
    ...
    def solids_species_add(self):
        ...
        sp.enable_lennard_jones(False)
        sp.enable_cantera(False)
        sp.popup()
    ...
    def solids_species_edit(self):
        ...
        if self.lock_level == LOCKED:
            return # prevent double-click from popping us up during run
        tw = ui.tablewidget_solids_species
        row = get_selected_row(tw)
    ...
    def reset_solids(self):
        ...
        ui.gsp_hertzian_warning_printed = False
        ui.groupbox_filter_particles.setChecked(False)
        ui.groupbox_filter_particles_checked = None
