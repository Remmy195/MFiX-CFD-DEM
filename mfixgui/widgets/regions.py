...
class RegionsWidget(QtWidgets.QWidget):
    ...
    def __init__(self, gui=None):
        ...
        self.checkbox_select_facets.clicked.connect(self.stl_type_changed)
        self.checkbox_select_facets.toggled.connect(lambda c: self.groupbox_stl.setEnabled(c))
    ...
    def update_region_geopool(self, name, geopool):
        data = self.tablewidget_regions.value
        data[name]['geometry_pool'] = geopool

    def reset_regions(self):
        self.tablewidget_regions.value.clear()
        self.tablewidget_regions.clear()
        self.parameter_key_map = {}
    ...
    def new_region(self, name=None, extents=None, rtype=None, defer_update=False):
        ...
        reg_dat['geometry_pool'] = self.vtkwidget.get_toplevel_geometry_names()
    ...
    def update_region_parameters(self):
        ...
        have_geo = self.listwidget_regions_geometry_pool.count() > 0
        if not have_geo:
            self.checkbox_select_facets.setChecked(False)
        self.checkbox_select_facets.setEnabled(bool(have_geo and enabled and (bc_flag or not users)))
        base = 'Select facets (STL)'
        if not have_geo:
            self.checkbox_select_facets.setText(base + " - no STL geometry available")
            self.checkbox_select_facets.setChecked(False)
            self.label_num_facets.setText("0")
        else:
            self.checkbox_select_facets.setText(base + ' - region in use' if (users and not bc_flag) else base)
        if self.gui.sms_mode and self.gui.mode=='mesher':
        ...
        self.checkbox_select_facets.setChecked(r_type == 'STL')
        ...
        self.label_num_facets.setText(str(data['facet_count']))
    ...
    def stl_type_changed(self):
        stl = self.checkbox_select_facets.isChecked()
        val = 'STL' if stl else None
        for wid in self.extent_lineedits:
            wid.error_check = not stl
        self.region_value_changed(self.checkbox_select_facets, {'type': val}, [])
    ...
    def enable_disable_widgets(self, name, enable_all=False):
        ...
        # input_enabled = self.gui.lock_level==UNLOCKED or not any(x in users for x in ('BCs', 'ICs', 'ISs'))
        input_enabled = self.gui.lock_level != LOCKED
    ...
    def regions_from_str(self, string):
        ...
        for region in loaded_data['order']:
            ...
            if loaded_region.get('geometry_pool', None) is None:
                ...
                loaded_region['geometry_pool'] = self.vtkwidget.get_toplevel_geometry_names()
