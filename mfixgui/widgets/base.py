...
class BaseWidget(QtCore.QObject):
    ...
    def emitUpdatedValue(self):
        if not self.emit_updated_value:
            return
    ...
    def setValInfo(self, max=None, min=None, exclude_min=None):
        if max is not None:
            self.max = max
        if min is not None:
            self.min = min
...
class LineEdit(QtWidgets.QLineEdit, BaseWidget):
    ...
    def check_range(self, val):
        if self.error_check:
            ...
            if self.min is not None:
                plim = pretty_eq_rep(self.min, self.dtype)
                if self.exclude_min and val <= self.min:
                    raise RangeError("Value below allowed range:\n {} <= {}".format(pval, plim))
                elif val < self.min:
                    raise RangeError("Value below allowed range:\n {} < {}".format(pval, plim))
            if self.max is not None:
                plim = pretty_eq_rep(self.max, self.dtype)
                if self.exclude_max and val >= self.max:
    ...
    def value(self):
        ...
        tlower = text.lower()
        if 'definition incomplete' in tlower or 'ramp from' in tlower:
            return None
...
class CustomDelegate(QtWidgets.QStyledItemDelegate):
    ...
    def get_widget_data(self, index):
    ...
    def createEditor(self, parent, option, index):
        widgetData = self.get_widget_data(index)
        model = index.model()
        value = model.data(index, Qt.EditRole)
        row_name = None
        if hasattr(model, '_rows'):
            row_name = model._rows[index.row()]
        col_name = None
        if hasattr(model, '_columns'):
            col_name = model._columns[index.column()]
        ...
        if value == '-':
            editor = None
        elif widgetData['widget'] == 'combobox':
        ...
        if editor:
            editor.emit_updated_value = False
            editor.key = col_name
            if 'dtype' in widgetData:
                editor.setdtype(widgetData['dtype'])
            if 'required' in widgetData:
                editor.required = widgetData.get('required', True)
            if 'default' in widgetData:
                editor.required = widgetData.get('default', 0.0)

            max_ = widgetData.get('max')
            min_ = widgetData.get('min')
