...
def init_template_manager(template_manager):
    ...
    for x in os.listdir(queue_template_dir):
        template_manager.add(QueueTemplate(queue_template_dir+'/'+x,
                                           is_builtin=True))

    if user_templates := SETTINGS.value("queue_templates"):
        for x in user_templates.split('|'):
            if x and exists(x):
                # Don't show builtin templates from previous versions,
                # which we shouldn't have saved in the first place
                if WINDOWS:
                    x = x.replace('\\', '/') # Silly Windows paths
                if ('share/mfix/templates/queue_templates' in x #Conda
                    or 'site-packages/queue_templates' in x): # pip
                    continue
                template_manager.add(QueueTemplate(x, is_builtin=False))
...
class QueueTemplateManager:
    ...
    def template_keys(self):
        ...
        if "Joule 3" in keys: # Move to head of list
            keys.remove("Joule 3")
            keys = ["Joule 3"] + keys
        return keys
    ...
    def save_settings(self):
        SETTINGS.setValue(
            "queue_templates",
            "|".join(template.path for template in self.templates.values()
...
class QueueTemplate:
    ...
    def __init__(self, path, is_builtin=False):
        ...
        if not self.submit():
            raise ValueError(
                "The template file %s  does not have a submit_cmd defined"%self.path)
        ...
        for key, value in template_values.items():
            ...
            if widget is not None:
                try:
                    ...
                    # FIXME report error
                    pass # Don't die on custom template errors - #1531
...
def _make_widget(template_field, parent):
    ...
    widget.help_text = template_field.get("help", '')
    if widget.help_text:
        widget.setToolTip(widget.help_text)
    return widget
