...
from qtpy.QtWidgets import (QGridLayout, QGroupBox, QHeaderView,
                            ...
                            QToolButton, QWidget, QTableWidgetItem)

from qtpy.QtGui import QPixmap, QPalette, QColor
...
def ramp_keys(k):
    return [k+'_ramp_t0', k+'_ramp_t1',
            k+'_ramp_v0', k+'_ramp_v1']
...
class BCS:
    ...
    def init_bcs(self):
        ...
        self.bcs_fluid_species_boxes = {} # In the 'wall' tab each species needs a groupbox
        ...
        ui.pushbutton_moving_wall.pressed.connect(lambda: self.bcs_change_tab(MOVING_WALL_TAB))
        ...
        get_combobox_item(cb, BC_TYPES.index("PSW")+1).setToolTip("A mixed wall can have a mix of no-slip, free-slip and partial-slip behavior for different phases.  Set the type for each phase using the <b>Wall type</b> control.")
        ...
        ui.tablewidget_fluid_mass_fraction.key = 'bc_x_g'
        ui.groupbox_solids_composition.key = 'bc_x_s'
        ui.tablewidget_solids_mass_fraction.key = 'bc_x_s'
        ...
        ui.label_bc_psd.keys = ui.combobox_bc_psd.keys = ['bc_psd_type',
                                                          ...
                                                          'bc_psd_max_dp']
        # Setup ramp buttons
        for (tb, key, tooltip) in (
                (ui.toolbutton_ramp_bc_t_g, 'bc_t_g', "fluid temperature."),
                (ui.toolbutton_ramp_bc_p_g, 'bc_p_g', "fluid pressure."),
                (ui.toolbutton_ramp_fluid_velocity_3, 'bc_w_g', "Z-axial fluid velocity."),
                (ui.toolbutton_ramp_bc_t_s, 'bc_t_s', "solids phase temperature."),
                (ui.toolbutton_ramp_bc_theta_m, 'bc_theta_m', "solids phase granular temperature."),
                (ui.toolbutton_ramp_solids_velocity_3, 'bc_w_s', "Z-axial solids velocity.")):
            tb.base_key = key
            tb.keys = ramp_keys(key) #locatability
            tb.setToolTip("Define ramp for "+ tooltip)

        for w in widget_iter(ui):
            if isinstance(w, QToolButton) and w.objectName().startswith('toolbutton_ramp'):
                w.clicked.connect(lambda arg, tb=w: self.handle_bc_ramp(tb))


        # enable parameters
        for w in widget_iter(ui):
            if isinstance(w, LineEdit) and w.dtype in (int, float):
                w.allow_parameters = True

        ui.checkbox_keyword_bc_gsp_random_q_args_BC_P.setVisible(False) # avoid seeing double

    def handle_bc_ramp(self, widget):
        if not self.bcs_current_indices:
            return
        ui = self.ui.boundary_conditions
        p = self.ramp_popup
        p.label.setText(widget.toolTip())
        try:
            key = widget.base_key
        except:
        ...
        unit_label = getattr(ui, 'label_%s_units' % key, None)
        if unit_label:
            unit = unit_label.text()
        else: # Not all label names match the pattern, e.g. 'label_fluid_velocity_1_units'
            if key.startswith('bc_volflow'):
                unit = 'm³/s'
            elif key.startswith('bc_v'):
                unit = 'm/s'
            elif key.startswith('bc_massflow'):
                unit = 'kg/s'
            else:
                unit = '?'

        try: # Still no good way to query if signal is connected
            p.finished.disconnect()
        except:
            pass
        p.finished.connect(self.bcs_setup_current_tab)

        p.popup(key,  unit, self.bcs_current_indices, self.bcs_current_solid)
    ...
    def change_bc_type(self, idx):
        ...
        # issues/1259
        #if new_type  != 'PSW':
        #    self.retain_keyword('bc_jj_ps')
        #    self.unset_keyword('bc_jj_ps')
    ...
    def change_bc_solids_wall_type(self, idx, P=None):
        ...
        if solids_model in ('DEM','CGP','SQP', 'GSP', 'PIC'):
            for c in 'huvw':
                key = 'bc_%sw_s' % c
    ...
    def bcs_cancel_add(self):
        ...
        if row is not None:
            for item in (ui.bottom_frame,
                         ui.toolbutton_delete):
                self.enable_widget(item, ignore_lock=True)
            ...
            self.set_widget_enabled(ui.toolbutton_up,
                                    row > 0 and not cyclic and not prev_cyclic)
            self.set_widget_enabled(ui.toolbutton_down,
                                    row != nrows-1 and not cyclic)

        else:
            self.disable_widget(ui.toolbutton_up)
            self.disable_widget(ui.toolbutton_down)
    ...
    def bcs_add_regions(self):
        ...
        if self.bcs_region_dict is None:
            self.bcs_region_dict = self.get_region_dict()
    ...
    def bcs_add_regions_1(self, selections, bc_type=None, mixed_wall=False,
        ...
        if bc_type == 'CYCLIC':
            ...
            for (i, region_name) in enumerate(selections):
                ...
                if region_data is None: # ?
                    self.warn("no data for BC region %s type %s" % (region_name, bc_type))
                    continue
        ...
        item_data = (tuple(indices), tuple(selections))
        item.setData(UserRole, item_data)
        if autoselect:
            self.bcs_current_indices, self.bcs_current_regions = item_data
    ...
    def bcs_delete_regions(self):
        ...
        self.set_widget_enabled(ui.toolbutton_up,
                                row is not None and row > 0 and not cyclic and not prev_cyclic)
        self.set_widget_enabled(ui.toolbutton_down,
                                row is not None and not cyclic and row != nrows-1)
    ...
    def handle_bcs_region_selection(self):
        ...
        self.set_widget_enabled(ui.toolbutton_delete, enabled, ignore_lock=True)
        self.set_widget_enabled(ui.bottom_frame, enabled, ignore_lock=True)
        if not enabled:
            self.disable_widget(ui.toolbutton_up)
            self.disable_widget(ui.toolbutton_down)
        else:
        ...
        if not enabled:
            ...
            for w in widget_iter(ui.bottom_frame):
                if isinstance(w, LineEdit):
                    w.setText('')
            ui.tablewidget_fluid_mass_fraction.setRowCount(0)
            ui.tablewidget_solids_mass_fraction.setRowCount(0)
            self.fixup_bcs_table(ui.tablewidget_fluid_mass_fraction)
            self.fixup_bcs_table(ui.tablewidget_solids_mass_fraction)
        ...
        if self.bc_is_cyclic(BC0):
            ...
            for i in range(ui.tab_layout.columnCount()-1): # Skip 'Cyclic'
                ...
                if not w:
                    continue
                w.setEnabled(False)
            self.enable_widget(ui.pushbutton_cyclic, ignore_lock=True)
            ...
            self.set_widget_enabled(ui.pushbutton_fluid, not self.fluid_solver_disabled,
                                    reason="Fluid solver disabled", ignore_lock=True)
            nscalar = self.project.get_value('nscalar',default=0)
            self.set_widget_enabled(ui.pushbutton_scalar, (nscalar != 0),
                                    reason="No scalar equations defined", ignore_lock=True)

            for i in range(1, ui.tab_layout.columnCount()-4): # Skip 'fluid', 'scalar', 'cyclic', 'transient', 'moving wall'
                item = ui.tab_layout.itemAtPosition(0, i)
                if item is None:
                    continue
                w = item.widget()
                if not w:
                    continue
                self.enable_widget(w, ignore_lock=True)

            self.disable_widget(ui.pushbutton_cyclic,
                                reason="Only available for cyclic boundary conditions.")
            ...
            if (self.project.get_value('bc_mi_start_time', args=[BC0]) is not None
                ...
                or (bc_type == 'MI' and self.project.solver in (DEM,CGP,GSP,SQP))):
                self.enable_widget(ui.pushbutton_transient, ignore_lock=True)
            else:
                self.disable_widget(ui.pushbutton_transient,
                                    reason="Only available for transient DEM/CGP/SQP/GSP mass inflows.")
        ...
        if BC0 is not None and self.bc_is_cyclic(BC0):
            ...
            return (SCALAR_TAB, None)
              ...
              and ((self.project.get_value('bc_type', default='', args=[BC0]).endswith('MI')
                    and self.project.solver in (DEM,CGP,SQP,GSP))
    ...
    def bcs_set_default_keys_W(self, BC):
        ...
        # Disabled for DEM, CGP, SQP and PIC solids# (covered by bc_jj_ps)
        # Sets keyword PHI_W
        ...
        #        if solids_model[P] not in ('DEM','CGP','SQP','GSP','PIC'):
        #            self.set_keyword_default(key, default, args=[BC,P])
        ...
        if bc_type in ('PSW', 'CG_PSW') or bc_jj_ps:
            for P in range(1, 1+mmax):
                if solids_model[P] == 'TFM':
                    ...
                    #check_bc_walls.f:
                    #! The wall velocities are not needed for non-TFM solids (triggers error)
                    for c in 'uvw':
                        key = 'bc_%sw_s'%c
    ...
    def get_bc_normal(self, BC):
        ...
        # load time, not depending on order of initialization
    ...
    def bcs_set_default_keys_I(self, BC):
        ...
        if bc_type == 'MI':
            for P, solid in enumerate(self.solids.values(), 1):
                if self.project.get_value('solids_model', default='TFM', args=[P]) == 'PIC':
        ...
        exclude_min = False
        if friction_model in ('SRIVASTAVA', 'GUO_BOYCE'):
            default = 1.0e-8
            exclude_min = True
        ...
        for le in (ui.lineedit_keyword_bc_theta_m_args_BC_P,):
    ...
    def bcs_check_wall_keys(self, indices=None):
        ...
        for BC in indices:
            ...
            if bc_type == 'CYCLIC':
                continue
            for P in range(1, 1+self.project.get_value('mmax', default=1)):
    ...
    def setup_bcs(self, allow_disabled_tab=False):
        ui = self.ui.boundary_conditions
        for b in (ui.pushbutton_fluid, ui.pushbutton_moving_wall,
                  ui.pushbutton_scalar, ui.pushbutton_cyclic, ui.pushbutton_transient):
        ...
        P = self.bcs_current_solid
        if self.lock_level:
            for w in (ui.toolbutton_add, ui.toolbutton_delete, ui.toolbutton_up, ui.toolbutton_down):
        ...
        #Enable/disable solids buttons
        for (i, solid_name) in enumerate(solids_names, 1):
            solids_model = self.project.get_value('solids_model', default='TFM', args=[i])
            enabled = (not cyclic
                       and not (bc_type.endswith('PO') and solids_model!='TFM'))
            b = ui.tab_layout.itemAtPosition(0,i).widget()
            b.setEnabled(enabled)
            if not enabled and not cyclic:
                b.setToolTip("Only available for TFM solids.")
            else:
                b.setToolTip('')
        ...
        b.setEnabled(enabled)
        column = 1+len(self.solids)
        if len(self.solids) != len(self.bcs_saved_solids_names):
            ui.tab_layout.removeWidget(b)
            ui.tab_layout.addWidget(b, 0, column)
        column += 1

        # Move the "Moving wall" button to the right of all solids, if needed
        b = ui.pushbutton_moving_wall
        font = b.font()
        font.setBold(self.bcs_current_tab==MOVING_WALL_TAB)
        b.setFont(font)
        enabled = bc_type.endswith('W')
        b.setEnabled(enabled)
        if len(self.solids) != len(self.bcs_saved_solids_names):
            ui.tab_layout.removeWidget(b)
            ui.tab_layout.addWidget(b, 0, column)
        ...
        enabled = (self.project.get_value('bc_mi_start_time', args=[BC0]) is not None
                   ...
                   or (not cyclic
                       ...
                       and self.project.solver in (DEM,CGP,GSP,SQP)))
        b.setEnabled(enabled)
        b.setToolTip("Only available for DEM/CGP/SQP/GSP mass inflows." if not enabled else '')
        ui.pushbutton_transient.setToolTip
        if len(self.solids) != len(self.bcs_saved_solids_names):
            ui.tab_layout.removeWidget(b)
            ui.tab_layout.addWidget(b, 0, column)
        column += 1
    ...
    def bcs_tab_to_index(self, tab, solid):
        return (0 if tab==FLUID_TAB
                ...
                else len(self.solids)+2 if tab==MOVING_WALL_TAB
                else len(self.solids)+3 if tab==TRANSIENT_TAB
                else len(self.solids)+4 if tab==CYCLIC_TAB
                else solid)
    ...
    def setup_bcs_tab(self, tab):
        ...
        elif self.bcs_current_tab == MOVING_WALL_TAB:
            self.setup_bcs_moving_wall_tab()
        else:
    ...
    def bcs_extract_regions(self):
        ...
        tw = ui.tablewidget_regions
        items = [tw.item(i,0).text() for i in range(tw.rowCount())]
        ...
        if items:
            ...
            # We do this here to work around issues/597 where cases with cyclic BCs were
            #  getting saved incorrectly
            for axis in 'X', 'Y', 'Z':
                q = axis.lower()
                keys = ('cyclic_'+q, 'cyclic_%s_pd'%q, 'delp_'+q)
                if any(self.project.get_value(key) for key in keys):
                    name = axis + '-cyclic'
                    if name not in items: # issues/597 ensure row present for cyclic BC
    ...
    def setup_bcs_fluid_W_tab(self):
        ...
        def setup_key_widget(key, default=None, enabled=True,
            ...
            for pat in ('label_%s', 'label_%s_units',
                ...
                if item:
                    self.set_widget_enabled(item, enabled) #XXX reason
        ...
        #    self.set_widget_enabled(w, mixed)
    ...
    def setup_bcs_solids_W_tab(self, P):
        ...
        # does not support ANY of the wall BC specifications. DEM/CGP/SQP/GSP only supports keywords associated with
        ...
        for w in widget_iter(ui.groupbox_solids_momentum_eq):
            if not hasattr(w, 'toolTip'): # not a control widget
                continue
            if not hasattr(w, 'tooltip0'):
                w.tooltip0 = w.toolTip()
            if not enabled:
                w.setToolTip(w.tooltip0 + "<br>Only available for TFM solids.")
            else:
        ...
        if solids_model == 'TFM':
            if not enabled:
                cb.setToolTip(cb.tooltip0 + "<br>Only available for partial slip wall with no Cartesian grid and stress model not algebraic or GHD.")
            else:
        ...
        for w in (ui.label_bc_jj_ps_type, ui.combobox_bc_jj_ps_type):
            self.set_widget_enabled(w, enabled,
                                    reason="Requires TFM solids and Johnson and Jackson partial slip boundary condition.")
        ...
        if enabled:
            ...
            #  Convective flux
            #    Disabled for DEM, CGP and PIC solids
            item = get_combobox_item(cb, CONVECTIVE_FLUX)
            enable = solids_model not in ('DEM', 'CGP', 'PIC')
            set_item_enabled(item, enable)
            if not enable:###
                item.setToolTip('Only available for TFM simulations.')
            else:
                item.setToolTip('')
            # Specified flux
            #    Disabled for DEM, CGP and PIC solids
            item = get_combobox_item(cb, SPECIFIED_FLUX)
            enable = solids_model not in ('DEM', 'CGP', 'PIC')
            set_item_enabled(item, enable)
            if not enable:
                item.setToolTip('Only available for TFM simulations.')
                ...
                item.setToolTip('')
            # TODO don't stay on disabled item (after change from TFM->DEM)
        ...
        #Define constant flux
        # Disabled for PIC/DEM solids
        # Specification only available with 'Specified flux' BC type
        # Sets keyword BC_C_T_S(#,#)
        # DEFAULT 0.0
        key = 'bc_c_t_s'
        enabled = energy_eq and (eq_type==SPECIFIED_FLUX) and solids_model not in ('PIC', 'DEM', 'CGP')
        default = 0.0 if enabled else None
        setup_key_widget(key, default, enabled)
    ...
    def update_bcs_fluid_mass_fraction_table(self):
        ...
        if not (self.fluid_species and self.bcs_current_indices):
            ...
            self.disable_widget(table)
            self.disable_widget(ui.groupbox_fluid_composition)
            return
        self.enable_widget(ui.groupbox_fluid_composition)
        self.enable_widget(table)
        BC0 = self.bcs_current_indices[0]
    ...
    def ramp_enabled(self, key, args):
        ret = (self.project.get_value(key + '_ramp_t0', args=args, default=0) != 0 or
                any(self.project.get_value(key+suff, args=args) is not None
                    for suff in ('_ramp_t1', '_ramp_v0', '_ramp_v1')))
        return ret

    def ramp_text(self, key, args):
        if any(self.project.get_value(k, args=args, default=0 if k.endswith('t0') else None) is None
               for k in ramp_keys(key)):
            return "Ramp definition incomplete"
        else:
            return "Ramp from %s to %s" % (
                self.project.get_value(key+'_ramp_v0', args=args),
                self.project.get_value(key+'_ramp_v1', args=args))
    ...
    def setup_bcs_fluid_I_tab(self):
        ...
        def show_velocity_3(enabled):
            ...
            for w in (ui.label_fluid_velocity_3,
                      ui.lineedit_keyword_bc_w_g_args_BC,
                      ui.toolbutton_ramp_fluid_velocity_3,
                      ui.label_fluid_velocity_3_units):
            ...
            if enabled:
                ...
                tb = ui.toolbutton_ramp_fluid_velocity_1
                tb.base_key = key
                tb.keys = ramp_keys(key)
                tb.setToolTip("Define ramp for X-axial velocity.")
                ...
                self.add_tooltip(le, key=le.key)
                self.enable_widget(le)
                tb = ui.toolbutton_ramp_fluid_velocity_2
                tb.base_key = key
                tb.keys = ramp_keys(key)
                tb.setToolTip("Define ramp for Y-axial velocity.")
            else:
        ...
        def setup_key_widget(key, default=None, enabled=True, suffix='',
            ...
            val = self.project.get_value(key, args=args)
            if val is None:
                val = self.get_retained_keyword(key, args=args, default=default)
            w = widget or get_widget(key+suffix)

            if self.ramp_enabled(key, args=args):
                text = self.ramp_text(key, args)
                w.setText(text)
                ss = 'QLineEdit {font: italic;'
                if 'incomplete' in text:
                    ss += 'background-color: pink;'
                ss += '}'
                w.setStyleSheet(ss)
                w.setReadOnly(True)
                # w.clicked.connect(self.handle_bc_ramp) TODO capture click on disabled lineedit
                #                                        and show ramp_popup
                for BC in self.bcs_current_indices:
                    args=mkargs(key, bc=BC)
                    self.retain_keyword(key, args=args)
                    self.unset_keyword(key, args=args)
            else:
                w.setStyleSheet('')
                ...
                for BC in self.bcs_current_indices:
                    args=mkargs(key, bc=BC)
                    self.update_keyword(key, val, args=args) # XXX
        ...
        #
        region_name = self.bcs.get(BC0,{}).get('region')
        ...
        if bc_type in ('MI', 'CG_MI'):
            ...
            tb = ui.toolbutton_ramp_fluid_inflow
            tb.base_key = key
            tb.keys = ramp_keys(key)
            text = cb.currentText()
            if text[0] not in 'XYZ':
                text = text.lower()
            tb.setToolTip("Define ramp for %s." % text)
            ...
            if bc_type != 'CG_MI': # show_tangential
                ...
                tb.base_key = key
                tb.keys = ramp_keys(key)
                tb.setToolTip("Define ramp for %s-axial velocity." % tangents[0])
                ...
                tb.base_key = key
                tb.keys = ramp_keys(key)
                tb.setToolTip("Define ramp for %s-axial velocity." % tangents[1])
            ...
            le.allow_parameters = True
            self.add_tooltip(le, key)
            setup_key_widget(key, default, enabled=True, widget=le)
            units = 'm/s'
            ui.label_fluid_inflow_units.setText(units)
            tb = ui.toolbutton_ramp_fluid_inflow
            tb.base_key = key
            ...
            default = 0.0
            label =  ui.label_fluid_velocity_1
            label.setText('Y-axial velocity') # no indent
            self.add_tooltip(label, key)
            le = ui.lineedit_fluid_velocity_1
            le.key = key
            le.args = ['BC']
            ...
            setup_key_widget(key, default, enabled=True, widget=le)
            units = 'm/s'
            ui.label_fluid_velocity_1_units.setText(units)
            tb = ui.toolbutton_ramp_fluid_velocity_1
            tb.base_key = key
            tb.keys = ramp_keys(key)
            tb.setToolTip("Define ramp for Y-axial velocity.")
            ...
            default = 0.0
            label =  ui.label_fluid_velocity_2
            label.setText('Z-axial velocity') # no indent
            self.add_tooltip(label, key)
            le = ui.lineedit_fluid_velocity_2
            le.key = key
            le.args = ['BC']
            ...
            setup_key_widget(key, default, enabled=True, widget=le)
            units = 'm/s'
            ui.label_fluid_velocity_2_units.setText(units)
            tb = ui.toolbutton_ramp_fluid_velocity_2
            tb.base_key = key
            tb.keys = ramp_keys(key)
            tb.setToolTip("Define ramp for Z-axial velocity.")
        ...
        default = 293.15
        setup_key_widget(key, default, enabled)
        ui.toolbutton_ramp_bc_t_g.setEnabled(enabled)
        ...
        if parent != ui.page_fluid_I:
            comp.hide()
            #https://stackoverflow.com/questions/22227376/how-to-change-the-font-of-a-qgroupboxs-title-only
            comp.setStyleSheet("QGroupBox { font-weight: bold;}")  # Restore bold title which may have been removed
    ...
    def update_bcs_solids_mass_fraction_table(self):
        ...
        if not (P and self.solids_species.get(P) and self.bcs_current_indices):
            self.fixup_bcs_table(table)
            self.disable_widget(table)
            self.disable_widget(ui.groupbox_solids_composition)
            return
        self.enable_widget(ui.groupbox_solids_composition)
        self.enable_widget(table)
        BC0 = self.bcs_current_indices[0]
    ...
    def setup_bcs_solids_I_tab(self, P):
        ...
        def show_velocity_3(enabled):
            # Show third velocity input, for CG_MI inflows
            for w in (ui.label_solids_velocity_3,
                      ui.lineedit_keyword_bc_w_s_args_BC_P,
                      ui.toolbutton_ramp_solids_velocity_3,
                      ui.label_solids_velocity_3_units):
                w.setHidden(not enabled)
            if enabled:
                ...
                self.enable_widget(w)
                self.add_tooltip(le, key=le.key)
                tb = ui.toolbutton_ramp_solids_velocity_1
                tb.base_key = key
                tb.keys = ramp_keys(key)
                tb.setToolTip("Define ramp for X-axial velocity.")
                ...
                self.add_tooltip(le, key=le.key)
                self.enable_widget(le)
                tb = ui.toolbutton_ramp_solids_velocity_2
                tb.base_key = key
                tb.keys = ramp_keys(key)
                tb.setToolTip("Define ramp for Y-axial velocity.")
            else:
        ...
        if P is None: # Nothing to do (Clear lineedits?)
            show_velocity_3(False)
            return
        if not self.bcs_current_indices:
            show_velocity_3(False)
        ...
        #  Available for DEM/CGP and PIC solids phases (#1356)
        ...
        if model == 'TFM':
            ...
            msg = "Not available for TFM solids."
            la.setToolTip(msg)
            cb.setToolTip(msg)
        elif bc_type == 'CG_MI':
            la.setEnabled(False)
        ...
        # Randomize orientation (#1644)
        key_sqp = 'bc_sqp_random_q'
        key_gsp = 'bc_gsp_random_q'
        cb_sqp = ui.checkbox_keyword_bc_sqp_random_q_args_BC_P
        cb_gsp = ui.checkbox_keyword_bc_gsp_random_q_args_BC_P
        if model in ('SQP', 'GSP'):
            cb_sqp.setVisible(model=='SQP')
            cb_gsp.setVisible(model=='GSP')
            cb_sqp.setEnabled(True)
            cb_sqp.setChecked(bool(self.project.get_value(key_sqp, args=[BC0,P])))
            cb_gsp.setChecked(bool(self.project.get_value(key_gsp, args=[BC0,P])))
            self.add_tooltip(cb_sqp, key_sqp)
            self.add_tooltip(cb_gsp, key_gsp)
        else: #SQP checkbox is the placeholder
            cb_sqp.setVisible(True)
            cb_sqp.setEnabled(False)
            cb_sqp.setChecked(False)
            cb_sqp.setToolTip('Only available for SQP/GSP solids.')
            cb_gsp.setVisible(False)
        ...
        def setup_key_widget(key, default=None, enabled=True, suffix='',
            ...
            for pat in ('label_%s', 'label_%s_units',
                ...
                item = getattr(ui, name, None)
                if item:
                    self.set_widget_enabled(item, enabled, reason)
        ...
        if bc_type in ('MI', 'CG_MI'):
            ...
            tb = ui.toolbutton_ramp_solids_inflow
            tb.base_key = key
            tb.keys = ramp_keys(key)
            text = cb.currentText()
            if text[0] not in 'XYZ':
            ...
            if bc_type != 'CG_MI': # show_tangential:
                ...
                if self.project.get_value('solids_model', default='TFM', args=[P]) == 'PIC':
                    ...
                    reason = "PIC model only."
                    self.disable_widget(le, reason=reason)
                    le.setText('')
                    self.disable_widget(ui.label_bc_pic_mi_const_statwt, reason=reason)
                ...
                for w in (ui.lineedit_solids_velocity_1,
                    ...
                    w.updateValue(w.key, val)
                le = ui.lineedit_keyword_bc_pic_mi_const_statwt_args_BC_P
                self.disable_widget(le, reason="Not available for STL mass inlet boundary.")
                le.setText('')
        ...
        # For BC_TYPE='PI'
            ...
            setup_key_widget(key, default, enabled=True, widget=le)
            units = 'm/s'
            ui.label_solids_velocity_1_units.setText(units)
            tb = ui.toolbutton_ramp_solids_velocity_1
            tb.base_key = key
            tb.keys = ramp_keys(key)
            tb.setToolTip("Define ramp for Y-axial velocity.")
            ...
            setup_key_widget(key, default, enabled=True, widget=le)
            units = 'm/s'
            ui.label_solids_velocity_2_units.setText(units)
            tb = ui.toolbutton_ramp_solids_velocity_2
            tb.base_key = key
            tb.keys = ramp_keys(key)
            tb.setToolTip("Define ramp for Z-axial velocity.")
        ...
        default = 293.15
        setup_key_widget(key, default, enabled)
        ui.toolbutton_ramp_bc_t_s.setEnabled(enabled)
        ...
        setup_key_widget(key, default, enabled,
                         reason="Not available with algebraic viscous stress model.")
        ui.toolbutton_ramp_bc_theta_m.setEnabled(enabled)
    ...
    def setup_bcs_fluid_PO_tab(self):
        ...
        def setup_key_widget(key, default=None, enabled=True, suffix='', widget=None):
            for pat in ('label_%s', 'label_%s_units',
                        'lineedit_keyword_%s_args_BC',
                        'lineedit_%s_args_BC',
                        'lineedit_keyword_%s',
        ...
        if parent != ui.page_fluid_PO:
            comp.hide()
            parent.layout().removeWidget(comp)
            # De-bold title since we are inserting it under "Optional" which is already bold
            #https://stackoverflow.com/questions/22227376/how-to-change-the-font-of-a-qgroupboxs-title-only
            comp.setStyleSheet("QGroupBox { font-weight: normal;}")
            ui.groupbox_fluid_PO_optional.layout().addWidget(comp, 3, 0, 1, 3)
            comp.show()
        self.update_bcs_fluid_mass_fraction_table()
    ...
    def setup_bcs_solids_PO_tab(self, P):
        ...
        if parent != ui.page_solids_PO:
            ...
            # De-bold title since we are inserting it under "Optional" which is already bold
            comp.setStyleSheet("QGroupBox { font-weight: normal;}")
            ui.groupbox_solids_PO_optional.layout().addWidget(comp, 3, 0, 1, 3)
            comp.show()
        self.update_bcs_solids_mass_fraction_table()
    ...
    def setup_bcs_moving_wall_tab(self):
        if not self.bcs_current_indices:
            return
        ui = self.ui.boundary_conditions
        BC0 = self.bcs_current_indices[0]
        for k in ('vel', 'omega', 'rot_center'):
            for i in (1,2,3):
                val = self.project.get_value('bc_wall_'+k, default=None, args=[BC0,i])
                txt = '' if val is None else str(val)
                getattr(ui, 'lineedit_keyword_bc_wall_%s_args_BC_%s'%(k,i)).setText(txt)
        # XXX Not sure why this hack is necessary, widgets should be unlocked on lock_level==PARTIAL ???
        for w in widget_iter(ui.page_moving_wall):
