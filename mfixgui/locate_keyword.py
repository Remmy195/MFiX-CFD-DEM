...
# TODO:  better navigation inside VTK pane
#  selection of table rows to enable panes?  or another way to force panes to display,
#  like the code that visits solid subtabs
# Locate keys in species popup - ljsig, ljeps, Cantera keys
...
ics_solids_keys_nophase = {'ic_p_star'}
...
def simple_search(key, args):
    # Return a 2-tuple, status:bool, err_msg:string or None
    found = False
    first = True
    err = None
    ...
    for pane in gui.ui.panes:
        # Prevent finding hidden bc_type in regions pane
        if key=='bc_type' and pane==gui.ui.regions:
            continue
...
def setup_pane(pane, widget, key, args=None):
    ...
    if pane_name == 'model':
        ...
        for (i,p) in enumerate([pane.materials, pane.TFM, pane.DEM, pane.PIC]):
            if is_ancestor(p, widget):
                if i==2: # DEM
                    if gui.project.solver == CGP:
                        i += 1
                    elif gui.project.solver == SQP:
                        i += 2
                    elif gui.project.solver == GSP:
                        i += 3
                elif i==3: # PIC
                    i += 3 # CGP, SQP, GSP
    ...
    elif pane_name == 'chemistry':
        for (i,p) in enumerate((pane.reactions, pane.options)):
            if is_ancestor(p, widget):
                gui.chemistry_change_tab(i)
                break
