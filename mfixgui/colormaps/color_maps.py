...
COLOR_MAP_EXT = ".rgb"
COLOR_MAPS = sorted(glob.glob(os.path.join(SCRIPT_DIR, "*" + COLOR_MAP_EXT)))
...
def get_color_map_dict():
    ...
    for name in COLOR_MAP_ORDER:
        colormap_path = os.path.join(SCRIPT_DIR, name + COLOR_MAP_EXT)
...
def get_color_map_pngs():
    ...
    for name in COLOR_MAP_ORDER:
        for r in ["", "_reversed"]:
            f = os.path.join(SCRIPT_DIR, name + r + ".png")
            pngs[name + r] = f
    return pngs
