...
from qtpy.QtCore import QProcess, QProcessEnvironment, QObject, QTimer
...
class ProcessManager(QObject):
    ...
    def start_solver(self, solver, template, omp_num_threads, nodes, project_file=None, mpirun_flags=None):
        ...
        run_command = self.get_run_command(solver, omp_num_threads, nodes, project_file=project_file, mpirun_flags=mpirun_flags)
        ...
        if template is None:
            ...
            submit_cmd = self.get_submit_command(run_command, template, mpirun_flags=mpirun_flags)
    ...
    def get_run_command(self, solver, omp_num_threads, nodes, project_file=None, mpirun_flags=None):
...
self.mfixgui install, and SMP/DMP info"""
        ...
        run_cmd += [os.path.realpath(str(solver.path))] # Symlinks on Windows, sigh
    ...
    def get_submit_command(self, cmd, template, mpirun_flags=None):
        """ Return a Named Tuple containing everything needed to start a queue job """
        ## TODO remove/deprecate Named Tuples
    ...
    def disconnect(self):
        """ disconnect stdout/stderr from mfix process """
    ...
    def start(self, cmd):
        ...
        # TODO none of this is correct for queued (Joule) jobs
        ...
        # FIXME we are now using Qt 5.15 - cgw
        saved_env = None
    ...
    def slot_read_out(self):
        data = bytes(self.mfix_process.readAllStandardOutput()).decode(
            "utf-8", errors="ignore")
        if data:
            self.mfixgui.signal_stdout.emit(data)

    def slot_read_err(self):
        data = bytes(self.mfix_process.readAllStandardError()).decode(
            "utf-8", errors="ignore")
        # Avoid duplicate message if already reported
        if data:
            if not self.mfixgui.job_manager.traceback_reported:
                self.mfixgui.signal_stderr.emit(data)

    def slot_finish(self, status):
        self.mfixgui.slot_rundir_timer()
        QTimer.singleShot(500, self.slot_finish_1)

    def slot_finish_1(self):
        self.mfixgui.slot_rundir_timer()
    ...
    def slot_error(self, error):
        if error == QProcess.FailedToStart:
            ...
            msg = None # "Process exit " + self.cmd_str
        ...
        if msg:
            self.mfixgui.signal_stderr.emit(msg)
