...
from mfixgui.external_libs.splipy import (
    curve_factory as cf,
    surface_factory as sf,
    volume_factory as vf)

from mfixgui.external_libs.splipy.stl_io import STL
...
# Example#1: Cylinder of radius 0.1 and height 0.5, with caps at both ends
    ...
    render(cyl, title='Capped cylinder')
...
#   - Front conical frustum: front radius, length, number of divisions
#   - Back conical frustum: back radius, length, number of divisions
...
# Revolve the segments. The segments are divided inside Revolve_Segments
    ...
    render(expansion, title='Expansion')
...
#Example#7: A Tube connecting a circle and a rectangle
    ...
    render(crossover, title='Crossover')
...
# This uses the Bend function with an offset
    ...
    render(coil2, title='Coil with extension')
    ...
    for segment in segments:
       # To avoid duplicate points, the last point of each segment, except the last segment is removed from the list
        ...
        if segment[0]=='line':
            ...
            pointlist.append(arc)

    points = np.vstack(pointlist)
...
# This returns the VTK Polydat object
    return revolve_points(points, theta_start, theta_end, theta_res, offset=offset)
...
def revolve_points_xyz(points, theta_start=0.0, theta_end=360.0,theta_res=32,offset=0.0):
    ...
    rpoints_shape = (npts, theta_res + 1)
    ...
    for p in points:
        x,y = p[0], p[1]
        r = np.vstack([x*cos_theta,  y + delta_y, x*sin_theta]).T
        pointlist.append(r)

    rpoints = np.vstack(pointlist)
...
def bend(rb1,rb2,theta1,theta2,bend_res,     r1,l1,n1,    r2,l2,n2,    theta_start=0, theta_end=360,theta_res=32, offset=0.0, n_front_cap=0, n_back_cap=0):
    """ Generates a bend, with optional front and back conical frustums, and optional front and back caps.
...
# Bend section (section of a torus)
    ...
    bend_section = [(theta_res + 1, bend_res + 1), xyz_bend[1]]
    bend = [bend_section]
...
# Connect them together to create the front section (conical frustum)
...
# Connect them together to create the back section (conical frustum)
...
# Connect them together to create a conical frustum
...
# Rodrigues' rotation formula
            prot = p*cos_t + np.cross(k,p)*sin_t + k*np.dot(k,p)*(1.0-cos_t)

# Translate rotated points as they follow the curve
            c = curve.evaluate(t)
            pointlist.append(c + prot)

    spoints = np.vstack(pointlist)
