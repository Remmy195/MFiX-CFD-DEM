...
PARTICLE_DATABASE = os.path.join(SCRIPT_DIR, 'particle_props.json')
...
class ParticlePopup(QDialog):
    ...
    def __init__(self, parent):
        ...
        tw.set_columns(['id', 'name', 'smd (m)', 'density (kg/m³)', 'umf (m/s)', 'group'])
    ...
    def emit_add(self):
        ...
        self.add.emit(p['name'],
                      p['smd (m)'] if p['smd (m)'] is not None
                              else default_values.d_p0,
                      p['density (kg/m³)'] if p['density (kg/m³)'] is not None
                              else default_values.ro_s0,
