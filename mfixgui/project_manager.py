...
from mfixgui.project import Project, Keyword, Equation, make_alias
...
class ProjectManager(Project):
    ...
    def change(self, widget, key, newValue, args=None):
        ...
        post_update = getattr(widget, 'post_update', None)
        if post_update:
            if callable(post_update):
                post_update()
            elif isinstance(post_update, (list, tuple)):
                for p in post_update:
                    if callable(p):
                        p()
                    else:
                        raise ValueError(p)
    ...
    def guess_solver(self):
        ...
        if solids_models == set(["TFM"]):
            ...
            return CGP
        elif solids_models == set(["SQP"]):
            return SQP
        elif solids_models == set(["GSP"]):
            return GSP
        ...
        msg = "Warning, cannot deduce model"
    ...
    def load_project_file(self, project_file):
        ...
        with warnings.catch_warnings(record=True) as ws:
            ...
            # Migrate 'dif_coef_kt' to 'dif_coeff_kt'
            for kw in list(self.keywordItems()):
                if kw.key == 'c':
                    self.gui.unset_keyword('dif_coef_kt')
                    self.gui.update_keyword('dif_coeff_kt', kw.value)

            # Migrate 'c' to 'usr_c', 'c_name' to 'usr_c_name':
            for kw in list(self.keywordItems()):
                if kw.key == 'c':
                    self.gui.unset_keyword('c', args=kw.args)
                    self.gui.update_keyword('usr_c', kw.value, args=kw.args)
                if kw.key == 'c_name':
                    self.gui.unset_keyword('c_name', args=kw.args)
                    self.gui.update_keyword('usr_c_name', kw.value, args=kw.args)

            # Set new kg_model keyword - (note, this seems to be partially duplicated below)
            kg_model = self.get_value('kg_model', default=None)
            if kg_model and kg_model.upper() not in KG_MODELS:
                self.gui.warning("Invalid kg_model %s, setting to AIR"%kg_model,
                                 popup=True)
                self.gui.update_keyword('kg_model', 'AIR')
                self.gui.unset_keyword('k_g0')
            if kg_model is None:
                if self.get_value('k_g0', default=None) is not None:
                    self.gui.update_keyword('kg_model', 'CONSTANT')
                elif self.get_value('usr_kg'):
                    self.gui.update_keyword('kg_model', 'USR')
                    self.gui.unset_keyword('usr_kg')
                    self.gui.unset_keyword('k_g0')
                elif self.get_value('energy_eq', default=True):
                    self.gui.update_keyword('kg_model', 'AIR')
            # ks_model
            for P in range(1, 1+len(self.solids)):
                val=self.get_value('usr_ks', args=[P], default=None)
                if val is not None:
                    self.gui.unset_keyword('usr_ks', args=[P])
                    if val:
                        self.gui.update_keyword('ks_model', 'USR', args=[P])


            # Don't allow specifying regions by cell indices.
            ...
            if units.lower() == 'cgs':
                ...
                for kw in self.keywordItems():
                    ...
                    if factor is not None:
                        ...
                        warnings.warn('No conversion factor for %s' % (kw.key))
                # Convert dh to SI
                if self.reactions:
                    for k,v in self.reactions.items():
                        if v.get('dh'):
                            v['dh'] *= 4184.0 # cal/mol = 4184 J/kmol
                            self.gui.print_internal('%s: dh = %s' % (k, v['dh']),
                                                font='Monospace')

                # issues/191 convert particle_input.dat if found
                particle_file = os.path.join(os.path.dirname(project_file), 'particle_input.dat')
                # Note, we should also convert particle_input.csv, but
                #  it's very unlikely that csv files will exist with CGS units
            ...
            # Parse THERMO DATA section NB: parsemfixdat does not handle this (why?)
            user_species = {}

            if self.thermo_lines_burcat:
                data = read_thermo.parse_section_burcat(self.thermo_lines_burcat)
                for (species, phase, temps, coeffs, composition, mol_weight, h_f, comment) in data:
                    species = make_alias(species.lower())
                    #tmin, tcom, tmax = temps
                    user_species[(species, phase)] = (temps, coeffs, composition, mol_weight, h_f, comment, "BURCAT")
            if self.thermo_lines_chemkin:
                data = read_thermo.parse_section_chemkin(self.thermo_lines_chemkin,
                                                         atomic=self.gui.species_popup.atomic)
                for (species, phase, temps, coeffs, composition, mol_weight, h_f, comment) in data:
                    species = make_alias(species.lower())
                    user_species[(species, phase)] = (temps, coeffs, composition, mol_weight, h_f, comment, "CHEMKIN")
            ...
            for g in self.gasSpecies:
                ...
                user_def = user_species.get((key, phase))
                # Look for mismatched phase
                if not user_def:
                    for ((s,p), v) in user_species.items():
                        if s == key:
                            # gasSpecies all have phase='g'.  Correct this with info from Burcat
                            phase = g.phase = p.upper()
                            user_def = v
                            break
                if user_def:
                    (temps, coeffs, composition, mol_weight, h_f, comment, format) = user_def
                    ...
                    if species_data:
                        ...
                        species_data['format'] = 'BURCAT'
                        species_data['composition'] = None
                        if mw_g is not None:
                            species_data['mol_weight'] = mw_g
                            source = 'Burcat*' # Modified mol. weight

                if not species_data:
                    excs.append("No definition found for species '%s' phase '%s'" % (species, phase))
                # Species/alias unification
                if len(species) > 18:
                    species_1 = self.gui.species_burcat_name(alias, 0, g.ind)
                    self.gui.set_unsaved_flag()
                    self.thermo_data_burcat.pop(species, None)
                    self.thermo_data_chemkin.pop(species, None)
            ...
            if species_data:
                # XXX TODO initialize the thermo_data with the lines read from
                # the file, instead of parsing and reformatting, to minimize
                # churn
                self.update_thermo_data(self.gui.fluid_species, loading=True)
            ...
            for s in self.solids:
                ...
                for (i,sp) in enumerate(species,1):
                    ...
                    if not user_def:
                        for ((s1,p), v) in user_species.items():
                            if s1 == key:
                                # Note solids objects all have phase='s'.  Correct this with data
                                # from Burcat
                                phase = sp.phase = p.upper()
                                user_def = v
                                break
                    if user_def:
                        (temps, coeffs, composition, mol_weight, h_f, comment, format) = user_def
                    ...
                    if not species_data:
                        excs.append(ValueError("No definition found for species '%s' phase '%s'" % (species, phase)))
                    else:
                ...
                if species_data:
                    ...
                    # the file, instead of parsing and reformatting, to minimize
                    # churn
                    self.update_thermo_data(self.gui.solids_species[s.ind], loading=True)
            ...
            # Default kg_model
            if not self.get_value('kg_model'):
                if self.get_value('usr_kg'):
                    self.gui.update_keyword('kg_model', 'USR')
                    self.gui.unset_keyword('usr_kg')
                elif self.get_value('k_g0'):
                    self.gui.update_keyword('kg_model', 'CONSTANT')
                elif self.get_value("energy_eq", default=True):
                    self.gui.update_keyword('kg_model', 'AIR')
            if self.get_value('kg_model') == 'KG_USR':
                self.gui.update_keyword('kg_model', 'USR')
            # Set mu_g_model
            if not self.get_value('mu_g_model'):
                if self.get_value('usr_mug'):
                    ...
                    self.gui.update_keyword('mu_g_model', 'SUTHERLAND')
            if self.get_value('mu_g_model') == 'MU_G_USR':
                self.gui.update_keyword('mu_g_model', 'USR')
            ...
            # issues/1389
            nr = len(self.reactions)
            if nr > 2000:
                for k in list(self.reactions.keys())[2000:]:
                    self.reactions.pop(k, None)
                    warnings.warn("Too many reactions (%s), limiting to 2000" % nr)
            ...
            for args in self.get_key_indices(key):
                val = self.get_value(key, args=args, default=False)
                if val:
                    warnings.warn(key.upper() + " is deprecated")
                self.gui.unset_keyword(key, args=args)

            # issue 1611
            if not self.get_value('cartesian_grid'):
                self.gui.unset_keyword('re_indexing')
            ...
            for kw in kwlist:
                ...
                if kw.value == 'TEMPERATURE_DEPENDENT_AIR':
                    kw.value = 'AIR'
                if kw.value == 'CANTERA_BASED_POLYNOMIAL':
                    kw.value = 'CANTERA_POLY'
    ...
    def update_thermo_data(self, species_dict, loading=False):
        d = {k:v for (k,v) in species_dict.items() if v.get('format')=='BURCAT'}
        self.update_thermo_data_burcat(d, loading=loading)

        # Make sure species is not in both dicts.
        # Maybe it was a mistake to split thermo_data into two
        # separate dicts for Burcat and Chemkin

        for k in d.keys():
            x=self.thermo_data_chemkin.pop(make_alias(k.lower()),None)
        d = {k:v for (k,v) in species_dict.items() if v.get('format')=='CHEMKIN'}
        self.update_thermo_data_chemkin(d, loading=loading)
        for k in d.keys():
            x = self.thermo_data_burcat.pop(make_alias(k.lower()),None)

    def update_thermo_data_burcat(self, species_dict, loading=False):
        """Update definitions in self.thermo_data_burcat based on data in species_dict."""
        update_dict = dict((data['species'].lower(), self.format_burcat(data))
                           for (species, data) in species_dict.items())

        changed = any(self.thermo_data_burcat.get(k) != update_dict[k] for k in update_dict)
        if changed:
            self.thermo_data_burcat.update(update_dict)
        # Why is unsaved_flag getting set at load time?
        # Because we are using this to initialize empty thermo_data_burcat
        # We could initialize it with the lines from the file, instead of
        #  parsing and reformatting.
        if changed and not loading:
            self.gui.set_unsaved_flag()


    def update_thermo_data_chemkin(self, species_dict, loading):
        """Update definitions in self.thermo_data_chemkin based on data in species_dict."""
        update_dict = dict((data['species'].lower(), self.format_chemkin(data))
                           for (species, data) in species_dict.items())

        changed = any(self.thermo_data_chemkin.get(k) != update_dict[k] for k in update_dict)

        # Why is unsaved_flag getting set at load time?
        # Because we are using this to initialize empty thermo_data_chemkin
        # We could initialize it with the lines from the file, instead of
        #  parsing and reformatting.
        if changed:
            self.thermo_data_chemkin.update(update_dict)
        if changed and not loading:
            self.gui.set_unsaved_flag()
    ...
    def format_burcat(self, data):
        ...
        if len(temps) == 3:
            ...
            tmin, tmax = temps
            tcom = 1000.0
        row = (species, tcom, data['phase'].upper(),
            tmin, tmax,
            calc_quality, digits, data['mol_weight'])
        lines.append('%-18stcom=%-19s0.%c%10.3f%10.3f%3s%10.*f 1' % row)
    ...
    def format_chemkin(self, data):
        """Return a list of lines in (extended) CHEMKIN format"""
        lines = []
        species = data['species']
        phase = data['phase']
        temps = data['temps']
        ...
        "MySpecies         120186H   0O   0          G  0300.00   5000.00  1000.00      1&"
        lines.append("%-44s%s  %07.2f   %07.2f  %07.2f      1&" %
                     (species, phase, temps[0], temps[-1], temps[1]))
        comp_text = ''
        for elt in sorted(composition.keys()):
            comp_text += ' %s %d'%(elt,composition[elt])
            if len(comp_text) > 60:
                comp_text += ' &'
                lines.append(comp_text.strip())
                comp_text = ''
        if comp_text:
            lines.append(comp_text.strip())
        lines.append("TEMP " + ' '.join(("%g"%t for t in temps)))
        # Reverse coeffs in blocks of 7
        groups = [coeffs[i:i+7] for i in range(0, len(coeffs), 7)]
        coeffs = ['%15.8E'%coeff for group in reversed(groups) for coeff in group]
        while(coeffs):
            lines.append(''.join(coeffs[:5]))
            lines.append(''.join(coeffs[5:7]))
            coeffs = coeffs[7:]
