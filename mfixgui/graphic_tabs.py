...
PLOT_ITEMS = dict([
    ...
    ['NIT', {
        ...
        'y_var':'nit'}],
    ['Simulation time', {
        'btn_name':'pushbutton_elapsed_time',
        'left':'Simulation time [s]',
        'bottom':'Elapsed real time [s]',
        'x_var':'elapsed_time',
        'y_var':'time'}],
    ['Residuals', {
...
class BaseGraphicTab(QtWidgets.QWidget):
    ...
    def create_vtk_widget(self, load):
        ...
        self._change_name(name)
        self.graphic_tabs.set_tab_tooltip(self,
            "Display simulation output with Visualization Toolkit")
        from mfixgui.vtk_widgets.results_viewer import GraphicsVtkWidget
        self.vtk_widget = GraphicsVtkWidget(self, load=load)
        if self.gui.project.get_value('no_k'):
            self.vtk_widget.change_interaction(style_2d=True)
        if not load:
            self.vtk_widget.default_view()
    ...
    def plot(self, x=None, y=None, append=False):
        ...
        if append:
            # append y
            if isinstance(y, (list, tuple)): # Residuals
                if self.y == []:
                    self.y = {}
                ...
                self.x.append(x)
            elif not is_valid(y) or x is None:
                return
            # if the value did not change, don't append.
            elif ylen >= 2 and y == self.y[-1] == self.y[-2]:
                self.x[-1] = x
            else:
                self.x.append(x)
                self.y.append(y)
...
class GraphicTabs:
    ...
    def set_tab_tooltip(self, tab, tooltip):
        tab_widget = self.ui.tabWidgetGraphics
        index = tab_widget.indexOf(tab)
        tab_widget.tabBar().setTabToolTip(index, tooltip)


    def remove_graphics_tabs(self):
        tab_widget = self.ui.tabWidgetGraphics
        for tab in self.plot_dict.values():
            if tab.vtk:
                # stop qtimers
                tab.vtk_widget.deleting = True # XXX issues/1825
                tab.vtk_widget.reset()
                # manually flush events (why?)
    ...
    def graphics_to_str(self):
        ...
        for key, tab in self.plot_dict.items():
            ...
            if vtk:
                ...
                isd = d.get('image_stack_dir')
                if isd and (os.path.abspath(isd)==os.path.abspath(os.getcwd())):
                    d.pop(isd)
        ...
        for key in order:
            ...
            if tab_data.get('plot', False) and key in PLOT_ITEMS:
                ...
                if self.loadvtk and 'MFIX_NO_VTK' not in os.environ and VTK_AVAILABLE:
                    ...
                    vtk_state = tab_data.get('vtk_state', {})
                    if vtk_state and 'image_stack_dir' in vtk_state:
                        isd = vtk_state.get('image_stack_dir')
                        if isd and not os.path.exists(isd):
                            vtk_state.pop('image_stack_dir')
                    if vtk_state:
                        vtk_widget.set_state(vtk_state)
                else:
                ...
                if self.loadvtk and 'MFIX_NO_VTK' not in os.environ and VTK_AVAILABLE:
                    ...
                    self.warn('The vtk module is not available, cannot create histogram window')
