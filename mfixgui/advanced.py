...
from mfixgui.tools import (keyword_args, format_description,
                           unicode_greek)
...
class AdvancedPopup(QDialog):
    ...
    def handle_selection(self):
        ...
        descr = format_description(descr, needle)
        ...
        if valids:
            ...
            for (k,v) in valids.items():
                ...
                if alias and k != alias:
                    val += '(%s)'% k
                note = format_description(v.get('note', ''), needle)
        ...
        if not isinstance(le.max, (int, float)):
            le.max = None
        le.min = validrange.get('min')
        if not isinstance(le.min, (int, float)):
            le.min = None
        le.exclude_min = validrange.get('exclude_min', False)
        le.exclude_max = validrange.get('exclude_max', False)
        ...
        if needle:
            ...
            if idx >= 0:
                ...
                ui.info_area.ensureCursorVisible()
