...
from mfixgui.tools.qt import sub_icon_size, set_alignment, append_stylesheet
...
class FileMenu:
    ...
    def init_file_menu(self, mfixgui):
        ...
        append_stylesheet(self.file_menu,"QWidget#file_menu{background-color: #E0E0E0;}")
        self.file_menu.hide()

        ui.file_menu_back = self.make_back_button()
        menu = ui.file_menu_list = make_list_widget()
        ui.stackedwidget_file_menu = QStackedWidget()
        ui.stackedwidget_file_menu.setObjectName("filemenu")
    ...
    def set_file_menu_width(self):
        menu = self.ui.file_menu_list
        fm = menu.fontMetrics()
        w = fm.width("Submit bug report") + 20 + sub_icon_size().width()
        menu.setMaximumWidth(w)
        menu.setMinimumWidth(w)
        self.ui.back_button.setMaximumWidth(w)
        self.ui.back_button.setMinimumWidth(w)
    ...
    def add_items(self):
        ...
        append_stylesheet(stacked, "QStackedWidget#filemenu{background-color: white;}")
        ...
        app_items = [MenuItem("Settings", 'settings', ui_item=(ui, self.settings_widget)),
                     ...
                     MenuItem("Submit bug report", 'bug', self.submit_bug_report),
    ...
    def open_file_menu(self, item=None):
        ...
        self.open_widget.populate_recent_projects() # 1542
    ...
    def show_file_menu(self):
        ...
        self.file_menu.setGeometry(-int(w/2) , 0, w, h) # Why w/2 here?
        self.file_menu.show()
        self.file_menu.raise_()
        ani = make_animation(self.file_menu, (-int(w/4), 0), (0, 0)) # w/4 ?
...
def make_animation(target, start, end):
    ...
    animation.setStartValue(QPoint(int(x_start), int(y_start)))
    animation.setEndValue(QPoint(int(x_end), int(y_end)))
    return animation
