...
class MeshStatsViewer:
    ...
    def init_mesh_stats_viewer(self):
        ...
        # file system watcher
        # XXX we know this doesn't work across NFS, should use a timer.  We
        #  already have "watch_run_dir_timer" in gui

        self.mesh_stats_watcher = QFileSystemWatcher()
        self.mesh_stats_watcher.directoryChanged.connect(self.mesh_stats_dir_changed)
        self.mesh_stats_watcher.fileChanged.connect(self.mesh_stats_file_changed)

        self.mesh_stats_timer = QTimer()
    ...
    def look_for_mesh_stats(self):
        ...
        if os.path.exists(b_name):
            self.mesh_stats_watcher.addPath(b_name)
            self.read_mesh_stats_file(b_name)
        else:
            self.mesh_stats_watcher.addPath(os.path.dirname(b_name))
    ...
    def mesh_stats_dir_changed(self, path):
        ...
        if os.path.exists(ms_name):
            self.mesh_stats_watcher.addPath(ms_name)
            self.mesh_stats_watcher.removePath(path)
    ...
    def mesh_stats_file_changed(self, path):
        if not self.watching_mesh_stats_file:
            ...
            # XXX Why?
            self.mesh_stats_timer.singleShot(1000, lambda:self.read_mesh_stats_file(path))

    def read_mesh_stats_file(self, path):
        self.watching_mesh_stats_file = False
        if not os.path.exists(path):
            # add the project dir back to the file watcher
            self.mesh_stats_watcher.addPath(os.path.dirname(path))
            return
    ...
    def reset_mesh_stats_viewer(self):
        paths = (self.mesh_stats_watcher.files()
                 + self.mesh_stats_watcher.directories())
        if paths:
            self.mesh_stats_watcher.removePaths(paths)
        self.mesh_stats_list.clear()
