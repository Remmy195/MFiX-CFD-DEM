...
__all__ = ['BSplineBasis', 'csr_matrix']

def csr_matrix(data, shape):
    #https://docs.scipy.org/doc/scipy/reference/generated/scipy.sparse.csr_matrix.html
    if len(data) == 3:
        data, indices, indptr = data
        ret = np.zeros(shape, dtype=data.dtype)
        for i in range(len(indptr)-1):
            ret[i, indices[indptr[i]:indptr[i+1]]] = data[indptr[i]:indptr[i+1]]
        return ret
    else:
        raise ValueError
