...
from mfixgui.tools import plural, get_mfix_src, get_full_path
from mfixgui.tools.qt import sub_icon_size, get_font_size

WINDOWS = sys.platform.startswith("win")
re_message_location = re.compile(r"(Error|Warning|Message|\s*) (from|at) ((?:[A-Z]:)?[^:]+):(\d+)\s*$", re.IGNORECASE)
...
class ConsolePrinter:
    ...
    def __init__(self, mfixgui):
        # TODO global singleton gui object!
        ...
        self.timer.timeout.connect(self.flush_traceback)
    ...
    def show_message_location_in_editor(self, _event):
        ...
        if result:
            path, lineno = result
            if os.path.exists(get_full_path(self.mfixgui.get_project_dir(), path)):
                self.mfixgui.change_mode("editor")
                self.mfixgui.editor_widget.show_location(path, lineno, error=True)
                # TODO highlight line
            else:
                self.mfixgui.error("Cannot find file %s" % path,
                                   popup=True)

    def find_path(self, text):
        match = re_message_location.match(text)
        if match is None:
            return None
        path = match.group(3)
    ...
    def print_welcome(self):
        ...
        self.print_internal("MFiX-GUI version %s" % get_version(), color="blue")
    ...
    def handle_stdout(self, text, is_stderr=False):
        if not text:
            return
        if WINDOWS:
            text = text.replace(r'\\', '\\')
        ...
        if self.mfixgui.job_manager.stopping:
            return
    ...
    def handle_stderr(self, text):
        if not text:
            return
        # Unfortunately Windows does not seem able to distinguish stdout from stderr (?)
        if WINDOWS:
            self.handle_stdout(text, is_stderr=True)
            return
        self.mfixgui.update_logger_solver_output(text)
        jm = self.mfixgui.job_manager
        if jm and jm.stopping:
            return
        if text.startswith('mpirun') and 'Forwarding signal' in text:
            return
        if text.strip().endswith('(ignored)'):
            return
        if text.strip() == 'Terminated':
            return
        if text.strip().upper() == 'ERROR STOP 1':
            return
        # Note, normal error messages from MFiX go to stdout. We only get stderr on a crash.
        if not self.traceback_lines:
            self.traceback_lines.append('>>>>>---TRACEBACK')
            self.traceback_lines.append('ERROR: Unexpected solver message:')
            self.timer.start(500) # plenty of time to consolidate traceback msgs
    ...
    def flush_traceback(self):
        sym = filename = None
        # Let's clean the traceback up a bit
        # Don't print extra line after traceback (command line invocation)
        if 'mfixgui.pymfix' in self.traceback_lines[-1]:
            self.traceback_lines.pop(-1)
        # Matches screwy code in error_manager_mod.f and ide_widget.py
        src_prefix = os.path.join(get_mfix_src(), 'model')
        prj_prefix = os.path.dirname(self.mfixgui.get_project_dir())
        count = 0
        for line in self.traceback_lines:
            ...
            if line.startswith('#'):
                if 'in ???' in line:
                    ...
                    sym = line.rsplit(maxsplit=1)[-1]
                    while sym.endswith('_'):
                        sym = sym[:-1]
                    while sym.startswith('_'):
                        sym = sym[1:]
            else:
                loc = line.split(maxsplit=1)[-1]
                if sym:
                    if '/python' in loc.lower(): # too deep in stack!
                        continue
                    try: # Handle symbolic links to source dir.  (Is this too much?)
                        fname, lno = loc.rsplit(':', maxsplit=1)
                        p1 = os.path.realpath(fname)
                        p2 = os.path.realpath(src_prefix)
                        if p1.startswith(p2):
                            fname = os.path.join(src_prefix, p1[len(p2)+1:])
                            loc = fname + ':' + lno
                    except OSError as e:
                        print(e)

                    for p in (src_prefix, prj_prefix):
                        if loc.startswith(p):
                            loc = loc[len(p)+1:]
                            break

                    self.message_scanner.add_line(f'#{count} {sym}')
                    self.message_scanner.add_line(f'        at {loc}')
                    count += 1

        self.traceback_lines.clear()
        self.message_scanner.add_line('<<<<<---TRACEBACK')
        self.mfixgui.job_manager.traceback_reported = True
    ...
    def is_delim(self, line):
        return line and line.strip().startswith(('<<<<<', '>>>>>'))

    def print_internal(self, line, color=None, font=None):
        if not (self.can_skip(line) or self.is_delim(line)):
            self.print_line(line, color, font)

    def print_line(self, line, color=None, font=None):
        ...
        m = re_message_location.match(line)
        if m:
            u_start, u_end = m.span(3)[0], m.span(4)[1]
        underline = bool(m)

        line = line.rstrip() + '\n'
        cursor = self.get_cursor(line, font, color, strikeout, underline=False)
        if underline:
            cursor.insertText(line[:u_start])
            cursor = self.get_cursor(line, font, color, strikeout, underline=True)
            cursor.insertText(line[u_start:u_end])
            cursor = self.get_cursor(line, font, color, strikeout, underline=False)
            cursor.insertText(line[u_end:])
        else:
        ...
        if self.autoscroll:
            scrollbar = self.console.verticalScrollBar()
            scrollbar.setValue(scrollbar.maximum())
            #Scrollbar value changed callback can reset autoscroll
            self.autoscroll = True
...
def get_char_format(line, font, color, strikeout=False, underline=False):
    ...
    #Is this needed? Why does monospace scale but only up to a point?
    #if fs:= get_font_size():
    #    char_format.setFontPointSize(fs)
