...
# we do not to expose paths like
#  'C:/Users/cgw/mambaforge/conda-bld/mfix-solver_1679106123663/work/model/monitors/average_des.f90'
re_conda_bld_dir = re.compile('[^ ]*/conda(-bld)?/mfix-solver.*/model/')

conda_prefix = os.environ.get('CONDA_PREFIX')
if conda_prefix:
    conda_prefix = conda_prefix.replace('\\', '/')
    if not conda_prefix.endswith('/'):
        conda_prefix += '/'
...
class MessageScanner:
    ...
    def add_line(self, line):
        ...
        #Stack trace from default solver
        line = re_conda_bld_dir.sub('', line)

        if conda_prefix and conda_prefix in line:
            # Keep full paths in the case of load error, for troubleshooting
            if not line.startswith("Using"): # see pymfix.py
                line = line.replace(conda_prefix, '')
                line = line.replace('share/mfix/src/model/', '')
            ...
            if line.lstrip().startswith(open_delim):
                ...
                for close_index in range(lineno + 1, len(lines)):
                    ...
                    if lines[close_index].lstrip().startswith(open_delim):
                        self.mfixgui.error("Solver message format error.\n"
                                           + ''.join(lines[lineno:]),
                                           popup=True)
