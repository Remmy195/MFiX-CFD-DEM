...
RE_ERR_1001_ILLEGAL_KEY = re.compile(r"Error 1001: Invalid or unknown input: (.*)")
...
class SolverLogMessage:
    ...
    # TODO, clean this up, especially the naming
    # (text should be plain-text, html should be called html)
    @property
    def text(self):
        """ Return the text of the message (as HTML)"""
        html_message = escape("\n".join(line.rstrip() for line in self.lines))
        #  not sure why this rstrip/join is needed
            ...
            for rgx in RE_ERR_1000_BAD_KEY, RE_ERR_2000_BAD_LINE:
                ...
                if match:
                    ...
                    if 0 <= lineno - 1 < len(datfile_lines):
                        ...
                        print(f"Project file does not have line #{lineno} in  {line}")
                    return
    ...
    def edit_keyword(self, newtext):
        ...
        if kwerr is None:
            print("edit_keyword called on non-keyword error message")
            return
        if not newtext:
    ...
    def drop_keyword(self):
        ...
        kwerr = self.keyword_error()
        if kwerr is None:
            print("drop_keyword called on non-keyword error message")
            return
        key, _ = kwerr
