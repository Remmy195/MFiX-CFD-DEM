...
from mfixgui.tools import get_full_path
from mfixgui.tools.qt import get_ui, get_icon, sub_icon_size
...
class MessageTab:
    def __init__(self, parent, console_printer, name, ui_widgets):
        (self.tab,
         ...
         self.next_button,
         self.last_button,
         self.textbrowser) = ui_widgets # FIXME
        ...
        self.first_button.setIconSize(sub_icon_size())
        ...
        self.prev_button.setIconSize(sub_icon_size())
        ...
        self.next_button.setIconSize(sub_icon_size())
        ...
        self.last_button.setIconSize(sub_icon_size())
        ...
        self.textbrowser.anchorClicked.connect(self.handle_anchor)

    def handle_mouse(self, ev):
        gui = self.console_printer.mfixgui
        loc = self.source_label.text()
        if loc and ':' in loc:
            file, line = loc.rsplit(':', maxsplit=1)
            try:
                line = int(line)
            except ValueError as e:
                print(e)
                line = 1
            if os.path.exists(get_full_path(gui.get_project_dir(), file)):
                gui.change_mode('editor')
                gui.editor_widget.show_location(file, line, error=True)
            else:
                gui.error("Cannot find file %s" % file,
                          popup=True)

    def handle_anchor(self, anchor):
        loc = anchor.toString()
        self.textbrowser.setSource(QUrl('')) #prevent navigation in dialog

        loc = loc.replace('%3A', ':')
        if ':' not in loc:
            print("Error, malformed URL", loc)
            return
        ...
        try:
            ...
            lno = 1
        gui = self.console_printer.mfixgui
        if os.path.exists(get_full_path(gui.get_project_dir(), fname)):
            gui.change_mode("editor")
            gui.editor_widget.show_location(fname, lno, error=True)
        else:
            gui.error("Cannot find file %s" % fname,
                      popup=True)
    ...
    def update(self):
        ...
        if message is not None:
            ...
            tok = first_line.split(' ')
            if len(tok) > 2 and tok[1] == 'from':
                src = tok[2]
                if src.startswith('<pre>'):
                    src = src[5:]
                self.source_label.setText(src)
            else:
                self.source_label.setText(' '.join(tok[1:]))
            text = message.text[len(first_line)+1:]
            width = 20 + QFontMetrics(tb.currentFont(), tb).size(0, strip_html(text)).width()
            ...
            for line in text.splitlines():
                ...
                lstrip = line.strip()
                if lstrip.startswith('at '):
                    if re_loc.match(lstrip):
                        lstrip = lstrip[3:]
                        fname, lno = lstrip.rsplit(':', maxsplit=1)
                        if lno.isdigit():
                            lno = int(lno)
                            self.source_label.setText('%s:%d'%(fname, lno))
                            line = '  at <a href="%s%%3A%s">%s:%s</a>' % (fname,lno, fname,lno)
                lines2.append(line)
            text = '\n'.join(lines2)

            ### I found a pretext to use f-strings!
            tb.setHtml(f'<pre>{text}</pre>')
