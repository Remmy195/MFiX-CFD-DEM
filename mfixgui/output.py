...
from qtpy.QtCore import Qt, QRegExp, QEvent

from qtpy.QtWidgets import (QCheckBox, QComboBox, QGroupBox, QHeaderView, QLabel,
                            QLineEdit, QPushButton, QTableWidgetItem, QWidget)

from qtpy.QtGui import QPixmap, QPalette, QRegExpValidator
...
from mfixgui.tools.qt import (get_combobox_item, get_selected_row, get_selected_rows,
                              set_combobox_tooltip, set_item_enabled, set_item_noedit,
                              sub_icon_height, trim_table_height, widget_iter)
...
PAGE_CELL, PAGE_PARTICLE, PAGE_GEOMETRY = range(3)
PAGE_FORCE = PAGE_PARTICLE # reuse page
...
re_no_space_in_filename = QRegExp(r'[^ \t\n]*')
...
class Output:
    ...
    def init_output(self):
        ...
        le.setValidator(QRegExpValidator(re_no_space_in_filename))
        ...
        self.add_tooltip(ui.tablewidget_fluid_mass_fraction, key="vtk_x_g")
        self.add_tooltip(ui.tablewidget_fluid_molar_fraction, key="vtk_y_g")
        self.add_tooltip(ui.tablewidget_particle_reactions, key="vtk_part_rrate")
        #self.add_tooltip(ui.tablewidget_reactions)
        ui.tablewidget_reactions.keys = ['vtk_des_rrate', 'vtk_fluid_rrate']
        ui.groupbox_cell_reactions_deprecated.setVisible(False)
        ui.groupbox_cell_reactions_deprecated.hidden_msg = 'ReactionRates array size (nrr) is 0.'
    ...
    def enable_disable_solid_inventory(self):
        ...
        self.set_widget_enabled(ui.checkbox_keyword_breakdown_solid_inventory_by_phase, val)

    def handle_combobox_vtk_gsp_output_type(self, val):
        if not self.vtk_current_indices:
            return
        for idx in self.vtk_current_indices:
            self.update_keyword('vtk_gsp_output_type', val+1, args=[idx])
        set_combobox_tooltip(self.ui.output.combobox_vtk_gsp_output_type)
    ...
    def handle_combobox_usr_region(self, index):
        ...
        if index == 0: # None
            ...
            if self.output_region_dict is None:
                self.output_region_dict = self.get_region_dict()
            text = cb.currentText()
    ...
    def init_output_vtk_tab(self):
        ...
        self.disable_widget(ui.toolbutton_delete) # Need a selection
        ...
        cb = ui.combobox_vtk_gsp_output_type
        key = 'vtk_gsp_output_type'
        cb.keys = [key]
        cb.currentIndexChanged.connect(
            self.handle_combobox_vtk_gsp_output_type)
        self.add_tooltip(get_combobox_item(cb,0), key, value=1)
        self.add_tooltip(get_combobox_item(cb,1), key, value=2)
    ...
    def handle_output_vtk_region_selection(self):
        ...
        self.set_widget_enabled(ui.toolbutton_delete, enabled)
        # This disables the bottom set of tabs, so navigation is limited
        #self.set_widget_enabled(ui.bottom_frame_vtk, enabled)

        # Allow the bottom tab set to still function during run
        for w in (ui.cell_common,
                  ui.subpage_particle,
                  ui.scrollarea_cell):
            self.set_widget_enabled(w, enabled, ignore_lock=True)
        self.set_widget_enabled(ui.bottom_tab_frame, enabled, ignore_lock=True) # Allow sub-tab navigation while locked
        self.set_widget_enabled(ui.label_select_cell_data, enabled)
    ...
    def output_show_regions_popup(self):
        ...
        #  Particle data
        #    Selection only available with DEM, CGP, SQP, GSP or PIC solids
        #    Sets keyword VTK_DATA(#) to 'P'
        solids_models = set(self.project.get_value('solids_model', default='TFM', args=[i])
        ...
        enabled = any(x in solids_models for x in ('DEM','PIC','CGP','SQP','GSP'))
        item = get_combobox_item(rp.combobox_2, 1)
        self.set_widget_enabled(item, enabled, reason="Not available for TFM solids.")
        if not enabled:
    ...
    def output_cancel_add(self):
        ...
        # always re-enable - used to block all widgets when the region selection
        # popup is active.
        self.enable_widget(ui.bottom_frame_vtk)
    ...
    def output_add_regions_1(self, selections, output_type=None, indices=None, autoselect=False):
        ...
        for (i, region_name) in enumerate(selections):
            ...
            if region_data is None: # ?
                self.warn("no data for output region %s" % region_name)
                continue
    ...
    def output_logs_from_str(self, s):
        ...
        # issues/1485 save solver output and dt by default
        if 'dt' not in self.output_logs:
            self.output_logs['dt'] = [True, 'dt_log', 'overwrite']
        if 'solver_output' not in self.output_logs:
            self.output_logs['solver_output'] = [True, 'solver_log', 'overwrite']
    ...
    def init_output_spx_tab(self):
        ...
        pb = ui.pushbutton_set_all_spx
        pb.clicked.connect(self.output_set_all_spx_dt)
        ui.lineedit_set_all_spx.dtype = float
        ui.lineedit_set_all_spx.min = 0.0

    def output_set_all_spx_dt(self):
        ui = self.ui.output
        try:
            val = ui.lineedit_set_all_spx.value
            if isinstance(val, (float, int, Equation)):
                for i in range(1,12):
        ...
        if enable:
            ...
            for i in range(1, 9):
                ...
                for j in range(len(cb)):
                    self.set_widget_enabled(get_combobox_item(cb, j),
                                            j==0 # <None> always enabled
    ...
    def init_output_log_tab(self):
        ...
        ui.widget_solid_inventory_frequency.key = 'report_solid_inventory_dt'
    ...
    def output_enable_spx(self, enabled):
        ...
        if enabled:
            ...
            ui.pushbutton_spx.setToolTip(None)
            ...
            ui.pushbutton_spx.setToolTip("SPx outputs are not enabled.")
            for i in range(1, MAX_SP+1):
                self.unset_keyword('spx_dt', args=[i])

    def setup_output(self, allow_disabled_tab=False):
        ui = self.ui.output
        # Is it OK to add/delete outputs when locked?
        #if self.lock_level:
        #    for w in (ui.toolbutton_add, ui.toolbutton_delete,
        #              ui.toolbutton_usr_add, ui.toolbutton_usr_delete,
        #              ui.toolbutton_usr_up, ui.toolbutton_usr_down):
        ...
        self.enable_widget(ui.checkbox_spx)
        ui.checkbox_spx.setChecked(activate_spx)
        ui.checkbox_spx.key = 'spx_dt'
        ui.pushbutton_spx.setEnabled(activate_spx)
        ui.pushbutton_spx.setToolTip(None if activate_spx
                                     else "SPx outputs are not enabled.")
        vtk_enabled = self.project.get_value('write_vtk_files', default=False)
        ui.pushbutton_vtk.setEnabled(vtk_enabled)
        ui.pushbutton_vtk.setToolTip(None if vtk_enabled else
                                     "VTK outputs are not enabled.")
    ...
    def setup_output_basic_tab(self):
        ...
        ui.pushbutton_vtk.setToolTip(None if write_vtk_files else
                                     "VTK outputs are not enabled.")
        ...
        ui.pushbutton_vtk.setToolTip(None if enabled else
                                     "VTK outputs are not enabled.")
        ...
        ui.lineedit_keyword_vtu_dir.setValidator(QRegExpValidator(re_no_space_in_filename))
        if enabled:
        ...
        ui.pushbutton_vtk.setToolTip(None if enabled else
                                     "VTK outputs are not enabled.")
        gb = ui.groupbox_write_part_out
        enabled = self.project.solver in (DEM,CGP,SQP,GSP)
        write_part_out = self.project.get_value('write_part_out', default=False)
        reason = 'Available for DEM/CGP/SQP/GSP models only.'
        self.set_widget_enabled(gb, enabled, reason=reason)
        gb.setChecked(enabled and write_part_out)
        # XXX set tooltips/reasdons on widgets inside groupbox?
        ...
        while len(dws) < n_species:
            n = 1+len(dws)
            cb = QComboBox()
            cb.setObjectName("combobox_%s_%d" % (key, n))
            setattr(ui, cb.objectName(), cb) # Keep a reference
            cb.addItem("Include")
            cb.addItem("Exclude")
            cb.key = key
            cb.args = [n]
    ...
    def setup_output_spx_tab(self):
        ...
        #    Selection only available if DEM, CGP, SQP, GSP or PIC solids
        ...
        #    No default (empty string)
        ui.lineedit_keyword_vtp_dir.setValidator(QRegExpValidator(re_no_space_in_filename))

        #Select particle data format
        #
        #Selection only available if DEM, CGP, SQP, GSP or PIC solids and PRINT_DES_DATA = .TRUE.
    ...
    def setup_output_vtk_tab(self):
        ...
        enabled = (row is not None)
        self.set_widget_enabled(ui.toolbutton_delete, enabled)
        self.set_widget_enabled(ui.bottom_frame_vtk, enabled, ignore_lock=True)
        ...
        #Specify write starting time
        #Specification is optional. Leave blank to start writing vtk file from initial simulation time.
        #Sets keyword VTK_T_START(#)
        #DEFAULT BLANK (Start writing VTK files from the beginning)
        key = 'vtk_t_start'
        default = None
        le = ui.lineedit_keyword_vtk_t_start_args_VTK
        val = self.project.get_value(key, args=[V0])
        le.updateValue(key, val)

        #Specify write ending time
        #Specification is optional. Leave blank to write vtk file until the end of simulation.
        #Sets keyword VTK_T_END(#)
        #DEFAULT BLANK (Write VTK files until the end of simulation)
        key = 'vtk_t_end'
        default = None
        le = ui.lineedit_keyword_vtk_t_end_args_VTK
        val = self.project.get_value(key, args=[V0])
        le.updateValue(key, val)
        ...
        if vtk_data in ('G', 'F'):
            for w in (ui.label_vtk_nxs, ui.label_vtk_nys, ui.label_vtk_nzs,
                ...
                name = 'geometry' if vtk_data=='G' else 'force chain'
                self.disable_widget(w, reason='Not available for %s data.' % name)
        ...
        # Force chain max only visible for force chain data
        ui.checkbox_keyword_vtk_fcmax_only_args_VTK.setVisible(vtk_data == 'F')
    ...
    def setup_output_vtk_cell(self):
        ...
        for (i, solid_name) in enumerate(self.solids.keys(), 1):
            model = self.project.get_value('solids_model', default='TFM', args=[i])
    ...
    def setup_output_fluid_subtab(self):
        ...
        # We should probably just disable this tab if fluid solver disabled
        ...
        # disable some controls if energy_eq disabled
        for key in 'vtk_t_g', 'vtk_k_g', 'vtk_c_pg':
            cb = getattr(ui, 'checkbox_keyword_%s_args_VTK'%key)
            enabled = not self.fluid_solver_disabled and self.project.get_value('energy_eq', default=True)
            self.set_widget_enabled(cb, enabled, reason='Requires energy equations and fluid solver.')
            if not enabled:
                cb.setChecked(False)
                for V in self.vtk_current_indices:
        ...
        self.set_widget_enabled(cb, enabled, reason='Fluid solver disabled, or no fluid species.')
        if not enabled:
            cb.setChecked(False)
            for V in self.vtk_current_indices:
                self.unset_keyword(key, args=[V])
        ...
        for key in ('vtk_k_turb_g', 'vtk_e_turb_g'):
            cb = getattr(ui, 'checkbox_keyword_%s_args_VTK' % key)
            self.set_widget_enabled(cb, enabled, reason='Requires K_EPSILON turbulence model.')
            if not enabled:
                cb.setChecked(False)
                for V in self.vtk_current_indices:
                    self.unset_keyword(key, args=[V])
        ...
        n_species = len(names)
        for key in 'vtk_x_g', 'vtk_y_g':
            tw = ui.tablewidget_fluid_mass_fraction if key == 'vtk_x_g' else ui.tablewidget_fluid_molar_fraction
            ...
            for i, name in enumerate(names):
                ...
                item.key = key
                item.tw = tw
                item.eventFilter = self.output_event_filter
    ...
    def output_event_filter(self, obj, event):
        if not self.vtk_current_indices:
            return False

        vtk = self.vtk_current_indices[0]
    ...
    def setup_output_reactions_subtab(self):
        ...
        for (i, info) in enumerate(rxn_info):
            name, des, idx = info
            cb = QCheckBox()
            key = 'vtk_des_rrate' if des else 'vtk_fluid_rrate'
            args = [V0, idx]
            cb.setChecked(bool(self.project.get_value(key, args=args)))
    ...
    def setup_output_solids_subtab(self, P):
        ...
        self.set_widget_enabled(cb, enabled, reason='Requires energy equations.')
        if not enabled:
            cb.setChecked(False)
            for V in self.vtk_current_indices:
        ...
        key = 'vtk_c_ps'
        cb = ui.checkbox_keyword_vtk_c_ps_args_VTK_P
        enabled = bool(energy_eq)
        self.set_widget_enabled(cb, enabled, reason="Requires energy equations.")
        if not enabled:
            cb.setChecked(False)
        ...
        enabled = (model=='TFM' and kt_type != 'ALGEBRAIC')
        self.set_widget_enabled(cb, enabled, reason="Requires TFM solids and non-algebraic viscous stress model.")
        if not enabled:
            cb.setChecked(False)
            for V in self.vtk_current_indices:
                self.unset_keyword(key, args=[V, P])
        ...
        for key in ('vtk_p_s', 'vtk_vel_s', 'vtk_u_s', 'vtk_v_s', 'vtk_w_s',
                    'vtk_ro_s', 'vtk_ep_s', 'vtk_rop_s', 'vtk_t_s', 'vtk_c_ps', 'vtk_theta_m'):
    ...
    def setup_output_other_subtab(self):
        ...
        if not enabled:
            ...
            val = self.project.get_value('vtk_p_star', default=False, args=[V0])
            cb.setChecked(val)
        ...
        #Enable writing scalar cell center coordinates
        # Sets keyword VTK_SCALAR_CC(#)
        # DEFULAT value .FALSE.
    ...
    def setup_output_vtk_particle(self, force=False):
        ...
        # Note 2, this groupbox is disabled completely if not DEM, CGP, SQP, GSP or PIC
        # Note 3, 'force' is set to True when particle pane is masquerading as
        #   force_chain
        ui = self.ui.output

        if not force:
            for w in (ui.combobox_vtk_select_mode,
                      ui.label_vtk_select_mode,
                      ui.groupbox_select_particle_data):
                w.show()
        ...
        # issues/1852
        for w in ui.label_vtk_gsp_output_type, ui.combobox_vtk_gsp_output_type:
            w.setVisible(self.project.solver == GSP)
        if self.project.solver == GSP:
            cb = ui.combobox_vtk_gsp_output_type
            val = self.project.get_value('vtk_gsp_output_type', default=1, args=[V0])
            if val not in (1,2):
                val = 1
                self.warning("Invalid VTK_GSP_OUTPUT_TYPE %s"%val)
                for v in self.vtk_current_indices:
                    self.update_keyword('vtk_gsp_output_type', 1, args=[v])
            cb.setCurrentIndex(val-1)
            set_combobox_tooltip(cb)

        energy_eq = self.project.get_value('energy_eq', default=True)

        # NB The following all require PIC or DEM/CGP/SQP/GSP solids and default to False
        enable_all = self.project.solver in (PIC,DEM,GSP,CGP,SQP,HYBRID)
        #
        ...
        #Enable writing particle mass
        # Sets keyword VTK_PART_MASS(#)

        # Enable writing particle volume
        # Sets keyword VTK_PART_VOLUME(#)

        # Enable writing particle drag coefficient
        # Sets keyword VTK_PART_DRAG_COEFF(#)

        #Enable writing particle cohesive force
        # Requires USE_COHESION=.TRUE.
        # Sets keyword VTK_PART_COHESION(#)

        #Enable writing particle collision force
        # Requires USE_COHESION=.TRUE.
        # Sets keyword VTK_PART_COL_FORCE(#)
        ...
        for key in ('vtk_part_angular_vel',
                    'vtk_part_c_ps',
                    'vtk_part_cgp_stat_wt',
                    'vtk_part_cohesion',
                    'vtk_part_col_force',
                    'vtk_part_coordination',
                    'vtk_part_density',
                    'vtk_part_drag_coeff',
                    'vtk_part_id',
                    'vtk_part_mass',
                    'vtk_part_gluemass',
                    'vtk_part_gluebounding',
                    'vtk_part_orientation',
                    'vtk_part_overlap_stats',
                    'vtk_part_phase_id',
        ...
        key = 'vtk_part_temp'
        enabled = bool(energy_eq) and enable_all
        self.set_widget_enabled(cb, enabled, reason='Requires energy equations.')
        if not enabled:
            cb.setChecked(False)
            for V in self.vtk_current_indices:
        ...
        # disable specific heat if energy_eq disabled
        cb = ui.checkbox_keyword_vtk_part_c_ps_args_VTK
        key = 'vtk_part_c_ps'
        enabled = bool(energy_eq) and enable_all
        self.set_widget_enabled(cb, enabled, reason='Requires energy equations.')
        if not enabled:
        ...
        self.set_widget_enabled(cb, enabled, reason='Requires <b>USE_COHESION</b>.')
        if not enabled:
            cb.setChecked(False)
            for V in self.vtk_current_indices:
                self.unset_keyword(key, args=[V])
        ...
        enabled = (self.project.solver == CGP) and enable_all
        self.set_widget_enabled(cb, enabled, reason="CGP model only.")
        if not enabled:
            cb.setChecked(False)
            for V in self.vtk_current_indices:
                self.unset_keyword(key, args=[V])
        ...
        # disable part_gluemass and part_gluebounding if not GSP
        enabled = (self.project.solver == GSP)
        for key in 'vtk_part_gluebounding', 'vtk_part_gluemass':
            cb = getattr(ui, 'checkbox_keyword_%s_args_VTK' % key)
            self.set_widget_enabled(cb, enabled, reason="GSP model only.")
            if not enabled:
                cb.setChecked(False)
                for V in self.vtk_current_indices:
                    self.unset_keyword(key, args=[V])
        ...
        enabled = (n_species > 0) and enable_all
        ...
        # Requires DEM/CGP/SQP/GSP solids
        ...
        for (i, cb) in enumerate(cbs, 1):
            ...
            enabled = enable_all
            self.set_widget_enabled(cb, enabled, reason="Not available for %s solids."%
                                    solver_name(self.project.solver))
            if enabled:
                cb.setChecked(bool(val))
            else:
                cb.setChecked(False)
    ...
    def setup_output_vtk_force(self):
        ui = self.ui.output
        ...
        key = 'vtk_fcmax_only'
        cb = ui.checkbox_keyword_vtk_fcmax_only_args_VTK
        if not self.vtk_current_indices:
            cb.setChecked(False)
            for w in widget_iter(ui.groupbox_particle):
                if isinstance(w, CheckBox):
                    w.setChecked(False)
            return
        ...
        val = self.project.get_value(key, args=[V0], default=False)
        cb.setChecked(val)
        # we are piggybacking on particle subpage
        self.setup_output_vtk_particle(force=True)
    ...
    def setup_output_vtk_geometry(self):
        ...
        for (prefix, items) in (('geometry', self.bcs.items),
                                ('is', self.iss.items)):
            for i, data in items():
                r = data.get('region')
                if regions.get(r,{}).get('type') == 'STL':
                    files.append(('%s_%04d.stl' % (prefix, i), r))
    ...
    def setup_output_log_tab(self):
        ...
        for which in 'solid_inventory', 'solver_output', 'dt', 'nit':
            ...
            gb = getattr(ui, 'groupbox_save_%s' % which)
            gb.setChecked(bool(enable))
            # These are not MFiX keywords so gui.enable_input locks them incorrectly
            self.set_widget_enabled(gb, self.lock_level != LOCKED,
                                    ignore_lock=True)
            for w in widget_iter(gb):
                if hasattr(w, 'toolTip'):
                  self.set_widget_enabled(w, enable and self.lock_level != LOCKED,
                                          ignore_lock=True)
            le = getattr(ui, 'lineedit_%s_filename' % which)
            le.setText(filename)
            if mode not in ('overwrite', 'append', 'increment'):
    ...
    def setup_output_usr_tab(self):
        ...
        enabled = (row is not None)
        self.set_widget_enabled(ui.toolbutton_usr_delete, enabled)
        self.set_widget_enabled(ui.toolbutton_usr_add, len(self.usr_outputs)<5)
        self.set_widget_enabled(ui.bottom_frame_usr, enabled, ignore_lock=True)
        ...
        self.set_widget_enabled(ui.toolbutton_usr_up, row > 0)
        self.set_widget_enabled(ui.toolbutton_usr_down, row < tw.rowCount()-1)
    ...
    def output_delete_usr_row(self):
        ...
        self.set_widget_enabled(ui.toolbutton_usr_delete, len(self.usr_outputs)>0) # XXX need selection?
        self.set_widget_enabled(ui.toolbutton_usr_add, len(self.usr_outputs)<5)
        self.update_usr_regions_table()
    ...
    def reset_output(self):
        ...
        ui.pushbutton_vtk.setToolTip("VTK outputs are not enabled")
