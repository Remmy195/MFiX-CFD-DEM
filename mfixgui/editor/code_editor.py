...
from qtpy.QtGui import (QFont, QFontMetrics, QTextFormat, QPainter, QTextOption,
    QKeySequence, QTextCursor, QTextDocument, QFontDatabase, QColor, QIcon)
...
class CodeEditorWidget(QWidget):
    def __init__(self, parent=None, mfixgui=None):
        ...
        self.file_name = None
        self.editor = CodeEditor(self, mfixgui)
        ...
        for key, callback in shortcuts:
            q = QShortcut(QKeySequence(key), self.editor)
    ...
    def save(self, fname=None):
        ...
        if ok:
            self.mfixgui.print_internal("Saved %s" % self.editor.file_name, color="blue")
        return ok
...
class CodeEditor(QPlainTextEdit):
    ...
    def __init__(self, parent=None, mfixgui=None):
        ...
        self.code_editor_widget = parent
        self.mfixgui = mfixgui
        self.unsaved_flag = False
        self.loading = False
        self.file_name = None
        self.watch_file_on_disk = True
        self.tab_char = '    ' # Replace tabs with spaces
        self.language = None
    ...
    def extend_context_menu(self):
        ...
        # How to set icon size for QAction?
        help_action = QAction(
            get_icon('search.svg'), '&Help (look up keyword)', menu)
        ...
        goto_action.triggered.connect(self.code_editor_widget.goto)
        insert_append_action(menu, goto_action, first_default_action)
        find_action = QAction(QIcon(""), "&Find/replace", menu)
        find_action.triggered.connect(self.code_editor_widget.find)
        insert_append_action(menu, find_action, first_default_action)
        save_action = QAction(QIcon(""), "&Save", menu)
        save_action.setEnabled(self.unsaved_flag)
        save_action.triggered.connect(lambda checked: self.code_editor_widget.save(None))
        insert_append_action(menu, save_action, first_default_action)
    ...
    def keyPressEvent(self, event):
        ...
        if completer_visible: #FIXME Make completer work on return/enter
            if key in [Qt.Key_Return, Qt.Key_Enter, Qt.Key_Escape, Qt.Key_Tab, Qt.Key_Backtab]:
                event.ignore()
                return
    ...
    def set_syntax_highlighter(self, language=None):
        # get the os default system font
        font = self.default_font = QFontDatabase.systemFont(QFontDatabase.FixedFont)
        self.language = language
        font.setFixedPitch(True)
        use_sys = SETTINGS.value('use_system_font',0) or 0
        if not int(use_sys):
        ...
        #font = self.default_font = text_format.font()
    ...
    def line_number_area_width(self):
        ...
        dpi = min(self.logicalDpiX(),
                  self.logicalDpiY())
        r = 5 + self.font_metrics.horizontalAdvance('9'*digits)
        if dpi > 96:
    ...
    def goto(self, line, column=0, error=False):
        if error:
        ...
        # FIXME does not highlight line when readonly
    ...
    def open(self, fname, read_only=False):
        ...
        try:
            ...
            self.large_file = size*1e-6>2  # over 2MB, don't highlight
        except OSError:
            pass
        ...
        try:
            ...
            self.error("Error loading file: %s" % e)
            text = ""
    ...
    def save(self, fname=None):
        ...
        # update file watcher
        watch_paths = self.file_watcher.files() + self.file_watcher.directories()
        if watch_paths:
