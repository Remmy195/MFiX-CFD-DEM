...
class FileWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        mfix_source = self.mfix_tree = FileTree(self, root_path=os.path.join(get_mfix_src(), 'model'), prj_dir=False)
    ...
    def close_file(self, fname, check_unsaved=True):

        index = self.ide_widget.is_open(fname)
        if index >= 0:
            return self.ide_widget.remove_tab(index, check_unsaved)
        return True
...
class FileTree(QTreeView):
    ...
    def show_context_menu(self, point):
        ...
        for label, action in self.menu_actions.items():
            action.setEnabled((is_file and not project_file) or label in ("Browse", "New file"))
        # Allow duplicating project file
        self.menu_actions["Duplicate"].setEnabled(is_file)
    ...
    def delete(self):
        ...
        reply = QMessageBox.question(self, "Delete?", "Are you sure you want to delete:\n{}".format(path),
                                QMessageBox.Yes|QMessageBox.No)
        if reply == QMessageBox.Yes:
            self.file_widget.close_file(path, check_unsaved=False)
            os.remove(path)
