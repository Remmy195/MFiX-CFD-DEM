...
class DistributionWizard(QDialog):
    def __init__(self, parent):
        ...
        ui.combobox_shape.currentIndexChanged.connect(self.shape_changed)
        ui.pushbutton_close.setFocus(False)

        ui.lineedit_number.dtype = float
        ui.lineedit_number.default_value = 1

    def dist_changed(self):
        enable = str(self.ui.combobox_distribution.currentText()).lower() == 'random'
        if enable:
    ...
    def shape_changed(self):
        ...
        enable = self.is_implicit(shape)
        self.ui.checkbox_union.setEnabled(enable)
        if not enable:
            self.ui.checkbox_union.setChecked(False)

    def is_implicit(self, shape):
        geo = self.vtk_widget.geometrydict.get(shape)
        if geo is not None:
            return geo.get('geo_type', 'none') == 'implicit'
        return False
    ...
    def apply_(self):
        ...
        shape = self.ui.combobox_shape.currentText()
        packing = str(self.ui.combobox_distribution.currentText()).lower()
        bounds = self.get_bounds(geo)
        union = self.ui.checkbox_union.isChecked()
        ...
        #     spacing = 1
        if packing == 'random':
            children = self.gen_random(geo, shape, bounds)
        elif packing == 'cubic':
            children = self.gen_cubic(geo, shape, spacing, bounds)
        elif packing == 'body centered cubic':
            children = self.gen_bcc(geo, shape, spacing, bounds)
        else:
            raise ValueError("Unknown packing type '%s'" % packing)

        if union:
            # implicit objects can all be unioned at the same time
            if self.is_implicit(shape):
                self.vtk_widget.boolean_operation(
                    booltype='union', children=children)
            else:
                last = children[0]
                for child in children[1:]:
                    last = self.vtk_widget.boolean_operation(
                        booltype='union', children=[last, child])
...
#        self.close()
    ...
    def gen_random(self, geo, shape, bounds):
        ...
        last = None
        children = []
        while n_points < int(self.ui.lineedit_number.value) and max_itr < 1000000000:
    ...
    def gen_cubic(self, geo, shape, spacing, bounds):
        ...
        return self.distribute(geo, shape, points)
