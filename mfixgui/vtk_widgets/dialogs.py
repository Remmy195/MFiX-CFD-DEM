...
FFMPEG_TIME_RX = re.compile(rb'time=(\d{2}):(\d{2}):(\d{2})\.\d{2}')
FFMPEG_DURATION_RX = re.compile(rb'Duration: (\d{2}):(\d{2}):(\d{2})\.\d{2}')
...
class CreateMoviePopUp(QtWidgets.QDialog):
    ...
    def create_movie(self):
        ...
        end = ui.doubleSpinBox_end.value()
    ...
    def process_image_stack(self, path, fps, start, end):
        ...
        padcount = len(re.findall(r'(\d+)', first_image)[-1])
    ...
    def take_snapshot(self):
        ...
        if self.parent.time >= end:
            self.ffmpeg_process.stdin.close()
            self.movie_finished()
        else:
            ui.progressBar.setValue(int(((self.parent.time-start)/(end-start))*100))
            self.parent.handle_next()
    ...
    def popup(self):
        ...
        ui.doubleSpinBox_start.setRange(ts[0], ts[-1])
        ui.doubleSpinBox_start.setValue(ts[0])
        ui.doubleSpinBox_end.setRange(ts[0], ts[-1])
        ui.doubleSpinBox_end.setValue(ts[-1])
