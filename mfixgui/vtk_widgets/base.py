...
class BaseVtkWidget(QtWidgets.QWidget):
    def __init__(self, parent=None):
        ...
        self.deleting = False # XXX issues/1825
        self.defer_render = False
        self.view_flip = [False]*3
        self.offscreen_vtkrenderer = None
        self.color_maps = None  # dict defined in the results_viewer
        ...
        c1 = QtGui.QColor(SETTINGS.value('vtk_bck_color1', '#aaffff'))
    ...
    def _mouseReleaseEvent(self, ev):
        ...
        if qrw.moving:
            ...
            if self.color_maps is None or not hasattr(self, 'change_color_pressed'):
                return

            # convert to % widget
            x0 = ev.x()/self.vtkWindowWidget.width()
            y0 = ev.y()/self.vtkWindowWidget.height()

            # did we click a color bar?
            for type_, vars in self.color_maps.items():
                for var, cbar_data in vars.items():
                    cb = cbar_data.get('actor', None)
                    if cb is None:
                        continue
                    # check if it is visible
                    if not cb.GetVisibility():
                        continue
                    # get the relative coordinates (% window)
                    pos1 = cb.GetPositionCoordinate().GetValue() # lower left
                    pos2 = [pos1[0]+cb.GetWidth(), pos1[1]+cb.GetHeight()]

                    if pos1[0] < x0 < pos2[0] and pos1[1] < 1-y0 < pos2[1]:
                        # change color bar
                        tw =  self.sidebar.treeWidget_colorbars
                        items = tw.findItems(var, QtCore.Qt.MatchExactly, 0)
                        tw.setCurrentItem(items[0])
                        # pop up the dialog
                        self.change_color_pressed()
    ...
    def set_view(self, view='xy'):
        ...
        camera = self.vtkrenderer.GetActiveCamera()
        foc = np.asarray(camera.GetFocalPoint())
        pos = np.asarray(camera.GetPosition())
        r = np.sqrt(np.sum((foc-pos)**2))

        idx = ['yz', 'xz', 'xy'].index(view)
        if self.view_flip[idx]:
            r *= -1
        foc[idx] = r
        self.view_flip[idx] = not self.view_flip[idx]
