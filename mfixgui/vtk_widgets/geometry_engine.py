...
from mfixgui.tools import topological_sort, get_unique_string, safe_int, safe_float
...
def dep_tree_iter(d):
    ...
    for key, val in d.items():
        if isinstance(val, dict):
            for (key2, dep) in dep_tree_iter(val):
                yield key2, dep
...
def is_file_in_directory(file_path, dir_path):
    ...
    return os.path.commonpath([abs_file_path, abs_dir_path]) == abs_dir_path
...
class GeometryEngine:
    ...
    def add_to_tree(self, name, icon=None, children=(), loading=False):
    ...
    def geometry_to_str(self):
        ...
        dep_tree = {}
        for key, dep in dep_tree_iter(self.geometrytree):
            dep_tree[key] = dep
            ...
            for node in nodes:
                if node in geo_dict and 'geo_type' in geo_dict[node]:
                    ...
                    if name is None and self.gui:
                        self.gui.error('Error loading geometry: {}'.format(node),
                                       popup=True)
                        return
                    ...
                    if self.gui:
                        self.gui.error('Error loading geometry: The geometry "{}" does not have parameters.'.format(node),
                                       popup=True)
                    return
    ...
    def update_parameter_map(self, new_value, name, key, check_old=True):
        ...
        if isinstance(new_value, Equation):
            ...
            for k, v in dict_iter(new_value):
                if isinstance(v, Equation):
                    new_params.extend(v.get_used_parameters())
    ...
    def add_stl(self, filename=None, name=None, data=None, loading=False,
        ...
        save_fname = filename
        if proj_dir is not None and is_file_in_directory(filename, proj_dir):
            save_fname = os.path.relpath(filename)
        ...
        # Add to tree
        self.add_to_tree(name, 'geometry.svg', loading=loading)
    ...
    def update_primitive(self, name):
        ...
        if primtype == 'sphere':
            ...
            source.SetHeight(safe_float(geo['height']))
            source.SetResolution(safe_int(geo['resolution']))
        elif primtype == 'superquadric':
            source.SetSize(safe_float(geo['size']))
            source.SetScale(
                safe_float(geo['scalex']),
            ...
            print(f'Warning: {primtype} not updated')
    ...
    def add_procedural(self, proceduraltype=None, name=None, data=None, loading=False):
        ...
        # Temp Source to keep pipeline happy
    ...
    def update_procedural(self, name):
        ...
        if proceduraltype == 'cylinder':
            poly_data = procedural_stl.cylinder(
                float(geo.get('radius', 0.5)),
                float(geo.get('height', 1.0)),
                theta_res=int(geo.get('circumferenceresolution', 32)),
                ...
                theta_start=float(geo.get('circumferencestartangle', 0)),
                theta_end=float(geo.get('circumferenceendangle', 360)),
            )
        elif proceduraltype == 'bend':
            poly_data = procedural_stl.bend(
                rb1=float(geo.get('bendmajorradius', 0.5)),
                rb2=float(geo.get('bendminorradius', 0.5)),
                theta1=float(geo.get('bendstartangle', 0.0)),
                theta2=float(geo.get('bendendangle', 360.0)),
                bend_res=int(geo.get('bendresolution', 32)),
                r1=float(geo.get('frontradius', 0.5)),
                l1=float(geo.get('frontlength', 1.0)),
                n1=int(geo.get('frontresolution', 1)),
                r2=float(geo.get('backradius', 0.5)),
                l2=float(geo.get('backlength', 1.0)),
                n2=int(geo.get('backresolution', 1)),
                theta_start=int(geo.get('circumferencestartangle', 0)),
                theta_end=int(geo.get('circumferenceendangle', 260)),
                theta_res=int(geo.get('circumferenceresolution', 32)),
                offset=float(geo.get('offset', 0.0)),
        ...
        elif proceduraltype == 'circle_to_rectangle':
            poly_data = procedural_stl.circle_to_rectangle(
                float(geo.get('radius', 0.5)),
                float(geo.get('width', 1.0)),
                int(geo.get('widthresolution', 20)),
                float(geo.get('height', 1.0)),
                int(geo.get('heightresolution', 20)),
                float(geo.get('length', 1.0)),
                int(geo.get('lengthresolution', 20)),
                x_offset=float(geo.get('offsetx', 0.0)),
                y_offset=float(geo.get('offsety', 0.0)),
                n_front_cap=int(0 if not geo.get('bottomcap', False) else geo.get('bottomresolution', 20)),
                back_cap=int(geo.get('topcap', False)),
                )
        elif proceduraltype == 'body_of_revolution':
            # build segment list from dict
            # ['line', [x1, y1, 0], [x2, y2, 0], n]
            # ['arc', [x1, y1, 0], r, theta1, theta2, n]
            # 'segment':shape, 'x1':0.0, 'y1':0.0, 'x2':1.0, 'y2':1.0, 'r':'-', 'theta1':'-', 'theta2':'-', 'div':20,
            segs = []
            for key, val in geo.get('segmentdict', {}).items():
                if val.get('segment', 'line') == 'line':
                    seg = [
                        'line',
                        [safe_float(val.get('x1', 0)), safe_float(val.get('y1', 0)), 0],
                        [safe_float(val.get('x2', 0)), safe_float(val.get('y2', 0)), 0],
                        safe_int(val.get('div', 20), 20),
                    ]
                else:
                    seg = [
                        'arc',
                        [safe_float(val.get('x1', 0)), safe_float(val.get('y1', 0)), 0],
                        safe_float(val.get('r', 1), 1),
                        safe_float(val.get('theta1', 0), 1),
                        safe_float(val.get('theta2', 30), 30),
                        safe_int(val.get('div', 20), 20),
                    ]
                segs.append(seg)
            if len(segs) > 0:
                poly_data = procedural_stl.revolve_segments(
                    segs,
                    theta_start=safe_float(geo.get('startangle', 0), 0),
                    theta_end=safe_float(geo.get('endangle', 360), 360),
                    theta_res=safe_int(geo.get('circumferenceresolution', 32), 32),
                    offset=safe_float(geo.get('offsety', 0), 0)
                )
    ...
    def add_implicit(self, implicittype=None, name=None, data=None, loading=False):
        ...
        # Add to tree
        self.add_to_tree(name, 'function.svg', loading=loading)
    ...
    def update_parametric(self, name):
        """Update the specified parametric object."""
        geo = self.geometrydict.get(name)
        paratype = geo['type']
    ...
    def add_filter(self, filtertype=None, name=None, data=None, child=None,
        ...
        # Add to tree
        self.add_to_tree(name, 'filter.svg', [child], loading=loading)
    ...
    def get_input_data(self, name):
        ...
        if geo is None:
            ...
            inputdata = geo['surface']
        else:
            raise ValueError('Unknown Geometry: {}'.format(geo))
        if hasattr(inputdata, "Update"):
            inputdata.Update()
        return inputdata
    ...
    def boolean_operation(self, booltype=None, boolname=None, data=None,
        ...
        # Add to tree
        self.add_to_tree(boolname, icon, children, loading=loading)

        return boolname

    def get_toplevel_geometry_names(self, visible_only=True):
    ...
    def collect_toplevel_geometry(self, visible_only=False):
        ...
        for top in self.get_toplevel_geometry_names(visible_only=visible_only):
    ...
    def select_facets(self, region):
        ...
        if geo_pool:
            ...
            for name in geo_pool:
                ...
                if geo is not None and hasattr(geo, 'GetOutput'):
        ...
        if region.get('filter_facets', False):
            ...
            if normals is not None:
                ...
                eql = region.get('equilibrant', False)
    ...
    def export_stl(self, file_name, bcs={}, iss={}):
        ...
        use_stl = False
        if geometry:
            # FIXME This is not quite right, if there are no CG_* BC's we get a
            # warning message from the solver
