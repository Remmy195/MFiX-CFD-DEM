...
from mfixgui.constants import CONVERSION_TO_METERS, SINGLE, TFM, DEM, CGP, SQP, GSP, PIC, HYBRID
from mfixgui.tools.qt import (widget_iter, get_icon, sub_icon_size,
                              get_image_path, deepcopy_dict, get_ui,
                              get_separator)
from mfixgui.tools.gsp_generator import q_to_m
from mfixgui.tools import get_unique_string, safe_int, safe_float
from mfixgui.widgets.base import LineEdit, ComboBox, Table
...
from mfixgui.vtk_widgets.tools import (clean_visual_dict, is_stl_ascii,
                                       purge_multi_solids, remove_vtk_objects)
...
class VtkWidget(BaseVtkWidget, GeometryEngine):
    ...
    def __init__(self, parent=None):
        ...
        self.particles_df_filtered = None
        self.particles_mtime = 0.0
        self.particles_fname = None
        self.particles_npoints = 0
        self.particles_dict = {
            'actors': []
        }

        self.cell_spacing_widgets = [
            self.ui.mesh.label_mesh_cell_size_x,
            self.ui.mesh.label_mesh_cell_size_y,
            self.ui.mesh.label_mesh_cell_size_z
        ]

        # enable parameters
        for widget in widget_iter(self.ui.geometry.groupBoxGeometryParameters):
            if isinstance(widget, LineEdit):
                # This seems to be the Geometry pane in the main GUI, not the modeler sidebar
    ...
    def __setup_body_of_rev(self):
        ui = self.ui.geometry

        # table
        cols = ['segment', 'x1', 'y1', 'x2', 'y2', 'r', 'theta1', 'theta2', 'div']
        col_del = {
            1: {'widget': 'lineedit',
                'required': True,
                'dtype':  'dp'},
        ...
        tw = ui.tablewidget_body_of_revolution_segmentdict
        tw.dtype = dict
        tw._setModel()
        ...
        tw.default_value = {}
        tw.set_delegate(col=col_del, row=None)
        tw.auto_update_rows(True)
        tw.new_selection.connect(self.segment_selected)
        tw.value_changed.connect(self.segment_changed)

        menu = QMenu('segments', self)
        action = QAction('line', menu)
        action.triggered.connect(lambda:self.add_body_of_rev(shape='line'))
        menu.addAction(action)
        action = QAction('arc', menu)
        action.triggered.connect(lambda:self.add_body_of_rev(shape='arc'))
        menu.addAction(action)

        ui.toolbutton_add_segment.setMenu(menu)
        ui.toolbutton_remove_segment.clicked.connect(self.remove_segment)
        ui.toolbutton_copy_segment.clicked.connect(self.copy_segment)
        ui.toolbutton_segment_down.clicked.connect(self.move_segment_down)
        ui.toolbutton_segment_up.clicked.connect(self.move_segment_up)
    ...
    def segment_changed(self):
        ...
        self.update_geometry_value(name, value, 'segmentdict')

    def segment_selected(self):
        ui = self.ui.geometry
        rows = ui.tablewidget_body_of_revolution_segmentdict.current_rows()
        enable = bool(rows)
        ui.toolbutton_remove_segment.setEnabled(enable)
        ui.toolbutton_copy_segment.setEnabled(enable)
        ui.toolbutton_segment_down.setEnabled(enable)
        ui.toolbutton_segment_up.setEnabled(enable)

    def fixup_table(self, tw, stretch_column=8):
        hv = QHeaderView
    ...
    def add_body_of_rev(self, shape='line'):
        ui = self.ui.geometry
        tw = ui.tablewidget_body_of_revolution_segmentdict
        data = tw.value
        keys = list(data.keys())
        name = get_unique_string(shape, keys)

        d = {
            'segment':shape, 'x1':0.0, 'y1':0.0, 'x2':1.0, 'y2':1.0, 'r':'-', 'theta1':'-', 'theta2':'-', 'div':20,
        }
        if shape == 'arc':
            d = {
                'segment':shape, 'x1':0.0, 'y1':0.0, 'x2':'-', 'y2':'-', 'r':0.5, 'theta1':0.0, 'theta2':30.0, 'div':20,
            }
        elif len(keys) > 0:
            prev = data.get(keys[-1])
            if prev.get('segment', 'line') == 'line':
                d['x1'] = prev.get('x2', 0.0)
                d['y1'] = prev.get('y2', 0.0)

        data[name] = d
        ui.tablewidget_body_of_revolution_segmentdict.set_value(data)
        tw.selectRow(len(data)-1)
    ...
    def copy_segment(self):
        ...
        keys = list(data.keys())
        key =  keys[rows[0]]
        new_key = get_unique_string(key, keys)
        data[new_key] = copy.deepcopy(data[key])
        ui.tablewidget_body_of_revolution_segmentdict.set_value(data)
        tw.selectRow(len(data)-1)
        self.segment_changed()
    ...
    def __add_sidebar(self):
        ...
        sb.toolButton_particles_vis.setIcon(get_icon('dem.svg'))
        ...
        for tb, opt in [
                ...
                (sb.toolButton_particles_toggle, sb.widget_particles_opts),
                ]:
            tb.clicked.connect(
        ...
        for ct, geo in zip([sb.toolButton_geometry_color,
            ...
            vbtns[geo]['color'] = ct
        ct = sb.toolButton_particles_color
        geo = 'particles'
        ct.clicked.connect(lambda ignore, g=geo, t=ct: self.change_color(g, t))
        vbtns[geo]['color'] = ct

        for sp, geo in zip([sb.doubleSpinBox_geometry_opacity,
                            ...
                            sb.doubleSpinBox_particles_opacity,
                            ], geo_list):
            sp.valueChanged.connect(lambda _, o=sp, g=geo: self.change_opacity(o, g))
            vbtns[geo]['opacity'] = sp
        le = sb.lineedit_particles_count
        le.dtype = int
        le.allow_parameters = False
        le.min = 0
        le.value_updated.connect(self.update_particle_preview)
        # Normal widgets
        ...
        # Mesh button
        # TODO auto-load msh on file watcher event
        ...
        # Region line width
        lw = sb.doubleSpinBox_regions_linewidth
        lw.valueChanged.connect(self.change_region_linewidth)
        vbtns['regions']['linewidth'] = lw

        # Allow mousewheel scrolling, don't disrupt combobox settings
        for w in widget_iter(sb):
            if isinstance(w, QComboBox):
                w.wheelEvent = lambda ev: None
            # This does not do what I expected,  I want to force
            # clicking in the spinbox before accepting wheel events - cgw
    ...
    def updateValue(self, key, newValue, args=None):
        ...
        if key == 'no_k':
            ...
            for tab in GUI.graphics_tabs:
                if tab.vtk:
                    tab.vtk_widget.change_interaction(newValue)
    ...
    def geometry_from_str(self, string):
        ...
        # check for mesh file
        m_name = self.get_mesh_name()
        if m_name is not None and os.path.exists(m_name):
            self.read_mesh_file(loading=True)
    ...
    def file_watcher_file_changed(self, path):
        if not self.watching_file:
            ...
            self.timer.singleShot(1000, lambda:self.read_mesh_file()) # mesh file, piggyback on boundary file change
    ...
    def get_mesh_name(self):
        ...
        prj_dir = GUI.get_project_dir()
        run_name = GUI.project.get_value('run_name', None)
        vtu_dir = GUI.project.get_value('vtu_dir', 'vtu_dir')

        if run_name is None:
            return None

        # search for the mesh file in the vtu_dir
        mesh_name = run_name.upper()+"_MESH.vtu"
        fname = None
        for path in [
                os.path.join(prj_dir, vtu_dir, mesh_name),
                os.path.join(prj_dir, vtu_dir.upper(), mesh_name),
                os.path.join(prj_dir, mesh_name)]:
    ...
    def visual_props_from_str(self, string):
        ...
        for geo, geo_dict in data.items():
            if geo=='geometry' and 'visible' not in geo_dict:
                geo_dict['visible'] = True
            for key, value in geo_dict.items():
                if key in ['color', 'edge']:
                    if isinstance(value, (list, tuple)):
                        data[geo][key] = QColor(*value)
                    else:
                        data[geo][key] = QColor(value)
    ...
    def change_object_name(self):
        ...
        # make sure the name actually changed, an item is selected, and the name is a valid geometry item
        if name == new_name or tree_item is None or name not in self.geometrydict:
            return
    ...
    def change_item_visibility(self, name=None, item=None, visible=True):
        ...
        self.ui.regions.update_toplevelgeo(self.get_toplevel_geometry_names(visible_only=False))
    ...
    def add_to_tree(self, name, icon='geometry.svg', children=(), loading=False):
        '''
        ...
        GeometryEngine.add_to_tree(self, name=name, children=children, loading=loading)
    ...
    def update_transform(self, name):
        ...
        if geo_type == 'stl':
            ...
            for key, bound, widget in zip(
                    ...
                    bounds,
                    [ui.label_stl_extentxmin, ui.label_stl_extentxmax,
                     ui.label_stl_extentymin, ui.label_stl_extentymax,
                     ui.label_stl_extentzmin, ui.label_stl_extentzmax]):
                geo[key] = bound
                # manually update stl bounds widgets
                widget.setText(str(bound))
    ...
    def add_stl(self, filename=None, name=None, data=None, loading=False,
        ...
        if filename is None:
            ...
            if proj_dir not in os.path.realpath(filename):
                btn = GUI.message(
                    text='The STL file is not in the current project directory. It is recommended to copy the STL file to the project directory.\n\n'
    ...
    def add_filter(self, filtertype=None, name=None, data=None, child=None,
        ...
        # update region top level geometry selection
        self.update_region_toplvl([name], [child])
    ...
    def boolean_operation(self, booltype=None, boolname=None, data=None,
        ...
        # update region top level geometry selection
        self.update_region_toplvl([boolname], children)
    ...
    def clear_all_geometry(self):
        ...
        self.geometrydict = {}
        self.geometrytree = {}
        ##self.gui.set_unsaved_flag() # gets triggered by reset at startup
    ...
    def remove_geometry(self):
        ...
        for selection in currentSelection:
            ...
            for child in children:
                ...
                child_name_list.append(str(child.text(0)).lower())
            ...
            # update top level region selection
            self.update_region_toplvl(child_name_list, [text])
    ...
    def new_region(self, name, region):
        ...
        actor = vtk.vtkActor()
        actor.SetMapper(mapper)
        self.set_region_actor_props(actor, name, region['color'])
...
#        self.balloon_widget.AddBalloon(actor, name, None)
    ...
    def update_region_toplvl(self, new, prev):
        '''
        Top level geometry changed, look for region references.
        '''
        for name, data in self.region_dict.items():
            gp = data.get('geometry_pool', [])
            mod = False
            for p in prev:
                if p in gp:
                    gp.remove(p)
            ...
            if mod:
                gp = list(set(gp))
                data['geometry_pool'] = gp
                GUI.ui.regions.update_region_geopool(name, gp)
                self.select_facets(name)
    ...
    def change_region_color(self, name, color):
        ...
        actor_props.SetColor(*reg['color'].color_float)
        actor_props.SetEdgeColor(*reg['color'].color_float)
    ...
    def set_region_actor_props(self, actor, name, color=None):
        ...
        self.set_representation(actor, props.get('rep', 'solid'))
        actor_props = actor.GetProperty()
        actor_props.SetOpacity(props.get('opacity', 0.5))
        actor_props.SetLineWidth(props.get('linewidth', 1.0))

        if color:
            c = color.color
            actor_props.SetColor(c.getRgbF()[:3])
            actor_props.SetEdgeColor(c.getRgbF()[:3])
        else:
            c = props['color']
            actor_props.SetColor(c.getRgbF()[:3])
            actor_props.SetEdgeColor(c.getRgbF()[:3])
    ...
    def change_region_linewidth(self, value):
        '''
        Update the line thickness for all the region actors.
        '''
        props = self.visual_props['regions']
        props['linewidth'] = value

        for actor in self.get_actors('regions'):
            actor_props = actor.GetProperty()
            actor_props.SetLineWidth(value)
        self.render()
    ...
    def update_particle_preview(self):
        ...
        def remove_actors():
            for actor in self.particles_dict['actors']:
                self.vtkrenderer.RemoveActor(actor)
            self.particles_dict['actors'] = []
            self.particles_npoints = 0

        sb = self.sidebar
        if GUI and GUI.project.get_value('gener_part_config', default=False):
            # Disable button and widget
            sb.toolButton_particles_vis.setDown(False)
            remove_actors()
            self.render()
            return

        fname = "particle_input.csv"
        df = None
        header = None
        ...
        sb = os.stat(fname)
        if (self.particles_fname != os.path.abspath(fname)
            or self.particles_mtime != sb.st_mtime
            or self.particles_df is None):
            try:
                with warnings.catch_warnings(record=True) as ws:
                    ...
                    if len(df) == 0:
                        ...
                        return
                for w in ws:
                    GUI.warning(str(w.message), popup=True)
            except Exception as e:
                remove_actors()
                GUI.error("Error loading %s: %s" % (fname, e), popup=True)
                self.render()
                return

            self.particles_fname = os.path.abspath(fname)
            self.particles_mtime = sb.st_mtime
            self.particles_df = df

            df = self.particles_df
            # lowercase column names for case-free comparison
        ...
        df = self.particles_df_filtered

        solver = GUI.project.solver
        # TODO factor out repeated code
        num_points = len(df)
        p = GUI.project.get_value('particles')
        if p:
            num_points = min(num_points, p)
        le = self.sidebar.lineedit_particles_count
        if le.text():
            p = le.value
            num_points = min(num_points, p)
        if num_points == 0:
        ...
        if num_points == self.particles_npoints:
            # What do we need to do here?
            return

        # Potential optimization:  if nparticles is reduced,
        #  reduce actor array size instead of deleting all actors
        #  and starting over.  Need to handle gsp case correctly ...

        self.particles_npoints = num_points
        remove_actors()

        df = df.iloc[:num_points, :]
        have_dia = 'diameter' in df.columns

        if solver in (DEM, CGP):
            # add new actors and mappers
            mapper = vtk.vtkOpenGLGlyph3DMapper()
            mapper.ScalarVisibilityOff()
            ...
            points_pd = df.loc[:, ['x','y','z']]
            if have_dia:
                radius_pd = df.loc[:, 'diameter']/2
            else:
                # Revise values to d_p0(M) based on 'phase_id' column
                radius_pd = df.loc[:, 'phase_id'].apply(lambda x: GUI.project.get_value('d_p0', args=[x]) / 2 )

            radius_vtk = np_to_vtk(radius_pd)
            radius_vtk.SetName("radius")
            polydata = vtk.vtkPolyData()
            points_vtk = vtk.vtkPoints()
            points_vtk.SetData(np_to_vtk(points_pd))
            ...
            actor = vtk.vtkActor()
            actor.SetMapper(mapper)
            self.set_particle_actor_props(actor)
            self.vtkrenderer.AddActor(actor)
            self.particles_dict['actors'].append(actor)

        elif solver == GSP:
            phases = sorted(list(set(df.loc[:,'phase_id'])))
            gsp_config = {}
            # Collect GSP configuration data for all phases
            for P in phases:
                # TODO automatic update if gsp_config.dat changes
                gsp_config_file = 'GSP-%s/gsp_config.csv' % P
                if not os.path.exists(gsp_config_file):
                    continue
                if os.stat(gsp_config_file).st_size == 0:
                    GUI.warning("Empty GSP configuration file %s" % gsp_config_file,
                                popup=True)
                    continue
                with open(gsp_config_file) as f:
                    lines = [l.strip() for l in f.readlines()]
                # if lines[0].split(',')[3].strip() != 'rel_d':
                #     GUI.warning("Unknown GSP configuration version: %s %s" %
                #                 (lines[0], gsp_config_file),
                #                 popup=True)
                #     continue
                # We expect the files in GSP-N/gsp_config.dat to only describe a single phase (N)
                #  so we ignore the "Phase_ID" and "spheres_per_particle" lines
                #gsp_df = pd.DataFrame(columns=[s.strip().lower().replace(' ','')
                #                               for s in lines[18].split('|')],
                #                      data=[l.split() for l in lines[20:]])
                # I don't trust these files to be well-formatted
                # GSP only works for 3 dimensional problems
                ...
                if lines[0].split(',')[2] == 'z':
                    ...
                    dimension = 2
                gsp_config[P] = (dimension,
                                 np.array([list(map(float,l.split(','))) for l in lines[1:]]))
            # Compute total number of component spheres
            num_points = sum(len(df['phase_id']==P) *
                             (len(gsp_config[P][1]) if P in gsp_config else 1)
                             for P in phases)

            # Preallocate so we're not growing arrays
            radius_np = np.zeros(num_points)
            points_np = np.zeros((num_points, 3))
            have_q = 'gsp_q1' in df.columns
            idx = 0
            for row in df.itertuples(index=True):
                P = row.phase_id
                gsp = gsp_config.get(P)
                if not gsp:
                    radius_np[idx] = (row.diameter if have_dia else GUI.project.get_value('d_p0', args=[P])) / 2
                    points_np[idx] = (row.x, row.y, row.z)
                    idx += 1
                else:
                    dim, gsp = gsp
                    center = np.array((row.x, row.y, row.z))
                    for sub in gsp:
                        d = (row.diameter if have_dia else GUI.project.get_value('d_p0', args=[P]))
                        r = d/2
                        rel_pos = (sub[:3] if dim==3
                                   else np.array((sub[0], sub[1], 0)))
                        if have_q:
                            q = (row.gsp_q1,row.gsp_q2,row.gsp_q3,row.gsp_q4)
                            if q != (1,0,0,0):
                                rel_pos = q_to_m((row.gsp_q1,row.gsp_q2,row.gsp_q3,row.gsp_q4)) @ rel_pos
                        # NB "relative position" scales to particle diameter not radius
                        points_np[idx] = center + d*rel_pos
                        # Relative size is column 3 if dim==3 else col 2
                        radius_np[idx] = r * sub[dim]
                        idx += 1
            mapper = vtk.vtkOpenGLGlyph3DMapper()
            mapper.ScalarVisibilityOff()
            source = vtk.vtkSphereSource()
            ...
            source.SetPhiResolution(50)
            source.SetCenter(0,0,0)
            radius_vtk = np_to_vtk(radius_np)
            radius_vtk.SetName("radius")
            polydata = vtk.vtkPolyData()
            points_vtk = vtk.vtkPoints()
            points_vtk.SetData(np_to_vtk(points_np))
            polydata.SetPoints(points_vtk)
            polydata.GetPointData().AddArray(radius_vtk)
            mapper.SetSourceConnection(source.GetOutputPort())
            mapper.SetInputData(polydata)
            ...
            # This is a inefficient.  We should sort the SQP
            #  particles into groups with the same a,b,c,m,n
            #  parameters instead of making a separate actor
            #  and mapper for each one.
            have_q = all('sqp_q'+s in df.columns for s in '1234')
            have_abc = all('sqp_'+s in df.columns for s in 'ab')
            have_mn  = all('sqp_'+s in df.columns for s in 'mn')
            phases = sorted(list(set(df.loc[:,'phase_id'])))
            if GUI.project:
                sqp_data = {P: [GUI.project.get_value('sqp_%s'%s, args=[P]) for s in 'abcmn']
                            for P in phases}
            else:
                sqp_data = {}
            ...
            for row in df.itertuples(index=True):
                ...
                source.SetAxisOfSymmetry(2)
                source.SetSize(1)
                source.SetThetaResolution(50)
                source.SetPhiResolution(50)
                if have_abc:
                    source.SetScale(row.sqp_a, row.sqp_b, row.sqp_c)
                elif P in sqp_data:
                    source.SetScale(*sqp_data[P][:3])
                else:
                    GUI.warning("No SQP shape parameters for phase %s." %P, popup=True)
                    break # Don't warn on every row
                if have_mn:
                    source.SetThetaRoundness(2/row.sqp_m)
                    source.SetPhiRoundness(2/row.sqp_n)
                elif P in sqp_data:
                    source.SetThetaRoundness(2/sqp_data[P][3])
                    source.SetPhiRoundness(2/sqp_data[P][4])
                else:
                    GUI.warning("No SQP shape parameters for phase %s." %P, popup=True)
                    break # Don't warn on every row
                ...
                mapper.SetSourceConnection(source.GetOutputPort())
                if have_q:
                    q = np.array([[row.sqp_q1, row.sqp_q2,
                                 row.sqp_q3, row.sqp_q4]])
                    q_vtk = np_to_vtk(q)
                    q_vtk.SetName("q")
                    polydata.GetPointData().AddArray(q_vtk)
                    mapper.OrientOn()
                    mapper.SetOrientationModeToQuaternion()
                    mapper.SetOrientationArray("q")
                mapper.SetInputData(polydata)
                actor = vtk.vtkActor()
                actor.SetMapper(mapper)
                self.set_particle_actor_props(actor)
    ...
    def set_particle_actor_props(self, actor):
        props = self.visual_props.get('particles')
        actor_props = actor.GetProperty()
        actor_props.SetColor(props['color'].getRgbF()[:3])
    ...
    def filter_particles(self):
        if self.particles_df is None:
            return
        gv = GUI.project.get_value
        query = []

        def append_query(key_name, column_name=None):
            if column_name is None:
                column_name = key_name
            if column_name not in self.particles_df.columns:
                return
            mn = gv("part_in_%s_min"%key_name)
            mx = gv("part_in_%s_max"%key_name)
            exclude = gv("part_in_%s_exclude"%key_name)
            if mn is not None:
                query.append("%s %s %s" %(column_name, "<" if exclude else ">", mn))
            if mx is not None:
                query.append("%s %s %s" %(column_name, ">" if exclude else "<", mx))
        ...
        # Phase
        for P in range(1, 1+len(GUI.solids)):
            if gv("part_in_phase", args=[P])==False:
                query.append("phase_id != %s" % P)

        # Diameter
        append_query("diameter")

        # Density
        append_query("density")
        ...
        # Temperature
        append_query("temp", "t_s")

        # Species mass fraction
        for key,rel in (("part_in_x_s_min","><"),
                        ("part_in_x_s_max","<>")):
#           for (p,s) in self.project.get_key_indices(key):
            for (s,) in self.project.get_key_indices(key):
                if "species%s" % s not in self.particles_df.columns:
                    continue
                exclude = gv("part_in_x_s_exclude", args=[s])
                sym = rel[int(exclude or 0)]
#               query.append("(phase_id != %s or species%s %s %s)" %
#                            (p, s, sym, gv(key, args=(p,s))))
                query.append("species%s %s %s" % (s, sym, gv(key,args=[s])))

        # User vars
        for key,rel in (("part_in_usr_var_min","><"),
                        ("part_in_usr_var_max","<>")):
            for (s,) in self.project.get_key_indices(key):
                if "usr_var%s" % s not in self.particles_df.columns:
                    continue
                exclude = gv("part_in_usr_var_exclude",args=[s])
                sym = rel[int(exclude or 0)]
                query.append("usr_var%s %s %s" % (s, sym, gv(key,args=[s])))


        if query:
            self.particles_df_filtered = self.particles_df.query(
                ' and '.join(query))
        else:
            self.particles_df_filtered = self.particles_df
    ...
    def change_visibility(self, name, toolbutton):
        ...
        if name == 'background_mesh':
            actors = self.grid_viewer_dict.get('actors')
        elif name == 'geometry':
            ...
            actors = [self.boundary_dict.get('actor')]
        elif name == 'mesh':
            actors = [self.mesh_dict.get('actor')]
        elif name == 'particles':
            actors = self.particles_dict.get('actors')
        if actors:
            for actor in actors:
                if actor is not None:
                    if visible:
    ...
    def get_actors(self, name):
        ...
        if name == 'background_mesh':
            ...
            a = self.boundary_dict.get('actor')
            if a:
                actors = [a]
        elif name == 'mesh':
            a = self.mesh_dict.get('actor')
            if a:
                actors = [a]
        elif name == 'particles':
            actors = self.particles_dict.get('actors',[])
    ...
    def change_color(self, name, button):
        ...
        if name == 'particles': # FIXME Why is this necessary?
