...
# This is needed for running the Mesher in SMS mode
from mfixgui.solver.manager import Solver, get_default_solver
...
and <b>stl_small_angle</b> to their default values.""")
        # key handler
        for p in (ui.pushbutton_delete_mesh,
                  ui.pushbutton_generate_mesh,
                  ui.pushbutton_accept_mesh,
                  ui.pushbutton_reset_tol):
            # See gui.enable_input
            p.lock_button = True
    ...
    def calc_total_cells(self, wid, dict, args):
        ...
        self.ui.mesh.label_total_background_cells.setText(str(tot))
    ...
    def setup_mesh(self, allow_disabled_tab=False):
        ...
        # Issues/1611
        cb = ui.checkbox_keyword_re_indexing
        if not self.project.get_value("cartesian_grid"):
            cb.setChecked(False)
            cb.setEnabled(False)
        else:
            cb.setEnabled(True)
            cb.setChecked(bool(self.project.get_value('re_indexing')))
    ...
    def mesh_dir_changed(self, d):
        ...
        mesh_exists = any (os.path.exists(RUN_NAME+x) for x in ('.msh', '_boundary.vtk'))
        for b in (ui.pushbutton_accept_mesh,
                  ui.pushbutton_delete_mesh):
            self.enable_widget(b, mesh_exists)
    ...
    def delete_mesh(self):
        run_name = self.project.get_value('run_name')
        vtu_dir = self.project.get_value('vtu_dir', 'vtu_dir')
        if run_name:
            RUN_NAME = run_name.upper()
            files = [f for f in (RUN_NAME+'_boundary.vtk',
                                 os.path.join(vtu_dir, RUN_NAME+'_boundary.vtk'),
                                 RUN_NAME+'_MESH.vtu',
                                 os.path.join(vtu_dir, RUN_NAME+'_MESH.vtu'),
                                 RUN_NAME+'.msh')
                     if os.path.exists(f)]
            if files:
    ...
    def generate_mesh(self):
        ...
        if not solver:
            s = "mfixsolver.bat" if WINDOWS else "mfixsolver"
            if os.path.exists(s):
                solver = s
        if not solver:
            self.error("No MFiX solver found.", popup=True)
            return
    ...
    def init_background_mesh(self):
        ...
        for (i, k) in enumerate(['x', 'y', 'z']):
            ...
            if indices:
                ...
                self.mesh_tables[i].set_value(table_dict)
    ...
    def extract_mesh_spacing(self, index, spacing):
        ...
        for (i, (val, count)) in enumerate((k, sum(1 for _i in g))
            ...
            table_dict[i] = {'position': loc, 'cells': count, 'stretch': 1.0,
                            'first': 0.0, 'last': 0.0}
        ...
        self.mesh_tables[index].set_value(table_dict)
