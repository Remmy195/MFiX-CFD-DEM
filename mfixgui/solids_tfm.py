...
from .tools.qt import get_combobox_item
...
class SolidsTFM:
    ...
    def setup_solids_tfm_tab(self):
        # Note - we are doing this setup on first show of this tab, rather
        # than at init or project load time
        ui = self.ui.solids
        ui.TFM.setEnabled(ui.pushbutton_solids_TFM.isEnabled()) # We may be just visiting due to locate_keyword
        ...
        for (i,e) in enumerate(enabled):
            ...
            if not e:
                self.disable_widget(item,  reason=reasons[i])
            else:
        ...
        for i in (1,2):
            item = get_combobox_item(cb, i)
            self.set_widget_enabled(item, (kt_type!='ALGEBRAIC'),
                             reason="Not available for algebraic viscous stress model.")
        ...
        for item in (ui.label_eps_f_min, ui.lineedit_keyword_eps_f_min):
            ...
            item.setEnabled(enabled)
            if not enabled:
                item.setToolTip(item.tooltip0 + "<br>Requires Srivastava or Guo-Boyce friction model")
            else:
        ...
        for item in (ui.label_c_e, ui.lineedit_keyword_c_e):
            self.set_widget_enabled(item, enabled, reason="Requires 2 or more solids phases" if mmax<2 else
                                    "Not compatible with GHD viscous stress model")
        ...
        for item in (ui.label_c_f, ui.lineedit_keyword_c_f):
            ...
            item.setEnabled(enabled)
            if not enabled:
                item.setToolTip(item.tooltip0 + "<br>Requires at least two solids phases")
            else:
                item.setToolTip(item.tooltip0)
        ...
        for item in (ui.label_phi, ui.lineedit_keyword_phi):
            self.set_widget_enabled(item, enabled, reason="Requires Schaeffer, Srivastava, or Guo-Boyce friction model")
        ...
        for (i,e) in enumerate(enabled):
            self.set_widget_enabled(get_combobox_item(cb,i), e) # XXX reason
        ...
        for item in (ui.label_blending_function, cb):
            self.set_widget_enabled(item, enabled, reason='Requires Schaeffer friction model.')
        if not enabled:
            self.unset_keyword(key)
        ...
        for item in (ui.label_segregation_slope_coefficient,
            ...
            self.set_widget_enabled(item, enabled,
                                    reason='Requires at least two solids phases and algebraic, Lun, Simonin or Amhadi viscous stress model.')
        ...
        for item in (ui.label_max_packing_correlation, ui.combobox_max_packing_correlation):
            self.set_widget_enabled(item, enabled,
                                    reason="Requires at least two solids phases and Schaffer friction model.")
        ...
        self.set_widget_enabled(get_combobox_item(cb, 2), enabled = mmax==2,
                                reason="Only valid for binary mixture.")
        ...
        if not cb.isEnabled():
            cb.setCurrentIndex(0)
            cb.setToolTip(ui.label_max_packing_correlation.toolTip())
    ...
    def handle_friction_model(self):
        if not self.project:
            return
        val = self.project.get_value('friction_model', default=DEFAULT_FRICTION_MODEL)
        # issues/1336
        exclude_min =  val in ('SRIVASTAVA', 'GUO_BOYCE')
        for le in (self.ui.initial_conditions.lineedit_keyword_ic_theta_m_args_IC_P,
                   self.ui.boundary_conditions.lineedit_keyword_bc_theta_m_args_BC_P):
        ...
        if exclude_min:
            ...
            for P in range(1, 1+len(self.solids)):
                key = 'ic_theta_m'
                for IC in self.ics:
                    if self.project.get_value(key, args=[IC,P], default=0) == 0:
                        self.update_keyword(key, val, args=[IC,P])
                key = 'bc_theta_m'
                for BC in self.bcs:
                    if self.project.get_value(key, args=[BC,P], default=0) == 0:
