...
from mfixgui.tools.read_thermo import expand_tabs
...
converted_keys = {'xlength', 'ylength', 'zlength', 'e', 'ic_des_fit_to_region'}

# these are getting migrated in project_manager
converted_keys.update({'c', 'c_name'})
...
class FloatExp(float):
    ...
    def __repr__(self):
        a = abs(self)
        return ('{:.{}e}'.format(self, self.fmt) if a>1e8 or (a and a<1e-3)
                else '%g'%self)
...
class Keyword:
    ...
    def updateValue(self, value):
        # 1854 Special handling for Cantera polynomial coeffs - we don't want to convert these
        # to FloatExp. This is a bit of a hack which can be removed when we fix issue 746
        if self.key in ('poly_mu_g', 'poly_kg'):
            self.dtype = float
            self.value = value
            return
...
class Solid(Base):
    ...
    def addSpecies(self, ind, phase='s'):
        return self.species.new(ind, phase)
...
def make_alias(name):
    # Would use make_alias in species_popup but we don't have access to the main
    # GUI object here due to pedantic object hierarchies.
    # So we just have this inline here for now
    for (pat, repl) in [(' ', '_'),
                        ('(', '_'),
                        (')', ''),
                        ('-', '_'),
...
class Project:
    ...
    def __init__(self, dat_file=None, keyword_doc=None):
        ...
        self.comment_block = []
        self.keyword_dict = {}
        self.thermo_data_burcat = {} # key=species[:18] value=list of lines
        self.thermo_data_chemkin = {} # key=species[:18] value=list of lines
        self.mfix_gui_comments = {} # lines starting with #!MFIX-GUI
    ...
    def parseKeywordLine(self, line, equation_str=None):
        ...
        if matches:
            for match in matches:
                ...
                # FIXME see issues 746, 1854, 1907
                if key in ('poly_mu_g', 'poly_kg'):
                    # Don't convert these to FloatExp
                    cleanVals = [float(val) for val in vals]
                else:
                    cleanVals = []
                    for val in vals:
    ...
    def parse_mfix_gui_comments(self, file):
        ...
        for line in file:
            ...
            if line.startswith('#!MFIX-GUI  '):  #continuation line
                ...
                elif line.endswith(' ='): #Empty value, line has been rstripped
                    key = line[11:-2]
                    val = '' # Use empty string to differentiate from None, so we can use 'get'
                    self.mfix_gui_comments[key] = val
    ...
    def parsemfixdat_inner(self, file):
        ...
        self.init_data()
        self.thermo_data_burcat.clear()
        self.thermo_data_chemkin.clear()
        reactionSection = False
        desReactionSection = False
        thermoSection = False
        thermoSectionChemkin = False
        thermo_lines_burcat = [] # Temporary holder for thermo_data section
        thermo_lines_chemkin = []
        ...
        for lineno, line in enumerate(file, 1):
            ...
            if '@(RXNS)' in line:
                ...
                if 'chemkin' in line.lower():
                    if thermoSectionChemkin:
                        raise SyntaxError('multiple THERMO DATA (CHEMKIN) sections')
                    thermoSectionChemkin = True
                    thermoSection = False
                else:
                    if thermoSection:
                ...
                thermo_lines_burcat.append(line0.rstrip()) # keep left padding
            elif thermoSectionChemkin:
                thermo_lines_chemkin.append(line0.rstrip()) # keep left padding
            elif not (reactionSection or thermoSection or thermoSectionChemkin):
                equation_str = None
                # remove comments
                commentedline = ''
        ...
        # These will be parsed further in project_manager.load_project_file (why not do it here?)
        self.thermo_lines_burcat = thermo_lines_burcat
        self.thermo_lines_chemkin = thermo_lines_chemkin
    ...
    def format_reaction(self, name):
        ...
        width = MAX_WIDTH-len('chem_eq')-7 # 4 space indent + 3 for ' = '
        chem_eq, rest = break_string(chem_eq, width)
        ...
        if not rest:
            ...
            yield '    chem_eq = "%s " &' % chem_eq
            while rest:
                width = MAX_WIDTH - 8 # indent
                chem_eq, rest = break_string(rest, width)
                chem_eq = chem_eq.strip()
                rest = rest.strip()
                if rest:
                    yield '        "%s " &' % chem_eq
        ...
        # Move dh and fracdh to the front
        for x in 'fracdh', 'dh':
            if x in data:
        ...
        for (key, val) in items:
            ...
            if isinstance(val, dict):
                ...
                val = str(val)
                width = 60-len(key)-7 # 4 space indent+3 for ' = '
                val, rest = break_string(val, width)
                val = val.strip()
                rest = rest.strip()
                if not rest:
                    yield '    %s = %s' % (key, val)
                    ...
                    yield '    %s = %s " &' % (key, val)
                    while rest:
                        val, rest = break_string(rest, width)
                        val = val.strip()
                        rest = rest.strip()
                        if rest:
                            yield '        "%s " &' % val
