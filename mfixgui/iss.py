...
class ISS:
    ...
    def iss_add_regions_1(self, selections,
        ...
        if self.iss_region_dict is None:
            self.iss_region_dict = self.get_region_dict()
        ...
        for (i, region_name) in enumerate(selections):
            ...
            if region_data is None: # ?
                self.warn("no data for IS region %s" % region_name)
                continue
    ...
    def setup_iss(self, allow_disabled_tab=False):
        ui = self.ui.internal_surfaces
        # Can we add internal surfaces when paused?
    ...
    def iss_setup_current_tab(self):
        ...
        #Input is only needed for semi-permeable surfaces.
        is_type = self.project.get_value('is_type', args=[IS0]).upper()
        ...
        enable = (is_type=='STL')
        gb = ui.groupbox_moving_surface
        if enable:
            gb.show()
        else:
            gb.hide()
        #gb.setEnabled(enable)
        #gb.setToolTip("" if enable else "Only available for STL internal surfaces")
        for k in ('vel', 'omega', 'rot_center'):
            for i in (1,2,3):
                if enable:
                    val = self.project.get_value('is_wall_'+k, default=None, args=[IS0,i])
                    txt = '' if val is None else str(val)
                else:
                    txt = ''
                getattr(ui, 'lineedit_keyword_is_wall_%s_args_IS_%s'%(k,i)).setText(txt)
