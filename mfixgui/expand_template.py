...
from mfixgui.chemistry import reaction_is_des
...
def alnum(line):
    ...
    if not end.strip():
        return (line,)
    args = tuple([try_int(x) for x in end.split(',')])
    if not isinstance(args[0], int):
        return (line,)
    return (start,)+args

def sorted_alnum(x):
    return sorted(x, key=alnum)

def get_loop_range(var):
    return sorted_alnum(loop_ranges.get(var, []))
        ...
        if key.startswith('category'):
            ...
            for (k, a) in all_keys:
                if cat in keyword_doc.get(k, {}).get('category', 'unknown'):
                    ...
                    ret2.append((k, a))
            return (bool(ret1), sorted_alnum(ret1), ret2)
...
MONITOR_TYPE_NAMES = dict(
    zip(MONITOR_TYPES_CELL, MONITOR_TYPE_CELL_NAMES))

MONITOR_TYPE_NAMES.update(dict(
    zip(MONITOR_TYPES_PARTICLE, MONITOR_TYPE_PARTICLE_NAMES)))

def monitor_type_name(args, args_raw):
    arg = args[0]
    arg_raw = args_raw[0]

    if not arg_raw.isdigit() and arg_raw != 'monitor':
        err('monitor_name: expected monitor argument, got "%s"' % arg_raw)

    key = 'monitor_type'
    args = (arg,)
    k = proj.get(key, args)
    return None if k is None else MONITOR_TYPE_NAMES.get(k.value)
...
def project_to_string(p, template='standard'):
    ...
    if all_keys:
        msg = "%s keys unmatched in template: %s" % (
            len(all_keys), ', '.join(sorted_alnum(format_key_with_args(k, a or None)
                                            for (k,a) in all_keys)))
        if len(msg) > 500:
            msg = msg[:500] + '...'
        err(msg)
    ...
    output_lines.extend(thermo_data_burcat_lines())
    output_lines.extend(thermo_data_chemkin_lines())
...
def chemistry_lines():
    ...
    for (name, reaction) in proj.reactions.items():
        if reaction.get('chem_eq') is None: # Don't save incompletely-defined reactions
            continue
        if reaction_is_des(proj, reaction):
...
def thermo_data_burcat_lines():
    if proj.thermo_data_burcat:
        d = proj.thermo_data_burcat
        yield '\nTHERMO DATA'
        for k in sorted_alnum(d.keys()):
            for line in d[k]:
                yield line.rstrip()
            yield '' # blank line between entries

def thermo_data_chemkin_lines():
    if proj.thermo_data_chemkin:
        d = proj.thermo_data_chemkin
        yield '\nTHERMO DATA (CHEMKIN)'
        for k in sorted_alnum(d.keys()):
            for line in d[k]:
                yield line.rstrip()
