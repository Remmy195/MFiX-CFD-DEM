...
class PSS:
    ...
    def init_pss(self):
        ...
        ui.tablewidget_fluid_mass_fraction.key = 'ps_x_g'
        ui.groupbox_solids_composition.key = 'ps_x_s'
        ui.tablewidget_solids_mass_fraction.key = 'ps_x_s'
    ...
    def pss_cancel_add(self):
        ...
        self.enable_widget(ui.tablewidget_regions, ignore_lock=True)
        self.enable_widget(ui.toolbutton_add)

        if get_selected_row(ui.tablewidget_regions) is not None:
            self.enable_widget(ui.bottom_frame, ignore_lock=True)
    ...
    def pss_add_regions_1(self, selections, indices=None, autoselect=False):
        ...
        if self.pss_region_dict is None:
            self.pss_region_dict = self.get_region_dict()
        ...
        for (i, region_name) in enumerate(selections):
            ...
            if region_data is None: # ?
                self.warn("no data for PS region %s" % region_name)
                continue
    ...
    def pss_update_enabled(self):
        if self.pss:
            ...
            tfm_solids = [s for (i,s) in enumerate(self.solids,1)
                          if self.project.get_value('solids_model', default='TFM', args=[i])=='TFM']
            disabled = (nregions==0
    ...
    def setup_pss(self, allow_disabled_tab=False):
        ...
        # Can we add point sources when locked?
        #if self.lock_level:
        ...
        self.set_widget_enabled(b, not self.fluid_solver_disabled,
                                reason="Fluid solver disabled",
                                ignore_lock=True)
        font = b.font()
        ...
        for (i, solid_name) in enumerate(self.solids.keys(),1):
            ...
            self.set_widget_enabled(b, model=='TFM',
                                    reason="Only TFM solids can be defined as point sources.",
                                    ignore_lock=True)
    ...
    def setup_pss_solids_tab(self, P):
        ...
        enabled = bool(species_eq)
        gb = ui.groupbox_solids_composition
        if enabled:
            self.update_pss_solids_mass_fraction_table()
        else:
            ui.tablewidget_solids_mass_fraction.setRowCount(0)
            self.fixup_pss_table(ui.tablewidget_solids_mass_fraction)
            self.disable_widget(gb)
