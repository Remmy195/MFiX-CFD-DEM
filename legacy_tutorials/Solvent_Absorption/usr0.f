...
! forces
      if (usr_c(5) == undefined) then
         write(err_msg, 1141)
         call log_warning()
 1141 format("Warning 1141: setting the calibration factor, usr_c(5), to ",&
        "1 for the",/"viscous term in gas-liquid interaction model")
            usr_c(5) = one
      elseif (usr_c(5) <= 0) then
         WRITE(err_msg, 1106) 'USR_C(5)', ival(USR_C(5))
         CALL LOG_ERROR()
      endif
      if (usr_c(6) == undefined) then
         write(err_msg, 1142)
         call log_warning()
 1142 format("Warning 1142: setting the calibration factor, usr_c(6), to ",&
        "1 for the",/"inertial term in gas-liquid interaction model")
            usr_c(6) = one
      elseif (usr_c(6) <= 0) then
         WRITE(err_msg, 1106) 'USR_C(6)', ival(USR_C(6))
         CALL LOG_ERROR()
      endif
      if (usr_c(7) == undefined) then
         write(err_msg, 1143)
         call log_warning()
 1143 format("Warning 1143: setting the calibration factor, usr_c(7), to ",&
        "1 for the",/"viscous term in gas-solids interaction model")
            usr_c(7) = one
      elseif (usr_c(7) <= 0) then
         WRITE(err_msg, 1106) 'USR_C(7)', ival(USR_C(7))
         CALL LOG_ERROR()
      endif
      if (usr_c(8) == undefined) then
         write(err_msg, 1144)
         call log_warning()
 1144 format("Warning 1144: setting the calibration factor, usr_c(8), to ",&
        "1 for the",/"inertial term in gas-solids interaction model")
            usr_c(8) = one
      elseif (usr_c(8) <= 0) then
         WRITE(err_msg, 1106) 'USR_C(8)', ival(USR_C(8))
         CALL LOG_ERROR()
      endif
      if (usr_c(9) == undefined) then
         write(err_msg, 1145)
         call log_warning()
 1145 format("Warning 1145: setting the calibration factor, usr_c(9), to ",&
        "1 for the",/"viscous term in liquid-solids interaction model")
            usr_c(9) = one
      elseif (usr_c(9) <= 0) then
         WRITE(err_msg, 1106) 'USR_C(9)', ival(USR_C(9))
         CALL LOG_ERROR()
      endif
      if (usr_c(10) == undefined) then
         write(err_msg, 1146)
         call log_warning()
 1146 format("Warning 1146: setting the calibration factor, usr_c(10), to ",&
        "1 for the",/"inertial term in liquid-solids interaction model")
            usr_c(10) = one
      elseif (usr_c(10) <= 0) then
         WRITE(err_msg, 1106) 'USR_C(10)', ival(USR_C(10))
         CALL LOG_ERROR()
      endif
...
! gas side mass transfer resistance is assumed negligible
               USR_C(2) = large_number
            CASE ('CONSTANT_MTC')
               MASS_TRANSFER_TYPE_ENUM = CONSTANT_MTC
               if (usr_c(2) == undefined) then
                   write(err_msg, 1190)
                   CALL LOG_ERROR()
 1190 format('ERROR 1190: The factor usr_c(2), the gas phase mass ',&
         'transfer coefficient,'/,'must be defined when specifying',&
         'MASS_TRANSFER_TYPE=""CONSTANT_MTC"".'/,&
         'Please correct the input file.')
               elseif (usr_c(2) <= 0) then
                  WRITE(err_msg, 1106) 'USR_C(2)', ival(USR_C(2))
                  CALL LOG_ERROR()
               endif
               if (usr_c(3) == undefined) then
                   write(err_msg, 1192)
                   call log_warning()
 1192 format('ERROR 1192: The factor usr_c(3), the liquid phase mass ',&
         'transfer coefficient,'/,'must be defined when specifying',&
         'MASS_TRANSFER_TYPE=""CONSTANT_MTC"".'/,&
         'Please correct the input file.')
               elseif (usr_c(3) <= 0) then
                  WRITE(err_msg, 1106) 'USR_C(3)', ival(USR_C(3))
                  CALL LOG_ERROR()
               endif
...
! EQUILIBRIUM_SEGREGATED scheme
         IF(ABSORPTION_CHEM_TYPE_ENUM == EQUILIBRIUM_SEGREGATED) THEN
            if (usr_c(11) == undefined) then
               write(err_msg, 1183)
               call log_warning()
 1183 format("Warning 1183: setting the forward reaction rate constant",&
           " USR_C(11) for",/"the carabmate (RNHCOO-) reversion equilbrium ",&
           "reaction to 1.")
               usr_c(11) = one
            elseif (usr_c(11) <= 0) then
               WRITE(err_msg, 1106) 'USR_C(11)', ival(USR_C(11))
               CALL LOG_ERROR()
            endif

            if (usr_c(12) == undefined) then
               write(err_msg, 1184)
               call log_warning()
 1184 format("Warning 1184: setting the forward reaction rate constant",&
           " USR_C(12) for",/"the dissociation of dissolved CO2 equilbrium",&
           " reaction to 1.")
               usr_c(12) = one
            elseif (usr_c(12) <= 0) then
               WRITE(err_msg, 1106) 'USR_C(12)', ival(USR_C(12))
               CALL LOG_ERROR()
            endif

            if (usr_c(13) == undefined) then
               write(err_msg, 1185)
               call log_warning()
 1185 format("Warning 1185: setting the forward reaction rate constant",&
           " USR_C(13) for",/"the dissociation of bicarbonate (HCO3-)",&
           " equilbrium reaction to 1.")
               usr_c(13) = one
            elseif (usr_c(13) <= 0) then
               WRITE(err_msg, 1106) 'USR_C(13)', ival(USR_C(13))
               CALL LOG_ERROR()
            endif

            if (usr_c(14) == undefined) then
               write(err_msg, 1186)
               call log_warning()
 1186 format("Warning 1186: setting the forward reaction rate constant",&
           " USR_C(14) for",/"the dissociation of protonated MEA (RNH2)",&
           " equilbrium reaction to 1.")
               usr_c(14) = one
            elseif (usr_c(14) <= 0) then
               WRITE(err_msg, 1106) 'USR_C(14)', ival(USR_C(14))
               CALL LOG_ERROR()
            endif

            if (usr_c(15) == undefined) then
               write(err_msg, 1187)
               call log_warning()
 1187 format("Warning 1187: setting the forward reaction rate constant",&
        " USR_C(15) for",/"the ionization of water equilibrium reaction",&
        " to 1.")
               usr_c(15) = one
            elseif (usr_c(15) <= 0) then
               WRITE(err_msg, 1106) 'USR_C(15)', ival(USR_C(15))
               CALL LOG_ERROR()
            endif
...
!-------------------------------------------------
         if (usr_c(1) == undefined) then
            write(err_msg, 1180)
            call log_warning()
 1180 format("Warning 1180: setting the calibration factor, usr_c(1), to ",&
           "1 for Henry's",/"constant for CO2 in aequous MEA.")
               usr_c(1) = one
            elseif (usr_c(1) <= 0) then
               WRITE(err_msg, 1106) 'USR_C(1)', ival(USR_C(1))
               CALL LOG_ERROR()
         endif
         ...
         if (usr_c(2) == undefined) then
             write(err_msg, 1181)
             call log_warning()
 1181 format('Warning 1181: setting the calibration factor, usr_c(2), to ',&
         ...
         if (usr_c(3) == undefined) then
             write(err_msg, 1182)
             call log_warning()
 1182 format('Warning 1182: setting the calibration factor, usr_c(3), to ',&
